...
export class RemoteProtocolFixtureState extends Schema.Class<RemoteProtocolFixtureState>(
	...
	phase: Schema.Literals(['unpaired', 'pairing', 'active', 'reconnecting', 'revoked', 'lost']),
	deviceId: Schema.optionalKey(Id),
	runtimeId: Schema.optionalKey(Id),
	sessionId: Schema.optionalKey(Id),
	keyEpoch: KeyEpoch,
	lastReceivedSequence: Sequence,
	interruptedOperationIds: Schema.Array(Id).check(Schema.isMaxLength(32), Schema.isUnique())
}) {}
...
export class RemoteProtocolFixtureFailure extends Schema.TaggedErrorClass<RemoteProtocolFixtureFailure>()(
	...
	{
		reason: Schema.Literals(['illegal-transition', 'replay', 'wrong-key-epoch', 'revoked']),
	...
	switch (event.type) {
		...
		case 'interrupt':
			return state.phase === 'active' || state.phase === 'reconnecting'
				? withState(state, {
						interruptedOperationIds: state.interruptedOperationIds.includes(event.operationId)
