...
describe("Flect control contracts", () => {
  it.effect(
    "decodes the complete protected sharing lifecycle as strict commands",
    () =>
      Effect.gen(function* () {
        const commands = [
          {
  ...
  it.effect(
    "decodes strict protected capability decisions and revocation",
    () =>
      Effect.gen(function* () {
  ...
  it.effect(
    "keeps provider authentication state outside inspectable workspace snapshots",
    () =>
      Effect.gen(function* () {
        ...
        const encoded = yield* Schema.encodeEffect(AgentWorkspaceSnapshot)(
          snapshot,
        );
  ...
  it.effect("round-trips a strict versioned command envelope", () =>
    Effect.gen(function* () {
      ...
      const encoded =
        yield* Schema.encodeEffect(FlectCommandEnvelope)(envelope);
      expect(yield* decodeFlectCommandEnvelope(encoded)).toEqual(envelope);
      yield* decodeFlectCommandEnvelope({ ...encoded, invented: true }).pipe(
        Effect.flip,
      );
    }),
  );
  ...
  it.effect(
    "strictly attributes commands created by an interactive agent",
    () =>
      Effect.gen(function* () {
  ...
  it.effect(
    "keeps enable-control in the closed union for UI authorization",
    () =>
      Effect.gen(function* () {
        const decoded = yield* decodeFlectCommandEnvelope({
          version: 1,
        ...
        expect(decoded.command).toEqual(
