...
describe('runtime contracts', () => {
	it.effect('requires a closed role on cancellation requests', () =>
		Effect.gen(function* () {
			...
			expect(yield* decode({ role: 'app' })).toEqual(CancelRequest.make({ role: 'app' }));
			expect(yield* decode({ role: 'shaper' })).toEqual(CancelRequest.make({ role: 'shaper' }));
			yield* decode({}).pipe(Effect.flip);
			yield* decode({ role: 'guardian' }).pipe(Effect.flip);
			yield* decode({ role: 'app', credential: 'not-a-real-secret' }).pipe(Effect.flip);
		})
	);
	...
	it.effect('decodes a typed busy turn without treating it as a fatal error', () =>
		Effect.gen(function* () {
	...
	it.effect('decodes visible tool lifecycle events without arbitrary details', () =>
		Effect.gen(function* () {
	...
	it.effect('supports automatic and explicit model selection', () =>
		Effect.gen(function* () {
			...
			yield* decodeSessionSelection({ reasoningLevel: 'extreme' }).pipe(Effect.flip);
		})
	);
