...
describe('capsule frame protocol', () => {
	...
	it.each([
		...
		await expect(Effect.runPromise(decodeCapsuleMessage(message))).rejects.toMatchObject({
			_tag: 'InvalidCapsuleMessage'
		});
	});
	...
	it.each([
		...
		await expect(Effect.runPromise(decodeCapsuleHostMessage(message))).rejects.toMatchObject({
			_tag: 'InvalidCapsuleMessage'
		});
	});
});
