...
export const ShareLineage = Schema.Literals([
  "new",
  "update",
  "fork",
  "replacement",
...
export class ShareInstallationSnapshot extends Schema.Class<ShareInstallationSnapshot>(
  ...
  warning: Schema.optionalKey(
    Schema.Literals(["invalid-record", "storage-unavailable"]),
  ),
}) {}

const failure = (
  reason: ShareInstallationFailure["reason"] = "invalid-record",
) =>
...
export const validateShareInstallationRecord = Effect.fn(
  "Flect.ShareInstallation.validate",
  ...
  if (
    ...
    (record.pending !== undefined &&
      record.pending.retainedAt > record.updatedAt) ||
    (record.pending !== undefined &&
