...
const ShareArtifactIds = Schema.Array(ShareId).check(Schema.isMinLength(1), Schema.isMaxLength(64));
...
export class WorkbenchHandoff extends Schema.Class<WorkbenchHandoff>('WorkbenchHandoff')({
	...
	failureSummary: Schema.optionalKey(Schema.String.check(Schema.isMaxLength(1_000)))
}) {}

export class WorkbenchSnapshot extends Schema.Class<WorkbenchSnapshot>('WorkbenchSnapshot')({
...
export class UserCommandSource extends Schema.Class<UserCommandSource>('UserCommandSource')({
...
export class AgentCommandSource extends Schema.Class<AgentCommandSource>('AgentCommandSource')({
...
export class SetRailCollapsed extends Schema.Class<SetRailCollapsed>('SetRailCollapsed')({
...
export class RefreshRuntime extends Schema.Class<RefreshRuntime>('RefreshRuntime')({
...
export class SetModelFavorite extends Schema.Class<SetModelFavorite>('SetModelFavorite')({
...
export class SubmitAppPrompt extends Schema.Class<SubmitAppPrompt>('SubmitAppPrompt')({
...
export class RequestShapeHandoff extends Schema.Class<RequestShapeHandoff>('RequestShapeHandoff')({
...
export class AcceptProposal extends Schema.Class<AcceptProposal>('AcceptProposal')({
	type: Schema.Literal('accept-proposal')
}) {}

export class RejectProposal extends Schema.Class<RejectProposal>('RejectProposal')({
	type: Schema.Literal('reject-proposal')
}) {}

export class RollbackRevision extends Schema.Class<RollbackRevision>('RollbackRevision')({
...
export class OpenShareSource extends Schema.Class<OpenShareSource>('OpenShareSource')({
...
export class CheckpointShareFork extends Schema.Class<CheckpointShareFork>('CheckpointShareFork')({
...
export class PrepareShareUpdate extends Schema.Class<PrepareShareUpdate>('PrepareShareUpdate')({
	type: Schema.Literal('prepare-share-update'),
	shareId: ShareId
}) {}

export class ContinueShareFork extends Schema.Class<ContinueShareFork>('ContinueShareFork')({
...
export class RestoreSafeMode extends Schema.Class<RestoreSafeMode>('RestoreSafeMode')({
...
export class DisableControl extends Schema.Class<DisableControl>('DisableControl')({
...
export class ToolActivity extends Schema.Class<ToolActivity>('ToolActivity')({
	...
	validationIssues: Schema.optionalKey(Schema.Array(ValidationIssue).check(Schema.isMaxLength(40)))
}) {}
...
export class OperationRecord extends Schema.Class<OperationRecord>('OperationRecord')({
	version: Schema.Literal(1),
	sequence: Sequence,
	operationId: OperationId,
	...
	phase: Schema.Literals(['accepted', 'started', 'updated', 'succeeded', 'failed', 'cancelled']),
...
export class RailStateSnapshot extends Schema.Class<RailStateSnapshot>('RailStateSnapshot')({
	collapsed: Schema.Boolean,
	width: Schema.Int.check(Schema.isBetween({ minimum: 340, maximum: 520 }))
}) {}
...
export class WorkspaceBuildSnapshot extends Schema.Class<WorkspaceBuildSnapshot>(
	...
	buildId: Schema.optionalKey(Schema.String.check(Schema.isMinLength(5), Schema.isMaxLength(80))),
	sourceRevision: Schema.optionalKey(
		Schema.String.check(Schema.isMinLength(1), Schema.isMaxLength(128))
	),
	artifactDigest: Schema.optionalKey(Schema.String.check(Schema.isPattern(/^[0-9a-f]{64}$/)))
}) {}

export class ShareSourceOpened extends Schema.Class<ShareSourceOpened>('ShareSourceOpened')({
...
export class ShareForkPrepared extends Schema.Class<ShareForkPrepared>('ShareForkPrepared')({
	status: Schema.Literal('forked'),
	shareId: ShareId,
	forkCommit: ShareObjectId
}) {}

export class ShareUpdatePrepared extends Schema.Class<ShareUpdatePrepared>('ShareUpdatePrepared')({
	status: Schema.Literals(['fast-forward', 'merged', 'conflict', 'replacement']),
	shareId: ShareId,
...
export class ShareExportPrepared extends Schema.Class<ShareExportPrepared>('ShareExportPrepared')({
	status: Schema.Literal('exported'),
	shareId: ShareId,
	archiveSha256: Schema.String.check(Schema.isPattern(/^[0-9a-f]{64}$/)),
	bytes: Schema.Int.check(Schema.isBetween({ minimum: 1, maximum: 64 * 1024 * 1024 }))
}) {}
...
export class FlectWorkspaceSnapshot extends Schema.Class<FlectWorkspaceSnapshot>(
	...
	workspaceId: WorkspaceId,
	sequence: Sequence,
	phase: Schema.Literals(['booting', 'ready', 'shaping', 'preview', 'safe-mode', 'unavailable']),
	mode: Schema.Literals(['edit', 'run']),
...
export class FlectWorkspaceEvent extends Schema.Class<FlectWorkspaceEvent>('FlectWorkspaceEvent')({
	version: Schema.Literal(1),
	id: EventId,
	sequence: Sequence,
	timestamp: Timestamp,
...
export class FlectCommandReceipt extends Schema.Class<FlectCommandReceipt>('FlectCommandReceipt')({
	version: Schema.Literal(1),
	commandId: CommandId,
	workspaceId: WorkspaceId,
	operationId: OperationId,
...
export class CommandConflict extends Schema.TaggedErrorClass<CommandConflict>()('CommandConflict', {
	message: Schema.Literal('The workspace changed before the command ran.'),
	currentSequence: Sequence
}) {}

export class CommandRejected extends Schema.TaggedErrorClass<CommandRejected>()('CommandRejected', {
	message: BoundedText(1, 500)
}) {}
