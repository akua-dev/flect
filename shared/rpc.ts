...
import { FlectCommandEnvelope, FlectWorkspaceEvent, FlectWorkspaceSnapshot } from './control';
import {
	ControlBrokerStatus,
	ControlCommandCompletion,
	ControlTransportFailed
} from './control-channel';
import { AgentIntegrationError, AgentIntegrationHost, AgentIntegrationStatus } from './setup';

const Identifier = Schema.Trim.check(Schema.isMinLength(1), Schema.isMaxLength(200));
...
export class ReplyProviderAuthSelection extends Rpc.make('ReplyProviderAuthSelection', {
...
export class Shape extends Rpc.make('Shape', {
	payload: {
		sessionId: Identifier,
		instruction: Schema.Trim.check(Schema.isMinLength(1), Schema.isMaxLength(4_000)),
