...
describe("role continuity contract", () => {
  ...
  it.effect(
    "rejects oversized fields, collections, and aggregate records",
    () =>
      Effect.gen(function* () {
        const base = emptyRoleContinuityRecord(0);
        ...
        const tooManyMessages = yield* decodeRoleContinuityRecord(
          JSON.stringify({
            ...base,
            app: Array.from({ length: 201 }, (_, index) =>
        ...
        const aggregate = RoleContinuityRecord.make({
          ...base,
          app: Array.from({ length: 6 }, (_, index) =>
            message(`large-${index}`, "x".repeat(100_000)),
          ),
        });
        const oversizedAggregate = yield* encodeRoleContinuityRecord(
          aggregate,
        ).pipe(Effect.flip);
  ...
  it.effect("rejects malformed JSON and malformed candidate bindings", () =>
    Effect.gen(function* () {
      const malformed = yield* decodeRoleContinuityRecord("{bad").pipe(
        Effect.flip,
      );
