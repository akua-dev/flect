...
describe("share installation records", () => {
  ...
  it.effect(
    "preserves a retained receipt after every artifact is removed",
    () =>
      Effect.gen(function* () {
        const decoded = yield* validateShareInstallationRecord({
          ...record(),
  ...
  it.effect(
    "rejects a persisted manifest that disagrees with its receipt",
    () =>
      Effect.gen(function* () {
  ...
  it.effect(
    "rejects duplicate artifacts, impossible refs, and private authority",
    () =>
      Effect.gen(function* () {
        const base = record();
        ...
        for (const input of cases) {
          const failure = yield* validateShareInstallationRecord(input).pipe(
            Effect.flip,
          );
