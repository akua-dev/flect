...
describe('interface document', () => {
	...
	it.effect('rejects pathological depth before recursive decoding', () =>
		Effect.gen(function* () {
			const error = yield* Effect.flip(validateInterfaceDocument(deeplyNestedDocument(2_000)));

			assert.strictEqual(error._tag, 'InvalidInterfaceDocument');
		})
	);

	it.effect('fails closed for malformed, unsupported, or missing stored state', () =>
		Effect.gen(function* () {
