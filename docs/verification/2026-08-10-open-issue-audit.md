...
| #31 | external-gate | The canonical workflow is checked in and its complete equivalent passes locally. The branch is pushed, but GitHub cannot dispatch a workflow that does not yet exist on the default branch; its first hosted run therefore requires a pull request or initial integration. Neither a pull request, merge, nor branch-protection change was authorized. |
