...
import { validateUpdaterArchiveEntries, validateUpdaterEvidence } from './release-update';
...
class FixtureManifest extends Schema.Class<FixtureManifest>('FixtureManifest')({
	...
	archiveUrl: Schema.String.check(Schema.isMinLength(1), Schema.isMaxLength(300)),
	signature: Schema.String.check(Schema.isMinLength(1), Schema.isMaxLength(16_384)),
...
export class NativeUpdateFixtureDogfoodReport extends Schema.Class<NativeUpdateFixtureDogfoodReport>(
	...
	preserved: Schema.Array(Schema.Literals(['workspace', 'settings', 'grants', 'extensions'])),
...
const fixtureError = (reason: NativeUpdateFixtureError['reason'], message: string) =>
...
const temporaryRoot = Effect.acquireRelease(
	Effect.tryPromise({
		...
		catch: () => fixtureError('filesystem', 'The fixture root could not be created.')
	}),
	(path) =>
		Effect.tryPromise({
...
const writeFixtureFile = Effect.fn('FixtureUpdate.writeFile')(
	(path: string, contents: string | Uint8Array) =>
		Effect.tryPromise({
			...
			catch: () => fixtureError('filesystem', 'A fixture file could not be written.')
		})
);
...
const writeFixtureApp = Effect.fn('FixtureUpdate.writeApp')(function* (
	...
	const runtime = join(parent, 'Flect.app', 'Contents', 'MacOS', 'flect-runtime');
		...
		[
			Effect.tryPromise({
				try: () => chmod(executable, 0o755),
					...
					fixtureError('filesystem', 'The fixture executables could not be made runnable.')
			}),
			Effect.tryPromise({
	...
	(command: ReadonlyArray<string>, reason: NativeUpdateFixtureError['reason']) =>
		Effect.tryPromise({
			try: async () => {
				...
				fixtureError(reason, `A fixture command failed: ${String(cause)}`.slice(0, 300))
		})
);
...
const createArchive = Effect.fn('FixtureUpdate.createArchive')(function* (
	...
	yield* runCommand(['tar', '-czf', archivePath, '-C', sourceRoot, 'Flect.app'], 'filesystem');
	return yield* Effect.tryPromise({
		try: async () => Uint8Array.from(await readFile(archivePath)),
		catch: () => fixtureError('filesystem', 'The fixture archive could not be read.')
	});
});
...
const signArchive = Effect.fn('FixtureUpdate.signArchive')(function* (archive: Uint8Array) {
	return yield* Effect.try({
		...
		catch: () => fixtureError('signature', 'The fixture archive could not be signed.')
	});
});
...
const corruptSignature = Effect.fn('FixtureUpdate.corruptSignature')(function* (signature: string) {
	return yield* Effect.try({
		...
		catch: () => fixtureError('signature', 'The fixture signature could not be corrupted.')
	});
});

const serveFixture = (version: string, archive: Uint8Array, signature: string) =>
	Effect.acquireRelease(
		Effect.callback<Server, NativeUpdateFixtureError>((resume) => {
			...
			const onError = () =>
				resume(Effect.fail(fixtureError('transport', 'The loopback fixture could not start.')));
...
const manifestFrom = Effect.fn('FixtureUpdate.fetchManifest')(function* (baseUrl: string) {
	const source = yield* Effect.tryPromise({
		...
		catch: () => fixtureError('transport', 'The fixture manifest could not be read.')
	});
	const manifest = yield* Schema.decodeUnknownEffect(Schema.fromJsonString(FixtureManifest), {
		errors: 'all',
		onExcessProperty: 'error'
	})(source).pipe(
		Effect.mapError(() => fixtureError('manifest', 'The fixture manifest is invalid.'))
	);
	const archiveUrl = yield* Effect.try({
		try: () => new URL(manifest.archiveUrl),
		catch: () => fixtureError('manifest', 'The fixture archive URL is invalid.')
	});
...
const fetchArchive = Effect.fn('FixtureUpdate.fetchArchive')((url: string) =>
	Effect.tryPromise({
		...
		catch: () => fixtureError('transport', 'The fixture archive could not be read.')
	})
);
...
const installFixture = Effect.fn('FixtureUpdate.install')(function* (
	...
	yield* validateUpdaterEvidence({
		...
		Effect.mapError(() => fixtureError('signature', 'The fixture updater signature is invalid.'))
	);
	const inventory = yield* runCommand(['tar', '-tzf', archivePath], 'install');
	yield* validateUpdaterArchiveEntries(inventory).pipe(
		Effect.mapError(() => fixtureError('install', 'The fixture updater inventory is invalid.'))
	);
	const extractedRoot = join(downloadRoot, 'extracted');
	yield* Effect.tryPromise({
		try: () => mkdir(extractedRoot, { recursive: true }),
		catch: () => fixtureError('filesystem', 'The fixture stage could not be created.')
	});
	yield* runCommand(['tar', '-xzf', archivePath, '-C', extractedRoot], 'install');
...
const toNativeUpdateError = (error: NativeUpdateFixtureError) =>
	NativeUpdateError.make({
		reason: error.reason === 'signature' ? 'invalid-signature' : 'install-failed',
		message:
...
const launchVersion = Effect.fn('FixtureUpdate.relaunch')(function* (installedApp: string) {
	...
	return yield* Schema.decodeUnknownEffect(Schema.fromJsonString(FixtureLaunch), {
		...
		Effect.mapError(() => fixtureError('relaunch', 'The relaunched fixture output is invalid.'))
	);
});
	...
	Effect.tryPromise({
		try: async () =>
			createHash('sha256')
				...
				.digest('hex'),
		catch: () => fixtureError('filesystem', 'A fixture file could not be hashed.')
	})
);
...
const treeDigest = Effect.fn('FixtureUpdate.treeDigest')(function* (root: string) {
	return yield* Effect.tryPromise({
		...
		catch: () => fixtureError('filesystem', 'The fixture tree could not be hashed.')
	});
});
...
const requireMatchingDigests = Effect.fn('FixtureUpdate.requireMatchingDigests')(function* (
	...
	for (const name of canaryNames) {
		if (before.get(name) !== after.get(name)) {
			return yield* Effect.fail(fixtureError('install', 'A durable fixture canary changed.'));
		}
	}
});
