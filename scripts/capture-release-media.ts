...
const waitForServer = async (url: string, child: ChildProcess, label: string) => {
...
const waitForShell = async (page: Page) => {
	...
	await expect(page.getByRole('textbox', { name: 'Message Shaper' })).toBeEnabled();
};
...
const openPreview = async (page: Page) => {
	...
	await page.getByRole('heading', { name: 'Focused project overview' }).waitFor();
	await page.getByRole('region', { name: 'Revision decision' }).waitFor();
};

const waitForAppAgentToSettle = async (page: Page) => {
	await expect(page.getByRole('button', { name: 'Stop App Agent' })).toBeHidden();
	await expect(page.getByRole('button', { name: 'Send to App Agent' })).toBeDisabled();
};
...
const recordDemo = async (videoDirectory: string) => {
	...
	try {
		...
		const captureFrames = async (count: number) => {
			const state = resolve(frameDirectory, `state-${String(frameNumber).padStart(4, '0')}.png`);
			...
			await run(['dwebp', '-quiet', normalizedStateWebp, '-o', normalizedState]);
		...
		await page.getByRole('button', { name: 'Keep change' }).click();
		await page.getByRole('region', { name: 'Revision decision' }).waitFor({ state: 'detached' });
		await page.mouse.move(1, 1);
...
try {
	...
	console.log(`Captured Flect v0.2 release media (${heroBytes.byteLength} byte hero).`);
} finally {
