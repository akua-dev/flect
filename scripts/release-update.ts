...
const evidenceError = (message: string) => UpdaterEvidenceError.make({ message });
...
const verifyMinisign = (
	...
	const payload = algorithm === 'ED' ? createHash('blake2b512').update(archive).digest() : archive;
...
export const validateUpdaterEvidence = Effect.fn('Flect.Updater.validateEvidence')(function* (
	...
	if (publicKey === undefined || publicKey.length === 0) {
		return yield* Effect.fail(evidenceError('A public release requires a public update key.'));
	}
	if (input.privateKeyConfigured !== true) {
		return yield* Effect.fail(evidenceError('A public release requires a private signing key.'));
	}
	if (input.target !== 'darwin-aarch64') {
		return yield* Effect.fail(evidenceError('The updater target must be darwin-aarch64.'));
	}
	...
	if (input.artifactUrl !== expectedUrl) {
		return yield* Effect.fail(
			evidenceError('The updater artifact must use the fixed HTTPS release URL.')
		);
	}
	...
	if (signature.length === 0 || signature.length > 16_384) {
		return yield* Effect.fail(evidenceError('The updater signature is empty or unbounded.'));
	}
...
export const writeStaticUpdateManifest = Effect.fn('Flect.Updater.writeStaticManifest')(function* (
	...
	yield* Effect.tryPromise({
		...
		catch: () => evidenceError('The static update manifest could not be written.')
	});
	return manifest;
});
