...
const comparisonError = (message: string) => ReleaseBuildComparisonError.make({ message });

const isSignatureEnvelope = (path: string) =>
	path === 'Contents/_CodeSignature' || path.startsWith('Contents/_CodeSignature/');
...
const digestTree = (entries: ReadonlyMap<string, TreeEntry>) =>
	sha256(
		[...entries.entries()]
			...
			.map(([path, entry]) =>
				[path, entry.kind, entry.mode, entry.digest ?? '', entry.target ?? ''].join('\0')
			)
			.join('\n')
	);
...
const changedOffsets = async (first: string, second: string) => {
	const [left, right] = await Effect.runPromise(
		Effect.all([Effect.promise(() => readFile(first)), Effect.promise(() => readFile(second))], {
		...
		Effect.tryPromise({
			try: async (): Promise<ReleaseBuildComparison> => {
				const [first, second] = await Effect.runPromise(
					Effect.all(
						[Effect.promise(() => readTree(firstApp)), Effect.promise(() => readTree(secondApp))],
						{ concurrency: 'unbounded' }
					)
				);
				const allPaths = [...new Set([...first.keys(), ...second.keys()])].sort();
				const changedPaths = allPaths
					.filter((path) => JSON.stringify(first.get(path)) !== JSON.stringify(second.get(path)))
					.slice(0, 100);
					...
					await Effect.runPromise(
						Effect.forEach(
							changedPaths.flatMap((path) => {
								...
								return left?.kind === 'file' && right?.kind === 'file' ? [path] : [];
							}),
							(path) =>
								Effect.promise(() =>
									changedOffsets(join(firstApp, path), join(secondApp, path)).then(
										(offsets) => [path, offsets] as const
									)
								),
							{ concurrency: 'unbounded' }
						)
				...
				return {
					verified: changedPaths.length === 0 && firstTreeSha256 === secondTreeSha256,
					firstTreeSha256,
					secondTreeSha256,
					changedPaths,
					binaryOffsets
			...
			catch: () => comparisonError('Independent Flect builds could not be compared.')
		})
);
...
export const compareReleaseBuilds = Effect.fn('Release.compareBuilds')((
	...
	if (!validAppPath(firstApp) || !validAppPath(secondApp)) {
		return Effect.fail(comparisonError('Independent comparison requires two Flect.app bundles.'));
	}
	return Effect.acquireUseRelease(
		Effect.tryPromise({
			try: () => mkdtemp(join(tmpdir(), 'flect-release-comparison-')),
			catch: () => comparisonError('The comparison workspace could not be created.')
		}),
		(temporary) =>
			Effect.tryPromise({
				try: async () => {
					...
					return await Effect.runPromise(compareUnsignedReleaseTrees(first, second));
				},
				catch: () => comparisonError('Signed Flect builds could not be normalized and compared.')
			}),
