...
describe('release updater evidence', () => {
	...
	it('keeps development packaging explicitly updater-unavailable', async () => {
		await expect(
			Effect.runPromise(validateUpdaterEvidence({ mode: 'development', version: '0.2.0' }))
	...
	it('fails every public updater boundary closed', async () => {
		...
		await expect(Effect.runPromise(validateUpdaterEvidence(valid))).resolves.toMatchObject({
			available: true,
		...
		const failures = [
			...
			[{ ...valid, artifactUrl: 'http://example.test/Flect.app.tar.gz' }, 'fixed HTTPS'],
			[{ ...valid, archivePath: join(temporary, 'missing.tar.gz') }, 'archive is missing'],
			[{ ...valid, signaturePath: join(temporary, 'missing.sig') }, 'signature is missing'],
			[{ ...valid, archivePath: join(temporary, 'changed.tar.gz') }, 'does not match']
		] as const;

		await writeFile(join(temporary, 'changed.tar.gz'), 'changed archive');

		for (const [input, message] of failures) {
			await expect(Effect.runPromise(validateUpdaterEvidence(input))).rejects.toMatchObject({
	...
	it('accepts only one bounded Flect app inventory', async () => {
		...
		for (const invalid of [
			...
			await expect(Effect.runPromise(validateUpdaterArchiveEntries(invalid))).rejects.toMatchObject(
				{
