...
describe('generated Flect skill', () => {
	it.effect('renders concise trigger metadata and content-first role guidance', () =>
		Effect.sync(() => {
	...
	it.effect('matches the checked-in skill and detects metadata drift', () =>
		Effect.gen(function* () {
			...
			const drifted = FLECT_COMMAND_METADATA.map((command, index) =>
				index === 0 ? { ...command, summary: `${command.summary} Changed.` } : command
			);
			...
			if (Result.isFailure(result) && result.failure._tag === 'FlectSkillGenerationError') {
				assert.strictEqual(result.failure.reason, 'stale');
			}
		}).pipe(Effect.provide(PlatformLive))
	);

	it.effect('writes the same deterministic skill through Effect FileSystem', () =>
		Effect.scoped(
			Effect.gen(function* () {
				const fs = yield* FileSystem.FileSystem;
