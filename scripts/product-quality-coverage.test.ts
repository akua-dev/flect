...
it.effect('classifies every canonical product-quality criterion exactly once', () =>
	Effect.gen(function* () {
		...
		const baselinePath = yield* path.fromFileUrl(
			new URL('../docs/verification/2026-08-01-product-quality-baseline.md', import.meta.url)
		);
...
it.effect('reports missing, duplicate, and unexpected classifications', () =>
	Effect.gen(function* () {
		const contract = ['- **FQ-01.1:** First outcome.', '- **FQ-01.2:** Second outcome.'].join('\n');
		const baseline = [
			'| FQ-01.1 | proven | Evidence. |',
			'| FQ-01.1 | partial | Duplicate. |',
			'| FQ-99.1 | implemented | Unexpected. |'
		].join('\n');
		const result = yield* Effect.result(validateProductQualityCoverage(contract, baseline));

		assert.isTrue(Result.isFailure(result));
		if (Result.isFailure(result) && result.failure._tag === 'ProductQualityCoverageError') {
...
it.effect('rejects a maturity classification without evidence', () =>
	Effect.gen(function* () {
		const result = yield* Effect.result(
			validateProductQualityCoverage('- **FQ-01.1:** First outcome.', '| FQ-01.1 | proven | |')
		);
