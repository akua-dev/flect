...
const processes = async (): Promise<ReadonlyArray<ProcessRecord>> =>
	(await command(['ps', '-axo', 'pid=,ppid=,command='])).split('\n').flatMap((line) => {
...
const waitFor = async <A>(description: string, probe: () => Promise<A | undefined>) => {
...
const numberPair = (value: string) => {
	const [first, second, ...rest] = value.split(',').map((part) => Number(part.trim()));
	if (
		first === undefined ||
		second === undefined ||
		rest.length !== 0 ||
...
const textarea = (pid: number, operation: string) => textareaOperation(pid, operation);
...
const cleanup = async () => {
	...
	if (
		...
		throw new Error(`Refusing unsafe temporary cleanup target: ${temporaryRoot}`);
	}
...
try {
	if (process.platform !== 'darwin' || process.arch !== 'arm64') {
		throw new Error('Packaged macOS verification requires the supported arm64 macOS host.');
	}
	...
	await command(['/usr/libexec/PlistBuddy', '-c', `Set :CFBundleIdentifier ${bundleId}`, plist]);
	await command([
	...
	await command(['codesign', '--force', '--deep', '--sign', '-', verificationApp]);
	await command(['codesign', '--verify', '--deep', '--strict', verificationApp]);

	const executable = await realpath(join(verificationApp, 'Contents', 'MacOS', 'flect'));
	const launch = async (priorPid?: number) => {
		await command(['open', '-n', '--env', `HOME=${isolatedHome}`, verificationApp]);
		return waitFor(
			'the isolated Flect process',
			async () =>
				(await processes()).find((entry) => entry.command === executable && entry.pid !== priorPid)
					?.pid
		);
	};
	...
	const windowButtonCount = Number(
		await appleScript(`${appProcess(mainPid)} to tell front window to get count of buttons`)
	);
	...
	const sidecar = await waitFor('one private runtime child', async () => {
		const children = (await processes()).filter(
			(entry) => entry.parentPid === mainPid && entry.command.endsWith('/flect-runtime')
		);
		return children.length === 1 ? children[0] : undefined;
	});
	process.kill(sidecar.pid, 'SIGKILL');
	await waitFor('the private runtime child to stop', async () =>
		(await processes()).some((entry) => entry.pid === sidecar.pid) ? undefined : true
	);
	...
	const windowCount = Number(await appleScript(`${appProcess(mainPid)} to get count of windows`));
	if (windowCount !== 1) {
