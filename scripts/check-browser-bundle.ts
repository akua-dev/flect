...
const assetPath = (reference: string) => {
  ...
  if (!path.startsWith(`${resolve(ASSETS)}/`)) {
    fail(
      `asset reference escapes the production asset directory: ${reference}`,
    );
  }
  return path;
};
...
const initialScriptPaths = initialReferences
  .filter(
    (reference) =>
      reference.startsWith("/assets/") && reference.endsWith(".js"),
  )
  .map(assetPath);
const activationCssReference = required(
  initialReferences.find(
    (reference) =>
      reference.startsWith("/assets/") && reference.endsWith(".css"),
  ),
...
if (
  initialNames.some(
    (name) => name.startsWith("workspace-entry") && name.endsWith(".css"),
  )
) {
