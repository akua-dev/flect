...
const paths = {
	...
	sidecar: resolve(root, 'src-tauri/binaries/flect-runtime-aarch64-apple-darwin'),
	app: resolve(root, 'src-tauri/target/release/bundle/macos/Flect.app'),
	builtDmg: resolve(root, 'src-tauri/target/release/bundle/dmg/Flect_0.2.0_aarch64.dmg'),
	...
	builtUpdaterArchive: resolve(root, 'src-tauri/target/release/bundle/macos/Flect.app.tar.gz'),
...
export const desktopBuildCommand = (mode: ReleaseTrustMode): ReadonlyArray<string> =>
	mode === 'development'
...
export const validateVersionManifest = Effect.fn('Release.validateVersions')(function* (
	...
	if (
		...
		return yield* Effect.fail(packagingError('Every public Flect version must be 0.2.0.'));
	}
});
...
export const validateReleaseLayout = Effect.fn('Release.validateLayout')(function* (
	...
	yield* requireEntry(layout.app, 'directory', 'The Flect application bundle is missing.');
	yield* requireEntry(
...
const captureCommand = async (command: ReadonlyArray<string>): Promise<CommandResult> => {
...
const copyReleaseAssets = Effect.fn('Release.copyAssets')(function* (mode: ReleaseTrustMode) {
	...
	if (mode === 'public') {
		yield* Effect.tryPromise({
			...
			catch: () => packagingError('The signed Flect updater archive could not be staged.')
		});
		yield* Effect.tryPromise({
			try: () => copyFile(paths.builtUpdaterSignature, paths.updaterSignature),
			catch: () => packagingError('The Flect updater signature could not be staged.')
		});
	}
	yield* run([
		'ffmpeg',
	...
	yield* Effect.tryPromise({
		try: () => writeFile(paths.checksum, `${digest}  ${basename(paths.releaseDmg)}\n`, 'utf8'),
	...
	if (recordedDigest !== digest || recordedName !== basename(paths.releaseDmg)) {
		return yield* Effect.fail(
...
const stageUpdaterEvidence = Effect.fn('Release.stageUpdaterEvidence')(
	function* (mode: ReleaseTrustMode) {
		...
		const updater = yield* validateUpdaterEvidence({
			...
			artifactUrl: 'https://github.com/akua-dev/flect/releases/download/v0.2.0/Flect.app.tar.gz'
		});
		if (!updater.available) {
			return updater;
		}
		const archiveEntries = yield* capture(['tar', '-tzf', paths.updaterArchive]);
	...
	const walk = async (current: string): Promise<void> => {
		...
		for (const name of entries) {
			...
			if (entry.isDirectory()) {
				...
				lines.push(`symlink\0${relativePath}\0${mode}\0${await readlink(path)}`);
...
const unsignedApplicationDigest = Effect.fn('Release.unsignedApplicationDigest')(() =>
	Effect.acquireUseRelease(
		Effect.tryPromise({
			...
			catch: () => packagingError('The unsigned-content workspace could not be created.')
		}),
		(temporary) =>
			Effect.tryPromise({
				try: async () => {
					...
					const copiedRuntime = join(copiedApp, 'Contents', 'MacOS', 'flect-runtime');
					for (const signedPath of [copiedRuntime, copiedApp]) {
						const result = await captureCommand(['codesign', '--remove-signature', signedPath]);
						if (result.exitCode !== 0) {
							throw new Error('A copied release signature could not be removed.');
						}
					}
					...
					packagingError('The normalized unsigned application digest could not be created.')
			}),
...
const parseSigningKind = (details: string): ReleaseTrustEvidence['signing']['kind'] =>
...
const writeReleaseEvidence = Effect.fn('Release.writeEvidence')(function* (
	...
	const privateRuntime = join(paths.app, 'Contents', 'MacOS', 'flect-runtime');
	const [
		commit,
		status,
		tagObservation,
		...
		[
			...
			observe(['spctl', '--assess', '--type', 'execute', '--verbose=4', paths.app]),
	...
	const teamIdentifier = teamMatch === undefined || teamMatch === 'not set' ? undefined : teamMatch;
	...
	const evidence: ReleaseTrustEvidence = {
		...
		source: {
			...
			tag: tagObservation.exitCode === 0 ? tagObservation.stdout.trim() : undefined
		},
		architectures: {
			...
			Object.fromEntries(
				await Effect.runPromise(
					Effect.forEach(
						inputPaths,
						(path) =>
							Effect.promise(async () => [path, await sha256File(join(root, path))] as const),
						{ concurrency: 'unbounded' }
					)
				)
			),
			...
			const values = await Effect.runPromise(
				Effect.forEach(
					[
						...
						paths.demoMp4,
						...(updater.available
							? [paths.updaterArchive, paths.updaterSignature, paths.updaterManifest]
							: [])
					],
					(path) =>
	...
	yield* Effect.tryPromise({
		try: () => writeFile(paths.manifest, `${JSON.stringify(manifest, null, 2)}\n`),
		catch: () => packagingError('The release evidence manifest could not be written.')
	});
});
	...
	yield* Effect.acquireUseRelease(
		Effect.gen(function* () {
			const mountPoint = yield* Effect.tryPromise({
				...
				catch: () => packagingError('A DMG verification mount could not be created.')
			});
			return yield* run([
				'hdiutil',
				'attach',
				...
				Effect.tapError(() =>
					Effect.promise(() => rm(mountPoint, { recursive: true, force: true }))
				)
			);
		}),
		(mountPoint) =>
			Effect.gen(function* () {
				...
				yield* requireEntry(mountedApp, 'directory', 'The mounted DMG does not contain Flect.app.');
				yield* run(['codesign', '--verify', '--deep', '--strict', '--verbose=2', mountedApp]);
			}),
		(mountPoint) =>
			run(['hdiutil', 'detach', mountPoint]).pipe(
				Effect.catch(() => Effect.void),
				Effect.andThen(Effect.promise(() => rm(mountPoint, { recursive: true, force: true })))
			)
	);
});
...
export const packageRelease = Effect.gen(function* () {
	...
	yield* requireEntry(paths.demoMp4, 'file', 'The Flect release demo is missing.');
	yield* requireEntry(paths.checksum, 'file', 'The Flect DMG checksum is missing.');
	yield* writeReleaseEvidence(updater);
	yield* requireEntry(paths.manifest, 'file', 'The Flect release evidence manifest is missing.');
