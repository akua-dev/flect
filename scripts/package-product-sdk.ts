...
const makeTempDirectory = (
  ...
  Effect.acquireRelease(
    ...
    (path) =>
      Effect.promise(() => rm(path, { force: true, recursive: true })).pipe(
        Effect.orDie,
      ),
  );
...
export const packageProductSdk = Effect.fn("Flect.ProductSdk.package")(
  function* (options?: { readonly outputDirectory?: string }) {
...
export const verifyProductSdkConsumer = Effect.fn(
  "Flect.ProductSdk.verifyConsumer",
  ...
  const executed = yield* runCommand(
    ["bun", "run", "index.ts"],
    consumer,
    "consumer-run",
  );
