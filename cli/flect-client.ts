...
import {
  type ControlDescriptorError,
  readControlDescriptor,
} from "../server/control-descriptor";
import {
...
const clientError = (
  reason: FlectControlClientError["reason"],
  message: string,
...
export const makeFlectControlClientLayer = (
  options: FlectControlClientOptions = {},
) =>
  Layer.effect(
    FlectControlClient,
    Effect.gen(function* () {
      ...
      const execute = Effect.fn("Flect.ControlClient.execute")(function* (
        ...
        const response = yield* Effect.tryPromise({
          try: (signal) =>
            ...
            clientError("unavailable", "Flect local control is unavailable."),
        });
        if (response.status === 401) {
          return yield* Effect.fail(
            clientError(
              "unauthorized",
              "Flect local control authorization was revoked.",
            ),
          );
        }
        return response;
      ...
      const decode = <A, R>(
        ...
        Effect.tryPromise({
          try: () => response.json(),
            ...
            clientError(
              "invalid-response",
              "Flect returned an invalid control response.",
            ),
        }).pipe(
      ...
      const workspacePath = readControlDescriptor(options.stateDirectory).pipe(
        Effect.map(
          (descriptor) =>
            `/v1/workspaces/${encodeURIComponent(descriptor.workspaceId)}`,
        ),
      );
      ...
      const events = (after = 0) =>
        Stream.unwrap(
          workspacePath.pipe(
            Effect.flatMap((path) =>
              execute(`${path}/events?after=${Math.max(0, after)}`),
            ),
              ...
              response.ok && response.body !== null
                ? Effect.succeed(
                    Stream.fromAsyncIterable(sseData(response.body), () =>
                      clientError(
                        "invalid-response",
                        "Flect returned an invalid event stream.",
                      ),
                    ).pipe(
                      Stream.mapEffect((input) =>
                        Schema.decodeUnknownEffect(
                          ...
                          Effect.mapError(() =>
                            clientError(
                              "invalid-response",
                              "Flect returned an invalid event.",
                            ),
                          ),
                        ),
                      ),
                ...
                : Effect.fail(
                    clientError(
                      "unavailable",
                      "Flect event streaming is unavailable.",
                    ),
                  ),
            ),
          ),
      ...
      const command = Effect.fn("Flect.ControlClient.command")(function* (
        ...
        if (value.type === "enable-control") {
          return yield* Effect.fail(
            clientError(
              "unauthorized",
              "Outside clients cannot enable Flect control.",
            ),
          );
        }
        const descriptor = yield* readControlDescriptor(options.stateDirectory);
