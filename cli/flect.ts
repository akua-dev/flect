...
import { flectApplicationPathFromExecutable, makeUninstallLayer } from '../src/lib/uninstall';
...
export class FlectCliError extends Schema.TaggedErrorClass<FlectCliError>()('FlectCliError', {
...
export const makeNativeFlectGatewayLayer = (options: NativeFlectGatewayOptions = {}) => {
	...
	const integrations = makeAgentIntegrationLayer(options.configRoot ?? homedir()).pipe(
		Layer.provide(platform)
	);
	const publicExecutable = options.publicExecutable ?? process.env.FLECT_PUBLIC_EXECUTABLE;
	...
	const uninstall = makeUninstallLayer({ applicationPath }).pipe(Layer.provide(setup));
