...
const grant = (
  registry: ProductCapabilityRegistryShape,
  capabilityId: string,
) => registry.decide(referenceProductContext, capabilityId, allow);

describe("reference product adapter", () => {
  it.effect(
    "composes offline, fixed GraphQL, authenticated mutation, and event operations",
    () => {
      ...
      return Effect.gen(function* () {
        ...
        assert.deepStrictEqual(fixture.authorizationHeaders, [
          null,
          `Bearer ${secret}`,
        ]);
  ...
  it.effect(
    "keeps credentials out of public encoded values and request bodies",
    () => {
      const fixture = makeFixture();
      return Effect.gen(function* () {
        const registry = yield* ProductCapabilityRegistry;
  ...
  it.effect(
    "keeps product authorization identical across inference owners",
    () => {
      ...
      return Effect.gen(function* () {
        assert.deepStrictEqual(
          yield* run(user.layer),
          yield* run(product.layer),
        );
