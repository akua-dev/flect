...
import { makeBrokeredIncidentsProduct, type ProductBrokerRequest } from './brokered-incidents';
...
describe('@flect/product reference adopters', () => {
	it.effect('keeps authorization inputs invariant across inference owners', () =>
		Effect.gen(function* () {
			const requests: Array<string> = [];
			const record =
				(product: string) =>
				(request: { readonly operationId: string; readonly input: ProductJson }) => {
	...
	it.effect('runs and personalizes the offline board without network or auth', () =>
		Effect.gen(function* () {
			...
			const capsule = yield* reference.integration.loadRecommendedExperience.pipe(
			...
			assert.strictEqual(detached.userState.exportedSnapshotDigest, state.exportedSnapshotDigest);
		})
	);
	...
	it.effect('runs fixed browser GraphQL and ordered cancellable events', () =>
		Effect.gen(function* () {
			...
			assert.strictEqual(offline.userState.forkRevision, 'refs/heads/user/personal');
		})
	);

	it.effect('keeps broker credentials private and product denial before callback', () =>
		Effect.gen(function* () {
			...
			const defect = yield* defecting.integration.operations[0]?.execute({}).pipe(Effect.result);

			assert.isTrue(Result.isFailure(result.denied));
			assert.strictEqual(requests.length, 1);
			assert.strictEqual(reference.integration.selectedInferenceOwner, 'product');
	...
	it.effect('keeps private-share credentials inside the trusted transport closure', () =>
		Effect.gen(function* () {
