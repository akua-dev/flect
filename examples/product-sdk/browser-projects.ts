...
export interface BrowserProjectsOptions {
	...
	readonly authorize?: (request: BrowserProjectsAuthorizationRequest) => Effect.Effect<boolean>;
}
...
export const makeBrowserProjectsProduct = Effect.fn('Flect.Examples.makeBrowserProjectsProduct')(
			...
			operations: [
				{
					...
					execute: (input) =>
						decodeOperationInput(LIST, ListInput, input).pipe(
							...
							Effect.mapError(() => makeProductOperationFailure(LIST, 'request-failed')),
							Effect.flatMap((response) =>
								response.data === undefined
									? Effect.fail(makeProductOperationFailure(LIST, 'invalid-output'))
