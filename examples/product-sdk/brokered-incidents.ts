...
const manifests = [
	ProductCapabilityManifest.make({
		...
		id: READ,
		name: 'Read incidents',
		description: 'Read bounded incidents through the configured product broker.',
		operationIds: [LIST],
...
export interface BrokeredIncidentsOptions {
	...
	readonly authorize?: (request: ProductBrokerRequest) => Effect.Effect<boolean>;
}
	...
	const broker = (request: ProductBrokerRequest) =>
		options.broker(request).pipe(
			Effect.catchCause((cause) => {
				...
				return failures.length === cause.reasons.length && Schema.is(ProductOperationFailure)(error)
					? Effect.fail(error)
					: Effect.fail(makeProductOperationFailure(request.operationId, 'request-failed'));
			})
		);
