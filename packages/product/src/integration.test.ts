...
const input = (
  overrides: Partial<ProductIntegrationInput> = {},
...
describe("@flect/product integration", () => {
  it.effect("validates and brands one complete integration", () =>
    Effect.gen(function* () {
      ...
      assert.strictEqual(
        integration.metadata.descriptor.id,
  ...
  it.effect("rejects invalid metadata and cross references", () =>
    Effect.gen(function* () {
      ...
      for (const candidate of cases) {
        const result = yield* defineProductIntegration(candidate).pipe(
          Effect.result,
        );
  ...
  it.effect("sanitizes a private archive-loader defect", () =>
    Effect.gen(function* () {
      const result = yield* defineProductIntegration(
        input({
          loadRecommendedExperience: Effect.die(
            new Error("private-host-secret"),
          ),
        }),
      ).pipe(Effect.result);
  ...
  it.effect("keeps inference choice outside product authorization", () =>
    Effect.gen(function* () {
      ...
      const user = yield* defineProductIntegration(
        input({ operations: [authorize(operation)] }),
      );
  ...
  it.effect("represents product denial without executing transport", () =>
    Effect.gen(function* () {
      ...
      const integration = yield* defineProductIntegration(
        input({ operations: [rejecting] }),
      );
      const result = yield* integration.operations[0]
        ?.authorize({})
        .pipe(Effect.result);
