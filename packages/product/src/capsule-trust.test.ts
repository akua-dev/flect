...
const keys = () => crypto.subtle.generateKey({ name: 'Ed25519' }, true, ['sign', 'verify']);
...
describe('capsule signing and trust', () => {
	it('verifies canonical Ed25519 content without creating permission authority', async () => {
		...
		expect(evaluateCapsuleTrustPolicy(assessment, { mode: 'require-verified' })).toEqual({
			allowed: true,
			reason: 'accepted',
	...
	it('distinguishes changed bytes, invalid claims, unknown keys, and revocation', async () => {
		...
		await expect(Effect.runPromise(verifyCapsuleSignatures(signed, []))).resolves.toMatchObject({
			status: 'unknown-key'
		});
		await expect(
			Effect.runPromise(
		...
		await expect(
			Effect.runPromise(
				verifyCapsuleSignatures(signed, [{ ...activeKey, publicKey: otherPair.publicKey }])
			)
		).resolves.toMatchObject({ status: 'invalid' });
	});
	...
	it('records local fork lineage and deliberately removes upstream signatures', async () => {
		...
		const forked = await Effect.runPromise(forkCapsule(signed, { revision: 'local-2' }));
		const decoded = await Effect.runPromise(decodeCapsule(forked));
		const assessment = await Effect.runPromise(verifyCapsuleSignatures(forked, []));
		...
		expect(evaluateCapsuleTrustPolicy(assessment, { mode: 'require-verified' })).toMatchObject({
			allowed: false,
