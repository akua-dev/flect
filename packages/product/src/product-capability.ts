...
export type ProductCapabilityConfirmationPolicy = typeof ProductCapabilityConfirmationPolicy.Type;
export const ProductCapabilityDecisionStatus = Schema.Literals(['granted', 'denied', 'revoked']);
export type ProductCapabilityDecisionStatus = typeof ProductCapabilityDecisionStatus.Type;
...
export type ProductCapabilityLifecycleState = typeof ProductCapabilityLifecycleState.Type;

const TimestampMillis = Schema.Int.check(Schema.isGreaterThanOrEqualTo(0));
const DurationMillis = Schema.Int.check(Schema.isBetween({ minimum: 1, maximum: 31_536_000_000 }));
...
export class ProductCapabilityRateLimit extends Schema.Class<ProductCapabilityRateLimit>(
	...
	maxInvocations: Schema.Int.check(Schema.isBetween({ minimum: 1, maximum: 10_000 })),
	intervalMs: Schema.Int.check(Schema.isBetween({ minimum: 100, maximum: 86_400_000 }))
}) {}

export class ProductCapabilityUsage extends Schema.Class<ProductCapabilityUsage>(
	'ProductCapabilityUsage'
)({
	totalInvocations: Schema.Int.check(Schema.isBetween({ minimum: 0, maximum: 10_000 })),
	windowInvocations: Schema.Int.check(Schema.isBetween({ minimum: 0, maximum: 10_000 })),
...
export class ProductCapabilityManifest extends Schema.Class<ProductCapabilityManifest>(
	...
	Schema.Struct({
		...
		description: Schema.Trim.check(Schema.isMinLength(1), Schema.isMaxLength(240)),
...
const ProductCapabilityDecisionFields = Schema.Struct({
	...
	Schema.makeFilter((decision) => {
		...
		if (decision.confirmationPolicy === 'workspace' && decision.workspaceId === undefined) {
			issues.push({
				path: ['workspaceId'],
...
export class ProductCapabilityDecisionRecord extends Schema.Class<ProductCapabilityDecisionRecord>(
	...
	Schema.Struct({
		version: Schema.Literal(2),
		decisions: Schema.Array(ProductCapabilityDecision).check(Schema.isMaxLength(128))
	}).check(
		Schema.makeFilter(
			(record) =>
				new Set(record.decisions.map((decision) => decision.decisionId)).size ===
...
export class ProductCapabilityProjection extends Schema.Class<ProductCapabilityProjection>(
	...
	operationIds: Schema.Array(ProductOperationId).check(Schema.isMaxLength(64), Schema.isUnique()),
	resourceIds: ResourceIds,
	dataClassIds: DataClassIds,
...
export class ProductCapabilityRequestContext extends Schema.Class<ProductCapabilityRequestContext>(
	...
	Schema.Struct({
		...
		requestDigest: ProductCapabilityRequestDigest,
		revision: Schema.String.check(Schema.isMinLength(1), Schema.isMaxLength(128)),
		capabilities: Schema.Array(ProductCapabilityRequestEntry).check(Schema.isMaxLength(128))
	}).check(
		Schema.makeFilter(
			(context) =>
				new Set(context.capabilities.map((capability) => capability.capabilityId)).size ===
