...
export class ProductHttp extends Context.Service<
  ProductHttp,
  ProductHttpShape
>()("flect/ProductHttp") {}
...
const boundedBody = (
  ...
  Effect.tryPromise({
    try: async (signal) => {
      ...
      Schema.is(ProductHttpFailure)(error)
        ? error
        : failed(policy.id, "transport"),
  });
...
export const makeProductHttpLayer = (options: {
  ...
  const policies = new Map(
    options.policies.map((policy) => [policy.id, policy]),
  );
    ...
    invoke: Effect.fn("Flect.ProductHttp.invoke")((request) =>
      Effect.gen(function* () {
        ...
        const allowed = new Set(
          policy.requestHeaders.map((name) => name.toLowerCase()),
        );
