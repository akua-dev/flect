...
export class ProductGraphql extends Context.Service<ProductGraphql, ProductGraphqlShape>()(
...
const digest = (document: string, policyId: string) =>
	Effect.tryPromise({
		try: async () => {
			const value = await crypto.subtle.digest('SHA-256', encoder.encode(document));
...
const documentMatchesOperation = (document: string, policy: ProductGraphqlPolicy) => {
	const operationName = policy.operationName.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
	return new RegExp(`\\b${policy.operationType}\\s+${operationName}\\b`).test(document);
};

const httpPolicy = (policy: ProductGraphqlPolicy): ProductHttpPolicy | undefined => {
	try {
		...
		Effect.gen(function* () {
			...
			for (const candidate of options.registrations) {
				...
				if (
					...
					(yield* digest(candidate.document, policy.id)) !== policy.documentSha256
				) {
			...
			const invoke = Effect.fn('Flect.ProductGraphql.invoke')(function* (
				...
				const request = yield* Schema.decodeUnknownEffect(
					ProductGraphqlRequest,
					strict
				)(candidate).pipe(Effect.mapError(() => failure(candidate.policyId, 'invalid-variables')));
				...
				if (encoded.byteLength > registration.policy.requestBytes) {
					return yield* Effect.fail(failure(request.policyId, 'invalid-variables'));
				}
				...
				const response = yield* http
					...
					.pipe(Effect.mapError((error) => mappedHttpFailure(request.policyId, error.reason)));
				return yield* decodeResponse(request.policyId, response.status, response.body);
			});
