...
export class CapsuleFileManifest extends Schema.Class<CapsuleFileManifest>(
  "CapsuleFileManifest",
)({
  path: Path,
  sha256: Hash,
...
export class CapsuleManifest extends Schema.Class<CapsuleManifest>(
  "CapsuleManifest",
)({
  ...
  capabilities: Schema.Array(
    Schema.Struct({ id: Text(120), required: Schema.Boolean }),
  ...
  compatibility: Schema.Struct({
    ...
    platforms: Schema.Array(
      Schema.Literals(["browser", "macos", "windows", "linux"]),
...
export class InvalidCapsule extends Schema.TaggedErrorClass<InvalidCapsule>()(
  "InvalidCapsule",
  { message: Schema.String },
) {}

const invalid = (message = "The .flect capsule is invalid.") =>
  InvalidCapsule.make({ message });
...
export const hashCapsuleArchive = Effect.fn("Flect.Capsule.hashArchive")(
  (archive: Uint8Array) => sha256(archive),
);

const decodeManifestSchema = Schema.decodeUnknownEffect(
  CapsuleManifest,
  strict,
);

const decodeManifest = Effect.fn("Flect.Capsule.decodeManifest")(function* (
  input: unknown,
) {
...
const verifyExtensionPayloads = (
  ...
  const valid = (manifest.extensions ?? []).every((extension) => {
    ...
    if (
      ...
      return false;
    if (
      extension.sourceMap === undefined ||
      extension.provenance.sourceMapSha256 === undefined
    )
      return (
        extension.sourceMap === undefined &&
        extension.provenance.sourceMapSha256 === undefined
    ...
    return (
      byPath.get(extension.sourceMap)?.sha256 ===
      extension.provenance.sourceMapSha256
    );
  });
  return valid
    ? Effect.void
    : Effect.fail(
        invalid("A portable extension payload is missing or invalid."),
      );
};

export const encodeCapsule = Effect.fn("Flect.Capsule.encode")(function* (
  ...
  for (const file of sorted) {
    if (paths.has(file.path) || file.path === "flect.json")
  ...
  if (
    sorted.length === 0 ||
    sorted.length > MAX_FILES ||
    total > MAX_CAPSULE_BYTES
  )
    return yield* Effect.fail(invalid());
...
export const decodeCapsule = Effect.fn("Flect.Capsule.decode")(function* (
  ...
  for (let index = 0; index < files.length; index += 1) {
    ...
    if (
      ...
      file.contents.byteLength !== expected.bytes
    )
      return yield* Effect.fail(invalid());
    if ((yield* sha256(file.contents)) !== expected.sha256)
      return yield* Effect.fail(
        invalid("A capsule payload failed integrity verification."),
      );
  }
  yield* verifyExtensionPayloads(manifest);
