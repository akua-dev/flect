...
export class ProductEventPolicy extends Schema.Class<ProductEventPolicy>(
  "ProductEventPolicy",
)({
  version: Schema.Literal(1),
  id: ProductAdapterPolicyId,
  operationId: ProductOperationId,
  bufferCapacity: Schema.Int.check(
    Schema.isBetween({ minimum: 1, maximum: 256 }),
  ),
  eventBytes: Schema.Int.check(
    Schema.isBetween({ minimum: 1, maximum: 1024 * 1024 }),
  ),
  reconnectAttempts: Schema.Int.check(
    Schema.isBetween({ minimum: 0, maximum: 10 }),
  ),
  reconnectDelayMs: Schema.Int.check(
    Schema.isBetween({ minimum: 100, maximum: 60_000 }),
  ),
  sequenceResume: Schema.Boolean,
}) {}

export class ProductEventRequest extends Schema.Class<ProductEventRequest>(
  "ProductEventRequest",
)({
  version: Schema.Literal(1),
