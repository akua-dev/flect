...
const archiveSha256 =
  "77ef986b6e6a2cf0ee10237668552e5f2ab4c5a890142e1f110a2cb436d20359";
...
const reasons = (
  snapshot: Awaited<ReturnType<typeof runEvaluation>>,
): ReadonlyArray<string> =>
  snapshot.diagnostics.map((diagnostic) => diagnostic.reason);

const runEvaluation = async (
  input: Parameters<typeof evaluateProductAdoption>[0],
) => Effect.runPromise(evaluateProductAdoption(input));

describe("@flect/product adoption diagnostics", () => {
  it.effect(
    "returns one deterministic ready state for initial offline adoption",
    () =>
      Effect.gen(function* () {
  ...
  it.effect(
    "reports browser-direct offline and broker authentication failures",
    () =>
      Effect.gen(function* () {
        ...
        assert.deepStrictEqual(reasons(unavailable), [
  ...
  it.effect(
    "blocks incompatible host and blocked migration deterministically",
    () =>
      Effect.gen(function* () {
        const previous = yield* defineProductIntegration(makeInput());
