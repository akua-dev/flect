...
const toSource = (
  ...
  manifest: CapsuleSource["manifest"],
): CapsuleSource => ({
  manifest,
...
const canonicalUnsigned = Effect.fn("Flect.CapsuleTrust.canonicalUnsigned")(
  function* (archive: Uint8Array) {
    ...
    const unsigned = yield* encodeCapsule(
      toSource(capsule, { ...manifest, signatures: [] }),
...
export const hashCapsuleSignedContent = Effect.fn(
  "Flect.CapsuleTrust.hashSignedContent",
)((archive: Uint8Array) =>
...
export const signCapsule = Effect.fn("Flect.CapsuleTrust.sign")(function* (
  ...
  if (
    ...
    options.keyId.length > 200 ||
    !validDate(options.signedAt)
  ) {
    return yield* Effect.fail(failure("invalid-key"));
  }
  const canonical = yield* canonicalUnsigned(archive);
  if (
    canonical.capsule.manifest.signatures.some(
      (claim) => claim.keyId === options.keyId,
    )
  ) {
...
export const verifyCapsuleSignatures = Effect.fn("Flect.CapsuleTrust.verify")(
  function* (archive: Uint8Array, keys: ReadonlyArray<CapsulePublisherKey>) {
    const canonical = yield* canonicalUnsigned(archive);
    const signatures = canonical.capsule.manifest.signatures;
    if (signatures.length === 0) {
      return CapsuleSignatureAssessment.make({
        status:
          canonical.capsule.manifest.lineage?.kind === "local-fork"
            ? "locally-forked"
            : "unsigned",
        keyIds: [],
        contentSha256: canonical.contentSha256,
    ...
    return CapsuleSignatureAssessment.make({
      status:
        statusPriority.find((status) => statuses.includes(status)) ?? "invalid",
...
export const CapsuleTrustPolicy = Schema.Union([
  ...
  Schema.Struct({
    ...
    keyIds: Schema.Array(Schema.String).check(
...
export const evaluateCapsuleTrustPolicy = (
  ...
  return CapsuleTrustDecision.make({
    allowed:
      !invalid &&
      (policy.mode === "allow-unverified" || (verified && approved)),
    reason: invalid
      ? "invalid-signature"
