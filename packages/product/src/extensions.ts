...
export const PortableExtensionBinding = Schema.Literals([
  "accepted",
  "candidate",
]);
...
export class PortableExtensionResources extends Schema.Class<PortableExtensionResources>(
  ...
  inputBytes: Schema.Int.check(
    Schema.isBetween({ minimum: 1, maximum: 1024 * 1024 }),
  ),
  outputBytes: Schema.Int.check(
    Schema.isBetween({ minimum: 1, maximum: 1024 * 1024 }),
  ),
...
export class PortableExtensionPackage extends Schema.Class<PortableExtensionPackage>(
  ...
  roles: Schema.Array(PortableExtensionRole).check(
    Schema.isMinLength(1),
    Schema.isMaxLength(2),
  ),
  ...
  capabilities: Schema.Array(PortableExtensionCapabilityRequest).check(
    Schema.isMaxLength(16),
  ),
  publicInstructions: Schema.String.check(Schema.isMaxLength(4_000)),
  commands: Schema.Array(PortableExtensionContribution).check(
    Schema.isMaxLength(32),
  ),
  tools: Schema.Array(PortableExtensionContribution).check(
    Schema.isMaxLength(16),
  ),
...
export class PortableExtensionRoleState extends Schema.Class<PortableExtensionRoleState>(
  ...
  requestedCapabilities: Schema.Array(ExtensionCapability).check(
    Schema.isMaxLength(16),
  ),
  requiredCapabilities: Schema.Array(ExtensionCapability).check(
    Schema.isMaxLength(16),
  ),
  grantedCapabilities: Schema.Array(ExtensionCapability).check(
    Schema.isMaxLength(16),
  ),
...
export class PortableExtensionCatalogRecord extends Schema.Class<PortableExtensionCatalogRecord>(
  ...
  version: Schema.Literal(1),
  entries: Schema.Array(PortableExtensionRoleState).check(
...
const hasDuplicates = (values: ReadonlyArray<string>) =>
...
export const validatePortableExtensionPackage = Effect.fn(
  ...
  if (
    ...
    (decoded.sourceMap === undefined) !==
      (decoded.provenance.sourceMapSha256 === undefined)
  ) {
...
export const intersectPortableExtensionGrants = (
  ...
  if ((role !== "app" && role !== "shaper") || !extension.roles.includes(role))
    return [];
  const requested = new Set(
    extension.capabilities.map((capability) => capability.id),
  );
...
export const assessPortableExtensionUpdate = (
  ...
  const currentCapabilities = new Set(
    current.capabilities.map(
      (capability) => `${capability.id}:${String(capability.required)}`,
    ),
  );
  const expandsAuthority =
    ...
    candidate.capabilities.some(
      (capability) =>
        !currentCapabilities.has(
          `${capability.id}:${String(capability.required)}`,
        ),
    );
