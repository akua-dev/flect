...
export class ShareGitRepository extends Schema.TaggedClass<ShareGitRepository>()(
  "git",
  { commit: ObjectId },
) {}

export const ShareRepositoryReceipt = Schema.Union([
...
export class ShareMigration extends Schema.Class<ShareMigration>(
  "ShareMigration",
)({
  fromVersion: SemanticVersion,
  toVersion: SemanticVersion,
  artifactIds: Schema.Array(Identifier).check(
    Schema.isMinLength(1),
...
export class ShareManifest extends Schema.Class<ShareManifest>("ShareManifest")(
  {
    ...
    compatibility: Schema.Struct({
      flect: Text(80),
      platforms: Schema.Array(Platform).check(
        Schema.isMinLength(1),
...
export const validateShareManifest = Effect.fn("Flect.Share.validateManifest")(
  function* (input: unknown) {
    ...
    for (const artifact of manifest.artifacts) {
      const capsuleRequired =
        artifact.kind === "experience" || artifact.kind === "extension";
      const capsuleValid =
...
export const hashShareArtifactSource = Effect.fn(
  "Flect.Share.hashArtifactSource",
)(function* (
  sourceRoot: string,
  files: ReadonlyArray<{
  ...
  return yield* digestBytes(
    encoder.encode(
      JSON.stringify({ formatVersion: 1, sourceRoot: root, files: normalized }),
    ),
  );
});

export class ShareLocalSource extends Schema.TaggedClass<ShareLocalSource>()(
  "local",
  {
...
export class ShareUrlSource extends Schema.TaggedClass<ShareUrlSource>()(
  "url",
  { url: HttpsUrl },
) {}

export class ShareGitSource extends Schema.TaggedClass<ShareGitSource>()(
  "git",
  { url: HttpsUrl, commit: ObjectId },
) {}

export class SharePrivateSource extends Schema.TaggedClass<SharePrivateSource>()(
  "private",
  {
