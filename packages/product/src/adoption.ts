...
export class ProductHostFacts extends Schema.Class<ProductHostFacts>('ProductHostFacts')({
...
export class ProductUserState extends Schema.Class<ProductUserState>('ProductUserState')({
	version: Schema.Literal(1),
	productId: ProductId,
...
const ProductAdoptionSeverity = Schema.Literals(['info', 'warning', 'blocking']);
...
const diagnosticFacts = (reason: ProductAdoptionReason): DiagnosticFacts => {
	switch (reason) {
		case 'incompatible-flect':
			return {
				severity: 'blocking',
				message: 'This Flect version is not compatible with the product integration.',
				action: 'Keep the current experience or use a compatible Flect version.'
			};
		...
		case 'authentication-unavailable':
			return {
				...
				action: 'Configure the declared authentication adapter or use the product offline.'
			};
		case 'migration-blocked':
			return {
				severity: 'blocking',
				message: 'This product update cannot be applied safely.',
				action: 'Keep the current experience and ask the product team for a migration.'
			};
		...
		case 'offline':
			return {
				severity: 'warning',
				message: 'The product connection is offline; the accepted interface remains available.',
		...
		case 'detached':
			return {
				severity: 'info',
				message: 'The product connection was removed without removing user-owned work.',
		...
		case 'ready':
			return {
				...
				action: 'Open the recommended experience or continue the personal fork.'
			};
	}
};
...
export const evaluateProductAdoption = Effect.fn('Flect.ProductAdoption.evaluate')(function* (
	...
	if (
		...
		(input.connection?.productId !== undefined && input.connection.productId !== descriptor.id)
	) {
		return yield* Effect.fail(invalidIntegration());
	}
	if (input.detached) {
	...
	if (!satisfiesRange(input.host.flectVersion, descriptor.compatibility.flect)) {
	...
	if (previous !== undefined && previous.integrationVersion !== descriptor.integrationVersion) {
		...
		reasons.push(migration?.disposition === 'blocked' ? 'migration-blocked' : 'migration-required');
	}
	...
	const update =
		...
		(previous.integrationVersion !== descriptor.integrationVersion ||
			...
			previous.capsuleVersion !== input.integration.metadata.experience.capsuleVersion ||
			previous.archiveSha256 !== input.integration.metadata.experience.archiveSha256);
	if (update) {
		reasons.push('product-update');
	}
	if (previous !== undefined && previous.capabilityDigest !== input.integration.capabilityDigest) {
		reasons.push('capability-review');
	}
	if (previous !== undefined && previous.extensionDigest !== input.integration.extensionDigest) {
		reasons.push('extension-review');
	}
	if (
		(update || reasons.includes('capability-review') || reasons.includes('extension-review')) &&
		(input.userState.forkRevision !== undefined ||
			input.userState.exportedSnapshotDigest !== undefined)
	) {
