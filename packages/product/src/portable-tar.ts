...
export const encodePortableTar = Effect.fn('Flect.PortableTar.encode')(function* (
	...
	for (const entry of entries) {
		...
		const payload = new Uint8Array(
			Math.ceil(entry.contents.byteLength / PORTABLE_TAR_BLOCK_BYTES) * PORTABLE_TAR_BLOCK_BYTES
		);
...
const readText = (header: Uint8Array, start: number, length: number) =>
	Effect.try({
		try: () => decoder.decode(header.subarray(start, start + length)).replace(/\0.*$/, ''),
		catch: failure
	});
...
export const decodePortableTar = Effect.fn('Flect.PortableTar.decode')(function* (
	...
	const paths = new Set<string>();
	let ended = false;
	for (let offset = 0; offset + PORTABLE_TAR_BLOCK_BYTES <= archive.byteLength;) {
		const header = archive.subarray(offset, offset + PORTABLE_TAR_BLOCK_BYTES);
		if (header.every((byte) => byte === 0)) {
			if (
				...
				!archive
					...
					.every((byte) => byte === 0) ||
				!archive.subarray(offset + PORTABLE_TAR_BLOCK_BYTES * 2).every((byte) => byte === 0)
			) {
				return yield* Effect.fail(failure());
			}
			ended = true;
		...
		const [path, sizeText, checksumText, magic, version, prefix] = yield* Effect.all([
		...
		const paddedSize = Math.ceil(size / PORTABLE_TAR_BLOCK_BYTES) * PORTABLE_TAR_BLOCK_BYTES;
		if (!archive.subarray(offset + size, offset + paddedSize).every((byte) => byte === 0)) {
			return yield* Effect.fail(failure());
		}
		offset += paddedSize;
	}
