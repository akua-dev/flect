...
export const ProductConnectionKind = Schema.Literals([
  "offline",
  "browser-direct",
  "brokered",
]);
export type ProductConnectionKind = typeof ProductConnectionKind.Type;
export const ProductAuthenticationOwner = Schema.Literals([
  "none",
  "product",
  "host",
]);
export type ProductAuthenticationOwner = typeof ProductAuthenticationOwner.Type;
export const ProductPlatform = Schema.Literals([
  "browser",
  "macos",
  "windows",
  "linux",
...
export class ProductInferencePolicy extends Schema.Class<ProductInferencePolicy>(
  ...
  Schema.Struct({
    ...
    Schema.makeFilter(
      (policy) => policy.allowedOwners.includes(policy.defaultOwner),
...
export class ProductDescriptor extends Schema.Class<ProductDescriptor>(
  "ProductDescriptor",
)(
  Schema.Struct({
    ...
    Schema.makeFilter(
      ...
      {
        expected:
          "offline authentication owner none or connected authentication owner product/host",
      },
    ),
  ),
) {}
...
export class ProductExperienceDescriptor extends Schema.Class<ProductExperienceDescriptor>(
  ...
  appExtensionIds: Schema.Array(ExtensionId).check(
    Schema.isMaxLength(32),
    Schema.isUnique(),
  ),
  shaperExtensionIds: Schema.Array(ExtensionId).check(
    Schema.isMaxLength(32),
    Schema.isUnique(),
  ),
}) {}

export class ProductMigration extends Schema.Class<ProductMigration>(
  "ProductMigration",
)({
  version: Schema.Literal(1),
...
export class ProductIntegrationMetadata extends Schema.Class<ProductIntegrationMetadata>(
  ...
  Schema.Struct({
    ...
    capabilities: Schema.Array(
      Schema.suspend(() => ProductCapabilityManifest),
    ...
    Schema.makeFilter(
      (metadata) =>
        ...
        new Set(metadata.capabilities.map((capability) => capability.id))
          .size === metadata.capabilities.length &&
        new Set(
          metadata.migrations.map(
            (migration) => `${migration.from}->${migration.to}`,
          ),
        ).size === metadata.migrations.length,
      {
        expected:
          "matching capsule/product IDs and unique capabilities/migrations",
      },
    ),
  ),
) {}
...
export class ProductIntegrationFailure extends Schema.TaggedErrorClass<ProductIntegrationFailure>()(
  "ProductIntegrationFailure",
  {
    ...
    recovery: Schema.Literal(
      "Review the product integration and keep the current experience.",
    ),
  },
) {}
...
export interface ProductIntegrationInput {
  ...
  readonly loadRecommendedExperience: Effect.Effect<
    Uint8Array,
    ProductIntegrationFailure
  >;
}
...
export const isProductIntegration = (
  value: ProductIntegration,
): value is ProductIntegration => validatedIntegrations.has(value);
...
const digestJson = Effect.fn("Flect.ProductIntegration.digestJson")(
  (value: ProductJson) =>
    digestBytes(encoder.encode(JSON.stringify(value)), "invalid-metadata"),
);

const sanitizeArchive = (
  effect: Effect.Effect<Uint8Array, ProductIntegrationFailure>,
) =>
  effect.pipe(
...
export const defineProductIntegration = Effect.fn(
  "Flect.ProductIntegration.define",
)(function* (input: ProductIntegrationInput) {
  ...
  const validDefinitions =
    ...
    metadata.capabilities.every((capability) =>
      capability.operationIds.every((operationId) =>
        definitionsById.has(operationId),
      ),
    ) &&
    new Set(input.events.map((event) => event.policyId)).size ===
      input.events.length;
  const validMigrations =
    metadata.migrations.every(
      (migration) =>
        migration.from !== migration.to &&
        migration.to === metadata.descriptor.integrationVersion,
    ) &&
  ...
  if (
    !validDefinitions ||
    !validMigrations ||
    !metadata.descriptor.inference.allowedOwners.includes(
...
export type ProductEventStream = Stream.Stream<
  ProductEvent,
