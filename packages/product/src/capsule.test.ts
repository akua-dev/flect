...
const extensionDigest = 'a4a348b6f4e91da9c77bf69cf56383647597fdf2b7a64d71ae214a0261892537';
...
describe('portable .flect capsules', () => {
	it('encodes identical inputs byte-identically and round-trips', async () => {
		...
		expect(decoded.files.map((file) => file.path)).toEqual(['assets/theme.css', 'ui/index.html']);
	});
	...
	it.each(['../escape', '/absolute', 'ui/../../escape', '.git/config'])(
		...
		async (path) => {
			const value = source();
			await expect(
				Effect.runPromise(encodeCapsule({ ...value, files: [{ path, contents: bytes('x') }] }))
	...
	it('rejects tampered payload hashes', async () => {
		...
		tampered[at] ^= 1;
		await expect(Effect.runPromise(decodeCapsule(tampered))).rejects.toMatchObject({
			_tag: 'InvalidCapsule'
		});
	});
	...
	it('rejects unsupported format versions and trailing archive data', async () => {
		...
		await expect(Effect.runPromise(decodeCapsule(trailing))).rejects.toMatchObject({
			_tag: 'InvalidCapsule'
		});
	});
		...
		await expect(
			Effect.runPromise(
				encodeCapsule({
					...value,
					manifest,
					files: [...value.files, { path: portableExtension.bundle, contents: bytes('() => null') }]
				})
			)
		).rejects.toMatchObject({ _tag: 'InvalidCapsule' });
	});
	...
	it('rejects a portable bundle larger than the executable source boundary', async () => {
		...
		const digest = [...new Uint8Array(await crypto.subtle.digest('SHA-256', oversized))]
