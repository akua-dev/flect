...
export class ProductHttpHeader extends Schema.Class<ProductHttpHeader>('ProductHttpHeader')({
	name: HeaderName,
	value: HeaderValue
}) {}

export class ProductHttpPolicy extends Schema.Class<ProductHttpPolicy>('ProductHttpPolicy')({
	id: Identifier,
	...
	methods: Schema.Array(Schema.Literals(['GET', 'POST', 'PUT', 'PATCH', 'DELETE'])).check(
		Schema.isMinLength(1),
	...
	requestBytes: Schema.Int.check(Schema.isBetween({ minimum: 0, maximum: 8 * 1024 * 1024 })),
	responseBytes: Schema.Int.check(Schema.isBetween({ minimum: 1, maximum: 8 * 1024 * 1024 })),
	deadlineMs: Schema.Int.check(Schema.isBetween({ minimum: 100, maximum: 60_000 }))
}) {}

export class ProductHttpRequest extends Schema.Class<ProductHttpRequest>('ProductHttpRequest')({
...
export class ProductHttpResponse extends Schema.Class<ProductHttpResponse>('ProductHttpResponse')({
	version: Schema.Literal(1),
