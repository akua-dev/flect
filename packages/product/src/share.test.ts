...
describe("@flect/product share contracts", () => {
  ...
  it.effect(
    "rejects duplicate IDs, wrong roots, ambiguous capsules, and private state",
    () =>
      Effect.gen(function* () {
        const base = manifest(
          ShareEmbeddedRepository.make({
        ...
        const cases: ReadonlyArray<unknown> = [
          ...
          },
          {
            ...base,
            artifacts: [
              artifact("component", "components/card", "artifacts/card.flect"),
            ],
          },
          {
            ...base,
  ...
  it.effect(
    "strictly decodes local, URL, Git, and private sources without credentials",
    () =>
      Effect.gen(function* () {
        const sources = [
  ...
  it.effect(
    "rejects malformed identities, receipts, bounds, and migrations",
    () =>
      Effect.gen(function* () {
        const base = manifest(
          ShareEmbeddedRepository.make({
  ...
  it.effect(
    "hashes source roots deterministically and rejects escaped files",
    () =>
      Effect.gen(function* () {
        const indexFile = {
        ...
        const second = yield* hashShareArtifactSource(
          "components/card",
          files.toReversed(),
        );
