...
const ByteLimit = Schema.Int.check(Schema.isBetween({ minimum: 1, maximum: 8 * 1024 * 1024 }));
const DeadlineMillis = Schema.Int.check(Schema.isBetween({ minimum: 100, maximum: 60_000 }));
...
export class ProductGraphqlResponse extends Schema.Class<ProductGraphqlResponse>(
	...
	Schema.Struct({
		...
		Schema.makeFilter(
			(response) => response.data !== undefined || (response.errors?.length ?? 0) > 0,
