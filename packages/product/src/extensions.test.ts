...
describe('portable extension packages', () => {
	it.effect('decodes an inspectable App and Shaper package without grants', () =>
		Effect.gen(function* () {
			const decoded = yield* validatePortableExtensionPackage(portablePackage);
	...
	it.effect('rejects duplicate roles, capabilities, and contribution ids', () =>
		Effect.gen(function* () {
			...
			assert.strictEqual(duplicateCapabilities._tag, 'InvalidExtensionManifest');
			assert.strictEqual(duplicateContributions._tag, 'InvalidExtensionManifest');
		})
	);

	it.effect('rejects Guardian, ambient authority, invalid versions, and excessive resources', () =>
		Effect.gen(function* () {
			...
			for (const failure of [guardian, credential, version, resources]) {
				...
				assert.notInclude(failure.message, 'must-never-enter-extension-state');
			}
		})
	);
	...
	it('intersects grants with the package request and target role', () => {
		assert.deepStrictEqual(
			intersectPortableExtensionGrants(portablePackage, 'app', ['interface:propose']),
			['interface:propose']
		);
		assert.deepStrictEqual(
			intersectPortableExtensionGrants(portablePackage, 'guardian', ['interface:read']),
			[]
		);
	});
	...
	it('requires explicit review for authority expansion and respects pins and forks', () => {
		...
		assert.strictEqual(
			assessPortableExtensionUpdate(portablePackage, {
				...portablePackage,
				version: '1.3.0',
				capabilities: [...portablePackage.capabilities, { id: 'interface:propose', required: true }]
			}).status,
			'authority-review'
		);
		assert.strictEqual(
