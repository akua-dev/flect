...
describe("FlectControlBroker", () => {
  it.effect(
    "rotates a private grant and interrupts queued work on disable",
    () =>
      Effect.gen(function* () {
        ...
        return yield* Effect.gen(function* () {
          ...
          const waiting = yield* broker
            .nextCommand(snapshot.workspaceId)
          ...
          Effect.provide(makeControlBrokerLayer({ stateDirectory: directory })),
        );
      }),
  );

  it.effect(
    "authenticates HTTP commands and returns the exact completion",
    () =>
      Effect.gen(function* () {
        const directory = yield* Effect.promise(() =>
          mkdtemp(join(tmpdir(), "flect-broker-http-")),
        );
        return yield* Effect.gen(function* () {
          const broker = yield* FlectControlBroker;
          yield* broker.enable(snapshot);
          ...
          const request = yield* Effect.tryPromise({
            try: () =>
              fetch(
                `${descriptor.url}/v1/workspaces/${snapshot.workspaceId}/commands`,
                {
                  method: "POST",
                  headers: {
  ...
  it.effect(
    "retains every pending completion across concurrent HTTP submissions",
    () =>
      Effect.gen(function* () {
        const directory = yield* Effect.promise(() =>
        ...
        return yield* Effect.gen(function* () {
          ...
          const commands = Array.from({ length: 32 }, (_, index) =>
            concurrentCommand(index),
          );
          const worker = yield* Effect.forEach(
            commands,
            (_, index) =>
              Effect.gen(function* () {
                const candidate = yield* broker.nextCommand(
