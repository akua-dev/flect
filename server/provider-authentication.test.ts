...
const makeLayer = (options: {
  ...
  const adapter = Layer.succeed(ProviderAuthAdapter)({
    ...
    check:
      options.check ??
      (() => Effect.succeed({ type: "oauth" as const, source: "OAuth" })),
  ...
  return ProviderAuthenticationLive.pipe(
    Layer.provide(Layer.merge(adapter, promptHost)),
  );
};
...
describe("ProviderAuthentication", () => {
    ...
    Effect.gen(function* () {
      ...
      yield* Effect.gen(function* () {
        ...
        const selection = [...events].find(
          (event): event is AuthSelectionRequired =>
            event.type === "auth_selection_required",
        );
  ...
  it.effect("rejects a duplicate login and cancels the active prompt", () =>
    Effect.gen(function* () {
      ...
      yield* Effect.gen(function* () {
        ...
        yield* auth.cancel(
          AuthLoginReference.make({ loginId: selection.loginId }),
        );
  ...
  it.effect(
    "maps malformed provider option ids without changing the reply",
    () =>
      Effect.gen(function* () {
        const selected = yield* Deferred.make<string>();
  ...
  it.effect(
    "fails closed on malformed public URLs without leaking provider copy",
    () =>
      Effect.gen(function* () {
        const layer = makeLayer({
          credentials: Effect.succeed([]),
            ...
            auth
              ...
              .pipe(
                Stream.tap((event) =>
                  event.type === "auth_started"
                    ? Deferred.succeed(
                        started[index] as Deferred.Deferred<void>,
                        undefined,
                      )
                    : Effect.void,
                ),
