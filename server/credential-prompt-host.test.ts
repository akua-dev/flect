...
describe("ProtectedPromptHost", () => {
  it.effect(
    "delivers one secret directly through a no-store one-use form",
    () =>
      Effect.gen(function* () {
  ...
  it.effect("rejects wrong methods, media types, and oversized bodies", () =>
    Effect.gen(function* () {
      ...
      const methodResponse = yield* Effect.promise(() =>
        fetch(wrongMethod.url, { method: "PUT" }),
      );
  ...
  it.effect("closes and fails the waiter on cancellation or expiry", () =>
    Effect.gen(function* () {
      ...
      const cancelledValue = yield* cancelled.value.pipe(
        Effect.flip,
        Effect.forkChild,
      );
      controller.abort();
      assert.strictEqual(
        (yield* Fiber.join(cancelledValue))._tag,
        "ProviderAuthOperationFailed",
      );
      ...
      const expiredValue = yield* expired.value.pipe(
        Effect.flip,
        Effect.forkChild,
      );
      yield* TestClock.adjust("10 minutes");
      assert.strictEqual(
        (yield* Fiber.join(expiredValue))._tag,
        "ProviderAuthOperationFailed",
      );
    }).pipe(Effect.provide(ProtectedPromptHostLive)),
  );
