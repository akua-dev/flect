...
describe('Flect RPC handlers', () => {
	it.effect('serves finite runtime operations through the shared RPC group', () =>
		Effect.gen(function* () {
			...
			const sessionId = yield* client.CreateSession(SessionSelection.make({}));
			...
			assert.strictEqual(
				shaped[0]?.type === 'shape_completed' && shaped[0].document !== undefined
					? shaped[0].document.name
					: '',
				'Focused Flect'
			);
			assert.strictEqual(diagnostic.message, 'The protected launcher remains available.');
