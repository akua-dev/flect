...
const testProvider = (connected: boolean) =>
	ProviderAuthSummary.make({
		...
		...(connected ? { sourceLabel: 'Pi credential store', credentialType: 'oauth' as const } : {}),
...
const shapedDocument = (_current: InterfaceDocument, instruction = ''): InterfaceDocument =>
...
export const FlectTestRuntimeLive = Layer.effect(
	...
	Effect.gen(function* () {
		...
		const sessionSelections = yield* Ref.make<ReadonlyMap<string, SessionSelection>>(new Map());
		...
		return {
			...
			loginProvider: (request) => {
				...
				return Stream.make(
					...
					Stream.tap((event) =>
						event.type === 'auth_connected' ? Ref.set(providerConnected, true) : Effect.void
					)
				);
			},
			...
			prompt: (sessionId, text) =>
				matchesUserRequest(text, 'Fail candidate extension')
					? Stream.unwrap(
							Effect.gen(function* () {
								const selection = (yield* Ref.get(sessionSelections)).get(sessionId);
								...
								const failure = yield* runTrustedExtensionFailureProbe(fixturePath, 'app');
					...
					: matchesUserRequest(text, 'Show the Markdown showcase')
						...
						: [
							  ...
							  /^Personalize shared (?:conflict )?fork [0-9a-f]{40}$/.test(text)
							? Stream.make(
									TurnStarted.make({ type: 'turn_started' }),
									InterfaceEditRequested.make({
										type: 'interface_edit_requested',
										requestId: `edit-${crypto.randomUUID()}`,
										instruction: matchesUserRequest(text, 'Explicitly change the interface')
							...
							: Stream.unwrap(
									Effect.gen(function* () {
										...
										return Stream.make(
											...
											Stream.concat(Stream.succeed(TurnCompleted.make({ type: 'turn_completed' })))
										);
									})
								),
				...
				instruction === 'Create a candidate that will be cancelled'
					...
					: Stream.unwrap(
							Effect.gen(function* () {
								...
								const forkPersonalization = /^Personalize shared fork ([0-9a-f]{40})$/m.exec(
									instruction
								)?.[1];
								const conflictingForkPersonalization =
									/^Personalize shared conflict fork ([0-9a-f]{40})$/m.exec(instruction)?.[1];
								const shareConflict = instruction.startsWith(
									'Resolve the shared Git conflict the user explicitly opened.'
								);
								const command = instruction.includes('Create deterministic local edit')
									...
									: shareConflict
										...
										: conflictingForkPersonalization !== undefined
											...
											: forkPersonalization !== undefined
												...
												: matchesUserRequest(instruction, 'Inspect embedded Git')
													? [
															'alias git=false',
								...
								return Stream.make(
									...
									Stream.concat(Stream.succeed(ShapeCompleted.make({ type: 'shape_completed' })))
								);
							})
						),
			cancel: () => Effect.void,
