...
describe('private sidecar mode', () => {
	it.effect('uses RPC only when no private marker or arguments are present', () =>
		Effect.gen(function* () {
	...
	it.effect('selects AXI and MCP while stripping only the private marker', () =>
		Effect.gen(function* () {
			const axi = yield* selectSidecarMode(['--flect-private-mode=axi', '--json', 'inspect']);
	...
	it.effect('fails closed on unknown, missing, misplaced, or duplicate modes', () =>
		Effect.gen(function* () {
			const cases = [
				['status'],
