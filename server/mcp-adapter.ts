...
import {
  FlectCommand,
  type FlectCommand as FlectCommandType,
} from "../shared/control";
...
export function createFlectMcpServer(options: FlectMcpOptions = {}): McpServer {
  ...
  server.registerTool(
    "flect_inspect",
    {
      title: "Inspect Flect",
      description:
        "Read the complete live, non-secret Flect workspace snapshot.",
    ...
    async () => {
      try {
        return result(
          await run(
            Effect.flatMap(FlectCommandGateway, (gateway) => gateway.inspect),
          ),
        );
      } catch {
        return failure();
  ...
  server.registerTool(
    "flect_wait",
    {
      ...
      description:
        "Wait until the live workspace sequence advances beyond a known sequence.",
        ...
        const waited = await run(
          Effect.gen(function* () {
            ...
            return {
              version: 1,
              advanced:
                event !== undefined || snapshot.sequence > afterSequence,
              snapshot,
            };
          }),
        );
  ...
  server.registerTool(
    "flect_logs",
    {
      ...
      description:
        "Read bounded, redacted operation evidence from the live Flect workspace.",
      inputSchema: z.object({
    ...
    async ({ afterSequence, limit }) => {
      try {
        const logs = await run(
          Effect.flatMap(FlectCommandGateway, (gateway) => gateway.logs),
        );
        return result({
          version: 1,
...
if (import.meta.main) {
  ...
  const stateDirectory =
    stateDirectoryIndex < 0 ? undefined : argv[stateDirectoryIndex + 1];
