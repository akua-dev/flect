	...
	FlectRpcs.toLayer(
		Effect.gen(function* () {
			...
			return FlectRpcs.of({
				...
				CancelProviderAuth: (reference) => runtime.cancelProviderAuth(reference),
				...
				DiagnoseRecovery: ({ sessionId, reason }) => runtime.diagnoseRecovery(sessionId, reason),
				ControlEnable: ({ snapshot }) => mapControl(control.enable(snapshot)),
				ControlStatus: () => mapControl(control.status),
				ControlDisable: () => control.disable.pipe(Effect.asVoid, Effect.mapError(controlFailure)),
				ControlPublishSnapshot: ({ snapshot }) => mapControl(control.publishSnapshot(snapshot)),
				ControlPublishEvent: ({ event }) => mapControl(control.publishEvent(event)),
				ControlNextCommand: ({ workspaceId }) => mapControl(control.nextCommand(workspaceId)),
				ControlComplete: ({ completion }) => mapControl(control.complete(completion)),
