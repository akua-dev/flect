...
export const ProviderAuthenticationLive = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const providers = Effect.fn("Flect.Auth.providers")(function* () {
    ...
    const emitTerminal = Effect.fn("Flect.Auth.emitTerminal")(function* (
      entry: ActiveLogin,
    ...
    const cancelEntry = Effect.fn("Flect.Auth.cancelEntry")(function* (
      entry: ActiveLogin,
    ) {
    ...
    const safeNotify = Effect.fn("Flect.Auth.notify")(function* (
      entry: ActiveLogin,
      event: PiAuthEvent,
    ) {
      switch (event.type) {
              ...
              ...(event.intervalSeconds === undefined
                ...
                : {
                    intervalSeconds: Math.max(
                      0,
                      Math.min(3_600, Math.floor(event.intervalSeconds)),
                    ),
                  }),
    ...
    const prompt = Effect.fn("Flect.Auth.prompt")(function* (
      entry: ActiveLogin,
      input: PiAuthPrompt,
    ) {
      ...
      if (input.type === "select") {
        ...
        const optionMap = new Map(
          options.map((option) => [option.publicId, option.originalId]),
        );
      ...
      Stream.unwrap(
        Effect.gen(function* () {
          ...
          const entry: ActiveLogin = {
            ...
            events: yield* Queue.sliding<AuthLoginEvent>(
              MAX_BUFFERED_AUTH_EVENTS,
            ),
            pending: yield* Ref.make<PendingSelection | undefined>(undefined),
            terminal: yield* Ref.make(false),
            fiber: yield* Ref.make<Fiber.Fiber<void, never> | undefined>(
              undefined,
            ),
          };
          const claimed = yield* Ref.modify(active, (current) => {
            if (
              current.has(request.providerId) ||
              current.size >= MAX_ACTIVE_LOGINS
            ) {
          ...
          return Stream.fromQueue(entry.events).pipe(
            Stream.takeUntil(isTerminal),
          );
        }),
      );

    const reply = Effect.fn("Flect.Auth.reply")(function* (
      input: AuthSelectionReply,
    ) {
      const entries = yield* Ref.get(active);
      const entry = [...entries.values()].find(
        (candidate) => candidate.loginId === input.loginId,
      );
      const pending =
        entry === undefined ? undefined : yield* Ref.get(entry.pending);
    ...
    const cancel = Effect.fn("Flect.Auth.cancel")(function* (
      reference: AuthLoginReference,
    ) {
      const entries = yield* Ref.get(active);
      const entry = [...entries.values()].find(
    ...
    const logout = Effect.fn("Flect.Auth.logout")(function* (
      providerId: string,
    ) {
      const descriptors = yield* adapter.providers;
