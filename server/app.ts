...
class SessionPathParams extends Schema.Class<SessionPathParams>('SessionPathParams')({
...
const controlCommandsJson = HttpServerResponse.schemaJson(ControlCommandsResponse);
const encodeEventJson = Schema.encodeEffect(Schema.fromJsonString(FlectEvent));
const encodeShapeEventJson = Schema.encodeEffect(Schema.fromJsonString(ShapeEvent));
const encodeAuthEventJson = Schema.encodeEffect(Schema.fromJsonString(AuthLoginEvent));
...
const runtimeFailure = () => publicError('The local runtime could not complete this request.', 500);
...
const sessionPath = HttpRouter.schemaPathParams(SessionPathParams, strictOptions).pipe(
	Effect.option
);
...
const closeSessionRoute = HttpRouter.add(
	...
	Effect.gen(function* () {
		...
		return yield* closeJson(new CloseSessionResponse({ version: 1, status: 'closed' }));
	}).pipe(Effect.catch(() => runtimeFailure()))
);
...
const cancelRoute = HttpRouter.add(
	...
	Effect.gen(function* () {
		...
		return yield* cancelJson(new CancelResponse({ version: 1, status: 'cancelled' }));
	}).pipe(Effect.catch(() => runtimeFailure()))
);
...
const guardianRoute = HttpRouter.add(
	...
	Effect.gen(function* () {
		...
		const runtime = yield* FlectRuntime;
		const diagnostic = yield* runtime.diagnoseRecovery(path.value.sessionId, recovery.value.reason);
		return yield* guardianJson(diagnostic);
	}).pipe(
		Effect.catchTag('SessionBusy', () => publicError('The session is busy.', 409)),
	...
	return Layer.mergeAll(
		HttpRouter.add(
			...
			Effect.gen(function* () {
				...
				return yield* providerAuthJson(ProviderAuthResponse.make({ version: 1, providers }));
			}).pipe(Effect.catch(() => runtimeFailure()))
		),
		HttpRouter.add(
			'POST',
		...
		HttpRouter.add(
			...
			mutation(AuthSelectionReply, (runtime, reply) =>
				runtime.replyProviderAuth(reply).pipe(Effect.as(HttpServerResponse.empty()))
			)
		),
		HttpRouter.add(
			'POST',
			...
			mutation(AuthLoginReference, (runtime, reference) =>
				runtime.cancelProviderAuth(reference).pipe(Effect.as(HttpServerResponse.empty()))
			)
		),
		HttpRouter.add(
			'POST',
				...
				runtime
					...
					.pipe(
						Effect.flatMap((providers) =>
							providerAuthJson(ProviderAuthResponse.make({ version: 1, providers }))
						)
					)
			)
		)
	...
	return Layer.mergeAll(
		...
		HttpRouter.add(
			...
			Effect.gen(function* () {
				...
				return yield* controlAckJson(ControlAck.make({ version: 1, status: 'accepted' }));
			}).pipe(Effect.catch(() => runtimeFailure()))
		),
		HttpRouter.add(
			'POST',
			...
			protectedRoute(ControlSnapshotPublication, (broker, input) =>
				broker
					...
					.pipe(Effect.andThen(controlAckJson(ControlAck.make({ version: 1, status: 'accepted' }))))
			)
		),
		HttpRouter.add(
			'POST',
...
export const makeFlectHttpApp = (allowedOrigins: ReadonlySet<string> = defaultAllowedOrigins) =>
	Layer.mergeAll(
		runtimeRoute,
		modelsRoute,
		createSessionRoute,
