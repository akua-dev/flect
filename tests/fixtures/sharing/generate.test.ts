...
const inspectRepository = Effect.fn("Flect.TestFixture.inspectRepository")(
  function* (fixture: SharingFixture) {
      ...
      (directory) =>
        Effect.gen(function* () {
          ...
          yield* Effect.promise(() =>
            writeFile(repositoryPath, decoded.repository),
          );
          yield* run(directory, [
            "tar",
            "-xf",
    ...
    () =>
      Effect.gen(function* () {
        ...
        for (const key of [
          ...
          assert.strictEqual(
            Buffer.compare(
              Buffer.from(first[key].archive),
              Buffer.from(second[key].archive),
            ),
            0,
          );
        }
        ...
        assert.strictEqual(
          conflicting.decoded.manifest.version,
          "2.0.0-conflict",
        );
        assert.deepStrictEqual(
          allArtifacts.decoded.manifest.artifacts.map(
        ...
        assert.strictEqual(
          first.compatibleUpdate.parentCommit,
          first.initial.commit,
        );
        assert.strictEqual(
          first.conflictingUpdate.parentCommit,
          first.initial.commit,
        );
        assert.match(first.initial.descriptorCommit, /^[0-9a-f]{40}$/);
        assert.notStrictEqual(
          first.initial.descriptorCommit,
          first.initial.commit,
        );
        ...
        assert.strictEqual(
          compatible.descriptorParents,
          first.compatibleUpdate.commit,
        );
        assert.strictEqual(conflicting.payloadParents, first.initial.commit);
        assert.strictEqual(
          conflicting.descriptorParents,
          first.conflictingUpdate.commit,
        );
        assert.strictEqual(allArtifacts.payloadParents, first.initial.commit);
        assert.strictEqual(
          allArtifacts.descriptorParents,
          first.allArtifacts.commit,
        );

        assert.strictEqual(first.privateAdapter.adapterId, "fixture-vault");
        assert.strictEqual(
          first.privateAdapter.reference,
          "weather/team-alpha",
        );
        assert.notInclude(
        ...
        assert.strictEqual(
          first.publicGit.descriptorCommit,
          first.initial.descriptorCommit,
        );
        assert.include(
        ...
        assert.isTrue(
          first.publicGit.files.some((file) =>
        ...
        const malicious = yield* Effect.exit(
          decodeShareArchive(first.malicious.archive),
        );
