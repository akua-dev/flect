...
import { makeRepositoryTar, type RepositoryArchiveEntry } from '../../../src/git/repository-tar';
	...
	const [exitCode, stdout] = yield* io(
		() =>
			new Promise<readonly [number, string]>((resolve, reject) => {
				...
				child.once('close', (code) => resolve([code ?? 1, Buffer.concat(output).toString('utf8')]));
			})
	);
...
const collectEntries = Effect.fn('Flect.TestFixture.collectEntries')(function* (
	...
	const visit = Effect.fn('Flect.TestFixture.visit')(function* (
		...
		const children = yield* io(() => readdir(absolute, { withFileTypes: true }));
...
const collectRepository = Effect.fn('Flect.TestFixture.collectRepository')(function* (
	directory: string
) {
	const topLevel = yield* io(() => readdir(directory, { withFileTypes: true }));
	const worktreeDirectories = topLevel
		.filter((entry) => entry.isDirectory() && entry.name !== '.git')
		.map((entry) => entry.name)
		.toSorted((left, right) => left.localeCompare(right));
	const entries = yield* Effect.forEach(['.git/objects', ...worktreeDirectories], (target) =>
		collectEntries(directory, target)
	);
	return makeRepositoryTar(entries.flat());
});

const collectPublicGitRemote = Effect.fn('Flect.TestFixture.collectPublicGitRemote')(function* (
	directory: string,
	refs: ReadonlyArray<{
...
const writeFiles = Effect.fn('Flect.TestFixture.writeFiles')(function* (
	...
	for (const file of files) {
		yield* io(() => mkdir(dirname(join(directory, file.path)), { recursive: true }));
...
const descriptorsFor = Effect.fn('Flect.TestFixture.descriptorsFor')(function* (
	...
	return yield* Effect.forEach(sources, (source) =>
		Effect.gen(function* () {
			const contentSha256 = yield* hashShareArtifactSource(source.sourceRoot, source.files).pipe(
...
const makeManifest = Effect.fn('Flect.TestFixture.makeManifest')(function* (input: {
	readonly id: string;
	readonly name: string;
	readonly version: string;
	readonly commit: string;
...
const packageFixture = Effect.fn('Flect.TestFixture.package')(function* (
	...
	return {
		...
		descriptorCommit,
		...(input.parentCommit === undefined ? {} : { parentCommit: input.parentCommit }),
		archive
	} satisfies SharingFixture;
});

const weatherSource = (version: string, source: string): ReadonlyArray<ArtifactSource> => {
...
const makeAllArtifactSources = Effect.fn('Flect.TestFixture.makeAllArtifactSources')(function* () {
			...
			files: [
				{
					...
					contents: encoder.encode('export const weather = { label: "Clear", temperature: 21 };\n')
				}
			]
		},
		{
			...
			files: [
				{
					...
					contents: encoder.encode('{"accent":"#46666f","surface":"#f5f7f7"}\n')
				}
			]
		},
		{
			...
			files: [
				{
					...
					contents: encoder.encode('{"steps":[{"id":"refresh","action":"weather.refresh"}]}\n')
				}
			]
		},
		{
...
export const makeSharingFixtureSet = Effect.fn('Flect.TestFixture.makeSharingFixtureSet')(
	function* () {
...
export const makeInitialSharingFixture = Effect.fn('Flect.TestFixture.makeInitialSharingFixture')(
	function* () {
