...
const renderedContrastRatio = async (
	...
	page.evaluate(
		({ backgroundSelector: backgroundQuery, foregroundSelector: foregroundQuery }) => {
			const foreground = document.querySelector(foregroundQuery);
			const background = document.querySelector(backgroundQuery);
			if (!(foreground instanceof HTMLElement) || !(background instanceof HTMLElement)) {
			...
			const foregroundLuminance = luminance(toRgb(getComputedStyle(foreground).color));
...
const shapeFirstInterface = async (page: Page) => {
	...
	await expect(page.getByRole('textbox', { name: 'Message Flect' })).toHaveAttribute(
		'data-composer-identity',
		'original'
	);
	await expect(page.getByRole('textbox', { name: 'Message Flect' })).toBeEnabled();
	...
	await expect(page.locator('.role-shell')).toHaveAttribute('data-use-disabled', 'false');
};
...
test('shapes a blank workspace in one continuous live canvas', async ({ page }) => {
	...
	await expect(page.getByRole('textbox', { name: 'Message Flect' })).toBeFocused();
	await expect(page.locator('.composer')).toHaveCount(1);
});

test('keeps completed agent work quiet behind the final answer', async ({ page }) => {
	await shapeFirstInterface(page);
	...
	await expect(work.getByRole('button', { name: 'Show 2 completed steps' })).toHaveAttribute(
		'aria-expanded',
		'false'
	);
	await expect(work.locator('.work-log__entries')).toBeHidden();
	await expect(page.getByText('Change complete: Focused project overview')).toBeVisible();
	...
	const expandedLayout = await work.evaluate((region) => {
		...
		const rows = [...region.querySelectorAll<HTMLElement>('.activity-card__summary')];
		return rows.map((row) => {
...
test('gives the static home screen one clear starting action', async ({ page }) => {
	...
	await expect(page.getByRole('button', { name: 'Create with Flect' })).toBeVisible();
	await page.getByRole('button', { name: 'A calm project planner for this week' }).click();
	await expect(page.getByRole('textbox', { name: 'Message Flect' })).toHaveValue(
	...
	await expect(page.getByRole('button', { name: 'Select element' })).toHaveCount(0);

	const dragHandle = await page.locator('.activation-shell__bar').evaluate((element) => {
...
test('keeps a visible, spacious drag handle above the workspace', async ({ page }) => {
	const dragHandle = await page.locator('.window-drag-region').evaluate((element) => {
		const style = getComputedStyle(element);
		const indicator = getComputedStyle(element, '::after');
		return {
			cursor: style.cursor,
...
test('selects and directly edits the running canvas through the same agent', async ({ page }) => {
	await shapeFirstInterface(page);
	...
	await expect(page.getByRole('textbox', { name: 'Message Flect' })).toBeFocused();
	await expect(page.locator('[data-node-id="headline"]')).toHaveAttribute('data-selected', 'true');
	...
	await expect(shell).not.toHaveAttribute('data-active-revision', afterPrompt ?? '');
	await expect(page.locator('.composer')).toHaveCount(1);
	await expect(page.getByRole('region', { name: 'Import decision' })).toHaveCount(0);
});
...
test('round-trips an accepted app through a verified .flect capsule', async ({ page }) => {
	await shapeFirstInterface(page);
	...
	await expect(page.getByRole('region', { name: 'Import decision' })).toContainText(
		'Imported app ready'
	);
	await expect(page.getByRole('button', { name: 'Activate app' })).toBeFocused();
	await expect(page.getByRole('textbox', { name: 'Message Flect' })).toBeVisible();
});

test('rejects a malformed capsule without replacing the accepted app', async ({ page }) => {
	...
	await expect(page.getByText('Flect app import failed safely.', { exact: true })).toBeVisible();
	await expect(page.getByRole('region', { name: 'Import decision' })).toHaveCount(0);
...
test('reviews imported provenance and blocks unavailable required capabilities', async ({
	...
	await expect(decision.getByText('Required · Unavailable on this host')).toBeVisible();
	await expect(page.getByRole('button', { name: 'Activate app' })).toBeDisabled();
...
test('blocks a capsule outside its declared Flect compatibility range', async ({ page }) => {
	...
	await expect(decision.getByText(/incompatible with this Flect version or host/)).toBeVisible();
	await expect(page.getByRole('button', { name: 'Activate app' })).toBeDisabled();
	await page.getByRole('button', { name: 'Discard' }).click();
});

test('projects verified capsule assets into the network-denied frame', async ({ page }) => {
	...
	await expect(frame.locator('main')).toHaveCSS('background-color', 'rgb(18, 24, 38)');
	await frame.getByRole('button', { name: 'Use assets' }).click();
	await expect(frame.getByRole('button', { name: 'Assets ready' })).toBeVisible();
});

test('imports an ordinary static HTML project as a reviewable Flect app', async ({ page }) => {
	await page.getByRole('button', { name: 'Actions' }).click();
	const chooserPromise = page.waitForEvent('filechooser');
	...
	await expect(frame.getByRole('heading', { name: 'Imported portfolio' })).toBeVisible();
	await expect(frame.locator('main')).toHaveCSS('color', 'rgb(26, 71, 120)');
	await expect(frame.getByRole('img', { name: 'Portfolio mark' })).toBeVisible();
	await frame.getByRole('button', { name: 'Open work' }).click();
	await expect(frame.getByRole('button', { name: 'Work opened' })).toBeVisible();
	await expect(
		page.getByRole('region', { name: 'Import decision' }).getByText('local-user · 1.0.0')
	).toBeVisible();
	await expect(
	...
	expect(exported.files.map((file) => file.path)).not.toContain('.env.production');
	expect(
		exported.files.some((file) =>
			new TextDecoder().decode(file.contents).includes('fixture-must-not-enter')
		)
	).toBe(false);
...
test('imports a bounded source archive through the ordinary project pipeline', async ({ page }) => {
	...
	await expect(
		page.getByText('Static app packaged · 1 source files · 1 ignored. Ready to activate.')
	).toBeVisible();
	const decision = page.getByRole('region', { name: 'Import decision' });
	await expect(decision.getByText('archive-import:archive-app.zip')).toBeVisible();
	await expect(decision.getByText(/^[0-9a-f]{64}$/)).toBeVisible();
});

test('requires exact immutable identity before importing public Git source', async ({ page }) => {
	await page.getByRole('button', { name: 'Actions' }).click();
	...
	await dialog.getByLabel('Public HTTPS repository').fill('https://example.test/project.git');
	await dialog.getByLabel('Exact commit ID').fill('main');
	await expect(dialog.getByRole('button', { name: 'Import exact commit' })).toBeDisabled();
	await dialog.getByRole('button', { name: 'Cancel' }).click();
	await expect(dialog).toHaveCount(0);
});

test('imports, builds, reviews, keeps, and exports a Vite source project', async ({ page }) => {
	await page.getByRole('button', { name: 'Actions' }).click();
	const chooserPromise = page.waitForEvent('filechooser');
	await page.getByRole('menuitem', { name: 'Import app project' }).click();
	const chooser = await chooserPromise;
	...
	await expect(
		page.getByText('Portable build verified · 3 source files. Ready to activate.')
	).toBeVisible({ timeout: 30_000 });
	const frame = page.frameLocator('iframe[title="vite-typescript"]');
	await expect(frame.getByRole('button', { name: 'Vite app ready' })).toBeVisible();
	await frame.getByRole('button', { name: 'Vite app ready' }).click();
	await expect(frame.getByRole('button', { name: 'Vite app used' })).toBeVisible();
	...
	expect(exported.manifest.entrypoints).toEqual([{ id: 'compiled-web', path: 'index.html' }]);
...
test('imports a Vite React project and rebuilds it with the registry offline', async ({
	...
	const importProject = async () => {
		...
		await expect(
			page.getByText('Portable build verified · 4 source files. Ready to activate.')
		).toBeVisible({ timeout: 45_000 });
	};

	await importProject();
	const candidate = page.frameLocator('iframe[title="vite-react"]');
	await expect(candidate.getByRole('heading', { name: 'React project imported' })).toBeVisible();
	await candidate.getByRole('button', { name: 'Use React app' }).click();
	await expect(candidate.getByRole('button', { name: 'React app used' })).toBeVisible();
	const decision = page.getByRole('region', { name: 'Import decision' });
	await expect(decision.getByText('Vite React · src/main.jsx')).toBeVisible();
	await expect(decision.getByText(/artifact [0-9a-f]{7}/)).toBeVisible();
	await expect(decision.getByText(/Locked in source Git · graph [0-9a-f]{7}/)).toBeVisible();
	await page.getByRole('button', { name: 'Activate app' }).click();
	await expect(page.getByRole('button', { name: 'Activate app' })).toHaveCount(0);
...
test('imports Vue and Svelte through the same portable workspace contract', async ({
	...
	const importFramework = async (
		...
		await expect(
			page.getByText('Portable build verified · 5 source files. Ready to activate.')
		...
		await expect(frame.getByRole('button', { name: `${framework} count 1` })).toBeVisible();
		await expect(
			page.getByRole('region', { name: 'Import decision' }).getByText(`${framework} · src/main.js`)
		).toBeVisible();
		await page.getByRole('button', { name: 'Activate app' }).click();
		await expect(page.getByRole('button', { name: 'Activate app' })).toHaveCount(0);
	};
...
test('downloads a capsule URL without credentials and opens verified review', async ({ page }) => {
	const archive = await assetFixtureArchive();
	...
	await page.getByRole('button', { name: 'Actions' }).click();
	await page.getByRole('menuitem', { name: 'Install Flect app from URL' }).click();
	...
	await expect(dialog).toHaveCount(0);
	await expect(
		page.frameLocator('iframe[title="Asset fixture"]').getByRole('button', { name: 'Use assets' })
	).toBeVisible();
	await expect(
		page.getByRole('region', { name: 'Import decision' }).getByText('fixture · 1.0.0')
	).toBeVisible();
	expect(credentialsHeader).toBeUndefined();
});

test('compares an installed capsule update and never overwrites it silently', async ({ page }) => {
	...
	await expect(page.getByRole('button', { name: 'Activate app' })).toHaveCount(0);
	await expect(
		page.frameLocator('iframe[title="Asset fixture"]').getByText('Version 1.0.0')
	).toBeVisible();

	await install('https://capsules.example/v2.flect');
	await expect(page.getByText('Review update 1.0.0 → 1.1.0')).toBeVisible();
	await expect(
		page.getByText('The installed app stays active until you explicitly activate this version.')
	).toBeVisible();
	await page.getByRole('button', { name: 'Discard' }).click();
	await expect(
		page.frameLocator('iframe[title="Asset fixture"]').getByText('Version 1.0.0')
...
test('previews and accepts compiled UI only inside the isolated capsule frame', async ({
	...
	await expect(frame.getByRole('heading', { name: 'Compiled product' })).toBeVisible();
	...
	await page.reload();
	const restoredAccepted = page.frameLocator('iframe[title="Compiled fixture"]');
	await expect(restoredAccepted.getByRole('heading', { name: 'Compiled product' })).toBeVisible();
	await page.getByRole('button', { name: 'Select element' }).click();
	await restoredAccepted.getByRole('heading', { name: 'Compiled product' }).click();
	await expect(page.locator('.canvas-selection-outline')).toContainText('Compiled product');
	await expect(page.getByRole('textbox', { name: 'Message Flect' })).toBeFocused();
	...
	await expect(restoredAccepted.getByRole('button', { name: 'Used' })).toBeVisible();
...
test('restores a compiled candidate for review without replacing accepted state', async ({
	...
	await expect(page.getByRole('region', { name: 'Import decision' })).toBeVisible();
	await page.reload();
	const restored = page.frameLocator('iframe[title="Compiled fixture"]');
	await expect(restored.getByRole('heading', { name: 'Compiled product' })).toBeVisible();
	await expect(page.getByRole('region', { name: 'Import decision' })).toBeVisible();
	await page.getByRole('button', { name: 'Discard' }).click();
	await expect(page.locator('iframe[title="Compiled fixture"]')).toHaveCount(0);
});
...
test('keeps one draft and one conversation while Flect routes product and edit work', async ({
	...
	await expect
		.poll(() =>
			previewConversation.evaluate((element) => element.scrollHeight > element.clientHeight)
		)
		.toBe(true);
	...
	expect(await previewConversation.evaluate((element) => element.scrollTop)).toBeLessThan(50);

	await page.getByRole('button', { name: 'Actions' }).click();
	await page.getByRole('menuitem', { name: 'Enable trusted Pi extensions' }).click();
	...
	await failedWork.getByRole('button', { name: 'Show 1 completed step' }).click();
	...
	await expect(page.getByText('FLECT_PRIVATE_EXTENSION_FIXTURE_FAILURE')).toHaveCount(0);
	...
	await expect(page.getByRole('dialog', { name: 'Flect actions' })).toHaveCount(0);
	await expect(page.getByText('Fail candidate extension')).toBeVisible();
	await expect(page.getByRole('button', { name: 'Fix with Flect' }).last()).toBeVisible();
	await page.getByRole('button', { name: 'Actions' }).click();
	await page.getByRole('menuitem', { name: 'Disable trusted Pi extensions' }).click();
	await composer.fill('Fail candidate extension');
	await composer.press('Enter');
	await expect(
		page.getByText('Trusted Pi extensions are disabled. The corrected candidate completed safely.')
	).toHaveCount(1);
	await expect(page.locator('.composer')).toHaveCount(1);
});
...
test('authors a real landing page website into the isolated canvas without ceremony', async ({
	...
	await expect(frame.getByRole('heading', { name: 'Driftwood Coffee', level: 1 })).toBeVisible();
	...
	await expect(page.getByRole('button', { name: 'Activate app' })).toHaveCount(0);
	await expect(page.locator('.role-shell')).toHaveAttribute('data-phase', 'accepted');
	await expect(page.getByText('Change complete: Driftwood Coffee')).toBeVisible();
...
test('keeps one chronological conversation across internal routing and reload', async ({
	...
	await latestWork.getByRole('button', { name: 'Show 1 completed step' }).click();
	...
	await expect(earlierTurn.locator('.historical-turn__context .message--user')).toBeHidden();
	await expect(earlierTurn.getByText('Change complete: Focused project overview')).toBeVisible();
	await earlierTurn
		.getByRole('button', {
			name: 'Show earlier request: Create a focused project overview'
		})
		.click();
	await expect(earlierTurn.locator('.historical-turn__context .message--user')).toBeVisible();
...
test('normalizes an interrupted Shaper turn without restoring partial output', async ({ page }) => {
	const input = page.getByRole('textbox', { name: 'Message Flect' });
	await input.fill('Create a candidate that will be cancelled');
	completedShapePages.add(page);
	await input.press('Enter');
	const conversation = page.getByRole('log', { name: 'Flect conversation' });
	await expect(conversation).toContainText('Create a candidate that will be cancelled');
	await expect(page.getByRole('status', { name: 'Workbench status' })).toContainText(
		'Flect is responding'
	);
	...
	await page.reload();
	await expect(page.getByRole('textbox', { name: 'Message Flect' })).toBeEnabled();
	await expect(page.getByText('Create a candidate that will be cancelled')).toBeVisible();
	await expect(page.getByRole('region', { name: 'Import decision' })).toHaveCount(0);
});

test('rejects a stale draft writer across two real same-origin tabs', async ({ context, page }) => {
	const second = await context.newPage();
	await second.goto(page.url());
	await expect(second.getByRole('textbox', { name: 'Message Flect' })).toBeEnabled();
	await expect(second.locator('.role-shell')).toBeVisible();
	await expect(second.getByRole('status', { name: 'Workbench status' })).toContainText(
		'Flect is ready'
	);

	await page.getByRole('textbox', { name: 'Message Flect' }).fill('Newer draft from the first tab');
	await expect
		.poll(() => page.evaluate(() => localStorage.getItem('flect.role-continuity.v1')))
		.toContain('Newer draft from the first tab');

	await second
		.getByRole('textbox', { name: 'Message Flect' })
		.fill('Stale draft from the second tab');
	expect(await page.evaluate(() => localStorage.getItem('flect.role-continuity.v1'))).toContain(
		'Newer draft from the first tab'
	);
	expect(await page.evaluate(() => localStorage.getItem('flect.role-continuity.v1'))).not.toContain(
		'Stale draft from the second tab'
	);

	await second.getByRole('button', { name: 'Open recovery mode' }).click();
	await expect(
		second.getByText('Saved conversation data needs repair before it can be restored.')
	).toBeVisible();
	await second.close();
});
...
test('preserves the prior continuity record when browser quota rejects a write', async ({
	...
	await page.getByRole('textbox', { name: 'Message Flect' }).fill('Last durable draft');
	await expect
		.poll(() => page.evaluate(() => localStorage.getItem('flect.role-continuity.v1')))
		.toContain('Last durable draft');
	const durable = await page.evaluate(() => localStorage.getItem('flect.role-continuity.v1'));
	...
	await page.getByRole('button', { name: 'Open recovery mode' }).click();
	await expect(
		page.getByText('Saved conversation data needs repair before it can be restored.')
	).toBeVisible();
	expect(await page.evaluate(() => localStorage.getItem('flect.role-continuity.v1'))).toBe(durable);
});

test('renders complete Markdown as a contained chat instrument', async ({ context, page }) => {
	...
	await expect(page.locator('.markdown-table')).toHaveAttribute('data-expanded', 'false');
	await expect(page.locator('.markdown-code .shiki')).toBeVisible();
	await expect(page.locator('.markdown-code')).toHaveAttribute('data-language', 'typescript');
	...
	await expect(page.locator('.markdown-table')).toHaveAttribute('data-expanded', 'true');

	await page.getByRole('button', { name: 'Copy code' }).click();
	await expect(page.getByRole('button', { name: 'Copied' })).toBeVisible();
	await page.getByRole('button', { name: 'Copy table as Markdown' }).click();
	await expect(page.locator('.markdown-table').getByRole('status')).toContainText(
		'Copied Markdown'
	);
...
test("keeps a reader's position until they choose to jump to new activity", async ({ page }) => {
	await page.setViewportSize({ width: 900, height: 620 });
	await shapeFirstInterface(page);
...
test('rolls back accepted local changes without exposing proposal ceremony', async ({ page }) => {
	await shapeFirstInterface(page);

	await page.getByRole('textbox', { name: 'Message Flect' }).fill('Create it again');
...
test('supports model search, keyboard resizing, collapse, and focus restoration', async ({
	...
	await expect
		.poll(() => page.locator('.role-shell').evaluate((shell) => shell.scrollLeft))
		.toBe(0);
	const modelPickerBox = await page.getByRole('dialog', { name: 'Choose model' }).boundingBox();
	expect(modelPickerBox).not.toBeNull();
	expect(modelPickerBox?.x ?? 0).toBeGreaterThanOrEqual(0);
	expect((modelPickerBox?.x ?? 0) + (modelPickerBox?.width ?? 0)).toBeLessThanOrEqual(
	...
	await reopen.click();
	await expect(page.getByRole('button', { name: 'Collapse agent' })).toBeFocused();
});

test('connects a Pi provider and selects reasoning entirely inside Flect', async ({ page }) => {
	await page.reload();
	await page.getByRole('button', { name: 'Model: Auto via Pi' }).click();

	await expect(page.getByRole('region', { name: 'Pi providers' })).toBeVisible();
	await expect(page.getByText('Flect browser test')).toBeVisible();
	await page.getByRole('radio', { name: 'High' }).click();
	await expect(page.getByRole('radio', { name: 'High' })).toHaveAttribute('aria-checked', 'true');

	await page.getByRole('button', { name: 'Connect' }).click();
	await expect
		.poll(async () => page.getByRole('dialog', { name: 'Choose model' }).isVisible())
		.toBe(false);
	await page.getByRole('button', { name: 'Model: Auto via Pi' }).click();
	...
	await expect(page.getByText('Active private sessions will restart.')).toBeVisible();
...
test('uses right and full-height sheets at compact breakpoints', async ({ page }) => {
	...
	await page.keyboard.press('Escape');
	await expect(page.getByRole('button', { name: 'Open Flect agent' })).toBeFocused();
...
test('keeps Settings in a foreground overlay above the running canvas', async ({ page }) => {
	await shapeFirstInterface(page);
...
test('keeps safe mode and promptless products inside the protected shell', async ({ page }) => {
	...
	await expect(page.getByRole('heading', { name: 'Read-only dashboard' })).toBeVisible();
	await expect(page.getByRole('complementary', { name: 'Flect agent' })).toBeVisible();
	await expect(page.getByRole('textbox', { name: 'Message Flect' })).toBeVisible();
	await page.getByRole('textbox', { name: 'Message Flect' }).fill('Unsent continuity to inspect');
	await expect
		.poll(() => page.evaluate(() => localStorage.getItem('flect.role-continuity.v1')))
		.toContain('Unsent continuity to inspect');

	await page.goto('/?safe=1');
	await expect(page.locator('.topbar').getByText('Recovery mode', { exact: true })).toBeVisible();
	await expect(page.getByText('Your interface is protected.')).toBeVisible();
	await expect(page.getByRole('textbox', { name: 'Message Flect' })).toBeDisabled();
	await expect(page.getByRole('textbox', { name: 'Message Flect' })).toHaveValue('');
	await expect(page.getByRole('button', { name: 'Restore working interface' })).toBeVisible();
	...
	await page.getByRole('button', { name: 'Discard saved conversation' }).click();
	await expect
		.poll(() => page.evaluate(() => localStorage.getItem('flect.role-continuity.v1')))
		.toBeNull();
	...
	await expect.poll(() => new URL(page.url()).searchParams.has('safe')).toBe(false);
	await page.reload();
	await expect(page.getByText('Your interface is protected.')).toHaveCount(0);
});
...
test('supports keyboard shaping and reduced motion', async ({ page }) => {
	...
	await page.reload();
	await expect(page.locator('.role-shell')).toHaveAttribute('data-reduced-motion', 'true');
...
test('lets an outside agent drive the same reactive workspace through flect', async ({ page }) => {
	await page.getByRole('button', { name: 'Open settings' }).click();
	await page.getByRole('button', { name: 'Enable local control' }).click();
	await expect(page.getByRole('button', { name: 'Disable local control' })).toBeVisible();
	await page.getByRole('button', { name: 'Close settings' }).click();
	...
	await page.getByRole('button', { name: 'Open settings' }).click();
	await expect(page.getByRole('button', { name: 'Enable local control' })).toBeVisible();
	await page.reload();
	await expect(page.locator('.topbar .safe-mode')).toHaveCount(0);
	await expect(page.getByRole('heading', { name: 'What do you need?' })).toBeVisible();
});
