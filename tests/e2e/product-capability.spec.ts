...
const runFlect = async (...args: ReadonlyArray<string>) => {
	try {
		...
		const output = Option.getOrUndefined(Schema.decodeUnknownOption(ProcessFailure)(cause));
		throw new Error(
...
test('drives digest-bound permission lifecycle through protected UI, controller, and public flect', async ({
	...
	await expect(decision.getByText('Required · Awaiting decision')).toBeVisible();
	await decision
		.getByRole('button', {
			name: 'This session product:projects:read'
		})
		.click();
	await expect(decision.getByText('Required · Granted · This session')).toBeVisible();
	...
	await expect
		.poll(async () => {
			const snapshot = await Effect.runPromise(
				Schema.decodeUnknownEffect(InspectProjection)(await runFlect('inspect'))
			);
	...
	const sessionPermissions = await Effect.runPromise(
		Schema.decodeUnknownEffect(PermissionListProjection)(await runFlect('permissions', 'list'))
	);
	...
	const firstInvocationFailure = await runFlectFailure('product', 'invoke', 'projects.list');
	...
	expect(
		await runFlectFailure('product', 'invoke', 'projects.list', '--input', '{"productDenied":true}')
	).toEqual(
		expect.objectContaining({
	...
	await expect(
		page.getByRole('region', { name: 'Import decision' }).getByText('Required · Awaiting decision')
	).toBeVisible();
	...
	await page
		...
		.click();
	await expect(page.getByText('Required · Granted · Always allow')).toBeVisible();
	...
	if (decisionId === undefined) throw new Error('Decision ID was not projected.');
	await runFlect('permissions', 'revoke', decisionId);
	await expect(page.getByText('Required · Revoked · Always allow')).toBeVisible();
