...
import {
  makeInitialSharingFixture,
  makeSharingFixtureSet,
} from "../fixtures/sharing/generate";
...
test("retains, previews, exports, removes, and explicitly deletes a real Git share", async ({
  ...
  await page
    .locator('input[type="file"][aria-label="Open shared file"]')
    .setInputFiles({
  ...
  await expect(
    sourceDialog.getByRole("button", { name: "Review source" }),
  ).toBeDisabled();
  await sourceDialog
    .getByRole("button", { name: "Close shared source dialog" })
    .click();
  ...
  await library
    .getByRole("button", { name: "Remove dev.flect.weather from app" })
    .click();
  await library
    .getByRole("button", { name: "Confirm remove dev.flect.weather" })
    .click();
  ...
  await verifyPortableFork(
    new Uint8Array(await readFile(retainedPath ?? "")),
    fixture.commit,
  );
  ...
  await library
    ...
    .click();
  await expect(
    library.getByText(/Browser deletion cannot be undone/),
  ).toBeVisible();
  await expect(library.getByText(fixture.commit)).toBeVisible();
  await library
    .getByRole("button", { name: "Confirm delete dev.flect.weather" })
    .click();
...
test("prepares and activates an exact fast-forward update from real Git history", async ({
  ...
  const fileInput = page.locator(
    'input[type="file"][aria-label="Open shared file"]',
  );
...
test("routes fork personalization through one composer and activates a real two-parent merge", async ({
  ...
  const path = await (await download).path();
  expect(path).not.toBeNull();
  const exported = await inspectPortableFork(
    new Uint8Array(await readFile(path ?? "")),
  );
...
test("resolves a real Git conflict with Flect and activates only the explicit resolution", async ({
  ...
  const path = await (await download).path();
  expect(path).not.toBeNull();
  const exported = await inspectResolvedWeather(
    new Uint8Array(await readFile(path ?? "")),
  );
  expect(exported.parents).toHaveLength(2);
  expect(exported.parents[0]).not.toBe(fixtures.initial.commit);
...
test("reviews a credential-free HTTPS share through the same inactive boundary", async ({
  ...
  await expect(review).toBeVisible();
  await expect(
    review.getByText("Shared file URL", { exact: true }),
  ).toBeVisible();
...
test("clones and reviews an exact public Git descriptor through wasm-git", async ({
  ...
  try {
    ...
    await dialog
      ...
      .fill("https://fixtures.flect.test/weather.git");
    await dialog
      .getByLabel("Exact commit")
      .fill(fixtures.publicGit.descriptorCommit);
    await dialog.getByRole("button", { name: "Review source" }).click();
    ...
    await expect
      .poll(
        async () =>
          (await review.isVisible()) || (await sourceError.isVisible()),
        { timeout: 30_000 },
      )
      .toBe(true);
    expect(
    ...
    await expect(
      review.getByText("Public Git revision", { exact: true }),
    ).toBeVisible();
    await review.getByText("Source and lineage details").click();
    await expect(
      review.getByText(fixtures.publicGit.descriptorCommit),
    ).toBeVisible();
    expect(requests.some((request) => request.includes("/info/refs"))).toBe(
      true,
    );
    expect(
      requests.some(
...
test("opens a host-composed private adapter without exposing its credential closure", async ({
  ...
  await dialog
    .getByLabel("Private reference")
    .fill(fixtures.privateAdapter.reference);
  await dialog.getByRole("button", { name: "Review source" }).click();

  const review = page.getByRole("region", { name: "Weather workspace" });
  await expect(review).toBeVisible();
  await expect(
    review.getByText("Private source · fixture-vault", { exact: true }),
  ).toBeVisible();
  await review.getByText("Source and lineage details").click();
  await expect(review.getByText("Opaque reference digest")).toBeVisible();
  await expect(page.locator("body")).not.toContainText(
    fixtures.privateAdapter.secretSentinel,
  );
  expect(
    await page.evaluate(() =>
