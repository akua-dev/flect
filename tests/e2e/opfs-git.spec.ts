...
test("retains, merges, conflicts, rejects, and deletes guarded shared Git history", async ({
  ...
  expect(result.mergeParents).toEqual([
    result.forkCommit,
  ...
  if (archive === null)
    throw new Error("The shared Git history download did not produce a file.");
...
test("imports an exact shared payload through disposable OPFS Git", async ({
  page,
}) => {
  test.setTimeout(120_000);
...
test("uses a persistent real Git repository without system Git", async ({
  page,
}) => {
  test.setTimeout(120_000);
  ...
  const reloaded = await Schema.decodeUnknownPromise(
    GitWorkspaceDiagnosticResult,
  )(JSON.parse((await resultNode.textContent()) ?? "{}"));
...
test("serializes competing browser checkpoints and rejects the stale writer", async ({
  ...
  const transactionResult = (target: typeof page) =>
    target.getByTestId("git-transaction-result");

  await page.goto(
    `/?git-transaction-diagnostic=1&workspace=${workspace}&value=initial`,
  );
  ...
  await expect(transactionResult(page)).toHaveAttribute(
    "data-state",
    "complete",
  );
  const initial = await Schema.decodeUnknownPromise(
    GitTransactionDiagnosticResult,
