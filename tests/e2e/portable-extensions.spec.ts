...
const acceptedExtensionSummary = (page: Page) =>
	page.locator('details.capsule-review > summary').filter({ hasText: /^Portable extensions$/ });

const promptFinished = (page: Page) =>
	page.waitForEvent('requestfinished', {
		predicate: (request) =>
			request.method() === 'POST' && /\/api\/sessions\/[^/]+\/prompts$/.test(request.url())
	});
...
test('reviews, tests, activates, discovers, disables, and removes role-scoped packages', async ({
	...
	const decision = await importCapsule(page, await portableExtensionCapsule({ variant: 'noop' }));

	await expect(decision.getByText('Unsigned')).toBeVisible();
	await expect(decision.getByRole('heading', { name: 'Project guide' })).toBeVisible();
	await expect(decision.getByText('akua-dev · 1.0.0 · fixture-1.0.0-noop')).toBeVisible();
	await expect(decision.getByRole('group', { name: 'App Agent' })).toBeVisible();
	...
	await expect(app.getByRole('checkbox', { name: 'interface:read · required' })).toBeChecked();
	await expect(
		app.getByRole('checkbox', { name: 'interface:propose · optional' })
	).not.toBeChecked();
	await app.getByRole('button', { name: 'Enable for App Agent' }).click();
	await expect(app.getByRole('button', { name: 'Test for App Agent' })).toBeVisible();
	...
	await expect(page.getByRole('button', { name: 'Activate app' })).toBeEnabled();
	...
	const candidateActivity = page.locator('button[aria-label="Bash details"]').last();
	...
	await expect(page.locator('form.composer').filter({ has: previewComposer })).toHaveAttribute(
		'aria-busy',
		'false'
	);
	...
	await expect(page.locator('form.composer').filter({ has: composer })).toHaveAttribute(
		'aria-busy',
		'false'
	);
	...
	await acceptedApp.getByRole('button', { name: 'Disable for App Agent' }).click();
	await expect(acceptedApp.getByRole('button', { name: 'Enable for App Agent' })).toBeVisible();

	await acceptedApp.getByText('Version controls').click();
	await acceptedApp.getByRole('button', { name: 'Remove from App Agent' }).click();
	await acceptedApp.getByRole('button', { name: 'Confirm remove from App Agent' }).click();
	await expect(page.getByRole('group', { name: 'App Agent' })).toHaveCount(0);
	await expect(page.getByRole('group', { name: 'Shaper' })).toBeVisible();
});

test('restores an accepted portable extension capsule after reload', async ({ page }) => {
	test.setTimeout(120_000);
	...
	await expect(page.getByRole('status').filter({ hasText: 'stopped safely' })).toHaveCount(0);
	await expect(acceptedExtensionSummary(page)).toBeVisible();
});

test('keeps the accepted baseline when a pinned update fails in its worker', async ({ page }) => {
	test.setTimeout(120_000);
...
test('fails network, storage, credential, flood, loop, memory, and oversized probes closed', async ({
	...
	for (const variant of ['network', 'storage', 'credential', 'flood', 'loop', 'memory'] as const) {
		const decision = await importCapsule(
			page,
		...
		await app.getByRole('button', { name: 'Enable for App Agent' }).click();
		await app.getByRole('button', { name: 'Test for App Agent' }).click();
		await expect(app.getByRole('alert')).toContainText('The portable extension failed safely.', {
			timeout: 30_000
		});
		await expect(page.getByRole('button', { name: 'Activate app' })).toBeDisabled();
		await page.getByRole('button', { name: 'Discard' }).click();
	...
	await expect(page.getByText(/fixture-private-key|FLECT_FIXTURE_SECRET/)).toHaveCount(0);
});
