...
const openUpdate = async (page: Page) => {
	...
	await expect(page.getByRole('region', { name: 'Flect update' })).toBeVisible();
};

test('keeps browser updates explicit and non-authoritative', async ({ page }) => {
	...
	await expect(page.getByRole('button', { name: 'Restart Flect' })).toHaveCount(0);
...
test('keeps update ownership readable in the compact protected shell', async ({ page }) => {
