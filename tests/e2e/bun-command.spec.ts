...
test('runs the browser Bun command and isolated preview in production Chromium', async ({
	...
	page.on('console', (message) => {
		if (message.type() === 'error' && !message.text().includes('status of 413')) {
	...
	await expect
		.poll(
			async () => {
				...
				if (status === 'failed') {
					throw new Error(
						(await page.getByTestId('bun-error').textContent()) ?? 'Browser Bun diagnostic failed.'
					);
				}
				return status;
			},
	...
	await expect(page.getByTestId('bun-packages')).toHaveText('Installed 1 package.');
	await expect(page.getByTestId('bun-preview-url')).toHaveText('/preview/3417/');

	const preview = page.frameLocator('iframe[title="Flect preview"]');
	await expect(preview.getByTestId('preview-heading')).toHaveText('Flect preview');
