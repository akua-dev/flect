...
test("builds a portable TSX and CSS project in a disposed browser Worker", async ({
  page,
}) => {
  test.setTimeout(120_000);
  await page.goto(
    `/?build-diagnostic=1&workspace=build-${Date.now().toString(36)}`,
  );
  ...
  await expect(
    preview.getByRole("button", { name: "Flect build ready" }),
  ).toBeVisible();
  await expect(preview.getByRole("button")).toHaveCSS(
    "background-color",
    "rgb(20, 86, 230)",
  );
