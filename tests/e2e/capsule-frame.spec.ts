...
test("runs an interactive capsule through an opaque typed bridge", async ({
  page,
}) => {
  const frame = page.frameLocator('iframe[title="Flect app"]');
  await expect(
    frame.getByRole("heading", { name: "Isolated product" }),
  ).toBeVisible();
  await frame.getByRole("button", { name: "Refresh product" }).click();
  await expect(frame.getByText("1", { exact: true })).toBeVisible();
  await expect(page.getByLabel("Capsule intent")).toHaveText(
    'product.refresh:{"count":1}',
  );
  await expect(frame.getByLabel("Product result")).toHaveText(
    '{"observed":{"count":1}}',
  );
});

test("keeps an unstyled capsule readable against the dark host canvas", async ({
  page,
}) => {
...
test("denies parent, storage, Tauri, and network authority", async ({
  page,
}) => {
...
for (const trigger of ["Send malformed", "Send oversized", "Flood host"]) {
  test(`fails closed when capsule tries to ${trigger.toLowerCase()}`, async ({
    page,
  }) => {
    const frame = page.frameLocator('iframe[title="Flect app"]');
    ...
    await expect(
      page.getByRole("button", { name: "Replace capsule" }),
    ).toBeEnabled();
  });
}
