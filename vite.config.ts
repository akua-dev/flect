...
export const flectViteConfig = {
	...
	plugins: [react(), tailwindcss(), browserBundleAnalysis, immutablePreviewAssets],
	worker: {
		format: 'es'
	},
	resolve: {
		alias: [
			{
				...
				replacement: fileURLToPath(new URL('./packages/product/src/contracts.ts', import.meta.url))
			},
			{
				find: '@flect/product/host',
				replacement: fileURLToPath(new URL('./packages/product/src/host.ts', import.meta.url))
			},
			{
				find: '@flect/product',
				replacement: fileURLToPath(new URL('./packages/product/src/index.ts', import.meta.url))
			},
			{
				find: '@',
			...
			{
				find: 'node:zlib',
				replacement: fileURLToPath(new URL('./src/shell/disabled-node-zlib.ts', import.meta.url))
			}
		]
	},
	server: {
