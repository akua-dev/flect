			...
			Effect.gen(function* () {
				...
				const duplicate = yield* preview
					.register({
						runId: 'run-a',
						port: 3001,
						handler: () => Effect.succeed({ status: 200, headers: {}, body: '' })
					})
					.pipe(Effect.flip);
		...
		it.effect('releases a port while preserving the stopped response', () =>
			Effect.gen(function* () {
				...
				yield* preview.register({
					runId: 'run-stop',
					port: 3002,
					handler: () => Effect.succeed({ status: 200, headers: {}, body: 'live' })
				});
				...
				yield* preview.register({
					runId: 'run-restarted',
					port: 3002,
					handler: () => Effect.succeed({ status: 200, headers: {}, body: 'restarted' })
				});
				const restarted = yield* preview.request(request('run-restarted', 3002));
	...
	it.layer(makeBunPreviewLayer({ handlerDeadline: '2 seconds' }))((it) => {
		it.effect('returns 504 when a handler exceeds its deadline', () =>
			Effect.gen(function* () {
				...
				const fiber = yield* preview.request(request('run-slow', 3003)).pipe(Effect.forkChild);
			...
			Effect.gen(function* () {
				...
				yield* preview.register({
					...
					onTimeout: Effect.sync(() => {
						reclaimed += 1;
					}).pipe(Effect.andThen(preview.stop('run-timeout').pipe(Effect.ignore)))
				});
				...
				assert.strictEqual((yield* preview.request(request('run-timeout', 3006))).status, 503);
				...
				const healthy = yield* preview.request(request('run-healthy-after-timeout', 3006));
