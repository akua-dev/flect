...
describe('BunPreviewExecution', () => {
	it.layer(makeBunPreviewLayer())((it) => {
		it.effect('releases interrupted registration and route ownership before reuse', () =>
			Effect.gen(function* () {
					...
					releaseBunPreviewExecution({
						...
						stopRoute: (ownerRunId, ownerPort) =>
							Ref.update(route, (current) =>
								current?.runId === ownerRunId && current.port === ownerPort ? undefined : current
							),
				...
				const response = yield* preview.request(request(restartedRunId, port));
				...
				const stillRunning = yield* preview.request(request(restartedRunId, port));
