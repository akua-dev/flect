...
const TARBALL_URL = 'https://registry.flect.invalid/flect-fixture/-/flect-fixture-1.0.0.tgz';
const INTEGRITY =
...
const tarball = Uint8Array.from(atob(TARBALL_BASE64), (character) => character.charCodeAt(0));

const makePackument = (integrity: string, tarball = TARBALL_URL): Packument => ({
	name: 'flect-fixture',
...
const makeRegistryFetch =
	...
	(url) => {
		...
		return Promise.reject(new Error('Fixture registry rejected an unknown URL.'));
	};
