          ...
          if (event.type === "ready") {
            ...
            resume(
              Effect.fail(
                executionFailure(
                  "startup",
                  "The browser JavaScript runtime could not start.",
                ),
              ),
            );
          }
  ...
  evaluate: (source, cwd) =>
    Effect.tryPromise({
      try: () => runtime.eval(source, cwd === undefined ? undefined : { cwd }),
        ...
        executionFailure(
          "worker",
          "Browser JavaScript execution failed safely.",
        ),
  ...
  writeFile: (path, source) =>
    Effect.tryPromise({
      try: () => runtime.fs.writeFile(path, source),
        ...
        executionFailure(
          "worker",
          "The browser JavaScript workspace could not be prepared.",
        ),
    }),
  output: Effect.suspend(() =>
    chunks.overflow
      ? Effect.fail(
          executionFailure(
            "invalid-result",
            "Browser JavaScript output exceeded its limit.",
          ),
        )
      : Effect.succeed({
...
export const RiftyJavaScriptRuntimeFactoryLive = Layer.succeed(
  RiftyJavaScriptRuntimeFactory,
)({
...
const makeRiftyJavaScriptExecutionLayer = Layer.effect(
  ...
  Effect.gen(function* () {
          ...
          Effect.timeoutOrElse({
            ...
            orElse: () =>
              Effect.fail(
                executionFailure(
                  "deadline",
                  "Browser JavaScript execution exceeded its deadline.",
                ),
              ),
          }),
        ),
    ...
    return {
      evaluate: Effect.fn("Flect.RiftyJavaScript.evaluate")((request) =>
      ...
      runModule: Effect.fn("Flect.RiftyJavaScript.runModule")((request) =>
        execute(
          ...
          ].join("\n"),
          {
            ...
            prepare: (runtime) =>
              Effect.forEach(
                Object.entries(request.files).sort(([left], [right]) =>
