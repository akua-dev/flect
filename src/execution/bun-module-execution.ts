...
import { BunPreviewExecution, BunPreviewExecutionLive } from './bun-preview-execution';
import { loadBrowserEsbuild } from './esbuild-browser';
import { RiftyJavaScriptExecution, RiftyJavaScriptLive } from './rifty-js-runtime';
...
class BunModuleRuntime extends Context.Service<BunModuleRuntime, BunModuleRuntimeShape>()(
...
const normalizeWorkspacePath = (base: string, input: string): string | undefined => {
...
const identifierName = (value: unknown): string | undefined =>
	isAstRecord(value) && value.type === 'Identifier' && typeof value.name === 'string'
		? value.name
		: undefined;

const stringLiteral = (value: unknown): string | undefined =>
	isAstRecord(value) && value.type === 'Literal' && typeof value.value === 'string'
		? value.value
		: undefined;
...
const isBunServeMember = (value: unknown) => {
	...
	return identifierName(value.property) === 'serve' || stringLiteral(value.property) === 'serve';
};
...
const aliasedServeNames = (ast: unknown) => {
	...
	while (changed) {
		...
		visitAst(ast, (node) => {
			...
			if (identifierName(node.init) !== undefined && aliases.has(identifierName(node.init) ?? '')) {
				for (const name of names) {
					if (!aliases.has(name)) {
						aliases.add(name);
						changed = true;
...
export const detectsBunPreview = (files: Readonly<Record<string, string | Uint8Array>>) =>
...
const buildDelta = (operation: BunModuleOperation, graph: PreparedModuleGraph): WorkspaceDelta => {
	const generated = new Map(
		Object.entries(graph.files).filter(([path]) => path.startsWith(`${BUILD_ROOT}/`))
	);
	...
	for (const path of Object.keys(operation.workspace.files)
		.filter((candidate) => candidate.startsWith(`${BUILD_ROOT}/`) && !generated.has(candidate))
		.sort()) {
...
const prepareModuleGraph = async (operation: BunModuleOperation): Promise<PreparedModuleGraph> => {
	if (
	...
	const visit = async (path: string): Promise<void> => {
		...
		if (source === undefined || encoder.encode(source).byteLength > SOURCE_LIMIT) {
			throw workspaceFailure();
		}
		graph.add(path);
		...
		for (const specifier of moduleSpecifiers(transformed.code)) {
			...
			const resolved = resolveSource(operation.workspace.files, dirname(path), specifier);
	...
	for (const path of [...graph].sort()) {
		...
		for (const [specifier, target] of imports.get(path) ?? []) {
			const replacement = relativePath(dirname(builtPath), compiledPath(target));
	...
	for (const [path, source] of Object.entries(operation.workspace.files)) {
		if (path === '/workspace/package.json' || path.startsWith('/workspace/node_modules/')) {
			output[path] = source;
		}
	}
			...
			runPermit.withPermit(
				Effect.gen(function* () {
					...
					const runEffect = runtime
						...
						.pipe(
							Effect.map((output) => result(output.stdout, output.stderr, output.previewUrl)),
							Effect.timeoutOrElse({
								duration: MODULE_DEADLINE,
								orElse: () =>
									Effect.fail(
										failure('deadline', 'The Bun-compatible module exceeded its deadline.')
									)
							}),
							Effect.catchDefect(() =>
								Effect.fail(failure('execution', 'The Bun-compatible module failed safely.'))
							)
						);
					...
					return yield* Effect.raceFirst(
						...
						Deferred.await(cancel).pipe(
							Effect.flatMap(() =>
								Effect.fail(failure('cancelled', 'The Bun-compatible module was stopped.'))
							)
						)
					).pipe(
						Effect.ensuring(
							Deferred.succeed(done, undefined).pipe(Effect.andThen(Ref.set(active, undefined)))
						)
					);
				})
			);
	...
	Effect.gen(function* () {
		...
		return {
			...
			stop: preview.stop.pipe(
				Effect.mapError(() =>
					failure('preview', 'The Bun-compatible preview could not be stopped.')
				)
			)
		};
	})
).pipe(Layer.provide(Layer.merge(RiftyJavaScriptLive, BunPreviewExecutionLive)));
