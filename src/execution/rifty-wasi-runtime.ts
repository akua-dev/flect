...
const wasiFailure = (
  reason: BrowserExecutionFailed["reason"],
  message: string,
) =>
...
export const makeRiftyWasiWorkerHandle = (
  worker: WasiWorker,
): RiftyWasiWorkerHandle => ({
  run: Effect.fn("Flect.RiftyWasiWorker.run")((request) =>
    Effect.callback<WasiExecutionResult, BrowserExecutionFailed>((resume) => {
      ...
      const settle = (
        effect: Effect.Effect<WasiExecutionResult, BrowserExecutionFailed>,
      ) => {
        if (completed) {
          return;
        }
      ...
      const onError = () => {
        settle(
          Effect.fail(
            wasiFailure("worker", "Browser WASI execution failed safely."),
          ),
        );
      };
          ...
          .catch(() => {
            settle(
              Effect.fail(
                wasiFailure(
                  "invalid-result",
                  "Browser WASI execution returned an invalid result.",
                ),
              ),
            );
          });
  ...
  acquire: Effect.acquireRelease(
    Effect.try({
      try: () =>
        ...
        wasiFailure("worker", "The browser WASI Worker could not start."),
    }),
    (worker) =>
      Effect.sync(() => {
        worker.terminate();
  ...
  Effect.gen(function* () {
    ...
    return {
      run: Effect.fn("Flect.RiftyWasi.run")((request) => {
        if (
          request.module.byteLength === 0 ||
          request.module.byteLength > MAX_MODULE_BYTES
        ) {
          return Effect.fail(
            wasiFailure(
              "invalid-input",
              "The browser WASI module size is invalid.",
            ),
          );
        }
            ...
            Effect.timeoutOrElse({
              ...
              orElse: () =>
                Effect.fail(
                  wasiFailure(
                    "deadline",
                    "Browser WASI execution exceeded its deadline.",
                  ),
                ),
            }),
          ),
...
export const makeRiftyWasiTestLayer = <E>(options: {
  readonly run: (
    request: WasiExecutionRequest,
  ) => Effect.Effect<WasiExecutionResult, E>;
}) => {
