...
export class BunPreviewRequest extends Schema.Class<BunPreviewRequest>('BunPreviewRequest')({
...
export class BunPreviewResponse extends Schema.Class<BunPreviewResponse>('BunPreviewResponse')({
...
export interface BunPreviewRegistration {
	...
	) => Effect.Effect<BunPreviewResponse | typeof BunPreviewResponse.Encoded, BunCommandFailed>;
...
export const makeBunPreviewLayer = (options?: { readonly handlerDeadline?: Duration.Input }) =>
	Layer.effect(
		BunPreview,
		Effect.gen(function* () {
			const registrations = yield* Ref.make<ReadonlyMap<number, BunPreviewRegistration>>(new Map());
					...
					mutation.withPermit(
						Effect.gen(function* () {
							...
							if (
								...
								[...current.values()].some((candidate) => candidate.runId === registration.runId)
							) {
						...
						Effect.flatMap((request) =>
							Ref.get(registrations).pipe(
								Effect.flatMap((current) => {
									...
									if (registration === undefined || registration.runId !== request.runId) {
									...
									return registration.handler(request).pipe(
										Effect.timeoutOrElse({
											duration: options?.handlerDeadline ?? DEFAULT_HANDLER_DEADLINE,
											orElse: () =>
										...
										Effect.flatMap((value) =>
											Schema.decodeUnknownEffect(
												...
												Effect.orElseSucceed(() =>
													response(502, 'Preview returned an invalid response.')
												)
											)
										),
										Effect.catch(() => Effect.succeed(response(502, 'Preview handler failed.'))),
										Effect.catchDefect(() =>
