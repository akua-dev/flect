import { type Fetcher, install, RegistryClient, type ResolvedPackage } from '@riftydev/npm-client';
...
const workspacePath = (path: string) =>
	path === WORKSPACE_ROOT ||
	(/^\/workspace(?:\/(?!\.{1,2}(?:\/|$))[^/]+)+$/.test(path) && !path.includes('\0'));
...
export const canonicalPackageVfsPath = (path: string) => {
	...
	if (!isOwnedMutationPath(normalized) || (!privateCachePath && path !== normalized)) {
		throw packageFailure();
	}
	return normalized;
};
...
const guardedPackageVfs = (vfs: Vfs): Vfs => {
	return {
		...
		readFileText: (path, encoding) => vfs.readFileText(canonicalPackageVfsPath(path), encoding),
		writeFile: (path, data) => {
		...
		openReadable: (path, options) => vfs.openReadable(canonicalPackageVfsPath(path), options)
	};
};
...
const cloneWorkspace = async (workspace: BunPackageWorkspace): Promise<MemoryVfs> => {
...
const makeDelta = (source: BunPackageWorkspace, staged: Readonly<Record<string, Uint8Array>>) => {
	...
	let byteLength = 0;
	for (const path of [...new Set([...Object.keys(original), ...Object.keys(staged)])].sort()) {
...
const readManifest = async (vfs: Vfs): Promise<PackageManifest> => {
	const value: unknown = JSON.parse(await vfs.readFileText('/workspace/package.json'));
...
const writeManifest = async (vfs: Vfs, manifest: PackageManifest) => {
	await vfs.writeFile('/workspace/package.json', `${JSON.stringify(manifest, null, 2)}\n`);
};

const parsePackageSpec = (
	specifier: string
): { readonly name: string; readonly version: string } | undefined => {
	const versionAt = specifier.startsWith('@') ? specifier.lastIndexOf('@') : specifier.indexOf('@');
	...
	Effect.tryPromise({
		try: async (signal) => {
			...
			if (kind === 'add') {
				...
				if (args.length === 0 || args.some((name) => !PACKAGE_NAME.test(name))) {
					throw packageFailure();
				}
				const removed = new Set(args);
				const withoutRemoved = (record: Record<string, string>): Record<string, string> =>
					Object.fromEntries(Object.entries(record).filter(([name]) => !removed.has(name)));
				const dependencies = withoutRemoved(stringRecord(manifest.dependencies) ?? {});
				const devDependencies = withoutRemoved(stringRecord(manifest.devDependencies) ?? {});
				const optionalDependencies = withoutRemoved(
			...
			const lockfile: unknown = JSON.parse(await vfs.readFileText('/workspace/package-lock.json'));
			if (
