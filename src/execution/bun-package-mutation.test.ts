...
const applyDelta = (
  ...
  for (const change of delta.files) {
    if (change.operation === "remove") {
      ...
      files[change.path] = decoder.decode(change.content);
    }
  }
  return { files };
};
...
describe("BunPackageMutation", () => {
  it.layer(makeBunPackageMutationLayer({ fetch: fixtureRegistryFetch }))(
    (it) => {
      it.effect(
        "installs verified packages and returns only a staged delta",
        () =>
          Effect.gen(function* () {
            ...
            assert.strictEqual(
              JSON.parse(
                textAt(output.delta, "/workspace/package-lock.json") ?? "{}",
              ).lockfileVersion,
              3,
            );
            assert.isFalse(
              output.delta.files.some(({ path }) =>
                path.includes("lifecycle-script-ran"),
              ),
            );
          }),
      );
      ...
      it.effect("adds and removes a dependency through portable deltas", () =>
        Effect.gen(function* () {
          ...
          const addedManifest = JSON.parse(
            textAt(added.delta, "/workspace/package.json") ?? "{}",
          );
          assert.strictEqual(
            addedManifest.dependencies["flect-fixture"],
            "1.0.0",
          );
  ...
  it.layer(makeBunPackageMutationLayer({ fetch: badIntegrityRegistryFetch }))(
    (it) => {
