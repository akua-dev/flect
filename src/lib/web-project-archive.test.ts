...
describe('web project archive import', () => {
	...
	])('imports $name without retaining secret files', async ({ name, archive }) => {
		const result = await Effect.runPromise(importWebProjectArchive(name, archive()));
	...
	it('rejects encryption, ZIP64 markers, and unsupported archive types', async () => {
		...
		const unsupportedFailure = await Effect.runPromise(
			importWebProjectArchive('project.rar', new Uint8Array(64)).pipe(Effect.flip)
		);
