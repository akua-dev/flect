...
export const projectAgentContinuity = (
  ...
  return RoleContinuityRecord.make({
    ...
    drafts:
      candidate === undefined
        ? { ...current.drafts, candidateUse: "" }
        : current.drafts,
    app: projectMessages(agent.app),
    previewApp:
      candidate === undefined ? [] : projectMessages(agent.previewApp),
...
export const restoreAgentContinuity = (
  ...
  const candidateMatches =
    candidate !== undefined && candidate === record.candidateRevisionId;
  return AgentWorkspaceSnapshot.make({
    ...current,
    app: restoreConversation(current.app, record.app),
    previewApp: restoreConversation(
      current.previewApp,
      candidateMatches ? record.previewApp : [],
    ),
