...
const revision = (
  id: string,
  source: InterfaceRevision["source"],
...
describe("workspacePhase", () => {
  ...
  it("routes an accepted custom revision to Run mode", () => {
    expect(
      workspacePhase(snapshot(revision("revision-1", "shaper")), false),
    ).toBe("accepted");
  });
  ...
  it("keeps an unresolved preview in Edit mode", () => {
    ...
    expect(workspacePhase(snapshot(builtIn, { proposal }), false)).toBe(
      "preview",
    );
  });
  ...
  it("gives explicit and persisted safe mode precedence", () => {
    ...
    expect(
      workspacePhase(
        snapshot(revision("revision-1", "shaper"), { safeMode: true }),
        false,
      ),
    ).toBe("safe");
  });
