...
export class OperationQuery extends Schema.Class<OperationQuery>(
  "OperationQuery",
)({
...
const redactTool = (tool: ToolActivity) =>
  ToolActivity.make({
    ...
    ...(tool.resultSummary === undefined
      ? {}
      : { resultSummary: redact(tool.resultSummary) }),
    ...(tool.previewUrl === undefined
      ? {}
      : { previewUrl: redact(tool.previewUrl) }),
  });

const encodedSize = Effect.fn("Flect.OperationJournal.encodedSize")(
...
export const OperationJournalLive = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const query = Effect.fn("Flect.OperationJournal.query")(
      (filter: OperationQuery) =>
    ...
    return {
      snapshot: SubscriptionRef.get(state).pipe(
        Effect.map((current) => current.records),
      ),
      changes: SubscriptionRef.changes(state).pipe(
        Stream.map((current) => current.records),
      ),
      append,
      query,
    };
