...
export { ShellLinkError, ShellLinkState, ShellLinkStatus } from '../../shared/setup';
...
export class ShellLink extends Context.Service<ShellLink, ShellLinkShape>()('flect/ShellLink') {}
...
const ioError = (message: string) => ShellLinkError.make({ reason: 'io', message });

const conflict = () =>
	ShellLinkError.make({
		reason: 'conflict',
		message: '~/.local/bin/flect is owned by another file or command-line link.'
	});
	...
	Layer.effect(
		...
		Effect.gen(function* () {
			...
			const status = Effect.fn('Flect.ShellLink.status')(function* () {
				...
				if (Result.isSuccess(target)) {
					...
					const exists = yield* fs
						.exists(link)
						.pipe(Effect.mapError(() => ioError('Flect could not inspect the command-line link.')));
			...
			const install = Effect.fn('Flect.ShellLink.install')(function* () {
				...
				const temporary = path.join(parent, `.flect-link-${crypto.randomUUID()}.tmp`);
				...
				yield* fs.makeDirectory(parent, { recursive: true, mode: 0o700 }).pipe(
					...
					Effect.mapError(() => ioError('Flect could not install the command-line link.'))
				);
			...
			const remove = Effect.fn('Flect.ShellLink.remove')(function* () {
				...
				yield* fs
					.remove(link)
					.pipe(Effect.mapError(() => ioError('Flect could not remove the command-line link.')));
				return ShellLinkStatus.make({
					state: 'absent',
					path: link,
					changed: true
