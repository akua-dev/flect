...
const decodeStoredPreferences = Effect.fn(
  "Flect.ShellPreferences.decodeStored",
)(function* (raw: string) {
...
export class ShellPreferences extends Context.Service<
  ShellPreferences,
  ShellPreferencesShape
  ...
  Effect.gen(function* () {
    ...
    const load = Effect.fn("Flect.ShellPreferences.load")(() =>
      storage.read(SHELL_PREFERENCES_KEY).pipe(
        Effect.flatMap((raw) =>
          raw === null
            ? Effect.succeed(defaultShellPreferences)
            : decodeStoredPreferences(raw),
        ),
        Effect.orElseSucceed(() => defaultShellPreferences),
      ),
    );

    const save = Effect.fn("Flect.ShellPreferences.save")(
      (value: ShellPreferencesValue) =>
