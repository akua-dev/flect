...
export class OperationContext extends Schema.Class<OperationContext>(
  "OperationContext",
)({
...
export class AgentPromptOutcome extends Schema.Class<AgentPromptOutcome>(
  "AgentPromptOutcome",
)({
...
export interface AgentWorkspaceShape {
  ...
  readonly diagnoseRecovery: (
    ...
    { readonly version: 1; readonly message: string },
    AgentWorkspaceError
  >;
...
const sessionSelection = (
  ...
  SessionSelection.make({
    ...
    ...(extensions.app || extensions.shaper
...
const workspaceFieldsWithoutReasoning = (current: AgentWorkspaceSnapshot) => {
  const { reasoningLevel: _reasoningLevel, ...fields } =
...
export const AgentWorkspaceLive = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const previewSession = yield* Ref.make<SessionHandle | undefined>(
      undefined,
    );
    ...
    const proposeShaperInterface = Effect.fn(
      "Flect.AgentWorkspace.proposeShaperInterface",
    ...
    const interruptFibers = Effect.fn("Flect.AgentWorkspace.interruptFibers")(
      function* () {
    ...
    const releaseSession = Effect.fn("Flect.AgentWorkspace.releaseSession")(
      function* () {
    ...
    const releasePreviewSession = Effect.fn(
      "Flect.AgentWorkspace.releasePreviewSession",
    )(function* () {
    ...
    const ensurePreviewSession = Effect.fn(
      "Flect.AgentWorkspace.ensurePreviewSession",
    )(() =>
      sessionPermit.withPermits(1)(
        Effect.gen(function* () {
          const snapshot = yield* SubscriptionRef.get(state);
    ...
    const finishCancelledActivities = Effect.fn(
      "Flect.AgentWorkspace.finishCancelledActivities",
      ...
      yield* updateRole(role, (current) =>
        RoleConversationSnapshot.make({
          ...
          activities: current.activities.map((activity) => {
            ...
            const output =
              `${activity.output ?? ""}${cancellationOutput}`.slice(-8_000);
        ...
        switch (event.type) {
          case "tool_execution_started":
            activity = ToolActivity.make({
              ...
              ...(event.inputSummary === undefined
                ? {}
                : { resultSummary: event.inputSummary }),
            });
            break;
          ...
          case "tool_execution_completed": {
            ...
            activity = ToolActivity.make({
              ...
              phase:
                event.status === "succeeded" && !failedShellExit
                  ? "succeeded"
                  : "failed",
              updatedAt: event.completedAt,
    ...
    const executeShellRequest = Effect.fn(
      "Flect.AgentWorkspace.executeShellRequest",
    )(function* (
      operation: OperationContext,
      sessionId: string,
      slot: ConversationSlot,
    ...
    const addValidationActivity = Effect.fn(
      "Flect.AgentWorkspace.addValidationActivity",
    )(function* (
      operation: OperationContext,
      event: Extract<
        ShapeEvent,
        { readonly type: "proposal_validation_failed" }
      >,
    ) {
      const now = yield* Clock.currentTimeMillis;
      const activity = ToolActivity.make({
    ...
    const selectModel = Effect.fn("Flect.AgentWorkspace.selectModel")(
      function* (selection: ModelSelection | undefined) {
        yield* resetSession;
        yield* SubscriptionRef.update(state, (current) => {
          const selectedModel =
            selection === undefined
          ...
          return AgentWorkspaceSnapshot.make({
            ...
            app: RoleConversationSnapshot.make({
              ...requiredRoleFields(current.app),
              status:
                current.app.status === "setup-required" ||
                current.app.status === "unavailable"
                  ? current.app.status
                  : "ready",
            }),
            previewApp: RoleConversationSnapshot.make({
            ...
            shaper: RoleConversationSnapshot.make({
              ...requiredRoleFields(current.shaper),
              status:
                current.shaper.status === "setup-required" ||
                current.shaper.status === "unavailable"
    ...
    const selectReasoning = Effect.fn("Flect.AgentWorkspace.selectReasoning")(
      function* (reasoningLevel: ReasoningLevel | undefined) {
        const current = yield* SubscriptionRef.get(state);
        const model = current.selectedModel ?? current.models[0];
        if (
          reasoningLevel !== undefined &&
          !model?.reasoningLevels.includes(reasoningLevel)
        ) {
          return;
        }
    ...
    const loginProvider = Effect.fn("Flect.AgentWorkspace.loginProvider")(
      function* (request: AuthLoginRequest) {
    ...
    const replyProviderAuth = (reply: AuthSelectionReply) =>
      client.replyProviderAuth(reply);
    ...
    const logoutProvider = Effect.fn("Flect.AgentWorkspace.logoutProvider")(
    ...
    const setModelFavorite = Effect.fn("Flect.AgentWorkspace.setModelFavorite")(
      function* (selection: ModelSelection, favorite: boolean) {
        const current = yield* SubscriptionRef.get(state);
    ...
    const setExternalExtensions = Effect.fn(
      "Flect.AgentWorkspace.setExternalExtensions",
      ...
      yield* SubscriptionRef.update(state, (snapshot) =>
        AgentWorkspaceSnapshot.make({
          ...
          app: RoleConversationSnapshot.make({
            ...
            status:
              snapshot.app.status === "setup-required" ||
              snapshot.app.status === "unavailable"
                ? snapshot.app.status
                : "ready",
          }),
          previewApp: RoleConversationSnapshot.make({
    ...
    const awaitRoleFiber = Effect.fn("Flect.AgentWorkspace.awaitRoleFiber")(
      function* <A, E>(role: ConversationSlot, fiber: Fiber.Fiber<A, E>) {
    ...
    const submitAppPrompt = Effect.fn("Flect.AgentWorkspace.submitAppPrompt")(
      function* (operation: OperationContext, text: string) {
        ...
        const editRequest = yield* Ref.make<InterfaceEditRequested | undefined>(
          undefined,
        );
              ...
              switch (event.type) {
                ...
                case "busy":
                  return setOperationalStatus(
                    ...
                    Effect.andThen(
                      event.type === "error" ? releaseSession() : Effect.void,
                    ),
                  );
              }
            }),
    ...
    const submitPreviewPrompt = Effect.fn(
      "Flect.AgentWorkspace.submitPreviewPrompt",
    )(function* (
      operation: OperationContext,
      text: string,
      document: InterfaceDocument,
      ...
      const request = Effect.gen(function* () {
        ...
        yield* client
          .prompt(sessionId, previewPrompt(prompt, document, revisionId))
          .pipe(
            Stream.runForEach((event) => {
              switch (event.type) {
                case "shell_request":
                ...
                case "interface_edit_requested":
                  return Ref.update(editRequest, (current) => current ?? event);
                case "external_extension_failed":
                  return recordExternalExtensionFailure(
                    operation,
                    event,
                    "previewApp",
                    revisionId,
    ...
    const submitShaperInstruction = Effect.fn(
      "Flect.AgentWorkspace.submitShaperInstruction",
    )(function* (
      operation: OperationContext,
      instruction: string,
      document: InterfaceDocument,
    ...
    const cancel = Effect.fn("Flect.AgentWorkspace.cancel")(function* (
      role: InteractiveAgentRole,
    ) {
    ...
    const cancelPreview = Effect.fn("Flect.AgentWorkspace.cancelPreview")(
      function* () {
        ...
        yield* Effect.all(
          [
            currentSession === undefined
              ? Effect.void
              : client.cancel(currentSession.id, "app"),
    ...
    const releasePreview = Effect.fn("Flect.AgentWorkspace.releasePreview")(
      function* () {
        const active = yield* Ref.get(fibers);
    ...
    const diagnoseRecovery = Effect.fn("Flect.AgentWorkspace.diagnoseRecovery")(
      function* (reason: RecoveryReason) {
        const sessionId = yield* ensureSession();
        return yield* client
