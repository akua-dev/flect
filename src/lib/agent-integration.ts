...
export interface AgentIntegrationShape {
  ...
  readonly statusAll: Effect.Effect<
    ReadonlyArray<AgentIntegrationStatus>,
    AgentIntegrationError
  >;
  readonly install: (
    host: AgentIntegrationHostType,
...
export class AgentIntegration extends Context.Service<
  AgentIntegration,
  AgentIntegrationShape
...
const readJsonObject = Effect.fn("Flect.AgentIntegration.readJson")(function* (
  ...
  const source = yield* fs
    ...
    .pipe(
      Effect.mapError(() =>
        integrationError(host, "io", `Flect could not read ${file}.`),
      ),
    );
...
const sessionStart = (
  ...
  if (!isObject(hooks)) {
    return Effect.fail(
      integrationError(host, "invalid-config", "hooks must be a JSON object."),
    );
  }
...
const ownedHookCount = Effect.fn("Flect.AgentIntegration.ownedHookCount")(
  function* (host: "codex" | "claude", root: JsonObject) {
    ...
    for (const group of groups) {
      ...
      for (const handler of handlers) {
        if (ownsHook(handler)) {
          owned += 1;
          if (
            group.matcher === SESSION_MATCHER &&
            JSON.stringify(handler) === expected
          ) {
            exact += 1;
          }
        }
...
const withoutOwnedHooks = Effect.fn("Flect.AgentIntegration.withoutOwnedHooks")(
  function* (host: "codex" | "claude", root: JsonObject) {
    const groups = yield* sessionStart(host, root);
    ...
    const hooks: Record<string, Json> = isObject(currentHooks)
      ? { ...currentHooks }
      : {};
...
const addOwnedHook = Effect.fn("Flect.AgentIntegration.addOwnedHook")(
  function* (host: "codex" | "claude", root: JsonObject) {
    const clean = yield* withoutOwnedHooks(host, root);
    const hooks: Record<string, Json> = isObject(clean.hooks)
      ? { ...clean.hooks }
      : {};
    const groups = Array.isArray(hooks.SessionStart)
      ? [...hooks.SessionStart]
      : [];
...
export const makeAgentIntegrationLayer = (root: string) =>
  Layer.effect(
    ...
    Effect.gen(function* () {
      ...
      const writePrivate = Effect.fn("Flect.AgentIntegration.writePrivate")(
        function* (
          host: AgentIntegrationHostType,
          file: string,
          content: string,
          ...
          yield* fs
            ...
            .pipe(
              ...
              Effect.mapError(() =>
                integrationError(host, "io", `Flect could not write ${file}.`),
              ),
            );
        },
      );
      ...
      const status = Effect.fn("Flect.AgentIntegration.status")(function* (
        ...
        const exists = yield* fs
          ...
          .pipe(
            Effect.mapError(() =>
              integrationError(host, "io", `Flect could not inspect ${file}.`),
            ),
          );
        if (!exists) {
        ...
        const count = yield* ownedHookCount(host, decoded.success).pipe(
          Effect.result,
        );
        return AgentIntegrationStatus.make({
          host,
            ...
            .pipe(
              Effect.mapError(() =>
                integrationError(
                  host,
                  "io",
                  `Flect could not inspect ${before.path}.`,
                ),
              ),
            );
          const rootObject =
            ...
            .pipe(
              Effect.mapError(() =>
                integrationError(
                  host,
                  "io",
                  `Flect could not remove ${before.path}.`,
                ),
              ),
            );
        } else {
