...
const digest = async (contents: Uint8Array) =>
  Array.from(
    new Uint8Array(
      await crypto.subtle.digest("SHA-256", Uint8Array.from(contents)),
    ),
...
const preflightZip = (archive: Uint8Array) => {
  ...
  for (let index = 0; index < entries; index += 1) {
    if (
      offset + 46 > end ||
      view.getUint32(offset, true) !== ZIP_CENTRAL_ENTRY
    ) {
...
export const importWebProjectArchive = Effect.fn(
  "Flect.WebProject.importArchive",
)(function* (
  fileName: string,
  archive: Uint8Array,
        ...
        : normalized.endsWith(".tar")
          ? (await Effect.runPromise(decodeRepositoryTar(archive)))
              .filter(
                (
                  entry,
                ): entry is typeof entry & { readonly contents: Uint8Array } =>
