...
const bindingFrom = (shaping: ShapingSnapshot) =>
	shaping.proposal === undefined ? ('accepted' as const) : ('candidate' as const);
...
const makeSnapshot = (
	...
	WorkbenchSnapshot.make({
		...
		transitionSequence,
		...(shaping.proposal === undefined ? {} : { candidateRevisionId: shaping.proposal.id }),
		...(handoff === undefined ? {} : { handoff })
	});

export const initialWorkbenchState = (shaping: ShapingSnapshot): WorkbenchSnapshot =>
	makeSnapshot(
		shaping.proposal !== undefined || hasUsableProduct(shaping) ? 'use' : 'shape',
		shaping,
		0
	);
...
export const synchronizeWorkbenchState = (
	...
	return makeSnapshot(target, shaping, current.transitionSequence + 1, current.handoff);
};
