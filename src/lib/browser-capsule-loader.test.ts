...
import {
  loadBrowserCapsule,
  loadBrowserCapsuleArchiveFromUrl,
} from "./browser-capsule-loader";
...
describe("browser capsule loader", () => {
  ...
  it("rejects unsafe schemes and oversized responses before decoding", async () => {
    ...
    await expect(
      Effect.runPromise(
        loadBrowserCapsuleArchiveFromUrl("file:///tmp/app.flect"),
      ),
    ).rejects.toThrow();
    ...
    await expect(
      Effect.runPromise(
        loadBrowserCapsuleArchiveFromUrl("https://example.test/large.flect"),
      ),
    ).rejects.toThrow();
  });
});
