...
export class ContinuityConflict extends Schema.TaggedErrorClass<ContinuityConflict>()(
  "ContinuityConflict",
  {
    message: Schema.Literal(
      "Role continuity changed before this update could be saved.",
    ),
...
export class ContinuityBlocked extends Schema.TaggedErrorClass<ContinuityBlocked>()(
  ...
  {
    message: Schema.Literal(
      "Recover or discard the saved role continuity before replacing it.",
    ),
...
export class ContinuityLock extends Context.Service<
  ContinuityLock,
  ContinuityLockShape
...
const recoveryReason = (
  raw: string,
): "corrupt-record" | "incompatible-record" => {
  try {
      ...
      storage.read(ROLE_CONTINUITY_KEY).pipe(
        Effect.flatMap((raw): Effect.Effect<RoleContinuityLoad> => {
          ...
          return decodeRoleContinuityRecord(raw).pipe(
            Effect.map(
              (record): RoleContinuityLoad => ({ status: "ready", record }),
            ),
            Effect.catch(() =>
              Effect.succeed({
        ...
        storage.read(ROLE_CONTINUITY_KEY).pipe(
          Effect.flatMap((raw) =>
            raw === null
              ...
              : decodeRoleContinuityRecord(raw).pipe(
                  Effect.flatMap(encodeRoleContinuityRecord),
                ),
          ),
        ),
      ),
