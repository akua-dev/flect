...
const input = (
  index: number,
  overrides: Partial<typeof OperationJournalInput.Type> = {},
) =>
...
describe("OperationJournal", () => {
  it.layer(OperationJournalLive)((it) => {
    ...
    it.effect(
      "stores and queries a bounded capability receipt without payloads",
      () =>
        Effect.gen(function* () {
          const journal = yield* OperationJournal;
          ...
          assert.deepStrictEqual(
            yield* journal.query(
              OperationQuery.make({ capabilityId: "product.projects.read" }),
            ),
            [record],
          );
    ...
    it.effect(
      "redacts authorization and provider-secret material at append",
      () =>
        Effect.gen(function* () {
          const journal = yield* OperationJournal;
          const record = yield* journal.append(
            input(4, {
              summary:
                "Authorization: Bearer super-secret-token with sk-provider-secret",
        ...
        const encodedBytes = new TextEncoder().encode(
          JSON.stringify(
            yield* Effect.forEach(records, (record) =>
              Effect.succeed(OperationRecord.make(record)),
            ),
          ),
        ).byteLength;
