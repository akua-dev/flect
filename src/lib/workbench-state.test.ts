...
const shaping = (active: InterfaceRevision, proposal?: InterfaceRevision, safeMode = false) =>
...
describe('workbench state', () => {
	...
	it('selects candidate Use as soon as a preview exists', () => {
		...
		const next = synchronizeWorkbenchState(current, shaping(accepted, candidate));
	...
	it.effect('supports rapid explicit Shape and Use selection on a candidate', () =>
		Effect.gen(function* () {
			const preview = shaping(accepted, candidate);
			const current = initialWorkbenchState(preview);
			const shape = yield* selectWorkbenchTarget(current, 'shape', preview, 0);
	...
	it.effect('rejects blank Use, stale selection, and safe-mode selection', () =>
		Effect.gen(function* () {
			...
			const unavailable = yield* selectWorkbenchTarget(blank, 'use', blankSnapshot, 0).pipe(
				Effect.flip
			);

			const productSnapshot = shaping(accepted);
			const product = initialWorkbenchState(productSnapshot);
			const stale = yield* selectWorkbenchTarget(product, 'shape', productSnapshot, 4).pipe(
				Effect.flip
			);
