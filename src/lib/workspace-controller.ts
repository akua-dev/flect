...
import { ExtensionCatalog, type PortableExtensionKey } from '../extensions/extension-catalog';
...
import { decodeShareArchive, encodeShareArchive } from '../sharing/share-archive';
import {
	ShareCandidateStore,
...
import { AgentWorkspace, OperationContext, type ProviderAuthUiState } from './agent-workspace';
import { isNativeHost } from './native-host';
import { OperationJournal, OperationJournalInput, OperationQuery } from './operation-journal';
...
export interface FlectWorkspaceControllerShape {
	...
	readonly privateShareSources?: Effect.Effect<ReadonlyArray<PrivateShareSourceSummary>>;
	...
	readonly setDraft: (key: keyof ContinuityDrafts, value: string) => Effect.Effect<void>;
	...
	readonly readShareExport: (shareId: string) => Effect.Effect<Uint8Array, CommandRejected>;
	...
	readonly loginProvider: (request: AuthLoginRequest) => Effect.Effect<void, unknown>;
	readonly replyProviderAuth: (reply: AuthSelectionReply) => Effect.Effect<void, unknown>;
	readonly cancelProviderAuth: (reference: AuthLoginReference) => Effect.Effect<void, unknown>;
...
const projectImportReport = (capsule: DecodedCapsule) => {
	const file = capsule.files.find((candidate) => candidate.path === PROJECT_IMPORT_REPORT_PATH);
...
const reviewCapsule = (
	...
	flectCompatible,
	platformCompatible: capsule.manifest.compatibility.platforms.includes(currentCapsulePlatform()),
	capabilities,
	...
	totalBytes: capsule.files.reduce((total, file) => total + file.contents.byteLength, 0),
		...
		: {
				build: {
					...
					...(capsule.manifest.build.dependencyGraphDigest === undefined
						? {}
						: {
								dependencyGraphDigest: capsule.manifest.build.dependencyGraphDigest
							})
				}
			}),
	...(() => {
	...
	activationBlocked:
		...
		!flectCompatible ||
		!capsule.manifest.compatibility.platforms.includes(currentCapsulePlatform()) ||
		capsule.manifest.capabilities.some(
			(capability) =>
				capability.required &&
				!capabilities.some(
					(projection) =>
						projection.capabilityId === capability.id && projection.state === 'granted'
				)
		),
...
const compiledCapsulePresentation = (
	...
	decodeCapsule(archive).pipe(
		Effect.flatMap((capsule) => {
			...
			if (entrypoint === undefined || file === undefined) return Effect.succeed(undefined);
			return Effect.try({
				try: () => ({
					id: capsule.manifest.id,
					name: capsule.manifest.name,
...
const modeFromWorkbench = (workbench: WorkbenchSnapshot, shaping: ShapingSnapshot) =>
...
const boundedHandoffInstruction = (handoff: WorkbenchHandoff) =>
	[
		...
		...(handoff.selection === undefined ? [] : [canvasSelectionSummary(handoff.selection)]),
...
export const FlectWorkspaceControllerLive = Layer.effect(
	...
	Effect.gen(function* () {
		...
		const capsuleStore = Option.getOrUndefined(yield* Effect.serviceOption(CapsuleStore));
		const proposalBuild = Option.getOrUndefined(yield* Effect.serviceOption(ProposalBuild));
		const productRegistry = Option.getOrUndefined(
			yield* Effect.serviceOption(ProductCapabilityRegistry)
		);
		const extensionCatalog = Option.getOrUndefined(yield* Effect.serviceOption(ExtensionCatalog));
		const portableExtensionHost = Option.getOrUndefined(
			yield* Effect.serviceOption(PortableExtensionHost)
		);
		const shareSourceResolver: ShareSourceResolverShape | undefined = Option.getOrUndefined(
			yield* Effect.serviceOption(ShareSourceResolver)
		);
		const privateShareSourceRegistry: PrivateShareSourceRegistryShape | undefined =
			Option.getOrUndefined(yield* Effect.serviceOption(PrivateShareSourceRegistry));
		const shareCandidateStore: ShareCandidateStoreShape | undefined = Option.getOrUndefined(
			yield* Effect.serviceOption(ShareCandidateStore)
		);
		const shareInstallationStore: ShareInstallationStoreShape | undefined = Option.getOrUndefined(
			yield* Effect.serviceOption(ShareInstallationStore)
		);
		const shareRepository: ShareRepositoryShape | undefined = Option.getOrUndefined(
			yield* Effect.serviceOption(ShareRepository)
		);
		const shareSignatureVerifier: ShareSignatureVerifierShape | undefined = Option.getOrUndefined(
			yield* Effect.serviceOption(ShareSignatureVerifier)
		);
		const capsuleTrustVerifier: CapsuleTrustVerifierShape | undefined = Option.getOrUndefined(
			yield* Effect.serviceOption(CapsuleTrustVerifier)
		);
		const sandboxedShell = Option.getOrUndefined(yield* Effect.serviceOption(SandboxedShell));
		const workspaceId = 'workspace-local-default';
		const permissionContextFor = Effect.fn('Workspace.permissionContextFor')(function* (
			capsule: DecodedCapsule,
			archive: Uint8Array
		) {
		...
		const reviewDecodedCapsule = Effect.fn('Workspace.reviewDecodedCapsule')(function* (
			capsule: DecodedCapsule,
			archive: Uint8Array
		) {
			...
			return reviewCapsule(
				capsule,
				archive,
				yield* satisfiesVersion(packageMetadata.version, capsule.manifest.compatibility.flect),
				context,
		...
		yield* agent.refresh;
		const [agentSnapshot, shellPreferences, operations, initialExtensions, initialShares] =
			yield* Effect.all([
				agent.snapshot,
				preferences.load,
				journal.snapshot,
				extensionCatalog === undefined ? Effect.succeed(undefined) : extensionCatalog.snapshot,
		...
		const initial = FlectWorkspaceSnapshot.make({
			...
			mode:
				shapingSnapshot.active.source === 'built-in' || shapingSnapshot.proposal !== undefined
					? 'edit'
					: 'run',
			...
			...(initialExtensions === undefined ? {} : { extensions: initialExtensions }),
			...(Option.isSome(initialRepository) ? { repository: initialRepository.value } : {}),
			...(initialShares === undefined ? {} : { shares: initialShares })
		});
		const state = yield* SubscriptionRef.make(initial);
		const appendCapabilityWarnings = Effect.fn('Workspace.appendCapabilityWarnings')(function* () {
		...
		const shareCandidate = yield* Ref.make<ShareCandidateState | undefined>(undefined);
		const shareActivation = yield* Ref.make<ShareActivationState | undefined>(undefined);
		const preparedShareExport = yield* Ref.make<PreparedShareExport | undefined>(undefined);
		const syncExtensionSnapshot = Effect.fn('Workspace.syncExtensionSnapshot')(function* () {
		...
		const capsuleLoad =
			capsuleStore === undefined ? undefined : yield* capsuleStore.load.pipe(Effect.result);
		...
		const restoredCapsuleArchives: CapsuleArchiveBindings =
			capsuleLoad === undefined || Result.isFailure(capsuleLoad) ? {} : capsuleLoad.success;
		const [
			restoredAccepted,
			restoredCandidate,
			...
			restoredCapsuleArchives.candidate === undefined || shapingSnapshot.proposal === undefined
				? Effect.succeed(undefined)
		...
		const initialCapsulePresentation: CapsulePresentationState = {
			...
			...(restoredCandidate === undefined ? {} : { candidate: restoredCandidate }),
			...(restoredLastKnownGood === undefined ? {} : { lastKnownGood: restoredLastKnownGood }),
			...(restoredAcceptedReview === undefined ? {} : { acceptedReview: restoredAcceptedReview }),
		...
		const capsulePresentation = yield* SubscriptionRef.make<CapsulePresentationState>(
			initialCapsulePresentation
		);
		const permissionsFromPresentation = (presentation: CapsulePresentationState) =>
			...
			capsuleStore === undefined
				...
				: capsuleStore.save({
						...(next.acceptedReview === undefined && next.accepted === undefined
							? {}
							: {
									accepted: next.acceptedReview?.archive ?? next.accepted?.archive
								}),
						...(next.candidateReview === undefined && next.candidate === undefined
							? {}
							: {
									candidate: next.candidateReview?.archive ?? next.candidate?.archive
								}),
						...(next.lastKnownGoodReview === undefined && next.lastKnownGood === undefined
							? {}
							: {
									lastKnownGood: next.lastKnownGoodReview?.archive ?? next.lastKnownGood?.archive
								})
					});
		const updateCapsulePresentation = Effect.fn('Workspace.updateCapsulePresentation')(function* (
		...
		const restoreCapsulePresentation = Effect.fn('Workspace.restoreCapsulePresentation')(
			(presentation: CapsulePresentationState) =>
				persistCapsulePresentation(presentation).pipe(
					Effect.mapError(() => commandRejected('The capsule presentation could not be restored.')),
		...
		const refreshCapabilityReviews = Effect.fn('Workspace.refreshCapabilityReviews')(function* () {
			const presentation = yield* SubscriptionRef.get(capsulePresentation);
			const [acceptedReview, candidateReview, lastKnownGoodReview] = yield* Effect.all([
				...
				presentation.candidateReview === undefined
					? Effect.succeed(undefined)
					: reviewWithProductPermissions(presentation.candidateReview.archive),
				presentation.lastKnownGoodReview === undefined
					? Effect.succeed(undefined)
					: reviewWithProductPermissions(presentation.lastKnownGoodReview.archive)
			]);
		...
		if (restoredCapsuleArchives.candidate !== undefined && restoredCandidate === undefined) {
		...
		const activeAgentParentOperations = yield* Ref.make<ReadonlySet<string>>(new Set());
		...
		const initialContinuity =
			continuityLoad.status === 'ready'
				? shapingSnapshot.safeMode
					...
					: projectAgentContinuity(agentSnapshot, shapingSnapshot, continuityLoad.record)
				: emptyRoleContinuityRecord(shapingSnapshot.lastEvent.sequence);
		const continuityState = yield* Ref.make(initialContinuity);
		const continuityBlocked = yield* Ref.make(continuityLoad.status === 'recovery');
		const continuityUi = yield* SubscriptionRef.make<RoleContinuityUiState>({
			...
			...(continuityLoad.status === 'recovery' ? { recovery: continuityLoad.reason } : {})
		});
		...
		const persistContinuity = Effect.fn('Workspace.persistContinuity')(
			(nextAgent: typeof initial.agent, shaping: ShapingSnapshot) =>
				continuityPermit.withPermits(1)(
					Effect.gen(function* () {
						...
						const saved = yield* continuityRepository.save(current.generation, next);
		...
		const setDraft = Effect.fn('Workspace.setDraft')((key: keyof ContinuityDrafts, value: string) =>
			continuityPermit.withPermits(1)(
				Effect.gen(function* () {
		...
		const discardContinuity = Effect.fn('Workspace.discardContinuity')(function* () {
		...
		const retryContinuity = Effect.fn('Workspace.retryContinuity')(function* () {
		...
		const transition = Effect.fn('Workspace.transition')(function* (
		...
		const syncAgentProjection = Effect.fn('Workspace.syncAgentProjection')(function* () {
		...
		const runProjectedAgentTurn = Effect.fn('Workspace.runProjectedAgentTurn')(function* <A, E, R>(
			turn: Effect.Effect<A, E, R>
		) {
			const fiber = yield* turn.pipe(Effect.forkChild({ startImmediately: true }));
			yield* Effect.yieldNow;
			yield* syncAgentProjection();
			return yield* Fiber.join(fiber).pipe(Effect.ensuring(syncAgentProjection()));
		});
		...
		const operationContext = (envelope: FlectCommandEnvelope, operationId: string) =>
		...
		const withNestedAgentCommands = <A, E, R>(
			...
			Ref.update(activeAgentParentOperations, (current) => new Set(current).add(operationId)).pipe(
		...
		const authorize = Effect.fn('Workspace.authorize')(function* (envelope: FlectCommandEnvelope) {
			switch (envelope.source.kind) {
				...
				case 'agent': {
					const binding =
						envelope.source.binding ?? (envelope.source.role === 'app' ? 'accepted' : 'candidate');
					const allowed =
		...
		const remember = (commandId: string, receipt: FlectCommandReceipt) =>
			Ref.update(cache, (current) => {
				...
				const order = alreadyKnown ? current.order : [...current.order, commandId];
		...
		const claim = Effect.fn('Workspace.claimCommand')((envelope: FlectCommandEnvelope) =>
			Effect.gen(function* () {
				yield* authorize(envelope);
				const existing = (yield* Ref.get(cache)).receipts.get(envelope.commandId);
				...
				const accepted = yield* transition(envelope.source, 'command-accepted', unchanged, {
					operationId,
					commandId: envelope.commandId
				});
		...
		const applyShapingSnapshot = (current: FlectWorkspaceSnapshot, shaping: ShapingSnapshot) =>
			(() => {
		...
		const transitionShaping = (
			...
			transition(envelope.source, type, (current) => applyShapingSnapshot(current, shaping), {
				operationId,
				commandId: envelope.commandId,
				...(revisionId === undefined ? {} : { revisionId })
			});

		const hashShareBytes = Effect.fn('Workspace.hashShareBytes')((value: Uint8Array) =>
			Effect.tryPromise({
				try: async () => {
				...
				catch: () => commandRejected('The shared source could not be reviewed.')
			})
		);

		const validateShareActivation = Effect.fn('Workspace.validateShareActivation')(function* () {
			...
			if (
				...
				return yield* Effect.fail(commandRejected('Shared activation is unavailable.'));
			}
			const candidate = yield* Ref.get(shareCandidate);
			const installation = yield* shareInstallationStore.get(activation.shareId);
			...
			if (
				...
				new Set(activation.artifactIds).size !== activation.artifactIds.length ||
				activation.artifactIds.some((id) => !declaredIds.includes(id)) ||
				(yield* shareCandidateStore.load(installation.pending.archiveSha256)) === undefined
			) {
				return yield* Effect.fail(
					commandRejected('Review the shared candidate again before activation.')
				);
			}
		...
		const finalizeShareActivation = Effect.fn('Workspace.finalizeShareActivation')(function* (
			metadata?: Ref.Ref<FinalizedShareActivation | undefined>
		) {
			const activation = yield* Ref.get(shareActivation);
			...
			const declaredIds = candidate.material.manifest.artifacts.map((artifact) => artifact.id);
			if (
				activation.artifactIds.length === 0 ||
				new Set(activation.artifactIds).size !== activation.artifactIds.length ||
				activation.artifactIds.some((id) => !declaredIds.includes(id))
			...
			const stored = yield* shareCandidateStore.load(installation.pending.archiveSha256);
			if (stored === undefined) {
				return yield* Effect.fail(
					commandRejected('Review the shared candidate again before activation.')
				);
					...
					.pipe(
						Effect.catchTag('ShareRepositoryFailure', (error) =>
							(error.rollbackRestored === true && metadata !== undefined
								...
								Effect.andThen(
									Effect.fail(commandRejected('The shared candidate changed before activation.'))
								)
							)
						)
					);
			...
			const installedArtifactIds = [
				...new Set([
					...installation.installedArtifactIds.filter((id) => declaredIds.includes(id)),
			...
			const acceptedInstallation = ShareInstallationRecord.make({
				...
				artifacts: candidate.material.manifest.artifacts.map((artifact) =>
					ShareInstalledArtifact.make({
						...
						...(artifact.capsule === undefined ? {} : { capsuleSha256: artifact.capsule.sha256 })
					})
				),
			...
			yield* shareInstallationStore.save(acceptedInstallation).pipe(
				Effect.mapError(() => commandRejected('The shared activation state could not be saved.')),
				Effect.catch((error) => restoreGit.pipe(Effect.andThen(Effect.fail(error))))
			);
		...
		const removeUnreferencedShareArchive = Effect.fn('Workspace.removeUnreferencedShareArchive')(
			function* (digest: string | undefined) {
				...
				if (
					shares.entries.some(
						(entry) =>
							entry.source.archiveSha256 === digest || entry.pending?.archiveSha256 === digest
					)
				) {
					return;
				}
		...
		const discardShareActivation = Effect.fn('Workspace.discardShareActivation')(function* (
			metadata?: Ref.Ref<DiscardedShareActivation | undefined>
		) {
			const activation = yield* Ref.get(shareActivation);
			...
			if (installation.refs.candidate !== undefined) {
				if (shareRepository === undefined) {
					return yield* Effect.fail(commandRejected('The shared candidate could not be rejected.'));
				}
				yield* shareRepository
					...
					.pipe(
						Effect.mapError(() => commandRejected('The shared candidate could not be rejected.'))
					);
			}
			const restoreRef: Effect.Effect<void, CommandRejected> =
				installation.refs.candidate === undefined
					? Effect.void
					: shareRepository === undefined
						? Effect.fail(commandRejected('The shared candidate could not be restored.'))
						: shareRepository
								.restoreCandidateRef({
									shareId: installation.shareId,
									candidate: installation.refs.candidate,
			...
			yield* shareInstallationStore.save(restored).pipe(
				Effect.mapError(() => commandRejected('The shared candidate state could not be saved.')),
				Effect.catch((error) => restoreRef.pipe(Effect.andThen(Effect.fail(error))))
			);
			yield* Ref.set(shareCandidate, undefined);
			yield* Ref.set(shareActivation, undefined);
			const shares = yield* shareInstallationStore.snapshot;
		...
		const runCompensations = Effect.fn('Workspace.runCompensations')(function* (
		...
		const enterDegradedRecovery = Effect.fn('Workspace.enterDegradedRecovery')(function* () {
		...
		const acceptProposal = Effect.fn('Workspace.acceptProposal')(function* (
			envelope: FlectCommandEnvelope,
			operationId: string
		) {
			const shaping = yield* kernel.snapshot;
			...
			if (proposal === undefined) {
				return yield* Effect.fail(commandRejected('There is no proposal to accept.'));
			}
			...
			const beforeCapsulePresentation = yield* SubscriptionRef.get(capsulePresentation);
			...
			const beforeExtensions =
				extensionCatalog === undefined ? undefined : yield* extensionCatalog.snapshot;
			...
			const acceptedResult = yield* Effect.gen(function* () {
				...
				yield* updateCapsulePresentation((current) => ({
					...(current.candidate === undefined ? {} : { accepted: current.candidate }),
			...
			yield* removeUnreferencedShareArchive(
				committedShare !== undefined &&
					committedShare.before.source.archiveSha256 !== committedShare.candidateArchiveSha256
					? committedShare.before.source.archiveSha256
					: undefined
			);
			yield* agent.releasePreview;
			const next = yield* kernel.snapshot;
			yield* transitionShaping(envelope, operationId, 'revision-accepted', next, accepted.id);
		});

		const rejectProposal = Effect.fn('Workspace.rejectProposal')(function* (
			envelope: FlectCommandEnvelope,
			operationId: string
		) {
			const shaping = yield* kernel.snapshot;
			const proposal = shaping.proposal;
			if (proposal === undefined) {
				return yield* Effect.fail(commandRejected('There is no proposal to reject.'));
			}
			const beforeWorkspace = yield* SubscriptionRef.get(state);
			const beforeCapsulePresentation = yield* SubscriptionRef.get(capsulePresentation);
			const beforeCandidate = yield* Ref.get(shareCandidate);
			...
			const rejectedResult = yield* Effect.gen(function* () {
				...
				yield* updateCapsulePresentation((current) => ({
					...
					...(current.lastKnownGood === undefined ? {} : { lastKnownGood: current.lastKnownGood }),
					...(current.acceptedReview === undefined
						? {}
			...
			yield* agent.releasePreview;
			const next = yield* kernel.snapshot;
			yield* transitionShaping(envelope, operationId, 'revision-rejected', next, rejected.id);
		});

		const rollback = Effect.fn('Workspace.rollback')(function* (
			envelope: FlectCommandEnvelope,
			operationId: string
		) {
			const beforeCapsulePresentation = yield* SubscriptionRef.get(capsulePresentation);
			yield* updateCapsulePresentation((current) => ({
				...(current.lastKnownGood === undefined ? {} : { accepted: current.lastKnownGood }),
				...(current.accepted === undefined ? {} : { lastKnownGood: current.accepted }),
				...(current.lastKnownGoodReview === undefined
					? {}
			...
			const next = yield* kernel.snapshot;
			yield* transitionShaping(envelope, operationId, 'revision-rolled-back', next, revision.id);
		});

		const enterSafeMode = Effect.fn('Workspace.enterSafeMode')(function* (
			envelope: FlectCommandEnvelope,
			operationId: string
		) {
			yield* kernel.enterSafeMode;
			const next = yield* kernel.snapshot;
			yield* transitionShaping(envelope, operationId, 'safe-mode-entered', next);
		});

		const shapeInstruction = Effect.fn('Workspace.shapeInstruction')(function* (
			envelope: FlectCommandEnvelope,
			operationId: string,
			instruction: string,
			...
			const shaping = yield* kernel.snapshot;
			if (shaping.safeMode) {
				return yield* Effect.fail(commandRejected('Leave safe mode before shaping.'));
			}
			const [presentation, pendingShare, extensionSnapshot] = yield* Effect.all([
			...
			if (
				...
				extensionSnapshot?.entries.some((entry) => entry.binding === 'candidate') === true
			) {
				return yield* Effect.fail(
					commandRejected(
			...
			const current = yield* SubscriptionRef.get(state);
			const previous = current.workbench ?? initialWorkbenchState(current.shaping);
			...
			const workbench =
				handoff === undefined ? selected : WorkbenchSnapshot.make({ ...selected, handoff });
			yield* transition(
				envelope.source,
				'turn-started',
				(snapshot) =>
			...
			const outcome = yield* withNestedAgentCommands(
				...
				runProjectedAgentTurn(
					agent.submitShaperInstruction(
						...
						handoff === undefined ? instruction : boundedHandoffInstruction(handoff),
						shaping.proposal?.document ?? shaping.active.document,
						instruction
					)
				)
			...
			yield* updateCapsulePresentation((current) => ({
				...(current.accepted === undefined ? {} : { accepted: current.accepted }),
				...(current.lastKnownGood === undefined ? {} : { lastKnownGood: current.lastKnownGood }),
				...(current.acceptedReview === undefined ? {} : { acceptedReview: current.acceptedReview }),
				...(current.lastKnownGoodReview === undefined
					? {}
					: { lastKnownGoodReview: current.lastKnownGoodReview })
			}));
			if (outcome.kind === 'app') {
				...
				yield* Effect.gen(function* () {
					if (shaping.proposal !== undefined) {
						return yield* Effect.fail(
							commandRejected('Resolve the current preview before authoring an app.')
						);
					}
			...
			if (shaping.proposal === undefined) {
				...
				const accepted = yield* kernel.applyLocalRevision(candidate, 'shaper');
				yield* agent.releasePreview;
				const next = yield* kernel.snapshot;
				yield* transitionShaping(envelope, operationId, 'revision-accepted', next, accepted.id);
			} else {
		...
		const submitAppConversation = Effect.fn('Workspace.submitAppConversation')(function* (
			envelope: FlectCommandEnvelope,
			operationId: string,
			text: string
		) {
			...
			const outcome = yield* withNestedAgentCommands(
				...
				runProjectedAgentTurn(
					current.shaping.proposal === undefined
						? agent.submitAppPrompt(operationContext(envelope, operationId), text)
			...
			if (outcome.editRequest !== undefined) {
				...
				const revisionId = latest.shaping.proposal?.id ?? latest.shaping.active.id;
				const handoff = WorkbenchHandoff.make({
					version: 1,
					instruction: outcome.editRequest.instruction,
					revisionId
				});
				yield* shapeInstruction(envelope, operationId, outcome.editRequest.instruction, handoff);
			}
		});

		const runShareConflictShaper = Effect.fn('Workspace.runShareConflictShaper')(function* (
			envelope: FlectCommandEnvelope,
			operationId: string,
			instruction: string
		) {
		...
		const hashShareReference = (value: string) => hashShareBytes(new TextEncoder().encode(value));
		...
		const shareOrigin = Effect.fn('Workspace.shareOrigin')(function* (
			source: ShareSource,
		...
		const encodeCandidateArchive = Effect.fn('Workspace.encodeShareCandidate')(
			(material: ShareCandidateMaterial) =>
				encodeShareArchive({
					manifest: {
						...material.manifest,
					...
					Effect.mapError(() => commandRejected('The shared candidate could not be retained.'))
				)
		);

		const persistShareCandidate = Effect.fn('Workspace.persistShareCandidate')(function* (
			material: ShareCandidateMaterial
		) {
			if (shareCandidateStore === undefined) {
				return yield* Effect.fail(commandRejected('Shared candidate storage is unavailable.'));
			}
		...
		const snapshotShareArtifacts = Effect.fn('Workspace.snapshotShareArtifacts')(function* (
			...
			if (shareRepository === undefined) {
				return yield* Effect.fail(commandRejected('The retained shared source is unavailable.'));
			}
			return (yield* Effect.forEach(
				...
				(artifact) =>
					shareRepository
						...
						.pipe(
							Effect.mapError(() =>
								commandRejected('The retained shared source could not be inspected.')
							)
						),
				{ concurrency: 1 }
			)).flat();
		...
		const snapshotInstalledShareArtifacts = Effect.fn('Workspace.snapshotInstalledShareArtifacts')(
			(
				installation: ShareInstallationRecord,
				artifacts: ShareCandidateMaterial['manifest']['artifacts']
			) => snapshotShareArtifacts(installation, artifacts, 'fork', installation.refs.fork)
		);

		const completeShareConflict = Effect.fn('Workspace.completeShareConflict')(function* (
			envelope: FlectCommandEnvelope,
			operationId: string,
			installation: ShareInstallationRecord,
			...
			if (
				...
				return yield* Effect.fail(
					commandRejected('Open the retained shared conflict before resolving it.')
				);
			}
			const expectedPaths = installation.pending.conflictPaths.toSorted();
			const resolutionPaths = [...files.map((file) => file.path), ...removals].toSorted();
			...
			const review = yield* buildShareReview({
				...
				Effect.mapError(() =>
					commandRejected('The resolved shared candidate could not be reviewed.')
				)
			);
		...
		if (
			...
			yield* Effect.gen(function* () {
				...
				if (
					...
					completed.value.candidate.manifest.id !== pendingInstallation.shareId ||
					completed.value.candidate.manifest.repository.commit !== pendingInstallation.refs.upstream
				) {
		...
		const stageCapsuleProposal = Effect.fn('Workspace.stageCapsuleProposal')(function* (
			envelope: FlectCommandEnvelope,
			operationId: string,
			archive: Uint8Array,
			options: {
			...
			const staging = yield* Effect.tryPromise({
				...
				catch: () => commandRejected('The capsule staging pipeline could not load safely.')
			});
		...
		const runCommand: (
			envelope: FlectCommandEnvelope,
			operationId: string
		) => Effect.Effect<unknown, unknown> = Effect.fn('Workspace.runCommand')(function* (
			envelope: FlectCommandEnvelope,
			operationId: string
		) {
			...
			switch (command.type) {
				...
				case 'open-share-source': {
					if (
						...
						return yield* Effect.fail(commandRejected('Shared source review is unavailable.'));
					}
					const completed = yield* shareSourceResolver.open(command.source).pipe(
						Stream.runLast,
						Effect.mapError((error) => commandRejected(error.message))
					);
					if (Option.isNone(completed) || completed.value.type !== 'completed') {
						return yield* Effect.fail(commandRejected('The shared source could not be reviewed.'));
					}
					const material = completed.value.candidate;
					const origin = yield* shareOrigin(command.source, material);
					const installed = yield* shareInstallationStore.get(material.manifest.id);
					const lineage: ShareLineage = installed === undefined ? 'new' : 'update';
					const signature = yield* shareSignatureVerifier.verify(
						material.manifest,
						material.archiveSha256
					);
					const previousFiles =
						installed === undefined
							? []
							: yield* snapshotInstalledShareArtifacts(installed, material.manifest.artifacts);
					const review = yield* buildShareReview({
						lineage,
						origin,
						manifest: material.manifest,
						...
						Effect.mapError(() => commandRejected('The shared source could not be reviewed.'))
					);
					yield* Ref.set(shareCandidate, { material, origin, lineage });
					yield* transition(
						envelope.source,
						'state-changed',
						(current) => FlectWorkspaceSnapshot.make({ ...current, shareReview: review }),
						{ operationId, commandId: envelope.commandId }
					);
					return {
						status: 'review-ready',
				...
				case 'reject-share-candidate': {
					...
					if (candidate === undefined) {
						return yield* Effect.fail(commandRejected('There is no shared candidate to reject.'));
					}
					let shares =
						shareInstallationStore === undefined
							? undefined
					...
					if (
						shareRepository !== undefined &&
						shareInstallationStore !== undefined &&
						(yield* shareInstallationStore.get(candidate.material.manifest.id))?.pending !==
							undefined
					) {
						const installation = yield* shareInstallationStore.get(candidate.material.manifest.id);
						if (installation !== undefined && installation.pending !== undefined) {
							if (installation.refs.candidate !== undefined) {
								yield* shareRepository
									.rejectCandidate({
										shareId: installation.shareId,
				...
				case 'retain-share-candidate': {
					if (
						...
						return yield* Effect.fail(commandRejected('Shared retention is unavailable.'));
					}
					const candidate = yield* Ref.get(shareCandidate);
					const review = (yield* SubscriptionRef.get(state)).shareReview;
					if (
						...
						return yield* Effect.fail(
							commandRejected('Open a new shared candidate before retaining it.')
						);
					}
					const declaredIds = candidate.material.manifest.artifacts.map((artifact) => artifact.id);
					if (
					...
					if ((yield* shareInstallationStore.get(candidate.material.manifest.id)) !== undefined) {
						return yield* Effect.fail(commandRejected('That shared source is already retained.'));
					}
					const signature = yield* shareSignatureVerifier.verify(
						candidate.material.manifest,
						candidate.material.archiveSha256
					...
					if (signature.status === 'invalid') {
						return yield* Effect.fail(commandRejected('The shared signature is invalid.'));
					}
					const candidateArchiveSha256 = yield* persistShareCandidate(candidate.material);
					const retained = yield* shareRepository
						...
						.pipe(
							Effect.mapError(() => commandRejected('The shared history could not be retained.'))
						);
					...
					const record = ShareInstallationRecord.make({
						...
						shareId: candidate.material.manifest.id,
						version: candidate.material.manifest.version,
						source: shareOriginWithArchive(candidate.origin, candidateArchiveSha256),
						manifest: candidate.material.manifest,
						manifestSha256,
						repositorySha256,
						artifacts: candidate.material.manifest.artifacts.map((artifact) =>
						...
						pending: SharePendingCandidate.make({
							...
							lineage: 'new',
							origin: shareOriginWithArchive(candidate.origin, candidateArchiveSha256),
							conflictPaths: [],
					...
					yield* shareInstallationStore
						.save(record)
						.pipe(
							Effect.mapError(() => commandRejected('The shared installation could not be saved.'))
						);
					const shares = yield* shareInstallationStore.snapshot;
					yield* transition(
						envelope.source,
				...
				case 'fork-share': {
					if (shareInstallationStore === undefined) {
						return yield* Effect.fail(commandRejected('Shared installation state is unavailable.'));
					}
					const installation = yield* shareInstallationStore.get(command.shareId);
					if (installation === undefined) {
						return yield* Effect.fail(commandRejected('That shared installation is unavailable.'));
					}
					return {
						status: 'forked',
						shareId: installation.shareId,
				...
				case 'checkpoint-share-fork': {
					if (shareRepository === undefined || shareInstallationStore === undefined) {
						return yield* Effect.fail(commandRejected('Shared fork editing is unavailable.'));
					}
					const installation = yield* shareInstallationStore.get(command.shareId);
					if (installation === undefined) {
						return yield* Effect.fail(commandRejected('That retained shared fork is unavailable.'));
					}
					if (installation.pending !== undefined) {
						return yield* Effect.fail(
							commandRejected('Finish or reject the current shared review before editing its fork.')
						);
					}
					if (installation.refs.fork !== command.expectedForkCommit) {
						return yield* Effect.fail(
							commandRejected('The shared fork changed. Inspect it again before editing.')
						);
					}
					const paths = [...command.files.map((file) => file.path), ...command.removals];
					...
					if (
						...
						return yield* Effect.fail(commandRejected('The shared fork changes are invalid.'));
					}
				...
				case 'prepare-share-update': {
					if (
						...
						return yield* Effect.fail(commandRejected('Shared update preparation is unavailable.'));
					}
					const candidate = yield* Ref.get(shareCandidate);
					const installation = yield* shareInstallationStore.get(command.shareId);
					if (
					...
					const update = yield* shareRepository
						...
						.pipe(
							Effect.mapError(() => commandRejected('The shared update could not be prepared.'))
						);
					...
					const review = yield* buildShareReview({
						...
						previousFiles,
						conflictPaths: update._tag === 'conflict' ? update.conflictPaths : [],
						flectVersion: packageMetadata.version,
						platform: currentCapsulePlatform(),
						signature
					}).pipe(
						Effect.mapError(() => commandRejected('The shared update could not be reviewed.'))
					);
					const refs = ShareInstallationRefs.make({
						base: installation.refs.base,
						upstream: update.upstream,
						fork: installation.refs.fork,
						...(update._tag === 'conflict' ? {} : { candidate: update.candidate })
					});
					const timestamp = Date.now();
					...
					yield* shareInstallationStore
						.save(pending)
						.pipe(
							Effect.mapError(() => commandRejected('The shared update state could not be saved.'))
						);
					yield* Ref.set(shareCandidate, {
						...candidate,
						lineage,
					...
					return {
						status: update._tag,
						shareId: installation.shareId,
						...(update._tag === 'conflict' ? {} : { candidateCommit: update.candidate })
					};
				}
				case 'continue-share-fork': {
					if (shareInstallationStore === undefined || shareRepository === undefined) {
						return yield* Effect.fail(
					...
					if (
						...
						return yield* Effect.fail(commandRejected('Open that retained shared conflict first.'));
					}
					const forkFiles = yield* snapshotInstalledShareArtifacts(
						installation,
						candidate.material.manifest.artifacts
					);
					const byPath = new Map(forkFiles.map((file) => [file.path, file.contents]));
					...
					const removals = installation.pending.conflictPaths.filter((path) => !byPath.has(path));
				...
				case 'open-share-conflict-in-shape': {
					if (
						...
						return yield* Effect.fail(commandRejected('Agent conflict resolution is unavailable.'));
					}
					const installation = yield* shareInstallationStore.get(command.shareId);
					const candidate = yield* Ref.get(shareCandidate);
					if (
					...
					yield* sandboxedShell
						...
						.pipe(
							Effect.mapError(() =>
								commandRejected('The conflict workspace could not be prepared safely.')
							)
						);
					const instruction = [
				...
				case 'activate-share-candidate': {
					if (
						...
						return yield* Effect.fail(commandRejected('Shared source activation is unavailable.'));
					}
					const candidate = yield* Ref.get(shareCandidate);
					const current = yield* SubscriptionRef.get(state);
					...
					if (!review.compatible || review.lineage === 'conflict') {
						return yield* Effect.fail(
							commandRejected('Resolve the shared review blockers before activation.')
						);
					}
					const signature = yield* shareSignatureVerifier.verify(
						candidate.material.manifest,
					...
					const descriptors = command.artifactIds.map((id) =>
						candidate.material.manifest.artifacts.find((artifact) => artifact.id === id)
					);
					if (descriptors.some((descriptor) => descriptor === undefined)) {
						return yield* Effect.fail(
							commandRejected('This shared artifact requires its dedicated activation path.')
						);
					}
					const kinds = new Set(
						descriptors.flatMap((descriptor) => (descriptor === undefined ? [] : [descriptor.kind]))
					);
					if (kinds.size === 1 && (kinds.has('experience') || kinds.has('extension'))) {
						...
						if (
							...
							(yield* hashCapsuleArchive(capsule.contents).pipe(
								Effect.mapError(() => commandRejected(`The shared ${kind} capsule is invalid.`))
							)) !== descriptor.capsule.sha256
						) {
							return yield* Effect.fail(commandRejected(`The shared ${kind} capsule is invalid.`));
						}
						yield* runCommand(
					...
					if (kinds.has('experience') || kinds.has('extension')) {
						return yield* Effect.fail(
							commandRejected('Activate shared experiences and extensions separately.')
						);
					}
					if (git === undefined) {
						return yield* Effect.fail(commandRejected('Shared source activation is unavailable.'));
					...
					const proposal = yield* kernel.propose(shaping.active.document, 'user');
					let preview = yield* kernel.preview(proposal.id);
					const role = installation.refs.candidate === undefined ? 'fork' : 'candidate';
					const expectedCommit =
						role === 'candidate' ? installation.refs.candidate : installation.refs.fork;
					if (expectedCommit === undefined) {
						yield* kernel.reject(preview.id).pipe(Effect.ignore);
						return yield* Effect.fail(commandRejected('The exact shared source is unavailable.'));
					}
					const files = (yield* Effect.forEach(descriptors, (descriptor) => {
						...
						return shareRepository
							...
							.pipe(
								...
								Effect.mapError(() => commandRejected('The exact shared source is unavailable.'))
							);
					})).flat();
					const stage = Effect.gen(function* () {
						...
						preview = yield* kernel.supersede(preview.id, shaping.active.document, 'user');
					}).pipe(
						Effect.tapError(() => kernel.reject(preview.id).pipe(Effect.ignore)),
						Effect.mapError(() => commandRejected('The shared source could not be staged safely.'))
					);
					yield* stage;
					const next = yield* kernel.snapshot;
					yield* transitionShaping(envelope, operationId, 'revision-previewed', next, preview.id);
					yield* Ref.set(shareActivation, {
						shareId: installation.shareId,
						artifactIds: [...command.artifactIds]
					});
					...
					const archive = yield* Effect.gen(function* () {
						...
						if (
							...
							const repository = yield* shareRepository
								...
								.pipe(
									Effect.mapError(() =>
										commandRejected('The reviewed shared candidate could not be exported.')
									)
								);
						...
						if (
							candidate?.material.manifest.id === command.shareId &&
							(installation === undefined ||
								(installation.pending === undefined && candidate.lineage === 'update'))
						) {
							return yield* encodeShareArchive({
								manifest: {
									...candidate.material.manifest,
						...
						const original = yield* decodeShareArchive(stored).pipe(
							Effect.mapError(() => commandRejected('That retained shared source is unavailable.'))
						);
						const repository = yield* shareRepository
							.exportFork({
								shareId: installation.shareId,
							...
							.pipe(
								Effect.mapError(() => commandRejected('The retained fork could not be exported.'))
							);
						return yield* encodeShareArchive({
							manifest: {
						...
						Effect.mapError(() => commandRejected('The shared archive could not be exported.'))
					);
				...
				case 'remove-share': {
					if (shareRepository === undefined || shareInstallationStore === undefined) {
						return yield* Effect.fail(commandRejected('Shared removal is unavailable.'));
					}
					const installation = yield* shareInstallationStore.get(command.shareId);
					if (installation === undefined) {
						return yield* Effect.fail(commandRejected('That shared installation is unavailable.'));
					...
					yield* shareRepository
						...
						.pipe(
							Effect.mapError(() =>
								commandRejected('The shared installation could not be removed.')
							)
						);
					...
					const retained = ShareInstallationRecord.make({
						...
						source:
							installation.pending === undefined
								? installation.source
								: shareOriginWithArchive(installation.source, installation.pending.archiveSha256),
						installedArtifactIds: [],
				...
				case 'delete-share-local-data': {
					if (shareRepository === undefined || shareInstallationStore === undefined) {
						return yield* Effect.fail(commandRejected('Shared deletion is unavailable.'));
					}
					...
					yield* shareRepository
						...
						.pipe(
							Effect.mapError(() => commandRejected('The shared local data could not be deleted.'))
						);
					yield* shareInstallationStore
				...
				case 'set-mode': {
					...
					const workbench = yield* selectWorkbenchTarget(
						...
						current.shaping,
						(current.workbench ?? initialWorkbenchState(current.shaping)).transitionSequence
					).pipe(Effect.mapError((error) => commandRejected(error.message)));
					yield* transition(
						envelope.source,
						'state-changed',
				...
				case 'refresh-runtime': {
					...
					yield* transition(
						envelope.source,
						'state-changed',
						(current) => FlectWorkspaceSnapshot.make({ ...current, agent: next }),
						{ operationId, commandId: envelope.commandId }
					);
					return;
				}
				...
				case 'set-model-favorite': {
					...
					yield* transition(
						envelope.source,
						'state-changed',
						(snapshot) => FlectWorkspaceSnapshot.make({ ...snapshot, agent: next }),
						{ operationId, commandId: envelope.commandId }
					);
					return;
				}
				...
				case 'test-portable-extension': {
					if (extensionCatalog === undefined || portableExtensionHost === undefined) {
						return yield* Effect.fail(
							commandRejected('Portable extension support is unavailable.')
						);
					}
					...
					if (entry === undefined) {
						return yield* Effect.fail(commandRejected('The portable extension is unavailable.'));
					}
				...
				case 'submit-app-prompt': {
					return yield* submitAppConversation(envelope, operationId, command.text);
				}
				case 'submit-shaper-instruction': {
					return yield* shapeInstruction(envelope, operationId, command.instruction);
				}
				case 'import-capsule': {
				...
				case 'request-shape-handoff': {
					const current = yield* SubscriptionRef.get(state);
					const revisionId = current.shaping.proposal?.id ?? current.shaping.active.id;
					...
					if (command.handoff.failureOperationId !== undefined && correlatedFailure === undefined) {
						return yield* Effect.fail(commandRejected('The selected failure is unavailable.'));
					}
					const trustedHandoff = WorkbenchHandoff.make({
						...
						...(correlatedFailure === undefined
							? {}
							: {
									failureSummary: correlatedFailure.tool?.resultSummary ?? correlatedFailure.summary
								})
					});
				...
				case 'invoke-interface-action': {
					...
					if (action === undefined) {
						return yield* Effect.fail(commandRejected('The interface action is unavailable.'));
					}
					if (!action.available) {
						return yield* Effect.fail(
							commandRejected(action.unavailableReason ?? 'The interface action is unavailable.')
						);
					}
				...
				case 'invoke-product-operation': {
					...
					if (
						review === undefined ||
						(envelope.source.kind === 'capsule' && review.id !== envelope.source.capsuleId)
					) {
						return yield* Effect.fail(commandRejected('The product operation was denied.'));
					}
				...
				case 'decide-product-capability': {
					if (productRegistry === undefined) {
						return yield* Effect.fail(commandRejected('The product capability is unavailable.'));
					}
					const presentation = yield* SubscriptionRef.get(capsulePresentation);
					const review = [presentation.candidateReview, presentation.acceptedReview].find(
						(candidate) => candidate?.id === command.capsuleId
					);
					if (review === undefined) {
						return yield* Effect.fail(commandRejected('The product capability is unavailable.'));
					}
					yield* productRegistry
						.decide(review.permissionContext, command.capabilityId, command.choice)
						.pipe(Effect.mapError((error) => commandRejected(error.message)));
					yield* refreshCapabilityReviews();
				...
				case 'revoke-product-capability': {
					...
					const visible = [
						...
						review?.capabilities.some((capability) => capability.decisionId === command.decisionId)
					);
					if (!visible) {
						return yield* Effect.fail(commandRejected('The product capability is unavailable.'));
					}
				...
				case 'restore-safe-mode': {
					...
					const next = yield* kernel.snapshot;
					yield* transitionShaping(envelope, operationId, 'safe-mode-restored', next, revision.id);
					return;
				}
				case 'enable-control':
					yield* transition(
		...
		const dispatchCommand = Effect.fn('Workspace.dispatchCommand')(function* (
			envelope: FlectCommandEnvelope
		) {
				...
				Effect.matchEffect({
					onFailure: (error) =>
						Effect.gen(function* () {
							...
							yield* transition(envelope.source, 'command-failed', unchanged, {
								operationId,
								commandId: envelope.commandId,
					...
					onSuccess: (result) =>
						Effect.gen(function* () {
							const encodedResult =
								result === undefined
									? undefined
									: yield* Schema.decodeUnknownEffect(Schema.Json)(result).pipe(
											Effect.mapError(() =>
							...
							const completed = yield* transition(envelope.source, 'command-completed', unchanged, {
								operationId,
								commandId: envelope.commandId
							});
							const receipt = FlectCommandReceipt.make({
								...
								status: 'completed',
								...(encodedResult === undefined ? {} : { result: encodedResult })
							});
							yield* remember(envelope.commandId, receipt);
							yield* appendCommandRecord(
								envelope,
		...
		const dispatch = Effect.fn('Workspace.dispatch')((envelope: FlectCommandEnvelope) =>
			Effect.gen(function* () {
				const nested =
					envelope.source.kind === 'agent' &&
					(yield* Ref.get(activeAgentParentOperations)).has(envelope.source.parentOperationId);
				if (nested) return yield* dispatchCommand(envelope);
				return yield* commandPermit.withPermits(1)(dispatchCommand(envelope));
			})
		);

		const connectClient = Effect.fn('Workspace.connectClient')(function* (
			...
			yield* transition(systemSource, 'client-connected', (snapshot) =>
				FlectWorkspaceSnapshot.make({
					...
					control: ControlStateSnapshot.make({
						...snapshot.control,
						clients: [
							...snapshot.control.clients.filter((candidate) => candidate.id !== client.id),
							client
						].slice(-50)
					})
				})
		...
		const disconnectClient = Effect.fn('Workspace.disconnectClient')((clientId: string) =>
			transition(systemSource, 'client-disconnected', (snapshot) =>
				FlectWorkspaceSnapshot.make({
					...snapshot,
					control: ControlStateSnapshot.make({
						...snapshot.control,
						clients: snapshot.control.clients.filter((candidate) => candidate.id !== clientId)
					})
				})
			).pipe(Effect.asVoid)
		);
		...
		return {
			...
			privateShareSources: privateShareSourceRegistry?.list ?? Effect.succeed([]),
				...
				Ref.get(preparedShareExport).pipe(
					Effect.flatMap((prepared) =>
						prepared?.shareId === shareId
							...
							: Effect.fail(commandRejected('Prepare that shared archive before reading it.'))
					)
				),
				...
				return yield* encodeCapsule({
					manifest: {
						...
						provenance: {
							...
							revision: current.repository?.acceptedCommit ?? current.shaping.active.id,
							builder: 'flect@0.2.0'
						},
						signatures: []
					},
