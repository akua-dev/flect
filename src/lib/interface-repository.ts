...
import { ShapingSnapshot, validateShapingSnapshot } from '../../shared/revisions';
...
export class RecoveryMarker extends Schema.Class<RecoveryMarker>('RecoveryMarker')({
...
export interface InterfaceRepositoryShape {
	...
	readonly save: (snapshot: ShapingSnapshot) => Effect.Effect<void, InterfaceStorageError>;
...
export const makeInterfaceRepositoryLayer = ({ safeMode }: { readonly safeMode: boolean }) =>
	Layer.effect(
		InterfaceRepository,
		Effect.gen(function* () {
			...
			const load = Effect.fn('Flect.InterfaceRepository.load')(function* () {
				...
				const decoded = yield* validateShapingSnapshot(input.value).pipe(Effect.option);
