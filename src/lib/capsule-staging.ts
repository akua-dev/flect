...
export interface CapsuleStagingDeps {
  ...
  readonly currentCapsulePlatform: () =>
    | "browser"
    | "macos"
    | "windows"
    | "linux";
...
export const stageCapsuleProposal = Effect.fn(
  "Flect.Workspace.stageCapsuleProposalLive",
)(function* (
  ...
  if (entrypoint !== undefined && file !== undefined) {
    ...
    const html = yield* Effect.try({
      try: () =>
        new TextDecoder("utf-8", { fatal: true }).decode(compiledFile.contents),
  ...
  if (
    ...
    const beforePresentation = yield* SubscriptionRef.get(
      deps.capsulePresentation,
    );
    ...
    const accepted = yield* kernel
      ...
      .pipe(
        Effect.tapError(() =>
          deps
            .restoreCapsulePresentation(beforePresentation)
            .pipe(Effect.ignore),
        ),
      );
    yield* deps.releasePreview;
    const next = yield* kernel.snapshot;
    yield* deps.transitionShaping(
      envelope,
      operationId,
      "revision-accepted",
      next,
  ...
  if (browserSource !== undefined) {
    ...
    candidateReview = yield* deps.reviewDecodedCapsule(
      builtCapsule,
      builtArchive,
    );
    compiledPresentation =
      yield* deps.compiledCapsulePresentation(builtArchive);
