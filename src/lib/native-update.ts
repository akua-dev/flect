...
export interface NativeUpdateShape {
	...
	readonly install: (token: string) => Effect.Effect<NativeUpdateSnapshot, NativeUpdateError>;
	readonly relaunch: Effect.Effect<void, NativeUpdateError>;
}

export type NativeUpdateAdapterShape = NativeUpdateShape;

export class NativeUpdate extends Context.Service<NativeUpdate, NativeUpdateShape>()(
	'flect/NativeUpdate'
) {}
...
export const makeGuardedNativeUpdateLayer = (adapter: NativeUpdateAdapterShape) =>
