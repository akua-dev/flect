	...
	const findings = [
		{
			...
			warning: 'Forms are contained and cannot submit from the isolated preview.'
		},
		{
			found:
				(!moduleGraphSupported && /<script\b[^>]*\btype\s*=\s*["']?module\b/i.test(source)) ||
				(!moduleGraphSupported && /\b(?:import|export)\s+(?:[^('"`]|$)/m.test(source)),
...
const moduleEntrypoint = (html: string, files: ReadonlyArray<{ readonly path: string }>) => {
...
const unsupportedViteConfiguration = (
	...
	const decoder = new TextDecoder('utf-8', { fatal: true });
	const config = files.find((file) => /^vite\.config\.(?:[cm]?[jt]s)$/i.test(file.path));
	const configSource = config === undefined ? undefined : textFile(config, decoder);
	if (
	...
	for (const file of files.filter((candidate) => /\.(?:[cm]?[jt]sx?)$/i.test(candidate.path))) {
...
export const importWebProject = Effect.fn('Flect.WebProject.import')(function* (
	...
	const stripRoot = parsed.every(({ parts }) => parts.length > 1 && parts[0] === firstRoot);
	...
	const ignoredFiles = normalized
		...
		.map((file) => file.path)
		.toSorted();
	const tooLongPath = included.find((file) => file.path.length > MAX_PROJECT_PATH_LENGTH);
	...
	const bytes = included.reduce((total, file) => total + file.contents.byteLength, 0);
	if (included.length === 0 || included.length > MAX_PROJECT_FILES || bytes > MAX_CAPSULE_BYTES) {
		return yield* Effect.fail(
			failure('The project exceeds the 255 source files or 32 MiB import limit.')
		);
	}
	const entrypoints = included.filter((file) => file.path.toLowerCase() === 'index.html');
	if (entrypoints.length !== 1) {
		return yield* Effect.fail(
			failure('Choose a single project folder with one index.html at its root.')
		);
	}
			...
			: dependencies.includes('vue')
				...
				: dependencies.includes('svelte')
					...
					: dependencies.includes('react') || dependencies.includes('react-dom')
						...
						: ('vite' as const);
	const displayName =
		options.name?.trim().slice(0, 80) || (stripRoot ? firstRoot.slice(0, 80) : 'Web project');
	const name = projectName(options.name ?? (stripRoot ? firstRoot : ''));
	const compatibilityReport = compatibility(included, sourceEntrypoint !== undefined);
	const adaptations =
		sourceEntrypoint === undefined
			? []
			: [
	...
	}).pipe(Effect.mapError(() => failure('The web project could not be packaged.')));
	return {
		archive,
		report
	};
