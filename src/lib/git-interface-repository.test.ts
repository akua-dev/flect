...
import { GitActivationReceipt, makeGitInterfaceRepositoryLayer } from './git-interface-repository';
...
const makeStorage = () => {
	...
	return {
		...
		layer: Layer.succeed(InterfaceStorage)({
			read: (key) => Ref.get(values).pipe(Effect.map((items) => items.get(key) ?? null)),
			write: (key, value) => Ref.update(values, (items) => new Map(items).set(key, value)),
			remove: (key) =>
...
const makeGit = () => {
	...
	>([]);
	const branches = new Map<string, Map<string, Uint8Array>>([['master', new Map()]]);
			...
			Ref.update(commands, (items) => [...items, args]).pipe(
				Effect.andThen(
					Effect.sync(() => {
						...
						if (command === 'checkout' && first === '-b' && second !== undefined) {
							const source = branches.get(current) ?? new Map();
							branches.set(
								second,
								new Map([...source].map(([path, contents]) => [path, contents.slice()]))
							);
						...
						if (command === 'rev-parse') {
							const ref = first === undefined || first === 'HEAD' ? current : first;
			...
			Ref.update(checkpoints, (items) => [
				...
				Effect.andThen(
					Effect.suspend(() => {
						...
						if (
							(options.expectedCommit !== undefined && actual !== options.expectedCommit) ||
							(options.expectedCommit === undefined &&
						...
						const sourceBranch =
							options.baseCommit === undefined
								? undefined
								: [...heads].find(([, commit]) => commit === options.baseCommit)?.[0];
						const files = new Map(
							branches.get(options.branch) ??
								(sourceBranch === undefined ? [] : (branches.get(sourceBranch) ?? new Map()))
						);
		...
		snapshotRef: (options) =>
			Effect.suspend(() => {
				if (
					...
					(options.guards ?? []).some((guard) => heads.get(guard.branch) !== guard.commit)
				) {
					return Effect.fail(
						GitWorkspaceFailure.make({
				...
				return Effect.succeed(
					GitRefSnapshot.make({
						...
						branch: options.branch,
						commit: options.expectedCommit,
						files: [...(branches.get(options.branch) ?? new Map())].map(([path, contents]) => ({
							path,
							contents
						}))
					})
		...
		moveRef: (options) =>
			Effect.suspend(() => {
				...
				const sourceBranch = [...heads].find(
					([branch, commit]) => branch !== options.branch && commit === options.targetCommit
				)?.[0];
				branches.set(
					options.branch,
					new Map(sourceBranch === undefined ? [] : (branches.get(sourceBranch) ?? new Map()))
				);
				return Effect.succeed(
					GitRefMoved.make({
						type: 'ref-moved',
...
describe('GitInterfaceRepository', () => {
	it.effect('carries guarded authoring source through proposal and acceptance', () =>
		Effect.gen(function* () {
			...
			const layer = makeGitInterfaceRepositoryLayer({
				safeMode: false,
				workspaceId: 'default'
			}).pipe(Layer.provide(Layer.merge(storage.layer, Layer.succeed(GitWorkspace)(git.service))));
			...
			yield* Effect.gen(function* () {
				...
				git.branches.set('flect/authoring', new Map(git.branches.get('flect/accepted')));
				git.branches
					.get('flect/authoring')
				...
				assert.strictEqual(
					new TextDecoder().decode(
						git.branches.get('flect/proposal/revision-1')?.get('src/shaped.ts')
					),
					'export {};\n'
				);
				assert.isFalse(
					git.branches.get('flect/proposal/revision-1')?.has('src/obsolete.ts') ?? true
				);

				yield* repository.save(acceptedSnapshot);
				assert.strictEqual(
					new TextDecoder().decode(git.branches.get('flect/accepted')?.get('src/shaped.ts')),
					'export {};\n'
				);
				assert.isFalse(git.branches.get('flect/accepted')?.has('src/obsolete.ts') ?? true);
				assert.strictEqual(git.heads.get('flect/authoring'), git.heads.get('flect/accepted'));
	...
	it.effect('keeps the accepted commit protected while a proposal branch advances', () =>
		Effect.gen(function* () {
			const storage = makeStorage();
			const git = makeGit();
			const layer = makeGitInterfaceRepositoryLayer({
			...
			assert.strictEqual(receipt.proposal?.branch, 'flect/proposal/revision-1');
	...
	it.effect('fails closed when a protected ref changes between saves', () =>
		Effect.gen(function* () {
			...
			yield* Effect.gen(function* () {
				...
				const error = yield* repository.save(proposalSnapshot).pipe(Effect.flip);
	...
	it.effect(
		...
		() =>
			Effect.gen(function* () {
				const storage = makeStorage();
				const git = makeGit();
				const dependencies = Layer.merge(storage.layer, Layer.succeed(GitWorkspace)(git.service));
				...
				const recoveredLayer = makeGitInterfaceRepositoryLayer({
					...
					Layer.provide(Layer.merge(storage.layer, Layer.succeed(GitWorkspace)(reopenedGit)))
				);
				...
				assert.strictEqual(receipt.acceptedCommit, git.heads.get('flect/accepted'));
				assert.strictEqual(receipt.lastKnownGoodCommit, git.heads.get('flect/last-known-good'));
			})
	);

	it.effect('repairs an existing legacy ref set when recovery metadata cannot be read', () =>
		Effect.gen(function* () {
			const storage = makeStorage();
			const git = makeGit();
			const dependencies = Layer.merge(storage.layer, Layer.succeed(GitWorkspace)(git.service));
			...
			git.branches.get('flect/last-known-good')?.delete('.flect/snapshot.json');
			...
			const recoveredLayer = makeGitInterfaceRepositoryLayer({
				safeMode: false,
				workspaceId: 'default'
			}).pipe(Layer.provide(Layer.merge(storage.layer, Layer.succeed(GitWorkspace)(reopenedGit))));
			...
			assert.isDefined(git.branches.get('flect/accepted')?.get('.flect/snapshot.json'));
			const values = yield* Ref.get(storage.values);
	...
	it.effect('loads protected refs in a one-shot safe launcher and persists restoration', () =>
		Effect.gen(function* () {
			const storage = makeStorage();
			const git = makeGit();
			const initialLayer = makeGitInterfaceRepositoryLayer({
	...
	it.effect('repairs an activation receipt that disagrees with protected proposal state', () =>
		Effect.gen(function* () {
			const storage = makeStorage();
			const git = makeGit();
			const dependencies = Layer.merge(storage.layer, Layer.succeed(GitWorkspace)(git.service));
			...
			git.branches.get('flect/accepted')?.set('.flect/snapshot.json', encodedProposal);
			git.branches.get('flect/last-known-good')?.set('.flect/snapshot.json', encodedProposal);
