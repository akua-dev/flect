...
import { AgentCommandSource, ControlCommandSource, UserCommandSource } from '../../shared/control';
import { defaultInterfaceDocument, InterfaceDocument } from '../../shared/interface-document';
import { RevisionId } from '../../shared/revisions';
import { SandboxedShell, type SandboxedShellShape } from '../shell/sandboxed-shell-service';
import {
	AgentWorkspace,
	AgentWorkspaceLive,
...
describe('AgentWorkspace', () => {
	it.effect('returns a typed edit request without treating questions as Shape', () => {
		...
		return Effect.gen(function* () {
			...
			const question = yield* workspace.submitAppPrompt(userOperation(55), 'Can you see it?');
			...
			assert.strictEqual(change.editRequest?.instruction, 'Make the primary action blue');
			assert.strictEqual(change.editRequest?.requestId, 'tool-edit-request-1');
		}).pipe(Effect.provide(layer));
	});

	it.effect('keeps a candidate-bound Preview App conversation in a separate session', () => {
		...
		return Effect.gen(function* () {
			...
			assert.strictEqual(createSession.mock.calls[0]?.[0].externalExtensions?.app, true);
			assert.strictEqual(createSession.mock.calls[0]?.[0].externalExtensions?.shaper, true);
			assert.strictEqual(createSession.mock.calls[1]?.[0].externalExtensions?.app, true);
			assert.strictEqual(createSession.mock.calls[1]?.[0].externalExtensions?.shaper, false);
			...
			assert.deepStrictEqual(
				...
				['Can you see the candidate?', 'Done.', 'What actions are available?', 'Done.']
			);
			assert.notStrictEqual(promptCalls[0]?.[0], promptCalls[1]?.[0]);
			assert.include(String(promptCalls[1]?.[1]), 'Preview candidate');
			assert.include(String(promptCalls[1]?.[1]), 'Can you see the candidate?');
			assert.notInclude(String(promptCalls[1]?.[1]), 'Use accepted');
		}).pipe(Effect.provide(layer));
	});

	it.effect('executes candidate Bash in the isolated Preview App workspace', () => {
		...
		return Effect.gen(function* () {
			...
			assert.strictEqual(shellExecute.mock.calls[0]?.[2]?.agentContext?.binding, 'candidate');
			assert.strictEqual(vi.mocked(client.completeShellRequest).mock.calls[0]?.[1], 'app');
			const evidence = yield* journal.query(OperationQuery.make({ category: 'tool', revisionId }));
			assert.isAbove(evidence.length, 0);
		}).pipe(Effect.provide(layer));
	});

	it.effect('disposes candidate authority without clearing accepted history', () => {
	...
	it.effect('isolates candidate overlap and cancellation from accepted App', () =>
		Effect.gen(function* () {
			...
			yield* Effect.gen(function* () {
				...
				assert.strictEqual(cancelSession.mock.calls[0]?.[0], 'session-agent-workspace-1');
	...
	it.effect('reuses one protected session across separate App and Shaper state', () => {
		...
		return Effect.gen(function* () {
			...
			assert.deepStrictEqual(
				...
				['Change the interface', `Change complete: ${defaultInterfaceDocument.name}`]
			);
			assert.deepStrictEqual(
	...
	it.effect('replaces the session for model or extension selection changes', () => {
		const { closeSession, createSession, layer } = makeLayer();
		return Effect.gen(function* () {
			const workspace = yield* AgentWorkspace;
			yield* workspace.refresh;
			yield* workspace.submitAppPrompt(userOperation(1), 'First');
			yield* workspace.selectModel(ModelSelection.make({ provider: model.provider, id: model.id }));
			yield* workspace.setExternalExtensions('shaper', true);
			yield* workspace.selectReasoning('high');
	...
	it.effect('keeps tool and validation activity visible without dropping the session', () => {
		const issues = [
		...
		return Effect.gen(function* () {
			...
			assert.deepStrictEqual(snapshot.shaper.activities[1]?.validationIssues, issues);
	...
	it.effect('retries once and fails when Shaper submits no proposal', () => {
		const { client, layer } = makeLayer({
			shape: () => Stream.succeed(ShapeCompleted.make({ type: 'shape_completed' }))
		});
		return Effect.gen(function* () {
			const workspace = yield* AgentWorkspace;
			yield* workspace.refresh;
			const error = yield* workspace
				.submitShaperInstruction(userOperation(21), 'Return prose only', defaultInterfaceDocument)
				.pipe(Effect.flip);
	...
	it.effect('latches the first Shaper Bash proposal and rejects duplicate authority', () => {
		const candidate = InterfaceDocument.make({
			...defaultInterfaceDocument,
		...
		const { client, layer } = makeLayer({
			...
			execute: (role, _line, options) =>
				Effect.gen(function* () {
					...
					const submit = (proposalSource: AgentCommandSource, document: InterfaceDocument) =>
					...
					const conflict = yield* Effect.result(submit(source, conflictingCandidate));
					const appAttempt = yield* Effect.result(
						submit(AgentCommandSource.make({ ...source, role: 'app' }), candidate)
					);
	...
	it.effect('confirms an authored app only when the controller concludes the turn', () => {
	...
	it.effect('reports a bounded activation failure without any completion claim', () => {
	...
	it.effect('cancels a running role through the runtime and its Effect fiber', () => {
	...
	it.effect('finishes a running shell activity when its role is cancelled', () => {
		...
		const { layer } = makeLayer({
			...
			execute: () => Deferred.succeed(shellStarted, undefined).pipe(Effect.andThen(Effect.never))
		});
		...
		return Effect.gen(function* () {
			...
			const snapshot = yield* workspace.snapshot;
			const activity = snapshot.app.activities.find((candidate) => candidate.callId === callId);
			assert.strictEqual(snapshot.app.status, 'ready');
	...
	it.effect('renders a bounded trusted-extension failure as transferable activity evidence', () => {
		const { layer } = makeLayer({
			prompt: () =>
				Stream.make(
					{ type: 'turn_started' },
		...
		return Effect.gen(function* () {
			const workspace = yield* AgentWorkspace;
			yield* workspace.refresh;
			yield* workspace.submitAppPrompt(userOperation(41), 'Exercise the trusted extension');
			const snapshot = yield* workspace.snapshot;
			const activity = snapshot.app.activities.find(
				(candidate) => candidate.callId === 'extension-failure-agent-workspace'
			);
			const journal = yield* OperationJournal;
			const entries = yield* journal.query(
			...
			assert.strictEqual(activity?.resultSummary, 'A trusted Pi extension failed.');
			assert.strictEqual(
				activity?.output,
			...
			assert.isTrue(
				entries.some(
					(entry) =>
						entry.toolCallId === 'extension-failure-agent-workspace' && entry.phase === 'failed'
				)
			);
