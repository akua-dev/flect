...
export interface WorkspaceControlTransportShape {
	...
	readonly publishEvent: (event: FlectWorkspaceEvent) => Effect.Effect<void, FlectUnavailableError>;
...
export const makeBrowserWorkspaceControlTransportLayer = (baseUrl = '/api/control') =>
	Layer.effect(
		WorkspaceControlTransport,
		Effect.gen(function* () {
			...
			const nextCommand = Effect.fn('ControlTransport.nextCommand')((workspaceId: string) =>
				HttpClientRequest.post('/commands/next').pipe(
					...
					Effect.flatMap(transport.execute),
					Effect.flatMap(HttpClientResponse.schemaBodyJson(ControlCommandsResponse)),
