...
import { ProposalBuild, type ProposalBuildShape } from '../build/proposal-build';
import {
...
import { ExtensionCatalog, makeExtensionCatalogLayer } from '../extensions/extension-catalog';
import {
...
import { AgentPromptOutcome, AgentWorkspace, type AgentWorkspaceShape } from './agent-workspace';
import { InterfaceStorage } from './interface-store';
import { OperationJournal, OperationJournalLive, OperationQuery } from './operation-journal';
...
const makeLayer = (options?: {
	...
	readonly extensionServices?: Layer.Layer<ExtensionCatalog | PortableExtensionHost>;
	...
	const selectModel = vi.fn((_selection: ModelSelection | undefined) => Effect.void);
	const setExternalExtensions = vi.fn((_role: 'app' | 'shaper', _enabled: boolean) => Effect.void);
	...
	const submitShaperInstruction = vi.fn<AgentWorkspaceShape['submitShaperInstruction']>(
	...
	const concludeShaperTurn = vi.fn<AgentWorkspaceShape['concludeShaperTurn']>(() => Effect.void);
	const agentLayer = Layer.effect(
		...
		Effect.gen(function* () {
			...
			return {
				...
				proposeShaperApp: (_source, _archive, name) => Effect.succeed({ status: 'proposed', name }),
				submitAppPrompt,
				submitPreviewPrompt,
				submitShaperInstruction,
				concludeShaperTurn,
	...
	const dependencies = Layer.mergeAll(
		baseDependencies,
		...(options?.git === undefined ? [] : [Layer.succeed(GitWorkspace)(options.git)]),
		...
		...(options?.extensionServices === undefined ? [] : [options.extensionServices]),
		...(options?.sharingServices === undefined ? [] : [options.sharingServices])
	);
	return {
...
const portablePackage = PortableExtensionPackage.make({
	...
	provenance: {
		...
		revision: 'v1.0.0',
		bundleSha256: 'a4a348b6f4e91da9c77bf69cf56383647597fdf2b7a64d71ae214a0261892537'
	}
});
...
const makeSharingServices = () => {
	...
	const candidateOverride = Ref.makeUnsafe<ShareCandidateMaterial | undefined>(undefined);
	const latestCandidate = Ref.makeUnsafe<ShareCandidateMaterial | undefined>(undefined);
	const signatureStatus = Ref.makeUnsafe<ShareSignatureAssessment['status']>('unsigned');
	...
	const candidateStore: ShareCandidateStoreShape = {
		...
		save: (archive) =>
			hashCapsuleArchive(archive).pipe(
				Effect.orDie,
				Effect.tap((digest) =>
					Ref.update(candidateArchives, (current) => new Map(current).set(digest, archive.slice()))
				)
			),
		load: (digest) =>
			Ref.get(candidateArchives).pipe(Effect.map((current) => current.get(digest)?.slice())),
		remove: (digest) =>
	...
	const store: ShareInstallationStoreShape = {
			...
			Effect.gen(function* () {
				...
				yield* Ref.update(installations, (snapshot) =>
					ShareInstallationSnapshot.make({
						formatVersion: 1,
						entries: [
							...snapshot.entries.filter((entry) => entry.shareId !== record.shareId),
							record
						]
					})
				);
		...
		remove: (shareId) =>
			Ref.update(installations, (snapshot) =>
				ShareInstallationSnapshot.make({
					formatVersion: 1,
					entries: snapshot.entries.filter((entry) => entry.shareId !== shareId)
				})
			)
	};
	...
	const repository: ShareRepositoryShape = {
		retain: (input) =>
			Ref.update(calls, (entries) => [...entries, `retain:${input.shareId}`]).pipe(
				Effect.as({
					refs: {
						base: input.commit,
		...
		prepareUpdate: (input) =>
			Ref.update(calls, (entries) => [...entries, `update:${input.shareId}`]).pipe(
				Effect.flatMap(() =>
		...
		checkpointFork: (input) =>
			Ref.update(calls, (entries) => [
				...
				Effect.as({
					fork: input.expectedForkCommit === 'a'.repeat(40) ? 'd'.repeat(40) : 'e'.repeat(40)
				})
			),
		restoreFork: (input) =>
			Ref.update(calls, (entries) => [
		...
		restoreCandidateRef: (input) =>
			Ref.update(calls, (entries) => [...entries, `restore-candidate-ref:${input.shareId}`]),
		acceptCandidate: (input) =>
			Ref.update(calls, (entries) => [...entries, `accept:${input.shareId}`]).pipe(
				Effect.as({
					refs: {
		...
		restoreCandidate: (input) =>
			Ref.update(calls, (entries) => [...entries, `restore-candidate:${input.shareId}`]),
		snapshotArtifact: (input) =>
			Ref.update(calls, (entries) => [...entries, `snapshot:${input.shareId}:${input.role}`]).pipe(
				Effect.as([
					{
						path: 'components/card/index.tsx',
						contents: new TextEncoder().encode('export const Card = 1')
		...
		exportFork: (input) =>
			Ref.update(calls, (entries) => [...entries, `export-fork:${input.shareId}`]).pipe(
				Effect.as(new Uint8Array([1, 2, 3]))
			),
		exportCandidate: (input) =>
			Ref.update(calls, (entries) => [...entries, `export-candidate:${input.shareId}`]).pipe(
				Effect.as(new Uint8Array([1, 2, 3]))
			),
...
describe('FlectWorkspaceController', () => {
	it.effect(
		...
		() => {
			...
			return Effect.gen(function* () {
				...
				assert.strictEqual(reviewing.shaping.active.id, before.shaping.active.id);
				assert.strictEqual(reviewing.shareReview?.shareId, 'dev.flect.shared-card');
				assert.isTrue(reviewing.shareReview?.inactive);
				assert.deepStrictEqual(reviewing.shares?.entries, []);

				yield* controller.dispatch(
					envelope(401, RejectShareCandidate.make({ type: 'reject-share-candidate' }))
				);
	...
	it.effect(
		...
		() => {
			...
			return Effect.gen(function* () {
				...
				assert.deepStrictEqual(yield* Ref.get(sharing.calls), ['retain:dev.flect.shared-card']);
				assert.deepStrictEqual(snapshot.shares?.entries[0]?.installedArtifactIds, [
					'dev.flect.shared-card.component'
				]);
				assert.strictEqual(snapshot.shareReview?.shareId, 'dev.flect.shared-card');
			}).pipe(Effect.provide(layer));
		}
	);

	it.effect('restores a retained inactive share review after controller restart', () => {
		const sharing = makeSharingServices();
		return Effect.gen(function* () {
			...
			assert.strictEqual((yield* first.snapshot).shares?.entries[0]?.pending?.lineage, 'new');
			...
			assert.strictEqual(restored.shareReview?.shareId, 'dev.flect.shared-card');
			assert.strictEqual(restored.shareReview?.lineage, 'new');
			assert.isTrue(restored.shareReview?.inactive);
			assert.strictEqual(restored.document.name, defaultInterfaceDocument.name);
		});
	});
	...
	it.effect('denies protected share retention to App Agent and Shaper', () => {
		...
		const { layer } = makeLayer({ sharingServices: sharing.layer });
		return Effect.gen(function* () {
			const controller = yield* FlectWorkspaceController.pipe(Effect.provide(layer));
			...
			for (const [index, command] of protectedCommands.entries()) {
				for (const source of [appAgent, shaperAgent]) {
					const failure = yield* controller
						.dispatch(envelope(410 + index * 2 + (source.role === 'app' ? 0 : 1), command, source))
						.pipe(Effect.flip);
	...
	it.effect(
		...
		() => {
			...
			return Effect.gen(function* () {
				...
				yield* controller.dispatch(
					envelope(432, RejectShareCandidate.make({ type: 'reject-share-candidate' }))
				);
				...
				const receipt = yield* controller.dispatch(envelope(434, checkpoint, shaperAgent));
	...
	it.effect(
		...
		() => {
			...
			return Effect.gen(function* () {
				...
				assert.strictEqual(
					(yield* decodeShareArchive(yield* controller.readShareExport('dev.flect.shared-card')))
				...
				assert.deepStrictEqual((yield* controller.snapshot).shares?.entries, []);
				assert.deepStrictEqual(yield* Ref.get(sharing.calls), [
	...
	it.effect('prepares a guarded upstream update and rejects only its candidate ref', () => {
		const sharing = makeSharingServices();
		const { layer } = makeLayer({ sharingServices: sharing.layer });
		return Effect.gen(function* () {
			const controller = yield* FlectWorkspaceController;
			...
			yield* open(442);
			assert.strictEqual((yield* controller.snapshot).shareReview?.lineage, 'update');
			assert.deepStrictEqual((yield* controller.snapshot).shareReview?.changes, []);
			...
			assert.strictEqual(exportedCandidate.manifest.repository.commit, 'c'.repeat(40));
			assert.deepStrictEqual(exportedCandidate.manifest.signatures, []);
			assert.include(yield* Ref.get(sharing.calls), 'export-candidate:dev.flect.shared-card');

			const restarted = yield* FlectWorkspaceController.pipe(
				Effect.provide(makeLayer({ sharingServices: sharing.layer }).layer)
			);
			assert.strictEqual((yield* restarted.snapshot).shareReview?.lineage, 'update');

			yield* restarted.dispatch(
				envelope(444, RejectShareCandidate.make({ type: 'reject-share-candidate' }))
			);
	...
	it.effect(
		...
		() => {
			...
			return Effect.gen(function* () {
				...
				yield* controller.dispatch(envelope(453, AcceptProposal.make({ type: 'accept-proposal' })));
				...
				yield* controller.dispatch(envelope(457, AcceptProposal.make({ type: 'accept-proposal' })));
				...
				assert.include(yield* Ref.get(sharing.calls), 'accept:dev.flect.shared-card');
			}).pipe(Effect.provide(layer));
		}
	);
	...
	it.effect(
		...
		() => {
			...
			return Effect.gen(function* () {
				...
				assert.strictEqual((yield* controller.snapshot).shareReview?.lineage, 'conflict');
				...
				const instruction = submitShaperInstruction.mock.calls.at(-1)?.[1] ?? '';
				assert.include(instruction, 'conflicts.txt');
				assert.include(instruction, 'flect share resolve');
				assert.strictEqual((yield* controller.snapshot).workbench?.target, 'shape');
				...
				assert.strictEqual(snapshot.shares?.entries[0]?.refs.candidate, 'e'.repeat(40));
				...
				assert.strictEqual(restored.shares?.entries[0]?.refs.candidate, 'e'.repeat(40));
			}).pipe(Effect.provide(layer));
		}
	);
	...
	it.effect(
		...
		() => {
			...
			return Effect.gen(function* () {
				...
				assert.include(yield* Ref.get(sharing.calls), 'resolve:dev.flect.shared-card:1:0');
				const snapshot = yield* controller.snapshot;
				assert.strictEqual(snapshot.shareReview?.lineage, 'fork');
				assert.notInclude(snapshot.shareReview?.blockers ?? [], 'conflict');
				assert.strictEqual(snapshot.shares?.entries[0]?.refs.candidate, 'e'.repeat(40));
	...
	it.effect('routes a shared experience through the existing capsule preview', () =>
		Effect.gen(function* () {
			...
			const snapshot = yield* controller.snapshot;
			const presentation = yield* (
				controller.capsulePresentation ?? Effect.die('capsule presentation unavailable')
			);
			...
			assert.strictEqual(snapshot.shaping.proposal?.document.name, 'Shared experience UI');
			assert.strictEqual(presentation.candidateReview?.id, 'dev.flect.shared-experience.app');
		})
	);

	it.effect('routes a shared extension through candidate enable, test, and Keep gates', () =>
		Effect.gen(function* () {
			const archive = yield* encodeCapsule({
				manifest: {
					formatVersion: 1,
				...
				files: [
					{
						path: 'ui/interface.json',
						contents: new TextEncoder().encode(JSON.stringify(defaultInterfaceDocument))
					},
					{
			...
			const { layer } = makeLayer({
				sharingServices: sharing.layer,
				extensionServices: makePortableExtensionServices('dev.flect.shared-extension.package')
			});
			const controller = yield* FlectWorkspaceController.pipe(Effect.provide(layer));
			yield* controller.dispatch(
				envelope(
			...
			const blocked = yield* controller
				.dispatch(envelope(469, AcceptProposal.make({ type: 'accept-proposal' })))
				.pipe(Effect.flip);
			...
			assert.deepStrictEqual(
				...
				[
					['dev.flect.shared-extension.package', portablePackage.id, 'app', 'candidate', false],
					['dev.flect.shared-extension.package', portablePackage.id, 'shaper', 'candidate', false]
				]
			);
			yield* controller.dispatch(
				envelope(
			...
			yield* controller.dispatch(envelope(471, AcceptProposal.make({ type: 'accept-proposal' })));
			const extensions = (yield* controller.snapshot).extensions;
			assert.isTrue(extensions?.entries.every((entry) => entry.binding === 'accepted') ?? false);
			assert.strictEqual(extensions?.entries.find((entry) => entry.role === 'app')?.tested, true);
		})
	);

	it.effect('rechecks signature authority immediately before share activation', () => {
		const sharing = makeSharingServices();
		const { layer } = makeLayer({
			sharingServices: sharing.layer,
			git: makeIdleGit()
		...
		return Effect.gen(function* () {
			...
			assert.strictEqual(failure.message, 'Resolve the shared review blockers before activation.');
			assert.strictEqual((yield* controller.snapshot).shaping.proposal, undefined);
		}).pipe(Effect.provide(layer));
	});

	it.effect('keeps a retained share inactive while the protected shell is in safe mode', () => {
		const sharing = makeSharingServices();
		const { layer } = makeLayer({
			sharingServices: sharing.layer,
			git: makeIdleGit()
		...
		return Effect.gen(function* () {
			...
			yield* controller.dispatch(envelope(465, EnterSafeMode.make({ type: 'enter-safe-mode' })));
			...
			assert.strictEqual(failure._tag, 'OperationFailed');
			assert.strictEqual(failure.message, 'Restore the interface before activating shared source.');
			assert.strictEqual(snapshot.phase, 'safe-mode');
			assert.strictEqual(snapshot.shareReview?.shareId, 'dev.flect.shared-card');
			assert.strictEqual(snapshot.shares?.entries[0]?.pending?.lineage, 'new');
		}).pipe(Effect.provide(layer));
	});

	it.effect('exports the exact reviewed candidate as a deterministic share archive', () => {
		const sharing = makeSharingServices();
		const { layer } = makeLayer({ sharingServices: sharing.layer });
		return Effect.gen(function* () {
			const controller = yield* FlectWorkspaceController;
			...
			const archive = yield* controller.readShareExport('dev.flect.shared-card');
	...
	it.effect('opens an imported declarative capsule as an isolated proposal', () => {
		const { layer } = makeLayer();
		return Effect.gen(function* () {
			const archive = yield* encodeCapsule({
				manifest: {
			...
			const snapshot = yield* controller.snapshot;
			assert.strictEqual(snapshot.shaping.proposal?.document.name, 'Imported product');
			assert.strictEqual(snapshot.shaping.proposal?.status, 'previewed');
			assert.strictEqual(snapshot.workbench?.binding, 'candidate');
			const review = yield* (
				controller.capsulePresentation ?? Effect.succeed<CapsulePresentationState>({})
			);
			...
			assert.deepStrictEqual(review?.candidateReview?.platforms, ['browser', 'macos']);
			const blocked = yield* controller
				.dispatch(
					envelope(
						70,
			...
			assert.strictEqual(retained.shaping.proposal?.document.name, 'Imported product');
	...
	it.effect(
		...
		() => {
			...
			return Effect.gen(function* () {
				...
				const controller = yield* FlectWorkspaceController;
				yield* controller.dispatch(
					envelope(720, ImportCapsule.make({ type: 'import-capsule', archive }))
				);
				let extensions = (yield* controller.snapshot).extensions;
				assert.deepStrictEqual(
					extensions?.entries.map((entry) => [entry.role, entry.binding, entry.state]),
					[
				...
				const untested = yield* controller
					.dispatch(envelope(723, AcceptProposal.make({ type: 'accept-proposal' })))
					.pipe(Effect.flip);
				assert.strictEqual(untested._tag, 'OperationFailed');
				assert.strictEqual((yield* controller.snapshot).shaping.proposal?.status, 'previewed');
				...
				yield* controller.dispatch(envelope(725, AcceptProposal.make({ type: 'accept-proposal' })));
	...
	it.effect('builds an imported Vite project from the exact Git proposal before review', () => {
		const acceptedCommit = 'a'.repeat(40);
		const lastKnownGoodCommit = 'b'.repeat(40);
		const proposalCommit = 'c'.repeat(40);
		const generatedLock = new TextEncoder().encode('{"lockfileVersion":3,"packages":{}}');
					...
					outputs: [
						{
							path: 'app.js',
							kind: 'chunk',
							contents: new TextEncoder().encode("document.body.dataset.ready='true'")
						}
					]
				})
			)
		...
		return Effect.gen(function* () {
			const imported = yield* importWebProject([
				...
				},
				{
					...
					contents: new TextEncoder().encode("document.querySelector('#root')!.textContent='Built'")
				}
			]);
			const controller = yield* FlectWorkspaceController;
			yield* controller.dispatch(
			...
			assert.strictEqual(workspaceSnapshot.build?.artifactDigest, 'e'.repeat(64));
			assert.strictEqual(presentation.candidateReview?.revision, proposalCommit);
			...
			assert.strictEqual(buildRequest.lastKnownGoodCommit, lastKnownGoodCommit);
			assert.strictEqual(buildRequest.entrypoint, 'src/main.ts');
			assert.ok(
				checkpoint.mock.calls[0]?.[0].files.some((file) => file.path === 'project/src/main.ts')
			);
			assert.ok(
				checkpoint.mock.calls[1]?.[0].files.some(
					(file) => file.path === 'project/package-lock.json' && file.contents === generatedLock
				)
			);
		}).pipe(Effect.provide(layer));
	});
	...
	it.effect(
		...
		() => {
			...
			return Effect.gen(function* () {
				...
				assert.strictEqual(snapshot.shaping.active.document.name, 'Driftwood Coffee');
				...
				assert.strictEqual(presentation.acceptedReview?.id, 'local.flect.driftwood-coffee');
				assert.deepStrictEqual(
	...
	it.effect(
		...
		() => {
			...
			return Effect.gen(function* () {
				...
				assert.strictEqual(snapshot.shaping.proposal, undefined);
				assert.deepStrictEqual(snapshot.shaping.active.document, before.shaping.active.document);
	...
	it.effect('keeps required capsule capabilities visible but blocks activation', () => {
		const { layer } = makeLayer();
		return Effect.gen(function* () {
			const archive = yield* encodeCapsule({
				manifest: {
			...
			const controller = yield* FlectWorkspaceController;
			yield* controller.dispatch(
				envelope(211, ImportCapsule.make({ type: 'import-capsule', archive }))
			);
			const review = (yield* (
				controller.capsulePresentation ?? Effect.succeed<CapsulePresentationState>({})
			)).candidateReview;
			...
			assert.match(review?.permissionContext.requestDigest ?? '', /^[0-9a-f]{64}$/);
			assert.strictEqual(review?.activationBlocked, true);

			const failure = yield* controller
				.dispatch(envelope(212, AcceptProposal.make({ type: 'accept-proposal' })))
				.pipe(Effect.flip);
			assert.strictEqual(failure._tag, 'OperationFailed');
			assert.strictEqual((yield* controller.snapshot).shaping.proposal?.status, 'previewed');
		}).pipe(Effect.provide(layer));
	...
	it.effect(
		...
		() => {
			...
			const projection = (context: Parameters<ProductCapabilityRegistryShape['permissions']>[0]) =>
			...
			const invokeDetailed = vi.fn<ProductCapabilityRegistryShape['invokeDetailed']>(
				(_context, invocation) =>
			...
			return Effect.gen(function* () {
				...
				const controller = yield* FlectWorkspaceController;
				yield* controller.dispatch(
					envelope(213, ImportCapsule.make({ type: 'import-capsule', archive }))
				);
				const candidateReview = (yield* (
					controller.capsulePresentation ?? Effect.succeed<CapsulePresentationState>({})
				)).candidateReview;
				assert.strictEqual(candidateReview?.activationBlocked, true);
				assert.strictEqual(candidateReview?.capabilities[0]?.state, 'available');
				assert.match(candidateReview?.permissionContext.requestDigest ?? '', /^[0-9a-f]{64}$/);
				...
				assert.deepStrictEqual(invokeDetailed.mock.calls[0]?.[0], grantedReview?.permissionContext);
				assert.deepStrictEqual(
				...
				const acceptedReview = (yield* (
					controller.capsulePresentation ?? Effect.succeed<CapsulePresentationState>({})
				)).acceptedReview;
				assert.strictEqual(acceptedReview?.capabilities[0]?.state, 'revoked');
				const capabilityRecords = (yield* controller.snapshot).operations.filter(
					(record) => record.capability !== undefined
				);
				assert.strictEqual(capabilityRecords.length, 2);
				assert.strictEqual(capabilityRecords[0]?.capability?.decisionId, decisionId);
	...
	it.effect('answers Use questions and applies only a typed edit-tool handoff', () => {
		...
		return Effect.gen(function* () {
			...
			assert.strictEqual(edited.workbench?.handoff?.instruction, 'Make the primary action blue');
			assert.include(submitShaperInstruction.mock.calls.at(-1)?.[1] ?? '', 'Revision:');
		}).pipe(Effect.provide(layer));
	});

	it.effect('routes an explicit landing-page build request directly to Shaper', () => {
		const { layer, submitAppPrompt, submitShaperInstruction } = makeLayer();
		return Effect.gen(function* () {
			const controller = yield* FlectWorkspaceController;
			yield* controller.dispatch(
			...
			assert.strictEqual(submitShaperInstruction.mock.calls.at(-1)?.[3], instruction);
			assert.notStrictEqual(after.shaping.active.id, before.shaping.active.id);
			assert.strictEqual(after.workbench?.target, 'use');
		}).pipe(Effect.provide(layer));
	});

	it.effect('keeps internal routing behind one continuously active product', () => {
		...
		return Effect.gen(function* () {
			...
			assert.strictEqual(submitAppPrompt.mock.calls.at(-1)?.[1], 'What can this product do?');
			...
			assert.notStrictEqual(corrected.shaping.active.id, first.shaping.active.id);
		}).pipe(Effect.provide(layer));
	});

	it.effect('returns a cancelled Shaper turn to a usable terminal state', () => {
		const { layer } = makeLayer();
		return Effect.gen(function* () {
			const controller = yield* FlectWorkspaceController;
			yield* controller.dispatch(
			...
			yield* controller.dispatch(
				envelope(66, CancelRole.make({ type: 'cancel-role', role: 'shaper' }))
			);
	...
	it.effect('rejects stale failure handoffs and injects only correlated evidence', () => {
		const { layer, submitShaperInstruction } = makeLayer();
		return Effect.gen(function* () {
			const controller = yield* FlectWorkspaceController;
			...
			yield* controller
				...
				.pipe(Effect.flip);
			const failed = (yield* journal.query(OperationQuery.make({ failuresOnly: true }))).at(-1);
	...
	it.effect('routes user and outside commands through one idempotent state machine', () => {
		const { layer } = makeLayer();
		return Effect.gen(function* () {
			const controller = yield* FlectWorkspaceController;
			const first = yield* controller.dispatch(
				envelope(1, SetMode.make({ type: 'set-mode', mode: 'edit' }), outside)
			);
	...
	it.effect('rejects stale sequences and outside attempts to enable control', () => {
		const { layer } = makeLayer();
		return Effect.gen(function* () {
			const controller = yield* FlectWorkspaceController;
			const initial = yield* controller.snapshot;
			yield* controller.dispatch(envelope(1, Inspect.make({ type: 'inspect' })));
			const conflict = yield* controller
				.dispatch(
					envelope(2, SetMode.make({ type: 'set-mode', mode: 'run' }), user, initial.sequence)
				)
				.pipe(Effect.flip);
			const unauthorized = yield* controller
				.dispatch(envelope(3, EnableControl.make({ type: 'enable-control' }), outside))
				.pipe(Effect.flip);
	...
	it.effect('enforces the role-scoped agent command policy before claim', () => {
		const { layer } = makeLayer();
		return Effect.gen(function* () {
			const controller = yield* FlectWorkspaceController;
			...
			yield* Effect.forEach(
				...
				({ command, source }, index) =>
					controller.dispatch(envelope(100 + index, command, source)).pipe(
						Effect.flip,
						Effect.tap((error) =>
							Effect.sync(() => assert.strictEqual(error._tag, 'ControlUnauthorized'))
						)
					),
				{ discard: true }
			);
	...
	it.effect('activates a local interface edit without a separate review decision', () => {
		const { layer } = makeLayer();
		return Effect.gen(function* () {
			const controller = yield* FlectWorkspaceController;
			yield* controller.dispatch(
			...
			assert.strictEqual(accepted.shaping.active.document.name, 'Controller proposal');
		}).pipe(Effect.provide(layer));
	});

	it.effect('does not let a stale kernel event re-enter safe mode after restore', () => {
		const { layer } = makeLayer();
		return Effect.gen(function* () {
			const controller = yield* FlectWorkspaceController;
			yield* controller.dispatch(envelope(1, EnterSafeMode.make({ type: 'enter-safe-mode' })));
			yield* controller.dispatch(envelope(2, RestoreSafeMode.make({ type: 'restore-safe-mode' })));
	...
	it.effect('lets outside clients use accepted edits without bypassing safe mode', () => {
		const { layer } = makeLayer();
		return Effect.gen(function* () {
			const controller = yield* FlectWorkspaceController;
			yield* controller.dispatch(
			...
			yield* controller.dispatch(envelope(5, EnterSafeMode.make({ type: 'enter-safe-mode' })));
	...
	it.effect('applies model, extension, and rail commands to the same reactive snapshot', () => {
		...
		return Effect.gen(function* () {
			...
			yield* controller.dispatch(
				envelope(3, SetRailWidth.make({ type: 'set-rail-width', width: 480 }))
			);
			yield* controller.dispatch(
				envelope(
					4,
	...
	it.effect('keeps unsent drafts reactive and outside public snapshots', () => {
		...
		return Effect.gen(function* () {
			...
			const snapshot = yield* controller.snapshot;
			assert.strictEqual(continuity.drafts.acceptedUse, 'Private unsent question');
			assert.strictEqual(continuity.drafts.shape, 'Private shape request');
			assert.strictEqual(JSON.stringify(snapshot).includes('Private unsent question'), false);
			assert.strictEqual(JSON.stringify(snapshot).includes('Private shape request'), false);
		}).pipe(Effect.provide(layer));
	});
});
