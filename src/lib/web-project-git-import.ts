...
export const importWebProjectFromGit = Effect.fn("Flect.WebProject.importGit")(
  function* (
    url: string,
    commit: string,
    options?: {
    ...
    if (
      ...
      return yield* Effect.fail(
        failure(
          "Use a credential-free HTTPS Git URL and the exact lowercase 40-character commit ID.",
        ),
      );
    }
      ...
      Effect.gen(function* () {
        ...
        yield* Effect.acquireRelease(
          workspace
            ...
            .pipe(
              Effect.mapError(() =>
                failure("The isolated Git workspace could not start."),
              ),
            ),
