...
export class Uninstall extends Context.Service<Uninstall, UninstallShape>()(
  "flect/Uninstall",
) {}
  ...
  Effect.gen(function* () {
    ...
    const inspect = Effect.fn("Flect.Uninstall.inspect")(function* () {
      ...
      return UninstallPlan.make({
        ...
        ownedIntegrations: [
          shellItem(shellStatus),
          ...agentStatuses.map(agentItem),
        ],
        retained,
      });
    });
      ...
      const ownedIntegrations = yield* Effect.forEach(
        ...
        (item) => {
          ...
          return item.host === undefined
            ? Effect.succeed(
                UninstallOwnedItem.make({ ...item, result: "failed" }),
              )
