...
describe("Uninstall", () => {
  it("derives only the fixed app bundle from a packaged public executable", () => {
    ...
    assert.isUndefined(
      flectApplicationPathFromExecutable("/usr/local/bin/flect"),
    );
    assert.isUndefined(
      flectApplicationPathFromExecutable(
        "/Applications/Other.app/Contents/MacOS/flect",
      ),
    );
  });
  ...
  it.effect("classifies owned, foreign, absent, and retained state", () =>
    Effect.gen(function* () {
      ...
      const agents: AgentIntegrationShape = {
        status: (host) =>
          Effect.succeed(
            statuses.find((status) => status.host === host) ?? statuses[0],
          ),
      ...
      assert.deepStrictEqual(
        result.plan.ownedIntegrations.map((item) => [
          item.kind,
          item.host,
          item.result,
        ]),
  ...
  it.effect(
    "rejects a path that is not the fixed Flect application bundle",
    () =>
      Effect.gen(function* () {
        const shell: ShellLinkShape = {
