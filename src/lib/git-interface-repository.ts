...
export class GitProposalReceipt extends Schema.Class<GitProposalReceipt>(
  "GitProposalReceipt",
)({
...
export const makeGitInterfaceRepositoryLayer = ({
  ...
  Layer.effect(
    ...
    Effect.gen(function* () {
      ...
      const receiptRef = yield* Ref.make<GitActivationReceipt | undefined>(
        undefined,
      );
      const openedRef = yield* Ref.make(false);
      const existedRef = yield* Ref.make(false);

      const ensureOpen = Effect.fn("Flect.GitInterfaceRepository.open")(
        function* () {
          if (yield* Ref.get(openedRef)) {
            return yield* Ref.get(existedRef);
          }
          const opened = yield* git
            .open({ workspaceId })
      ...
      const persistReceipt = Effect.fn(
        "Flect.GitInterfaceRepository.persistReceipt",
      )(function* (receipt: GitActivationReceipt) {
        yield* storage.write(ACTIVATION_RECEIPT_KEY, JSON.stringify(receipt));
        yield* Ref.set(receiptRef, receipt);
      });

      const checkpointSnapshot = Effect.fn(
        "Flect.GitInterfaceRepository.checkpointSnapshot",
      )(function* (
        ...
        const document =
          snapshot.proposal?.document ?? snapshot.active.document;
        const snapshotBytes = new TextEncoder().encode(
          `${JSON.stringify(snapshot, null, 2)}\n`,
        );
        const interfaceBytes = new TextEncoder().encode(
      ...
      const initialize = Effect.fn("Flect.GitInterfaceRepository.initialize")(
        function* (snapshot: ShapingSnapshot) {
      ...
      const readSnapshot = Effect.fn(
        "Flect.GitInterfaceRepository.readSnapshot",
      )(function* (
        branch: string,
        ...
        return yield* validateShapingSnapshot(input).pipe(
          Effect.mapError(storageFailure),
        );
      });

      const loadReceipt = Effect.fn("Flect.GitInterfaceRepository.loadReceipt")(
        function* () {
          ...
          return yield* decodeReceipt(input).pipe(
            Effect.mapError(storageFailure),
          );
        },
      );
      ...
      const recoverActivation = Effect.fn(
        ...
        const snapshot = yield* readSnapshot(
          LAST_KNOWN_GOOD_BRANCH,
          status.lastKnownGoodCommit,
          [
            {
              branch: ACCEPTED_BRANCH,
      ...
      const repairExisting = Effect.fn(
        "Flect.GitInterfaceRepository.repairExisting",
      )(function* (snapshot: ShapingSnapshot) {
        const status = yield* git
          .status()
          .pipe(Effect.mapError(storageFailure));
      ...
      const load = Effect.fn("Flect.GitInterfaceRepository.load")(function* () {
        ...
        const loaded = yield* Effect.gen(function* () {
          ...
          const branch =
            activation.proposal?.branch ?? activation.acceptedBranch;
          const expectedCommit =
            activation.proposal?.commit ?? activation.acceptedCommit;
          const snapshot = yield* readSnapshot(
            branch,
            expectedCommit,
      ...
      const save = Effect.fn("Flect.GitInterfaceRepository.save")(function* (
        ...
        if (snapshot.proposal !== undefined) {
          ...
          const branch =
            existingProposal?.branch ??
            `flect/proposal/${snapshot.proposal.id}`;
          const repositoryStatus = yield* git
            .status(
              existingProposal === undefined
                ? {}
                : { proposalBranch: existingProposal.branch },
            )
            .pipe(Effect.mapError(storageFailure));
        ...
        const repositoryStatus = yield* git
          .status(
            receipt.proposal === undefined
              ? {}
              : { proposalBranch: receipt.proposal.branch },
          )
          .pipe(Effect.mapError(storageFailure));
        const sourceRef =
        ...
        const acceptedCommit = yield* checkpointSnapshot(snapshot, {
          ...
          guards: [
            ...
            ...(sourceRef === undefined ||
            sourceRef.branch === LAST_KNOWN_GOOD_BRANCH
              ? []
              : [sourceRef]),
          ],
        });
