...
const makeHarness = () => {
  ...
  const controllerLayer = Layer.effect(
    ...
    Effect.gen(function* () {
      ...
      releasePrompt = Deferred.succeed(promptGate, undefined).pipe(
        Effect.asVoid,
      );
      setControl = (enabled) =>
...
describe("WorkspaceControlBridge", () => {
  it.effect(
    "renders external commands through the same reactive controller",
    () => {
      ...
      return Effect.gen(function* () {
        ...
        assert.strictEqual(
          harness.complete.mock.calls[0]?.[0].outcome.status,
          "succeeded",
        );
  ...
  it.effect(
    "delivers cancellation while another outside command is still active",
    () => {
      const harness = makeHarness();
      return Effect.gen(function* () {
        const bridge = yield* WorkspaceControlBridge;
