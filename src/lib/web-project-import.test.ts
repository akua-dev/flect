...
describe("plain web project import", () => {
  it("packages a directory-root index and local assets into a verified capsule", async () => {
    ...
    expect(capsule.manifest.entrypoints).toEqual([
  ...
  it("fails closed with an actionable report when no unambiguous index exists", async () => {
    const failure = await Effect.runPromise(
      importWebProject([
        file("one/index.html", "one"),
        file("two/index.html", "two"),
      ]).pipe(Effect.flip),
    );
  ...
  it("recognizes a standard Vite React entrypoint without executing Vite", async () => {
    ...
    expect(result.report.ignoredFiles).toEqual([
      ".env.local",
    ...
    expect(capsule.files.map((asset) => asset.path)).toContain(
      "metadata/import-report.json",
    );
  });
  ...
  it("rejects a source file that collides with Flect import metadata", async () => {
    const failure = await Effect.runPromise(
      importWebProject([
        ...
        file(
          "site/metadata/import-report.json",
          JSON.stringify({ owner: "source project" }),
        ),
      ]).pipe(Effect.flip),
    );
  ...
  it("reports capsule file and path limits before packaging", async () => {
    const tooManyFiles = [
      ...
      ...Array.from({ length: 255 }, (_, index) =>
        file(`site/assets/${index}.txt`, "x"),
      ),
    ];
    const countFailure = await Effect.runPromise(
      importWebProject(tooManyFiles).pipe(Effect.flip),
    );
