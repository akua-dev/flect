...
const harness = (initial: string | null = null, overrides: Partial<InterfaceStorageShape> = {}) => {
	let stored = initial;
	...
	const lock = Layer.effect(
		...
		Effect.gen(function* () {
			...
			return {
				exclusive: <A, E>(effect: Effect.Effect<A, E>) => semaphore.withPermits(1)(effect)
			};
		})
	);
	return {
		layer: makeRoleContinuityRepositoryLayer.pipe(Layer.provide(Layer.merge(storage, lock))),
		stored: () => stored
	};
};
...
describe('RoleContinuityRepository', () => {
	...
	it.effect('reports corrupt and incompatible records without replacing them', () => {
		const corrupt = harness('{bad');
		const incompatible = harness(JSON.stringify({ ...emptyRoleContinuityRecord(0), version: 2 }));
		return Effect.gen(function* () {
	...
	it.effect('rejects stale writes and preserves the newer record', () => {
		...
		return Effect.gen(function* () {
			...
			const conflict = yield* repository.save(0, changed(0, 'stale')).pipe(Effect.flip);
	...
	it.effect('leaves the prior record intact when quota or storage rejects a write', () => {
		...
		return Effect.gen(function* () {
			const repository = yield* RoleContinuityRepository;
			const error = yield* repository.save(1, changed(1, 'must not replace')).pipe(Effect.flip);
