...
export const WorkspaceControlBridgeLive = Layer.effect(
	...
	Effect.gen(function* () {
		...
		const activeCommandFibers = yield* Ref.make<ReadonlySet<Fiber.Fiber<unknown, unknown>>>(
			new Set()
		);
		...
		const reconcileTransportFailure = Effect.fn('ControlBridge.reconcileTransportFailure')(
			function* () {
				...
				yield* confirmTransportRevocation().pipe(
					Effect.catch(() =>
						Effect.logWarning('Flect control revocation cleanup is still pending.')
					)
				);
			}
		);
				...
				Effect.ensuring(
					revoking
						? Ref.set(revocationInFlight, false).pipe(
								Effect.andThen(reconcile().pipe(Effect.catch(() => Effect.void)))
							)
						: Effect.void
				)
			);
		...
		const reconcileObserved = reconcile().pipe(
			Effect.catch(() => Effect.logWarning('Flect control transport reconciliation failed.'))
		);
		...
		yield* Effect.forever(Effect.sleep('1 second').pipe(Effect.andThen(reconcileObserved))).pipe(
			Effect.forkScoped
		);
		yield* controller.events.pipe(
		...
		yield* Effect.forever(
			runNext().pipe(
				Effect.catch(() =>
					reconcileTransportFailure().pipe(Effect.andThen(Effect.sleep('200 millis')))
				)
			)
		...
		yield* Effect.addFinalizer(() =>
			Ref.get(enabled).pipe(
				Effect.flatMap((active) =>
					active ? transport.disable.pipe(Effect.catch(() => Effect.void)) : Effect.void
				)
			)
		);
