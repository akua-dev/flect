...
export const consumeLegacyInterfaceDocument = Effect.fn(
  "Flect.InterfaceDocument.consumeLegacy",
)(function* () {
  const storage = yield* InterfaceStorage;
  yield* storage.write(
    INTERFACE_DOCUMENT_MIGRATED_KEY,
    INTERFACE_DOCUMENT_MIGRATED_VALUE,
  );
  yield* storage
    .remove(INTERFACE_DOCUMENT_KEY)
...
export const loadInterfaceDocument = Effect.fn("Flect.InterfaceDocument.load")(
  function* ({
    ...
    if (
      ...
      migrationMarker === INTERFACE_DOCUMENT_MIGRATED_VALUE
    ) {
      return defaultInterfaceDocument;
    }
