...
describe('NativeUpdate', () => {
	...
	it.effect('rejects an install token after a newer check', () => {
		const checks = [available('candidate-token-0001'), available('candidate-token-0002', '0.2.2')];
		let checkIndex = 0;
		...
		return Effect.gen(function* () {
			...
			const stale = yield* Effect.result(updates.install('candidate-token-0001'));
			...
			const replay = yield* Effect.result(updates.install('candidate-token-0002'));
