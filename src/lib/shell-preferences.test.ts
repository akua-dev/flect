...
const withStorage = <A, E>(
	...
	effect.pipe(
		Effect.provide(
			makeShellPreferencesLayer.pipe(Layer.provide(Layer.succeed(InterfaceStorage)(storage)))
		)
	);
...
describe('ShellPreferences', () => {
	...
	it.effect('falls back for malformed, excess, invalid, and unreadable values', () =>
		Effect.gen(function* () {
			const invalidValues = [
				'{bad json',
				JSON.stringify({
	...
	it.effect('keeps storage write failures typed', () =>
		withStorage(
			...
			Effect.gen(function* () {
				const preferences = yield* ShellPreferences;
				const error = yield* preferences.save(defaultShellPreferences).pipe(Effect.flip);
