...
const withFixture = <A, E>(
	use: (root: string) => Effect.Effect<A, E, AgentIntegration | FileSystem.FileSystem | Path.Path>
) =>
	Effect.scoped(
		Effect.gen(function* () {
			...
			return yield* use(root).pipe(Effect.provide(makeAgentIntegrationLayer(root)));
		})
...
describe('AgentIntegration', () => {
	it.effect('keeps the shipped OpenCode asset identical to the installer', () =>
		Effect.gen(function* () {
			...
			const asset = yield* path.fromFileUrl(
				new URL('../../assets/agent-integrations/opencode/flect.js', import.meta.url)
			);

			assert.strictEqual(yield* fs.readFileString(asset), OPENCODE_PLUGIN_SOURCE);
		}).pipe(Effect.provide(PlatformLive))
	);

	it.effect('merges Codex hooks idempotently and removes only its owned hook', () =>
		withFixture((root) =>
			Effect.gen(function* () {
				const fs = yield* FileSystem.FileSystem;
				...
				assert.include(afterInstall.source, '"description": "Robin\'s hooks"');
	...
	it.effect('merges Claude local settings without disturbing unrelated values', () =>
		withFixture((root) =>
			Effect.gen(function* () {
				const fs = yield* FileSystem.FileSystem;
				...
				assert.include(installed.source, '"matcher": "startup|resume|clear|compact"');
	...
	it.effect('installs a dependency-free OpenCode V2 plugin and preserves conflicts', () =>
		withFixture((root) =>
			Effect.gen(function* () {
				const fs = yield* FileSystem.FileSystem;
				const integrations = yield* AgentIntegration;
				...
				yield* fs.writeFileString(path, "export default { id: 'user.plugin' }\n");
				const conflict = yield* Effect.result(integrations.install('opencode'));
	...
	it.effect('reports malformed host JSON as a conflict instead of overwriting it', () =>
		withFixture((root) =>
			Effect.gen(function* () {
				const fs = yield* FileSystem.FileSystem;
				yield* fs.makeDirectory(`${root}/.codex`, { recursive: true });
				...
				assert.strictEqual(yield* fs.readFileString(`${root}/.codex/hooks.json`), '{broken\n');
			})
		)
	);
});
