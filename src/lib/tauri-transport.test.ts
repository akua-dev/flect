...
describe("Tauri RPC transport", () => {
  it.effect(
    "routes agent integration lifecycle through the private runtime",
    () =>
      Effect.gen(function* () {
        ...
        const result = yield* Effect.scoped(
          Effect.gen(function* () {
            ...
            Effect.provide(
              makeTauriAgentIntegrationLayer().pipe(
        ...
        assert.deepStrictEqual(yield* Ref.get(tags), [
          "SetupAgentStatus",
          "SetupAgentInstall",
        ]);
      }),
  ...
  it.effect("routes only fixed native update operations through the host", () =>
    Effect.gen(function* () {
      ...
      const values = yield* Effect.gen(function* () {
        ...
        Effect.provide(
          makeTauriNativeUpdateLayer().pipe(
            Layer.provide(Layer.succeed(TauriNativeHost)(host)),
          ),
        ),
      );
  ...
  it.effect(
    "routes only fixed shell-link operations through the native host",
    () =>
      Effect.gen(function* () {
  ...
  it.effect("encodes shape documents before crossing the private bridge", () =>
    Effect.gen(function* () {
      ...
      assert.strictEqual(
        result[0]?.type === "shape_completed" &&
          result[0].document !== undefined
