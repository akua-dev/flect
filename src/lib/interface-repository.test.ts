		...
		layer: Layer.succeed(InterfaceStorage)({
			read: (key) =>
				Ref.update(reads, (count) => count + 1).pipe(
					Effect.andThen(Ref.get(value).pipe(Effect.map((current) => current.get(key) ?? null)))
				),
			write: (_key, next) =>
				Ref.update(writes, (count) => count + 1).pipe(
					Effect.andThen(Ref.update(value, (current) => new Map(current).set(_key, next)))
				),
			remove: (key) =>
...
describe('InterfaceRepository', () => {
	...
	it.layer(safeLayer)((it) => {
		it.effect('bypasses journal storage and requests recovery in safe mode', () =>
			Effect.gen(function* () {
				const repository = yield* InterfaceRepository;
				const loaded = yield* repository.load;
				yield* repository.save(snapshot);
	...
	const invalidDocumentStorage = makeStorage(JSON.stringify(duplicateNodeSnapshot));
	...
	const invalidActiveStatusStorage = makeStorage(JSON.stringify(invalidActiveStatusSnapshot));
	...
	const rejectedProposalStorage = makeStorage(JSON.stringify(rejectedProposalSnapshot));
	...
	const initializedCustomStorage = makeStorage(JSON.stringify(initializedCustomSnapshot));
	...
	const forgedBuiltInStorage = makeStorage(JSON.stringify(forgedBuiltInSnapshot));
