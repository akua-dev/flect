...
const packageManifest = (name: string, exports: Readonly<Record<string, string>>) =>
	JSON.stringify({
		name,
		version: '19.2.8',
		main: 'index.js',
...
button { border: 0; border-radius: 999px; background: rgb(20, 86, 230); color: white; padding: 14px 22px; font: inherit; }`
	...
	file('node_modules/react/cjs/react-jsx-runtime.production.js', reactJsxProduction),
	file(
	...
	file('node_modules/react-dom/cjs/react-dom.production.js', reactDomProduction),
	file('node_modules/react-dom/cjs/react-dom-client.production.js', reactDomClientProduction),
	file('node_modules/scheduler/package.json', packageManifest('scheduler', { '.': './index.js' })),
	file('node_modules/scheduler/index.js', schedulerIndex),
	file('node_modules/scheduler/cjs/scheduler.production.js', schedulerProduction)
];
...
export function BrowserBuildDiagnostic() {
	...
	useEffect(() => {
		const packageResolver = Layer.succeed(BrowserPackageResolver)({
			resolve: () => Effect.die(new Error('The fixed build diagnostic has no package request.'))
		});
		const dependencies = Layer.mergeAll(GitWorkspaceLive, BrowserBuildLive, packageResolver);
		const runtime = ManagedRuntime.make(ProposalBuildLive.pipe(Layer.provideMerge(dependencies)));
		const restorationKey = 'flect.browser-build-diagnostic.complete';
		const restoreOnly = sessionStorage.getItem(restorationKey) === '1';
		const workspaceId =
			new URLSearchParams(globalThis.location.search).get('workspace') ?? 'build-diagnostic';
		let active = true;
		void runtime
			.runPromise(
				Effect.gen(function* () {
					...
					if (restoreOnly) {
						...
						if (restored === undefined) {
							return yield* Effect.fail(
								new Error('The last successful browser build was not restored.')
							);
						}
						return {
							artifact: restored,
