...
const portableHtml = (source: string, hasCss: boolean, hasJavaScript: boolean) => {
...
export const buildFrameworkCapsule = Effect.fn('Flect.FrameworkCapsule.build')(function* ({
	...
	const source = yield* decodeCapsule(sourceArchive).pipe(
		Effect.mapError(() => failure('The imported project capsule is invalid.'))
	);
	const sourceHtml = source.manifest.entrypoints.find(
		(entrypoint) => entrypoint.id === 'source-html'
	);
	const htmlFile = source.files.find((file) => file.path === sourceHtml?.path);
	if (sourceHtml === undefined || htmlFile === undefined) {
		return yield* Effect.fail(failure('The imported project has no portable HTML shell.'));
	}
	const html = yield* Effect.try({
		try: () => decoder.decode(htmlFile.contents),
		catch: () => failure('The imported project HTML is invalid.')
	});
	const report = source.files.find((file) => file.path === PROJECT_IMPORT_REPORT_PATH);
	const hasCss = artifact.outputs.some((output) => output.path === 'app.css');
	const hasJavaScript = artifact.outputs.some((output) => output.path === 'app.js');
	if (!hasJavaScript) {
		return yield* Effect.fail(
			failure('The verified browser build emitted no application entrypoint.')
		);
	}
	return yield* encodeCapsule({
		...
		files: [
			...
			...(report === undefined ? [] : [{ path: report.path, contents: report.contents }])
		]
	}).pipe(
