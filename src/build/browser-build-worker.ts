...
const decodeRequest = Schema.decodeUnknownEffect(BrowserBuildWorkerRequest, strictOptions);

const buildFailure = (buildId: string, reason: BrowserBuildFailure['reason'], message: string) =>
	...
	Effect.tryPromise({
		try: async () => {
			...
			for (const file of files) {
				...
				const transformed = await transformFrameworkSource(file.path, file.contents);
			...
			const absoluteFiles = new Set(files.map((file) => `${root}/${file.path}`));
				...
				plugins: [
					{
						...
						load(id) {
							...
							return transformed === undefined ? null : { code: transformed, moduleType: 'js' };
						}
					}
				],
				transform: {
			...
			try {
				...
				if (generated.output.length === 0 || generated.output.length > MAX_OUTPUT_FILES) {
					throw buildFailure(
						request.buildId,
						'oversized',
				...
				const outputs = generated.output.map((output) => {
					...
					const contents =
						output.type === 'chunk' ? encoder.encode(output.code) : bytes(output.source);
					outputBytes += contents.byteLength;
					if (contents.byteLength > MAX_FILE_BYTES || outputBytes > MAX_BUILD_BYTES) {
						throw buildFailure(
							request.buildId,
							'oversized',
				...
				const css = new Uint8Array(restrictedCss.byteLength + frameworkCss.byteLength);
			...
			const detail =
				error instanceof Error
					? error.message.replaceAll(/\/flect\/build-[^/\s]+\//g, '').slice(0, 360)
