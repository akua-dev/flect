...
const vueSource = async (path: string, contents: Uint8Array) => {
	...
	if (descriptor.template.src !== undefined) {
		throw failure(`${path} uses an external Vue template source, which is not portable.`);
	}
...
export const transformFrameworkSource = async (path: string, contents: Uint8Array) => {
	try {
