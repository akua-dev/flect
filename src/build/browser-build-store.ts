...
class StoredBuildOutput extends Schema.Class<StoredBuildOutput>(
  "StoredBuildOutput",
)({
  path: BuildPath,
...
class StoredBuildArtifact extends Schema.Class<StoredBuildArtifact>(
  "StoredBuildArtifact",
)({
...
class StoredBuildBindings extends Schema.Class<StoredBuildBindings>(
  "StoredBuildBindings",
)({
  version: Schema.Literal(1),
...
export interface BrowserBuildStoreShape {
  readonly load: Effect.Effect<
    BrowserBuildArtifact | undefined,
    BrowserBuildStoreError
  >;
  readonly save: (
    artifact: BrowserBuildArtifact,
  ) => Effect.Effect<void, BrowserBuildStoreError>;
}

export class BrowserBuildStore extends Context.Service<
  BrowserBuildStore,
  BrowserBuildStoreShape
...
const decodeArtifact = Schema.decodeUnknownPromise(
  StoredBuildArtifact,
  strictOptions,
);
...
const makeStore = (vfs: Vfs, requestedRoot: string): BrowserBuildStoreShape => {
  ...
  const load = Effect.tryPromise({
    try: async () => {
      ...
      const binding = await decodeBindings(
        JSON.parse(await vfs.readFileText(bindings)),
      );
    ...
    Effect.tryPromise({
      try: async () => {
        if (
          ...
          (await digestBuildEntries(artifact.outputs)) !==
...
export const makeBrowserBuildStoreLayer = (
  vfs: Vfs,
  root = "/flect-builds/default",
