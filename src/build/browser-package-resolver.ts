...
const failure = (inputDigest: string, reason: BrowserPackageFailure['reason'], message: string) =>
	BrowserPackageFailure.make({ inputDigest, reason, message });

const record = (value: unknown): value is Readonly<Record<string, unknown>> =>
	typeof value === 'object' && value !== null && !Array.isArray(value);

const validateLockfile = (lockfile: Uint8Array, registryOrigin: string, inputDigest: string) => {
	let value: unknown;
	try {
		value = JSON.parse(decoder.decode(lockfile));
	} catch {
		throw failure(inputDigest, 'invalid-lock', 'The resolved browser package lock is invalid.');
	}
	if (!record(value) || value.lockfileVersion !== 3 || !record(value.packages)) {
		throw failure(inputDigest, 'invalid-lock', 'The resolved browser package lock is invalid.');
	}
	let packageCount = 0;
...
export const makeBrowserPackageResolverLayer = (options?: { readonly registryOrigin?: string }) =>
	Layer.effect(
		BrowserPackageResolver,
		Effect.gen(function* () {
