...
import { BrowserPackageCache, makeBrowserPackageCacheLayer } from './browser-package-cache';
import {
...
describe('BrowserPackageResolver', () => {
	it.effect('resolves once and reuses the integrity-bearing cache offline', () =>
		Effect.gen(function* () {
			...
			const cache = makeBrowserPackageCacheLayer(new MemoryVfs(), '/flect-packages/test');
			...
			assert.isTrue(
				second.files.some((file) => file.path === 'node_modules/flect-fixture/package.json')
			);
			const lock: unknown = JSON.parse(new TextDecoder().decode(second.lockfile));
			assert.strictEqual((lock as { lockfileVersion?: unknown }).lockfileVersion, 3);
		})
	);
	...
	it.effect('does not cache a package whose integrity check fails', () =>
		Effect.gen(function* () {
			...
			const cache = makeBrowserPackageCacheLayer(new MemoryVfs(), '/flect-packages/integrity');
			const layer = makeBrowserPackageResolverLayer({
				registryOrigin: 'https://registry.flect.invalid'
			}).pipe(Layer.provideMerge(Layer.mergeAll(packages, cache)));
