...
export const makeLiveProposalBuild = async ({ git }: { readonly git: GitWorkspaceShape }) => {
	...
	const live = ProposalBuildLive.pipe(
		Layer.provideMerge(Layer.mergeAll(Layer.succeed(GitWorkspace)(git), browserBuild, packages))
	);
