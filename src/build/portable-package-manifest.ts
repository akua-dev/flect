...
const failure = (message: string) =>
  PortablePackageManifestFailure.make({ message });

const record = (value: unknown): value is Readonly<Record<string, unknown>> =>
  typeof value === "object" && value !== null && !Array.isArray(value);

export const portablePackageManifest = Effect.fn(
  "Flect.PortablePackageManifest.decode",
  ...
  if (!record(parsed)) {
    return yield* Effect.fail(
      failure("The project package manifest is invalid."),
    );
  }
  const dependencies = yield* Schema.decodeUnknownEffect(StringRecord)(
    ...
    Effect.mapError(() =>
      failure("The portable build requires string runtime dependencies."),
    ),
  );
  if (
    record(parsed.optionalDependencies) &&
    Object.keys(parsed.optionalDependencies).length > 0
  ) {
    return yield* Effect.fail(
      failure(
        "Optional dependencies are not supported by the portable acceptance build.",
      ),
    );
  }
