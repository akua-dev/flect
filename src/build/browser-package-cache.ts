...
import { BrowserPackageFile, BrowserPackageResolution } from '../../shared/browser-package';
...
class CachedPackageFile extends Schema.Class<CachedPackageFile>('CachedPackageFile')({
	path: PackagePath,
	bytes: Schema.Int.check(Schema.isBetween({ minimum: 0, maximum: MAX_FILE_BYTES })),
	digest: Digest
}) {}

class CachedPackageGraph extends Schema.Class<CachedPackageGraph>('CachedPackageGraph')({
	version: Schema.Literal(1),
	inputDigest: Digest,
	lockDigest: Digest,
	graphDigest: Digest,
	packageCount: Schema.Int.check(Schema.isBetween({ minimum: 0, maximum: 2_048 })),
	lockBytes: Schema.Int.check(Schema.isBetween({ minimum: 1, maximum: MAX_FILE_BYTES })),
	files: Schema.Array(CachedPackageFile).check(Schema.isMaxLength(MAX_FILES))
}) {}

class PackageCacheBindings extends Schema.Class<PackageCacheBindings>('PackageCacheBindings')({
	version: Schema.Literal(1),
	entries: Schema.Record(Digest, Digest).check(Schema.isMaxProperties(MAX_BINDINGS))
}) {}
...
export interface BrowserPackageCacheShape {
	readonly load: (
		inputDigest: string
	) => Effect.Effect<BrowserPackageResolution | undefined, BrowserPackageCacheError>;
	readonly save: (
...
const decodeBindings = Schema.decodeUnknownPromise(PackageCacheBindings, strictOptions);
const decodeGraph = Schema.decodeUnknownPromise(CachedPackageGraph, strictOptions);

const makeCache = (vfs: Vfs, requestedRoot: string): BrowserPackageCacheShape => {
	...
	const load = (inputDigest: string) =>
		Effect.tryPromise({
			try: async () => {
				...
				if (manifest.inputDigest !== inputDigest || manifest.graphDigest !== graphDigest) {
					throw failure();
				}
		...
		Effect.tryPromise({
			try: async () => {
				if (
					...
					(await digestBuildBytes(resolution.lockfile)) !== resolution.lockDigest ||
					(await digestBuildEntries([
				...
				await vfs.writeFile(`${objectRoot}/package-lock.json`, resolution.lockfile);
				for (const [index, file] of resolution.files.entries()) {
					await vfs.writeFile(`${objectRoot}/files/${index}.bin`, file.contents);
				}
				await vfs.writeFile(
				...
				while (Object.keys(entries).length > MAX_BINDINGS) {
					const index = keys.findIndex((key) => key !== resolution.inputDigest && key in entries);
					if (index < 0) {
						throw failure();
					}
...
export const makeBrowserPackageCacheLayer = (vfs: Vfs, root = '/flect-packages/default') =>
	Layer.succeed(BrowserPackageCache)(makeCache(vfs, root));

export const BrowserPackageCacheLive = Layer.effect(
	BrowserPackageCache,
	Effect.promise(() =>
		browserPersistentStorage().then(({ vfs }) => makeCache(vfs, '/flect-packages/default'))
	)
);
