...
export const makeLazyProposalBuildLayer = (options?: {
  readonly load?: LiveProposalBuildLoader;
}) =>
  Layer.effect(
    ProposalBuild,
    Effect.gen(function* () {
      ...
      yield* Effect.addFinalizer(() => {
        ...
        return current === undefined
          ? Effect.void
          : Effect.promise(() =>
              current.then((loaded) => loaded.dispose()).catch(() => undefined),
            );
      });
      return {
        resolvePackageLock: (request) =>
          service.pipe(
            Effect.flatMap((build) => build.resolvePackageLock(request)),
          ),
        compile: (request) =>
          service.pipe(Effect.flatMap((build) => build.compile(request))),
