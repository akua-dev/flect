...
export class BrowserBuild extends Context.Service<
  BrowserBuild,
  BrowserBuildShape
>()("flect/BrowserBuild") {}
...
const requestWorker = (
  ...
  Effect.callback<BrowserBuildArtifact, BrowserBuildFailure>((resume) => {
    ...
    const settle = (
      result: Effect.Effect<BrowserBuildArtifact, BrowserBuildFailure>,
    ) => {
      if (completed) {
        return;
      }
      ...
      settle(
        Effect.fail(
          failure(
            request.buildId,
            "worker",
            "The restricted browser build Worker failed safely.",
          ),
        ),
      );
    };
    ...
    try {
      worker.postMessage(
        BrowserBuildWorkerRequest.make({ version: 1, id, request }),
      );
    } catch {
      settle(
        Effect.fail(
...
export const makeBrowserBuildLayer = (options?: {
  ...
  Layer.effect(
    ...
    Effect.gen(function* () {
      ...
      const lastSuccessful = yield* Ref.make<BrowserBuildArtifact | undefined>(
        restored,
      );
      ...
      const compile = Effect.fn("Flect.BrowserBuild.compile")(
        (request: BrowserBuildRequest) =>
                ...
                Effect.acquireUseRelease(
                  ...
                  (worker) =>
                    requestWorker(worker, request).pipe(
                      ...
                      Effect.tap((artifact) =>
                        Ref.set(lastSuccessful, artifact),
                      ),
                    ),
