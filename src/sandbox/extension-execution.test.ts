...
import { ExtensionExecution, ExtensionExecutionLive } from './extension-execution';
...
describe('ExtensionExecution', () => {
	...
	const sandbox = makeExtensionSandboxTestLayer({
		run: () => Ref.update(sandboxCalls, (count) => count + 1).pipe(Effect.as(sandboxResult))
	});
	...
	const dependencies = Layer.mergeAll(sandbox.layer, broker, makeShapingKernelTestLayer());
	const testLayer = ExtensionExecutionLive.pipe(Layer.provideMerge(dependencies));

	it.layer(testLayer)((it) => {
		it.effect('disables an extension after three denied executions and does not run it again', () =>
			Effect.gen(function* () {
				...
				const failures = yield* Effect.forEach([1, 2, 3], () =>
					execution.execute(deniedManifest, {}, ['interface:propose']).pipe(Effect.flip)
				);
				...
				assert.deepStrictEqual(recovered.disabledExtensions, ['weather-card']);
