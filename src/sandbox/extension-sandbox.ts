...
type SandboxWorker = Pick<Worker, 'addEventListener' | 'removeEventListener' | 'postMessage'>;
...
interface SandboxWorkerFactoryShape {
	readonly acquire: Effect.Effect<SandboxWorkerHandle, SandboxExecutionFailed, Scope.Scope>;
}
...
export class ExtensionSandbox extends Context.Service<ExtensionSandbox, ExtensionSandboxShape>()(
...
export const makeSandboxWorkerHandle = (worker: SandboxWorker): SandboxWorkerHandle => ({
	run: Effect.fn('Flect.SandboxWorker.run')((request) =>
		Effect.callback<SandboxResult, SandboxExecutionFailed>((resume) => {
			...
			const settle = (effect: Effect.Effect<SandboxResult, SandboxExecutionFailed>) => {
				if (completed) {
					return;
				}
				completed = true;
