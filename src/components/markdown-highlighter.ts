...
const cacheKey = (code: string, language: string): string =>
  `${language}\u0000${code}`;
...
export const highlightMarkdownCode = Effect.fn("Flect.Markdown.highlightCode")(
  function* (code: string, language: string) {
...
export const __markdownHighlightTest = {
  has: (code: string, language: string): boolean =>
    cache.has(cacheKey(code, language)),
  reset: (): void => {
    cache.clear();
    cacheBytes = 0;
  },
