...
const entry = (role: 'app' | 'shaper', state: PortableExtensionRoleState['state'] = 'available') =>
...
describe('ExtensionReview', () => {
	it('exposes provenance, roles, requested authority, and bounded resources', () => {
		...
		expect(screen.getByRole('heading', { name: 'Project guide' })).toBeVisible();
	...
	it('requires an explicit per-role enable action and keeps optional grants off by default', async () => {
		...
		expect(within(app).getByRole('checkbox', { name: /interface:read/i })).toBeChecked();
		expect(within(app).getByRole('checkbox', { name: /interface:read/i })).toBeDisabled();
		expect(within(app).getByRole('checkbox', { name: /interface:propose/i })).not.toBeChecked();

		await userEvent.click(within(app).getByRole('button', { name: 'Enable for App Agent' }));
	...
	it('makes candidate testing visible and reports a safe extension failure', async () => {
		...
		await userEvent.click(screen.getByRole('button', { name: 'Test for App Agent' }));
	...
	it('offers pin, fork, conflict recovery, and confirmed removal', async () => {
		...
		await userEvent.click(screen.getByRole('button', { name: 'Pin Shaper version' }));
		...
		await userEvent.click(screen.getByRole('button', { name: 'Fork for Shaper' }));
		expect(actions.onFork).toHaveBeenCalledWith(
			expect.objectContaining({ role: 'shaper' }),
			'local-b'
		);

		await userEvent.click(screen.getByRole('button', { name: 'Use upstream for Shaper' }));
		expect(actions.onResolveUpdate).toHaveBeenCalledWith(
			expect.objectContaining({ role: 'shaper' }),
			'upstream'
		);

		await userEvent.click(screen.getByRole('button', { name: 'Remove from Shaper' }));
		expect(screen.getByText('Removal needs confirmation.')).toHaveAttribute('role', 'status');
		await userEvent.click(screen.getByRole('button', { name: 'Confirm remove from Shaper' }));
		expect(actions.onRemove).toHaveBeenCalledWith(expect.objectContaining({ role: 'shaper' }));
	});
});
