...
describe("AgentRail", () => {
  ...
  it("keeps typed browser-build progress visible in the protected rail", () => {
    ...
    expect(
      screen.getByText("Compiling exact proposal").closest("[role=status]"),
    ).toHaveTextContent("Browser build: Compiling exact proposal");
  });
  ...
  it("shows capsule provenance and lets the protected user grant a registered capability", async () => {
      ...
      <AgentRail
        capsuleReview={{
          ...
          importReport: {
            ...
            adaptations: [
              "Flect used its restricted compiler instead of Vite.",
            ],
    ...
    expect(
      screen.getByText("Flect used its restricted compiler instead of Vite."),
    ).toBeVisible();
