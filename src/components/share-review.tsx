...
const hardBlockers: ReadonlySet<ShareReviewContract["blockers"][number]> =
  new Set([
    "invalid-signature",
    "incompatible",
    "conflict",
...
export function ShareReview({
  ...
  const authorityChangeCount = review.changes.filter(
    (change) => change.authorityAffecting,
  ).length;
    ...
    >
      ...
      <div className="share-review__facts">
        ...
        <div>
          ...
          <strong>
            {review.compatible
              ? "Compatible with this Flect"
              : "Not compatible with this Flect"}
          </strong>
        </div>
        <div>
          <span>Authority</span>
      ...
      <p className="share-review__signature-note">
        {signatureLabel(review.signature.status)}. A signature identifies a
        claim; it does not grant access or approve capabilities.
      </p>
        ...
        {review.lineage === "conflict" ? (
          ...
          <>
            <button
              className="decision-button decision-button--primary"
              disabled={
                busy || blocked || alreadyCurrent || selected.length === 0
              }
              onClick={() => {
