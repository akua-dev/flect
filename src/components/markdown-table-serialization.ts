...
const tableRows = (table: HTMLTableElement): ReadonlyArray<ReadonlyArray<string>> =>
	Array.from(table.rows, (row) => Array.from(row.cells, (cell) => normalizeCellText(cell))).filter(
		(row) => row.length > 0
	);
