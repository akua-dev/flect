...
import { ComposerActionsMenu, type ComposerActionsMenuProps } from './composer-actions-menu';

afterEach(cleanup);

const props = (overrides: Partial<ComposerActionsMenuProps> = {}): ComposerActionsMenuProps => ({
	disabled: false,
...
describe('ComposerActionsMenu', () => {
	it('exposes implemented protected and extension actions', async () => {
		...
		expect(screen.getByRole('menuitem', { name: 'Undo last change' })).toBeVisible();
		expect(screen.getByRole('menuitem', { name: 'Open safe mode' })).toBeVisible();
		expect(screen.getByRole('menuitem', { name: 'Enable trusted Pi extensions' })).toBeVisible();
		expect(screen.getByRole('menuitem', { name: 'Export source and history' })).toBeVisible();
	});
	...
	it('exports the complete source repository', async () => {
		...
		await user.click(screen.getByRole('button', { name: 'Actions' }));
		await user.click(screen.getByRole('menuitem', { name: 'Export source and history' }));
	...
	it('opens shared URLs or local archives from the protected actions menu', async () => {
		...
		await user.click(screen.getByRole('button', { name: 'Actions' }));
		await user.click(screen.getByRole('menuitem', { name: 'Review shared source' }));
		expect(onOpenShareSource).toHaveBeenCalledOnce();

		rerender(<ComposerActionsMenu {...props({ onOpenShareFile, onOpenShareSource })} />);
		await user.click(screen.getByRole('button', { name: 'Actions' }));
		await user.click(screen.getByRole('menuitem', { name: 'Open shared file' }));
		...
		await user.click(screen.getByRole('button', { name: 'Actions' }));
		await user.click(screen.getByRole('menuitem', { name: 'Manage shared sources' }));
	...
	it('shows accepted and isolated candidate object IDs', async () => {
		...
		expect(screen.getByRole('status')).toHaveTextContent('External change waiting for activation');
	});
	...
	it('toggles trusted Pi extensions for the active role', async () => {
		...
		await user.click(screen.getByRole('button', { name: 'Actions' }));
		await user.click(screen.getByRole('menuitem', { name: 'Enable trusted Pi extensions' }));
	...
	it('runs rollback only when a prior revision is available', async () => {
		...
		const { rerender } = render(
			<ComposerActionsMenu {...props({ onRollback, rollbackAvailable: false })} />
		);

		await user.click(screen.getByRole('button', { name: 'Actions' }));
		expect(screen.getByRole('menuitem', { name: 'Undo last change' })).toBeDisabled();

		rerender(<ComposerActionsMenu {...props({ onRollback, rollbackAvailable: true })} />);
		await user.click(screen.getByRole('menuitem', { name: 'Undo last change' }));
	...
	it('dismisses with Escape and restores focus to the trigger', async () => {
		...
		expect(screen.queryByRole('menu', { name: 'Flect actions' })).not.toBeInTheDocument();
