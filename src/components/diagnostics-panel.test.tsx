...
describe("DiagnosticsPanel", () => {
  ...
  it("confirms exact native setup mutations and exposes definitive states", async () => {
    ...
    await user.click(
      screen.getByRole("button", { name: "Install command-line link" }),
    );
    await user.click(
      screen.getByRole("button", { name: "Remove Codex context" }),
    );
  ...
  it("confirms ownership-safe uninstall preparation and names retained data", async () => {
    ...
    await user.click(
      screen.getByRole("button", { name: "Prepare to uninstall" }),
    );
  ...
  it("shows a reviewed native update and confirms installation", async () => {
    ...
    expect(
      screen.getByText("Security and reliability improvements."),
    ).toBeVisible();
  ...
  it("reports the browser update boundary without an install action", async () => {
    ...
    await user.click(screen.getByRole("button", { name: /Diagnostics/i }));
    expect(
      screen.getByText("Updates are available in a signed desktop release."),
    ).toBeVisible();
    expect(
  ...
  it("exposes bounded update progress accessibly", async () => {
    ...
    await user.click(screen.getByRole("button", { name: /Diagnostics/i }));
    expect(
      screen.getByRole("progressbar", { name: "Update progress" }),
    ).toHaveAttribute("value", "512");
    expect(
      screen.getByRole("progressbar", { name: "Update progress" }),
