...
export interface AgentRailProps {
	...
	readonly onForkPortableExtension?: (key: ExtensionReviewKey, revision: string) => Promise<void>;
	readonly onResolvePortableExtensionUpdate?: (
		key: ExtensionReviewKey,
		choice: 'upstream' | 'fork'
	) => Promise<void>;
	readonly onRemovePortableExtension?: (key: ExtensionReviewKey) => Promise<void>;
...
export function ProductCapabilities({
	...
	const decide = async (capabilityId: string, choice: ProductCapabilityDecisionChoice) => {
	...
	const revokeCapability = (capability: CapsuleReview['capabilities'][number]) =>
	...
	const lifecycleLabel = (state: CapsuleReview['capabilities'][number]['state']) => {
		switch (state) {
			case 'available':
				return 'Available';
			case 'requested':
						...
						<li key={capability.capabilityId}>
							<span className='capsule-review__capability-copy'>
								...
								<details className='capsule-review__capability-scope'>
									...
									{capability.expiresAtMillis !== undefined && (
										<small>Expires {new Date(capability.expiresAtMillis).toISOString()}</small>
									)}
							...
							{capability.availability === 'available' && (
								<fieldset className='capsule-review__capability-actions'>
									<legend className='sr-only'>Decide {capability.capabilityId}</legend>
									...
									onRevoke !== undefined ? (
										<button
											...
											type='button'
										>
											{pendingCapabilityId === capability.capabilityId ? 'Saving…' : 'Revoke'}
										</button>
									) : (
										onDecide !== undefined && (
											<>
					...
					<div className='conversation__empty flex size-full flex-col items-center justify-center gap-3 p-8 text-center'>
						<div className='space-y-1'>
							...
							<p className='text-muted-foreground text-sm'>
								Ask for a change, use the current interface, or call an approved action. Flect
								routes the work for you.
							</p>
						</div>
					</div>
				</div>
...
export function AgentRail({
	...
	const conversationControllers = [workspace.app, workspace.previewApp, workspace.shaper];
	const preferredController = preview
		...
		: useDisabled
			...
			: workspace.app;
	const controller =
		conversationControllers.find((entry) => isAgentSessionActive(entry.status)) ??
		preferredController;
	...
	const cancel = async () => {
		const active = conversationControllers.filter((entry) => isAgentSessionActive(entry.status));
		...
		mode === 'safe'
			...
			: canvasSelection !== undefined
				...
				: preview && candidateRevisionId !== undefined
					? (text: string) => workspace.previewApp.submit(text, document, candidateRevisionId)
	...
	const finishProjectImport = async ({ archive, report }: WebProjectImportResult) => {
		setCapsuleNotice(
			...
			void import('../lib/web-project-import')
				.then(({ importWebProject, shouldAvoidReadingWebProjectFile }) =>
					Effect.runPromise(
						...
						).pipe(Effect.flatMap((projectFiles) => importWebProject(projectFiles)))
					)
				)
	...
	const importProjectArchive = () => {
		...
		input.addEventListener('change', () => {
			...
			void file
				.arrayBuffer()
				.then(async (buffer) => {
					const { importWebProjectArchive } = await import('../lib/web-project-archive');
					return Effect.runPromise(importWebProjectArchive(file.name, new Uint8Array(buffer)));
				})
				.then(finishProjectImport)
				.catch((error: unknown) => setCapsuleNotice(projectImportFailureMessage(error)));
		});
		input.click();
	};
	const importGitProject = async (event: FormEvent<HTMLFormElement>) => {
		...
		try {
			const { importWebProjectFromGit } = await import('../lib/web-project-git-import');
	...
	const installCapsule = async (event: FormEvent<HTMLFormElement>) => {
		...
		try {
			const { loadBrowserCapsuleArchiveFromUrl } = await import('../lib/browser-capsule-loader');
			const archive = await Effect.runPromise(loadBrowserCapsuleArchiveFromUrl(installUrl));
			await workspace.importCapsule?.(archive);
			setCapsuleNotice('Flect app verified. Ready to activate.');
			setInstallOpen(false);
			setInstallUrl('');
		} catch {
			setInstallError('Install failed safely. Check the URL, CORS access, and capsule integrity.');
	...
	}, [candidateExtensionBlocked, capsuleReview?.activationBlocked, shaping.status]);
		...
		>
			...
			<div className='agent-rail__dock'>
				...
				{mode === 'safe' && (
					<section className='recovery-banner' role='status'>
						<div>
							...
							<p>
								Flect temporarily hid custom changes. Restore the last working interface when you
								are ready.
							</p>
							<small>
				...
				{shaping.status !== 'preview' &&
					...
					portableExtensionActions !== undefined && (
						<details className='capsule-review'>
							...
							<Suspense fallback={<SurfaceFallback label='Opening portable extensions' />}>
								<ExtensionReview
									binding='accepted'
						...
						{capsuleReview !== undefined && (
							<details className='capsule-review' open>
								...
								<dl>
									...
									<div>
										...
										<dd>
											...
											{capsuleReview.flectCompatible ? 'compatible' : 'incompatible'}
										</dd>
									</div>
									<div>
										<dt>This host</dt>
										<dd>
											...
											{capsuleReview.platformCompatible ? 'supported' : 'unsupported'}
										</dd>
									</div>
									{capsuleReview.build !== undefined && (
										<>
											<div>
												...
												<dd>
													artifact {capsuleReview.build.artifactDigest.slice(0, 7)}
													{' · source '}
													{capsuleReview.build.sourceRevision.slice(0, 7)}
												</dd>
											</div>
											{capsuleReview.build.dependencyGraphDigest !== undefined && (
												<div>
													<dt>Dependencies</dt>
													<dd>
														Locked in source Git · graph{' '}
														{capsuleReview.build.dependencyGraphDigest.slice(0, 7)}
													</dd>
												</div>
											)}
										</>
								...
								{capsuleReview.importReport !== undefined && (
									<div className='capsule-review__capabilities'>
										...
										{capsuleReview.importReport.adaptations.length > 0 && (
											<ul>
												{capsuleReview.importReport.adaptations.map((adaptation) => (
													<li key={adaptation}>
										...
										{capsuleReview.importReport.ignoredFiles.length > 0 && (
											<small>
												{capsuleReview.importReport.ignoredFiles.length} ignored secret or generated
												file
												{capsuleReview.importReport.ignoredFiles.length === 1 ? '' : 's'}
											</small>
										)}
									</div>
								)}
								...
								{capsuleReview.activationBlocked && (
									<p className='capsule-review__warning' role='alert'>
										{!capsuleReview.trustDecision.allowed
											...
											: !capsuleReview.flectCompatible || !capsuleReview.platformCompatible
								...
								{candidateExtensionBlocked && (
									<p className='capsule-review__warning' role='alert'>
										Every enabled candidate extension must pass its bounded test before this app can
										be activated. Disable failed extensions or ask Flect to fix them.
									</p>
								)}
							</details>
						)}
				...
				{controller.status === 'setup-required' && (
					<section aria-labelledby='provider-setup-title' className='provider-setup'>
						<Card>
							<CardHeader>
								<CardTitle id='provider-setup-title'>Connect an agent</CardTitle>
								<CardDescription>
									Sign in once, then tell Flect what you want to make. Your draft stays private
									until the connection succeeds.
								</CardDescription>
							</CardHeader>
							<CardContent>
								<Suspense fallback={<SurfaceFallback label='Opening provider setup' />}>
									<ProviderAuthPanel
				...
				{controller.status === 'unavailable' && (
					<div className='runtime-alert' role='alert'>
						...
						<button className='retry-button' onClick={() => void workspace.refresh()} type='button'>
							<RefreshIcon />
							Restart runtime
						</button>
					</div>
					...
					>
						<form onSubmit={(event) => void installCapsule(event)}>
							<header>
								...
								<p>
									Flect downloads without credentials, verifies the manifest and every file, then
									opens a reviewable preview.
								</p>
							</header>
					...
					>
						<form onSubmit={(event) => void importGitProject(event)}>
							<header>
								...
								<p>
									Flect clones public HTTPS source in its isolated Git worker, checks out exactly
									one commit, and never runs repository code.
								</p>
							</header>
							...
							<input
								...
								maxLength={40}
								minLength={40}
								onChange={(event) => setGitImportCommit(event.currentTarget.value)}
								pattern='[0-9a-f]{40}'
							...
							<div>
								...
								<button
									className='decision-button decision-button--primary'
									disabled={
										gitImporting || gitImportUrl.length === 0 || gitImportCommit.length !== 40
									}
									type='submit'
								>
