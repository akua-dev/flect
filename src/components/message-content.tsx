...
const MarkdownDetails: NonNullable<Components["details"]> = ({
  children,
  open,
}) => (
...
const MarkdownSection: NonNullable<Components["section"]> = ({
  children,
  ...props
}) => {
  const { node: _node, ...elementProps } = props;
  const role =
    "data-footnotes" in elementProps ? "doc-endnotes" : elementProps.role;
  return (
...
const MarkdownCode: NonNullable<Components["code"]> = ({
  ...
  const { node: _node, ...elementProps } = props;
  const language =
    className === undefined ? undefined : extractFenceLanguage(className);
  return (
...
const extractPreCode = (
  ...
  const codeNode = (node as HastElementNode | undefined)?.children?.find(
    (child) =>
      (child as HastElementNode).type === "element" &&
      (child as HastElementNode).tagName === "code",
...
export function MessageContent({
  ...
  const remarkPlugins =
    messageRole === "assistant" ? REMARK_PLUGINS : REMARK_PLUGINS_WITH_BREAKS;
