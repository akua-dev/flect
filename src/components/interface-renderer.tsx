...
export interface InterfaceRendererProps {
	...
	readonly onSelectionChange?: (selection: CanvasSelection, nodeId: string) => void;
}
	...
	switch (node.type) {
		...
		case 'text':
			return node.style === 'headline' ? (
				...
				<p className={`interface-text interface-text--${node.style}`} {...selectable}>
					{node.text}
				</p>
			);
		case 'prompt':
...
export function InterfaceRenderer({
	...
	const choose = (event: MouseEvent<HTMLElement> | KeyboardEvent<HTMLElement>) => {
