...
import { AgentRail, type AgentRailProps, type ShapingController } from './agent-rail';
...
export interface RoleAwareShellProps {
	...
	readonly onCapsuleIntent?: (intent: CapsuleIntent) => Promise<CapsuleIntentOutcome>;
	...
	readonly onRetainShare?: (artifactIds: ReadonlyArray<string>) => Promise<void>;
	...
	readonly onActivateShare?: (artifactIds: ReadonlyArray<string>) => Promise<void>;
	...
	readonly onOpenSharePrivate?: (adapterId: string, reference: string) => Promise<void>;
	...
	readonly onDeleteShare?: (shareId: string, expectedForkCommit: string) => Promise<void>;
}
...
const isStarterInterface = (document: InterfaceDocument) => {
	...
	const actions =
		root.type === 'stack'
			? root.children.flatMap((node) => (node.type === 'stack' ? node.children : [node]))
			: [];
	return (
		document.name === 'Flect' &&
		actions.some(
			(node) =>
				node.type === 'button' && node.id === 'shape-interface' && node.label === 'Start building'
		)
	);
};
...
export function RoleAwareShell({
	...
	const [localMode, setLocalMode] = useState<ShellMode>(() => initialMode(phase));
	...
	const mode: ShellMode = phase === 'safe' ? 'safe' : (controlledMode ?? localMode);
	const target: ConversationTarget =
		phase === 'safe' ? 'shape' : (controlledTarget ?? (mode === 'run' ? 'use' : 'shape'));
	...
	const hasShaperActivity = workspace.shaper.messages.length > 0 || shaping.status !== 'idle';
	...
	const collapsed = docked && phase !== 'safe' && preferences.value.railCollapsed;
	...
	useLayoutEffect(() => {
		...
		const composer = railContainerRef.current?.querySelector<HTMLElement>('.composer');
	...
	const restoreLostFocus = useCallback(() => {
		...
		container?.querySelector<HTMLElement>('.agent-rail__header button:not(:disabled)')?.focus();
	}, [collapsed]);
	...
	const focusComposer = useCallback(() => {
		...
		queueMicrotask(() => {
			railContainerRef.current
				?.querySelector<HTMLTextAreaElement>('.composer textarea[name="message"]')
	...
	const applyDirectManipulation = useCallback(
		(instruction: string) => {
			...
			setSelectionMode(false);
			void (
				shaping.requestTargeted?.(instruction, canvasSelection, selectedNodeId) ??
	...
	const resizeBy = useCallback(
		(delta: number) => preferences.setRailWidth(preferences.value.railWidth + delta),
		[preferences]
	);
	...
	const handlePointerMove = (event: PointerEvent<HTMLDivElement>) => {
		...
		void preferences.setRailWidth(origin.width + (origin.pointerX - event.clientX));
	};
	...
	return (
		<div
			...
			data-reduced-motion={
				globalThis.matchMedia?.('(prefers-reduced-motion: reduce)').matches ? 'true' : 'false'
			}
			data-target={target}
		...
		>
			...
			{shareDialogOpen && onOpenShareUrl !== undefined && onOpenShareGit !== undefined && (
				<Suspense fallback={<ShareSurfaceFallback />}>
					<ShareSourceDialog
						candidateOpen={shareReview !== undefined}
						onClose={() => setShareDialogOpen(false)}
						onOpenGit={onOpenShareGit}
						{...(onOpenSharePrivate === undefined ? {} : { onOpenPrivate: onOpenSharePrivate })}
				...
				<input
					...
					onChange={(event) => {
						...
						if (file !== undefined) {
							void file
								.arrayBuffer()
								.then((buffer) => onOpenShareFile(file.name, new Uint8Array(buffer)))
								.catch(() => setShareFileError('The shared file could not be reviewed safely.'));
						}
			...
			>
				...
				{docked && !starterWorkspace && phase === 'accepted' && !preview && (
					<button
						aria-label={selectionMode ? 'Cancel element selection' : 'Select element'}
				...
				{starterWorkspace ? (
					<section className='blank-invitation'>
						...
						<p>
							Flect makes a live interface from your outcome. You can keep using and changing it
							here.
						</p>
					</section>
					...
					<>
						<Suspense fallback={<ShareSurfaceFallback />}>
							<InterfaceRenderer
								...
								onSelectionChange={(selection, nodeId) => chooseCanvasSelection(selection, nodeId)}
								renderPrompt={() => (
									<button className='canvas-agent-entry' onClick={focusComposer} type='button'>
