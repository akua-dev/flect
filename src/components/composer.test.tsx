...
describe('Composer', () => {
	...
	it('keeps one draft while internal routing changes', async () => {
		...
		await user.type(screen.getByRole('textbox', { name: 'Message Flect' }), 'Continuous draft');

		rerender(<Composer {...props({ mode: 'run' })} />);
		expect(screen.getByRole('textbox', { name: 'Message Flect' })).toHaveValue('Continuous draft');
	...
	it('hydrates the single private draft and reports it as accepted continuity', async () => {
		...
		expect(onDraftChange).toHaveBeenLastCalledWith('acceptedUse', 'Saved Flect message now');
	});
	...
	])('describes the %s state without exposing a false action', (status, help) => {
	...
	it('keeps the first message editable while provider setup blocks sending', async () => {
		const user = userEvent.setup();
		const onDraftChange = vi.fn(() => Promise.resolve());
		render(<Composer {...props({ status: 'setup-required', onDraftChange })} />);
		...
		expect(onDraftChange).toHaveBeenLastCalledWith('acceptedUse', 'Build my first interface');
		expect(screen.getByRole('button', { name: 'Send to Flect' })).toBeDisabled();
	});
	...
	it('turns the primary action into a single Flect stop control', async () => {
		...
		expect(screen.queryByRole('button', { name: 'Send to Flect' })).not.toBeInTheDocument();
	});
	...
	it('locks protected actions synchronously while submission starts', async () => {
		...
		await user.type(screen.getByRole('textbox', { name: 'Message Flect' }), 'Change the interface');
		await user.click(screen.getByRole('button', { name: 'Send to Flect' }));

		expect(screen.getByRole('button', { name: 'Actions' })).toBeDisabled();
		expect(screen.getByRole('button', { name: 'Send to Flect' })).toBeDisabled();
		expect(screen.getByRole('button', { name: 'Send to Flect' })).toHaveAccessibleDescription(
