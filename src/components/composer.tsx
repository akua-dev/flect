...
import { type AgentSessionStatus, isAgentSessionActive } from '../hooks/use-agent-session';
import {
	PromptInput,
	PromptInputBody,
	PromptInputFooter,
...
export interface ComposerProps {
	...
	readonly onDraftChange?: (key: keyof ContinuityDrafts, value: string) => Promise<void>;
...
export function Composer({
	...
	const inputUnavailable =
		disabled || mode === 'safe' || status === 'booting' || status === 'unavailable';
	...
	const canSubmit = prompt.trim().length > 0 && !submissionUnavailable && !protectedActionsLocked;
	...
	const submit = async () => {
		...
		if (!nextPrompt || submittingRef.current || isActive || submissionUnavailable) {
			return;
		}
		...
		>
			...
			<PromptInputFooter className='composer__rail'>
				<PromptInputTools className='composer__tools'>
					...
					/>
					{mode === 'safe' && <span className='composer__safe-label'>Recovery mode</span>}
					<ModelMenu
						authEvent={authEvent}
