...
describe('ShareSourceDialog', () => {
	it('submits a labeled HTTPS source and closes through the callback', async () => {
		...
		render(<ShareSourceDialog onClose={vi.fn()} onOpenGit={vi.fn()} onOpenUrl={onOpenUrl} open />);
		...
		expect(onOpenUrl).toHaveBeenCalledWith('https://example.test/weather.flect-share');
	});

	it('requires an exact commit and rejects credential-bearing URLs', () => {
		render(<ShareSourceDialog onClose={vi.fn()} onOpenGit={vi.fn()} onOpenUrl={vi.fn()} open />);
	...
	it('returns focus to the control that opened it', async () => {
		...
		await waitFor(() => expect(screen.getByLabelText('Shared file URL')).toHaveFocus());
	...
	it('opens an opaque private reference only after replacing an existing candidate', async () => {
		...
		fireEvent.click(screen.getByRole('checkbox', { name: /replace the inactive candidate/i }));
		fireEvent.click(screen.getByRole('button', { name: 'Review source' }));

		await waitFor(() =>
			expect(onOpenPrivate).toHaveBeenCalledWith('company-share', 'designs/weather')
		);
	});
});
