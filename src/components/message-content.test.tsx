...
[^1]: Supporting detail.`}
    ...
    expect(container.querySelector("code:not(pre code)")).toHaveTextContent(
      "inline()",
    );
    ...
    expect(screen.getByRole("doc-noteref")).toHaveAttribute(
      "href",
      "#user-content-fn-1",
    );
    expect(screen.getByRole("doc-endnotes")).toHaveTextContent(
      "Supporting detail.",
    );
  ...
  it("uses chat hard breaks only for user and activity messages", () => {
    ...
    expect(assistant.container.querySelector("p")).toHaveTextContent(
      "line one line two",
    );
  });
...
<form><input value="secret" /></form>`}
    ...
    expect(
      container.querySelector(
        "script, style, iframe, object, form, img, audio, video",
      ),
    ).not.toBeInTheDocument();
    expect(
      container.querySelector("[onclick], [style]"),
    ).not.toBeInTheDocument();
...
\`\`\``}
    ...
    expect(highlightMarkdownCode).toHaveBeenCalledWith(
      "const answer: number = 42\n",
      "typescript",
    );
...
| Flect | Ready |`}
    ...
    expect(container.querySelector(".markdown-table")).toHaveAttribute(
      "data-expanded",
      "false",
    );
    expect(
    ...
    expect(
      screen.getByRole("button", { name: "Copy table as CSV" }),
    ).toBeVisible();
  });
});
