...
import type { AgentSessionStatus, AgentWorkspaceController } from '../hooks/use-agent-session';
...
const shaping = (overrides: Partial<ShapingController> = {}): ShapingController => ({
	status: 'idle',
...
function ShellHarness({
	...
	const preferences: ShellPreferencesController = {
		...
		setRailCollapsed: async (railCollapsed) =>
			setValue((current) => ShellPreferencesValue.make({ ...current, railCollapsed })),
...
describe('RoleAwareShell', () => {
	it('keeps the starter workspace free of canvas-only actions', () => {
		render(<ShellHarness document={defaultInterfaceDocument} phase='accepted' />);
		...
		expect(screen.queryByRole('button', { name: 'Select element' })).not.toBeInTheDocument();
		...
		expect(screen.queryByRole('link', { name: 'Flect home' })).not.toBeInTheDocument();
	...
	it('keeps an untouched starter workspace centered until work begins', () => {
		...
		expect(container.querySelector('.role-shell')).toHaveClass('role-shell--centered');
		expect(container.querySelector('.agent-rail-container[data-layout="center"]')).toBeVisible();
		expect(container.querySelector('.workspace-canvas')).toHaveClass('workspace-canvas--starter');
		expect(screen.queryByRole('separator', { name: 'Resize agent panel' })).not.toBeInTheDocument();
	});
	...
	it('opens Settings across the canvas instead of inside the agent rail', async () => {
		...
		expect(document.querySelector('.workspace-canvas')).toHaveClass('workspace-canvas--settings');
		await waitFor(() =>
	...
	it('announces the active agent and protected workbench state atomically', () => {
		...
		rerender(<ShellHarness phase='preview' shapingController={shaping({ status: 'preview' })} />);
		expect(screen.getByRole('status', { name: 'Workbench status' })).toHaveTextContent(
		...
		expect(screen.getByRole('status', { name: 'Workbench status' })).toHaveTextContent(
			'Recovery mode. Custom interface changes are hidden.'
		);
	});
	...
	it('does not expose internal mode controls', () => {
		...
		const { rerender } = render(
			<ShellHarness controlledMode='run' onModeChange={onModeChange} phase='accepted' />
		);
		...
		rerender(<ShellHarness controlledMode='edit' onModeChange={onModeChange} phase='accepted' />);
		expect(screen.getByRole('textbox', { name: 'Message Flect' })).toBeVisible();
	});
	...
	it('moves the same focused composer from blank Edit into the rail', async () => {
		...
		expect(screen.getByText('What do you need?')).toBeVisible();
		expect(container.querySelector('.role-shell--centered')).toBeInTheDocument();

		await user.type(input, 'Create a project overview{Enter}');

		expect(request).toHaveBeenCalledWith('Create a project overview');
		await waitFor(() => expect(container.querySelector('.role-shell--split')).toBeInTheDocument());
		expect(screen.getByRole('textbox', { name: 'Message Flect' }).closest('.composer')).toBe(
			originalComposer
		);
		expect(screen.getByRole('textbox', { name: 'Message Flect' })).toHaveFocus();
	});

	it('restores focus to the composer when the focused canvas control disappears', async () => {
		const { rerender } = render(<ShellHarness document={documentWithAction} phase='accepted' />);
		...
		rerender(<ShellHarness document={documentWithoutPrompt} phase='accepted' />);
	...
	it('routes a selected visible element through one targeted agent handoff', async () => {
		...
		await waitFor(() =>
			expect(document.querySelector('.canvas-edit-palette__target')).toHaveTextContent('Projects')
		);
	...
	it('turns direct manipulation into a targeted attributable edit', async () => {
		const user = userEvent.setup();
		const requestTargeted = vi.fn(() => Promise.resolve());
		render(<ShellHarness phase='accepted' shapingController={shaping({ requestTargeted })} />);
	...
	it('shows one history and one stop control during internal work', async () => {
		const controller = workspace({
			...
			shaper: {
				...shaperRole(),
				messages: [{ id: 'shape-1', role: 'assistant', content: 'Shaper history' }],
				shape: vi.fn(() =>
					Promise.resolve({
						kind: 'document' as const,
						document: documentWithoutPrompt
		...
		const { rerender } = render(<ShellHarness phase='accepted' workspaceController={controller} />);
	...
	it('keeps preview decisions next to the protected composer', async () => {
		...
		expect(
			within(screen.getByRole('region', { name: 'Import decision' })).getByText('Projects')
		).toBeVisible();
	...
	it('keeps the protected rail when the interface has no prompt node', () => {
		...
		expect(screen.getByRole('complementary', { name: 'Flect agent' })).toBeVisible();
		expect(screen.getByRole('textbox', { name: 'Message Flect' })).toBeVisible();
	});
	...
	it('collapses, restores focus, and reopens the rail', async () => {
		...
		await waitFor(() =>
			expect(screen.getByRole('button', { name: 'Collapse agent' })).toHaveFocus()
		);
	});
	...
	it('reports a rejected local share without an unhandled file-input failure', async () => {
		...
		expect(screen.getByRole('alert')).not.toHaveTextContent('private archive detail');
