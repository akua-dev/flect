...
describe('MarkdownTable', () => {
	it('renders semantic cells, expands, and copies both formats', async () => {
		...
		await user.click(screen.getByRole('button', { name: 'Expand table cells' }));
		expect(wrapper).toHaveAttribute('data-expanded', 'true');
		expect(screen.getByRole('button', { name: 'Collapse table cells' })).toHaveAttribute(
			'aria-pressed',
			'true'
		);

		await user.click(screen.getByRole('button', { name: 'Copy table as Markdown' }));
		await waitFor(() =>
			expect(clipboardHarness.writes).toEqual(['| Name | Note |\n| --- | --- |\n| Flect | A, B |'])
		);
