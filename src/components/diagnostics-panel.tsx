...
const stateLabel = (state: ShellLinkStatus['state'] | AgentIntegrationStatus['state']) => {
	switch (state) {
		case 'absent':
...
export function DiagnosticsPanel({
	...
	const confirmAgent = (host: AgentIntegrationHost, action: 'install' | 'remove') => {
		...
		if (
			...
			void (action === 'install' ? setup?.installAgent(host) : setup?.removeAgent(host));
		}
	};
	const confirmUpdate = () => {
	...
	const uninstallPending =
		uninstall?.ownedIntegrations.some((item) => item.result === 'pending') === true;
	...
	const panelBody = (
		<div className='diagnostics-panel__body'>
			...
			{update !== undefined && (
				...
				>
					<div className='diagnostics-panel__setup-heading'>
						<div>
							...
							{updateSnapshot === undefined ? (
								...
								<>
									...
									<p>Apple silicon macOS · {updateSize(updateSnapshot.candidate.contentLength)}</p>
								</>
						...
						{updateSnapshot?.state === 'current' ? (
							<button disabled={update.loading} onClick={() => void update.check()} type='button'>
								Check for updates
							</button>
						) : updateSnapshot?.state === 'available' ? (
							<button disabled={update.loading} onClick={confirmUpdate} type='button'>
								Install update
							</button>
						) : updateSnapshot?.state === 'ready-to-relaunch' ? (
							<button
					...
					{(updateSnapshot?.state === 'downloading' || updateSnapshot?.state === 'installing') && (
						<progress
			...
			>
				<div className='diagnostics-panel__setup-heading'>
					...
					{!nativeAvailable ? (
						...
						<button disabled={setup.loading} onClick={() => confirmShell('remove')} type='button'>
						...
						<button
							...
							onClick={() => confirmShell(shell.state === 'stale' ? 'repair' : 'install')}
							type='button'
						>
							{shell.state === 'stale' ? 'Repair command-line link' : 'Install command-line link'}
						</button>
					)}
				</div>
				...
				{nativeAvailable ? (
					<ul aria-label='Agent context integrations' className='diagnostics-panel__integrations'>
							...
							<li key={agent.host}>
								...
								{agent.state === 'installed' ? (
									...
									<button
										...
										type='button'
									>
										{agent.state === 'stale' ? 'Repair' : 'Enable'} {hostLabel(agent.host)} context
									</button>
								)}
							</li>
						))}
					...
					<div className='diagnostics-panel__uninstall'>
						<div className='diagnostics-panel__setup-heading'>
							...
							<button
								...
								type='button'
							>
								{uninstallPending ? 'Prepare to uninstall' : 'Integrations prepared'}
							</button>
						</div>
			...
			{operations.length > 0 && (
				<ol aria-label='Recent operations' className='diagnostics-panel__operations'>
					{operations
						.slice(-20)
						.reverse()
						.map((operation) => (
