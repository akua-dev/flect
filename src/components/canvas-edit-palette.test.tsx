...
import { CanvasEditPalette, placeCanvasEditPalette } from './canvas-edit-palette';
...
describe('CanvasEditPalette', () => {
	it('places editing controls below a target when the viewport has room', () => {
		expect(
			placeCanvasEditPalette(CanvasSelectionRect.make({ x: 300, y: 80, width: 200, height: 60 }), {
				width: 800,
				height: 600
			})
	...
	it('moves above and clamps within the viewport near an edge', () => {
		expect(
			placeCanvasEditPalette(CanvasSelectionRect.make({ x: 760, y: 560, width: 32, height: 24 }), {
				width: 800,
				height: 600
			})
	...
	it('exposes compact semantic actions and one clear action', async () => {
		...
		await user.click(screen.getByRole('button', { name: 'Clear canvas selection' }));

		expect(onAction).toHaveBeenCalledWith('move-later');
		expect(onClear).toHaveBeenCalledTimes(1);
		expect(screen.getByRole('toolbar', { name: 'Edit Project heading' })).toHaveAttribute(
			'data-placement',
			'below'
		);
	});
