...
export function ModelMenu({
	...
	const selectedValue = selectedModel === undefined ? 'auto' : modelValue(selectedModel);
	...
	const searchedModels = useMemo(
		() =>
			models.filter((model) =>
				`${model.name} ${model.id} ${model.provider}`.toLocaleLowerCase().includes(normalizedQuery)
			),
	...
	const filtered = useMemo(() => {
		...
		return visible.toSorted((left, right) => {
			const favoriteDelta =
				Number(favorites.has(modelValue(right))) - Number(favorites.has(modelValue(left)));
			return (
				favoriteDelta || left.name.localeCompare(right.name) || left.id.localeCompare(right.id)
			);
		});
	}, [favorites, normalizedQuery.length, providerFilter, searchedModels]);
	const providerGroups = useMemo(
		() =>
			Array.from(new Set(filtered.map((model) => model.provider))).map((provider) => ({
				provider,
	...
	return (
		<div className='model-menu'>
			<button
				...
				aria-label={`Model: ${selectedModel === undefined ? 'Auto via Pi' : selectedLabel}`}
			...
			{open && (
				...
				>
					{!normalizedQuery && (
						<nav aria-label='Model providers' className='model-menu__provider-rail'>
							<button
							...
							<span aria-hidden='true' className='model-menu__provider-separator' />
					...
					<div className='model-menu__content'>
						...
						>
							...
							{filtered.length === 0 && <p className='model-menu__empty'>{emptyMessage}</p>}
						</div>
						{reasoningModel !== undefined && (
							<section aria-label='Reasoning effort' className='model-menu__reasoning'>
								<div>
