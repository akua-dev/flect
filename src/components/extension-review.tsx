...
const roleLabel = (role: PortableExtensionRole) =>
  role === "app" ? "App Agent" : "Shaper";
...
const stateLabel = (entry: PortableExtensionRoleState) => {
  switch (entry.state) {
    ...
    case "enabled":
      return entry.binding === "candidate" && !entry.tested
        ? "Enabled · test required"
        : "Enabled";
    case "disabled":
      return "Disabled";
    case "failed":
...
function ExtensionRoleReview({
  ...
  const canEnable =
    entry.state !== "incompatible" && entry.state !== "conflict";
  ...
  return (
    ...
    >
      ...
      <span className="extension-review__state" id={`${id}-state`}>
        {stateLabel(entry)}
        {entry.pinned ? " · pinned" : ""}
        {entry.forkRevision === undefined
          ? ""
          : ` · fork ${entry.forkRevision}`}
      </span>
        ...
        {extension.capabilities.length === 0 ? (
          ...
          <ul>
            {extension.capabilities.map((capability) => {
              const required = entry.requiredCapabilities.includes(
                capability.id,
              );
              return (
                <li key={capability.id}>
                  <label>
                    <input
                      ...
                      onChange={(event) =>
                        toggleGrant(capability.id, event.currentTarget.checked)
                      }
                      type="checkbox"
                    />
                    <span>
      ...
      <div className="extension-review__actions">
        {canEnable && (
          <button
            ...
            onClick={() =>
              void perform(
                `${enabled ? "Disabling" : "Enabling"} ${role}`,
        ...
        <button
          ...
          onClick={() =>
            void perform(
              `${entry.pinned ? "Unpinning" : "Pinning"} ${role}`,
        ...
        <div className="extension-review__conflict">
          ...
          <div className="extension-review__actions">
            <button
              className="decision-button"
              onClick={() =>
                void perform(`Using upstream for ${role}`, () =>
                  onResolveUpdate(key, "upstream"),
                )
              }
              type="button"
            >
      ...
      <details className="extension-review__advanced">
        ...
        <div className="extension-review__fork">
          ...
          <button
            ...
            onClick={() =>
              void perform(`Forking for ${role}`, () =>
                onFork(key, forkRevision),
              )
            }
            type="button"
          >
        ...
        {confirmRemove ? (
          <div className="extension-review__remove-confirm">
            ...
            <button
              ...
              onClick={() =>
                void perform(`Removing from ${role}`, () => onRemove(key))
              }
              type="button"
            >
      ...
      {status !== undefined && (
        <p
          aria-live="polite"
          className="extension-review__operation"
          role="status"
        >
          {status}
        </p>
  ...
  return (
    <section aria-label="Portable extensions" className="extension-review">
      <header className="extension-review__intro">
        ...
        <p>
          Untrusted package code stays in a bounded browser sandbox. Each role
          receives only the access you enable here.
        </p>
      </header>
