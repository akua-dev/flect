...
describe('CapsuleFrame', () => {
			...
			[
				{
					...
					contents: new TextEncoder().encode(`main{background-image:url("../../assets/mark.svg")}`)
				},
				{
					path: 'assets/mark.svg',
					contents: new TextEncoder().encode(`<svg xmlns="http://www.w3.org/2000/svg"></svg>`)
				},
				{
					path: 'ui/app.js',
					contents: new TextEncoder().encode(`document.querySelector("#run").textContent="Ready"`)
				}
			]
		);
	...
	it('connects when the opaque frame loaded before effect subscription', async () => {
		...
		await waitFor(() => expect(FakeMessageChannel.created).toBeGreaterThanOrEqual(2));
	});
	...
	it('returns a correlated bounded result to the requesting capsule', async () => {
		...
		await waitFor(() =>
			expect(port?.posted).toContainEqual(expect.objectContaining({ type: 'visibility' }))
		);
		port?.receive({
			version: 1,
			type: 'intent',
		...
		expect(onIntent).toHaveBeenCalledWith(expect.objectContaining({ action: 'projects.list' }));
	});
	...
	it('keeps the authenticated channel when host callbacks change', async () => {
		...
		const rendered = render(<CapsuleFrame html='<main>Product</main>' onIntent={firstIntent} />);
		fireEvent.load(screen.getByTitle('Flect app'));
		await new Promise((resolve) => globalThis.setTimeout(resolve, 0));
		...
		await waitFor(() =>
			expect(channel?.port1.posted).toContainEqual(expect.objectContaining({ type: 'visibility' }))
		);
		const createdBeforeRerender = FakeMessageChannel.created;

		rendered.rerender(<CapsuleFrame html='<main>Product</main>' onIntent={nextIntent} />);
	...
	it('exchanges a bounded selection without reconnecting the capsule', async () => {
		...
		await waitFor(() => expect(onSelectionChange).toHaveBeenCalledWith(selection));
		rendered.rerender(
			<CapsuleFrame
				html='<article>Late delivery</article>'
				onClearSelection={onClearSelection}
