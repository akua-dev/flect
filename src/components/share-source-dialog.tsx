...
export function ShareSourceDialog({
  ...
  const [privateAdapterId, setPrivateAdapterId] = useState(
    privateSources[0]?.id ?? "",
  );
  ...
  const privateAvailable =
    privateSources.length > 0 && onOpenPrivate !== undefined;
  ...
  const submit = async (event: FormEvent) => {
    ...
    const decoded =
      mode === "url"
        ? Effect.runSync(
            Effect.result(
              Schema.decodeUnknownEffect(ShareUrlSource)({ _tag: "url", url }),
            ),
          )
        : mode === "git"
          ? Effect.runSync(
    ...
    try {
      ...
      setError(
        "The shared source could not be opened. Check it and try again.",
      );
    } finally {
      setBusy(false);
    }
    ...
    >
      <form onSubmit={(event) => void submit(event)}>
        <header>
          ...
          <button
            aria-label="Close shared source dialog"
            onClick={onClose}
            type="button"
          >
            Close
        ...
        <div aria-live="polite" id={panelId} role="tabpanel">
          {mode !== "private" && (
            <label>
              <span>
                {mode === "url" ? "Shared file URL" : "Repository URL"}
              </span>
              <input
            ...
            <>
              <label>
                ...
                <select
                  ...
                  onChange={(event) =>
                    setPrivateAdapterId(event.currentTarget.value)
                  }
              ...
              <label>
                ...
                <input
                  ...
                  onChange={(event) =>
                    setPrivateReference(event.currentTarget.value)
                  }
                  ref={urlRef}
                  type="text"
        ...
        {candidateOpen && (
          <label className="share-source-dialog__replacement">
            <input
              ...
              onChange={(event) =>
                setReplaceConfirmed(event.currentTarget.checked)
              }
              type="checkbox"
            />
            <span>
              Replace the inactive candidate. Your accepted app stays unchanged.
            </span>
          </label>
        )}
        <footer>
