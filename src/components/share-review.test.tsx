...
describe("ShareReview", () => {
  ...
  it("explains signature, compatibility, origin, and authority without implying trust", () => {
    ...
    expect(
      screen.queryByText(/token|credential|endpoint/i),
  ...
  it("blocks invalid signatures even when the share is otherwise compatible", () => {
    ...
    expect(
      screen.getByRole("button", { name: "Retain selected" }),
    ).toBeDisabled();
  });
  ...
  it("announces a newly opened review without scrolling the document", async () => {
    ...
    await waitFor(() =>
      expect(
        screen.getByRole("region", { name: "Weather workspace" }),
      ).toHaveFocus(),
    );
  ...
  it("does not prepare a no-op update that is already installed", () => {
    ...
    expect(
      screen.getByRole("button", { name: "Already current" }),
    ).toBeDisabled();
  });
  ...
  it("offers explicit model-free conflict choices and never enables Keep", () => {
    ...
    expect(
      screen.queryByRole("button", { name: "Preview selected" }),
    ).not.toBeInTheDocument();
    fireEvent.click(
