...
import { type CanvasEditAction, CanvasEditPalette } from './canvas-edit-palette';
...
const base64 = (contents: Uint8Array) => {
	...
	for (let offset = 0; offset < contents.length; offset += 0x8000) {
		binary += String.fromCharCode(...contents.subarray(offset, offset + 0x8000));
	}
...
export const projectCapsuleDocument = (
	...
	for (const script of parsed.querySelectorAll<HTMLScriptElement>('script[src]')) {
		...
		try {
			...
			script.textContent = decodeText(asset.contents).replace(/<\/script/gi, '<\\/script');
		} catch {
			script.remove();
		}
	}
	...
	for (const element of parsed.querySelectorAll<HTMLElement>('video[poster]')) {
		const path = resolvePath(entrypointPath, element.getAttribute('poster') ?? '');
		const asset = path === undefined ? undefined : assets.get(path);
	...
	for (const style of parsed.querySelectorAll<HTMLStyleElement>('style')) {
		style.textContent = rewriteCss(style.textContent ?? '', entrypointPath, assets);
	}
...
export interface CapsuleFrameProps {
	...
	readonly onDirectManipulation?: (kind: 'move' | 'resize', deltaX: number, deltaY: number) => void;
...
export function CapsuleFrame({
	...
	const nonce = useMemo(() => bridgeNonceFor(projectedDocument), [projectedDocument]);
	const source = useMemo(() => documentFor(projectedDocument, nonce), [nonce, projectedDocument]);
	...
	useEffect(() => {
		...
		const connect = () => {
			if (disposed || frame.contentWindow === null || portRef.current !== undefined) {
				return;
			}
			...
			channel.port1.onmessage = (event) => {
				...
				void Effect.runPromise(decodeCapsuleMessage(event.data)).then((message) => {
					if (disposed) return;
					if (message.type === 'ready') {
						...
						void (async () => {
							...
							try {
								const decoded = await Effect.runPromise(decodeCapsuleHostMessage(requested));
								outcome =
									decoded.type === 'intent-result' && decoded.id === message.id
										? decoded
										: invalidResult;
							} catch {
			...
			channel.port1.start();
			frame.contentWindow.postMessage({ version: 1, type: 'flect:connect', nonce }, '*', [
				channel.port2
			]);
		};
	...
	const beginManipulation = (kind: 'move' | 'resize', event: ReactPointerEvent<HTMLElement>) => {
	...
	return (
		<div className='capsule-surface'>
			<iframe
				...
				style={height?.source === source ? { height: `${height.value}px` } : undefined}
				title={title}
			/>
			{selection !== undefined && (
				<fieldset
					...
					style={{
						left:
							selection.rect.x +
							(manipulationPreview?.kind === 'move' ? manipulationPreview.deltaX : 0),
						top:
							selection.rect.y +
							(manipulationPreview?.kind === 'move' ? manipulationPreview.deltaY : 0),
						width: Math.max(
							16,
							selection.rect.width +
								(manipulationPreview?.kind === 'resize' ? manipulationPreview.deltaX : 0)
						),
						height: Math.max(
							16,
							selection.rect.height +
								(manipulationPreview?.kind === 'resize' ? manipulationPreview.deltaY : 0)
						)
					}}
				>
