...
describe("ProductCapabilityDecisionStore", () => {
  it.effect("round-trips a strict normalized v2 decision record", () => {
    ...
    const layer = makeProductCapabilityDecisionStoreLayer.pipe(
  ...
  it.effect("migrates only a matched and currently bounded v1 decision", () => {
    ...
    return Effect.gen(function* () {
      ...
      assert.deepStrictEqual(loaded.decisions[0]?.operationIds, [
        "projects.list",
      ]);
      assert.strictEqual(
        loaded.decisions[0]?.requestDigest,
  ...
  it.effect(
    "migrates false to revoked and never invents a disallowed policy",
    () => {
      const storage = makeStorage({
        [V1_KEY]: JSON.stringify({
          version: 1,
  ...
  it.effect(
    "keeps v1 and returns no migrated authority when v2 save fails",
    () => {
      const storage = makeStorage({
        [V1_KEY]: JSON.stringify({
          version: 1,
