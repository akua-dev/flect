import { Clock, Context, Effect, Layer, Result, Schema, type SchemaAST } from 'effect';
import {
...
export class ProductCapabilityDecisionStoreFailure extends Schema.TaggedErrorClass<ProductCapabilityDecisionStoreFailure>()(
	...
	{
		reason: Schema.Literals(['invalid-record', 'persistence-failed', 'storage-unavailable']),
...
const decodeV2 = (raw: string) =>
	parseJson(raw).pipe(
		Effect.flatMap(Schema.decodeUnknownEffect(ProductCapabilityDecisionRecord, strict)),
...
const decodeV1 = (raw: string) =>
	parseJson(raw).pipe(
		Effect.flatMap(Schema.decodeUnknownEffect(ProductCapabilityGrantRecord, strict)),
...
export const makeProductCapabilityDecisionStoreLayer = Layer.effect(
	...
	Effect.gen(function* () {
		...
		const save = Effect.fn('ProductCapabilityDecisionStore.save')(function* (
			...
			const normalized = normalize(
				yield* Schema.decodeUnknownEffect(
					Schema.Array(ProductCapabilityDecision).check(Schema.isMaxLength(128)),
					strict
		...
		const load = Effect.fn('ProductCapabilityDecisionStore.load')(function* (
			...
			const byKey = new Map(
				contexts.map((context) => [`${context.scopeId}\u0000${context.manifest.id}`, context])
			);
			const represented = new Set(
				durable.map((decision) => `${decision.scopeId}\u0000${decision.capabilityId}`)
			);
			const migrated = [...legacy.success]
				...
				.filter(
					(entry) =>
						!represented.has(`${entry.state.scopeId}\u0000${entry.state.capabilityId}`) &&
						entry.context?.manifest.confirmationPolicies.includes('persistent')
				)
				.map((entry) => {
			...
			const complete = legacy.success.every((state) =>
				decisions.some(
					(decision) =>
						decision.scopeId === state.scopeId && decision.capabilityId === state.capabilityId
				)
			);
			if (complete) {
