    ...
    () =>
      runtime.runPromise(
        Effect.gen(function* () {
          ...
          setProjects(
            projectPermissions.filter((permission) => permission.requested),
          );
          setReports(
            reportPermissions.filter((permission) => permission.requested),
          );
        }),
      ),
    [runtime],
      ...
      runtime
        .runPromise(
          Effect.gen(function* () {
            const service = yield* ProductCapabilityRegistry;
            const request =
              scopeId === projectsScope ? projectsRequest : reportsRequest;
