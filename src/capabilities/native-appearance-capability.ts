...
export const NativeAppearanceCapabilityManifest =
...
export const makeNativeAppearanceOperation = (
  systemAccentColor: Effect.Effect<
    NativeAccentColor,
    NativePlatformCapabilityFailure
  >,
  ...
  authorize: (input) =>
    ...
    (typeof input === "object" &&
      ...
      : Effect.fail(
          productOperationFailure("native.appearance.current", "invalid-input"),
        ),
  execute: () =>
    systemAccentColor.pipe(
      Effect.map((accent) => ({ ...accent })),
      Effect.mapError(() =>
        productOperationFailure("native.appearance.current", "unavailable"),
      ),
    ),
});
