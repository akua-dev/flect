...
describe("ProductCapabilityBroker", () => {
  it.effect(
    "projects available, requested, denied, granted, and revoked states",
    () => {
  ...
  it.effect(
    "consumes once and resets a bounded rate window with TestClock",
    () => {
      const { layer } = makeLayer();
      return Effect.gen(function* () {
        const broker = yield* ProductCapabilityBroker;
        ...
        const consumed = yield* Effect.result(
          broker.reserve(request, operation),
        );
  ...
  it.effect(
    "allows only one concurrent reservation for a once decision",
    () => {
      const { layer } = makeLayer();
      return Effect.gen(function* () {
        const broker = yield* ProductCapabilityBroker;
        ...
        assert.strictEqual(
          results.filter((result) => Result.isSuccess(result)).length,
          1,
        );
        assert.strictEqual(
          results.filter((result) => Result.isFailure(result)).length,
          1,
        );
      }).pipe(Effect.provide(layer));
    },
  ...
  it.effect(
    "keeps durable authority unchanged when revocation cannot persist",
    () => {
      ...
      return Effect.gen(function* () {
        const broker = yield* ProductCapabilityBroker;
        assert.strictEqual(
          (yield* broker.catalog(request))[0]?.state,
          "granted",
        );
  ...
  it.effect(
    "denies scope widening and invalidates an old request digest",
    () => {
      const { layer } = makeLayer();
      return Effect.gen(function* () {
        const broker = yield* ProductCapabilityBroker;
        ...
        assert.strictEqual(
          (yield* broker.catalog(changed))[0]?.state,
          "requested",
        );
  ...
  it.effect(
    "invalidates an exact reservation after expiry or replacement",
    () => {
      const { layer } = makeLayer();
      return Effect.gen(function* () {
        const broker = yield* ProductCapabilityBroker;
