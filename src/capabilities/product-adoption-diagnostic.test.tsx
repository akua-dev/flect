...
describe("ProductAdoptionDiagnostic", () => {
  it("renders three SDK products with public connection, auth, inference, and experience facts", async () => {
    ...
    expect(
      screen.getByRole("heading", { name: "Product adoption SDK" }),
    ).toBeVisible();
    ...
    expect(
      within(offline).getByText("User inference · Product optional"),
    ).toBeVisible();
    expect(within(offline).getByText("Recommended 1.0.0")).toBeVisible();
    expect(
      within(offline).getByText("The product integration is ready."),
    ).toBeVisible();
  ...
  it("switches deterministically between recovery states without losing user-owned facts", async () => {
    ...
    expect(
      within(browser).getByText("Protected review required"),
    ).toBeVisible();
