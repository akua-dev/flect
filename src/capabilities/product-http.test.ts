...
describe("ProductHttp", () => {
  it.effect(
    "rejects path, method, and secret-header authority before fetch",
    () =>
      Effect.gen(function* () {
        ...
        for (const denied of [
          ...
          assert.strictEqual(
            error.message,
            "The product request failed safely.",
          );
        }
  ...
  it.effect(
    "injects host credentials privately and returns only bounded safe output",
    () =>
      Effect.gen(function* () {
        ...
        assert.strictEqual(
          observed?.url,
          "https://api.example.com/v1/projects?limit=2",
        );
        assert.strictEqual(
  ...
  it.effect(
    "cancels oversized responses and reports only a typed failure",
    () =>
      Effect.gen(function* () {
        let cancelled = false;
