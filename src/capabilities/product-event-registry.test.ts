...
const makeLayer = (connector: ProductEventConnector) => {
  const authorize = vi.fn((input: ProductJson) => {
    ...
    return Effect.succeed(
      AuthorizedProductOperation.make({
        ...
        operationId,
        resourceIds: [
          widened ? "projects.administration" : "projects.workspace",
        ],
        dataClassIds: ["projects.summary"],
      }),
    );
  ...
  const registry = makeProductEventRegistryLayer({
    operations: [
      { id: operationId, capabilityId, policyId, authorize, request },
    ],
...
describe("ProductEventRegistry", () => {
  it.effect(
    "denies an ungranted stream before product policy or connector",
    () =>
      Effect.gen(function* () {
        let opened = false;
  ...
  it.effect("keeps product denial and user scope authoritative", () =>
    Effect.gen(function* () {
      ...
      yield* Effect.gen(function* () {
        ...
        const deniedInputs: ReadonlyArray<ProductJson> = [
          { productDenied: true },
          { widen: true },
        ];
  ...
  it.effect("revokes an idle stream and aborts its owned connector", () =>
    Effect.gen(function* () {
      ...
      const error = yield* Effect.gen(function* () {
        ...
        yield* (yield* ProductCapabilityBroker).revoke(
  ...
  it.effect(
    "caller cancellation releases the connector without public failure",
    () =>
      Effect.gen(function* () {
        let aborted = false;
        const connector: ProductEventConnector = {
