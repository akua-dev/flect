...
const connectionLabel = (
  connection: ProductAdoptionProductModel["connection"],
) => {
  switch (connection) {
    case "offline":
      return "Offline";
...
const authenticationLabel = (
  owner: ProductAdoptionProductModel["authenticationOwner"],
) => {
  switch (owner) {
    case "none":
...
const inferenceLabel = (model: ProductAdoptionProductModel) => {
  ...
  return model.inference.defaultOwner === "user"
    ? "User inference"
    : "Product inference";
};
  ...
  return (
    <article aria-labelledby={headingId} className="product-adoption-card">
      <header>
        ...
        <span
          className={`product-adoption-card__state is-${snapshot.state}`}
          role="status"
        >
      ...
      <div className="product-adoption-card__ownership">
        <span>
          {personalFork ? "Personal fork preserved" : "Recommended experience"}
        </span>
...
export function ProductAdoptionDiagnostic({
  ...
  return (
    <main
      className="product-adoption-diagnostic"
      data-testid="product-adoption-diagnostic"
    >
      <header className="product-adoption-diagnostic__hero">
        ...
        <p>
          Three connection models, one protected authority boundary, and
          user-owned interfaces that remain recoverable.
        </p>
      </header>
      <section
        aria-label="Reference products"
        className="product-adoption-diagnostic__grid"
      >
...
export function ProductAdoptionDiagnosticRoute() {
  const [products, setProducts] = useState<
    ReadonlyArray<ProductAdoptionProductModel>
  >([]);
  ...
  if (failed)
    return (
      <main role="alert">The product adoption diagnostic failed safely.</main>
    );
  if (products.length === 0)
    return <main aria-busy="true">Loading product adoption proof…</main>;
