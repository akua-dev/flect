...
interface BrokerState {
	...
	readonly activeLeases: ReadonlyMap<string, ReadonlySet<Fiber.Fiber<unknown, unknown>>>;
}
...
export interface ProductCapabilityBrokerShape {
	...
	) => Effect.Effect<ProductCapabilityProjection, ProductCapabilityBrokerFailure>;
	readonly revoke: (decisionId: string) => Effect.Effect<void, ProductCapabilityBrokerFailure>;
	readonly reserve: (
		context: ProductCapabilityRequestContext,
		operation: AuthorizedProductOperation
	) => Effect.Effect<ProductCapabilityReservation, ProductCapabilityBrokerFailure>;
	...
	readonly warnings: Effect.Effect<ReadonlyArray<ProductCapabilityDecisionStoreFailure>>;
}
...
const brokerFailure = (reason: ProductCapabilityBrokerFailure['reason'], capabilityId?: string) =>
...
const transition = <A, B>(result: A, state: B): readonly [A, B] => [result, state];

const isDurable = (decision: ProductCapabilityDecision) =>
	decision.confirmationPolicy === 'workspace' || decision.confirmationPolicy === 'persistent';

const durableDecisions = (state: BrokerState) => state.decisions.filter(isDurable);

const addWarning = (
	warnings: ReadonlyArray<ProductCapabilityDecisionStoreFailure>,
	warning: ProductCapabilityDecisionStoreFailure
) =>
	warnings.some((current) => current.reason === warning.reason) ? warnings : [...warnings, warning];

const isSubset = (selected: ReadonlyArray<string>, available: ReadonlyArray<string>) =>
...
const lifecycle = (decision: ProductCapabilityDecision, now: number) => {
	...
	if (
		(decision.expiresAtMillis !== undefined && decision.expiresAtMillis <= now) ||
		(decision.confirmationPolicy === 'once' && (decision.usage?.totalInvocations ?? 0) >= 1)
	) {
		return 'expired';
	}
	return 'granted';
...
const makeProjection = (options: {
	...
	return ProductCapabilityProjection.make({
		...
		confirmationPolicies: manifest === undefined ? [] : [...manifest.confirmationPolicies],
		...
		...(decision === undefined
			...
			: {
					...
					...(decision.rateLimit === undefined ? {} : { rateLimit: decision.rateLimit })
				})
	});
};
	...
	Layer.effect(
		...
		Effect.gen(function* () {
			...
			const ensureLoaded = Effect.fn('ProductCapabilityBroker.load')(function* (
				...
				yield* SynchronizedRef.updateEffect(state, (current) => {
					...
					return store.load(migrationContexts).pipe(
						Effect.map((loaded) => {
							const byId = new Map(
								loaded.decisions.map((decision) => [decision.decisionId, decision])
							);
							...
							return {
								...
								loadedContexts: new Set([...current.loadedContexts, contextKey]),
								warnings:
			...
			const catalog = Effect.fn('ProductCapabilityBroker.catalog')(function* (
				...
				const requested = new Map(context.capabilities.map((entry) => [entry.capabilityId, entry]));
				const capabilityIds = [...new Set([...manifests.keys(), ...requested.keys()])].sort(
				...
				return capabilityIds.map((capabilityId) => {
					...
					return makeProjection({
						...
						decision:
							request === undefined ? undefined : latestDecision(current, context, capabilityId),
						now
					});
				});
			});
			...
			const decide = Effect.fn('ProductCapabilityBroker.decide')(function* (
				...
				yield* ensureLoaded(context);
				const requested = context.capabilities.some((entry) => entry.capabilityId === capabilityId);
				if (!requested) {
					return yield* Effect.fail(brokerFailure('not-requested', capabilityId));
				}
				const manifest = manifests.get(capabilityId);
				if (manifest === undefined) {
					return yield* Effect.fail(brokerFailure('unknown-capability', capabilityId));
				}
				const now = yield* Clock.currentTimeMillis;
				const operationIds =
				...
				if (
					...
					return yield* Effect.fail(brokerFailure('invalid-scope', capabilityId));
				}
				if (
					choice.type === 'allow' &&
					(!manifest.confirmationPolicies.includes(choice.confirmationPolicy) ||
						...
						(choice.rateLimit !== undefined &&
							...
							choice.rateLimit.maxInvocations * manifest.maxRate.intervalMs >
								manifest.maxRate.maxInvocations * choice.rateLimit.intervalMs))
				) {
					return yield* Effect.fail(brokerFailure('invalid-scope', capabilityId));
				}
				const selectedRate =
					choice.type === 'allow' ? (choice.rateLimit ?? manifest.maxRate) : undefined;
				const decision = ProductCapabilityDecision.make({
					version: 2,
					decisionId: `decision-${crypto.randomUUID()}`,
					scopeId: context.scopeId,
					...(choice.type === 'allow' && choice.confirmationPolicy === 'workspace'
						? { workspaceId: context.workspaceId }
						: {}),
					requestDigest: context.requestDigest,
					capabilityId,
					status: choice.type === 'deny' ? 'denied' : 'granted',
					confirmationPolicy: choice.type === 'deny' ? 'persistent' : choice.confirmationPolicy,
				...
				yield* SynchronizedRef.updateEffect(state, (current) => {
					const replaced = current.decisions.filter(
						(candidate) =>
							candidate.scopeId === context.scopeId && candidate.capabilityId === capabilityId
					);
					...
					return mustSave
						? store.save(durableDecisions(next)).pipe(
								Effect.as(next),
								Effect.mapError(() => brokerFailure('persistence-failed', capabilityId))
							)
						: Effect.succeed(next);
				});
				const projections = yield* catalog(context);
				const projection = projections.find((candidate) => candidate.capabilityId === capabilityId);
				return projection === undefined
					? yield* Effect.fail(brokerFailure('unknown-capability', capabilityId))
					: projection;
			});
				...
				const fibers = yield* SynchronizedRef.modifyEffect(state, (current) => {
					...
					return (
						isDurable(existing)
							? store.save(durableDecisions(next)).pipe(
									Effect.as(next),
									Effect.mapError(() => brokerFailure('persistence-failed', existing.capabilityId))
								)
							: Effect.succeed(next)
			...
			const reserve = Effect.fn('ProductCapabilityBroker.reserve')(function* (
				...
				if (manifest === undefined) {
					return yield* Effect.fail(brokerFailure('unavailable', operation.capabilityId));
				}
				if (
					!context.capabilities.some((entry) => entry.capabilityId === operation.capabilityId) ||
					!manifest.operationIds.includes(operation.operationId)
				) {
					return yield* Effect.fail(brokerFailure('denied', operation.capabilityId));
				}
				if (
				...
				return yield* SynchronizedRef.modifyEffect(state, (current) => {
					const decision = latestDecision(current, context, operation.capabilityId);
					if (decision === undefined || decision.status === 'denied') {
						return Effect.fail(brokerFailure('denied', operation.capabilityId));
					}
					if (decision.status === 'revoked') {
						return Effect.fail(brokerFailure('revoked', operation.capabilityId));
					}
					if (
						(decision.expiresAtMillis !== undefined && decision.expiresAtMillis <= now) ||
						(decision.confirmationPolicy === 'once' && (decision.usage?.totalInvocations ?? 0) >= 1)
					) {
						return Effect.fail(brokerFailure('expired', operation.capabilityId));
					}
					if (
					...
					const windowReset =
						...
						now - previous.windowStartedAtMillis >= decision.rateLimit.intervalMs;
					const windowInvocations = windowReset ? 0 : previous.windowInvocations;
					if (
						decision.rateLimit !== undefined &&
						windowInvocations >= decision.rateLimit.maxInvocations
					) {
						return Effect.fail(brokerFailure('rate-limited', operation.capabilityId));
					}
					const updated = ProductCapabilityDecision.make({
						...
						usage: ProductCapabilityUsage.make({
							...
							windowStartedAtMillis: windowReset ? now : previous.windowStartedAtMillis
						})
					});
					const next: BrokerState = {
						...current,
						decisions: current.decisions.map((candidate) =>
							candidate.decisionId === decision.decisionId ? updated : candidate
						)
					};
					...
					return isDurable(decision)
						? store.save(durableDecisions(next)).pipe(
								...
								Effect.mapError(() => brokerFailure('persistence-failed', operation.capabilityId))
							)
						...
						yield* SynchronizedRef.modifyEffect(state, (current) =>
							Effect.gen(function* () {
								...
								activeLeases.set(
									...
									new Set(activeLeases.get(reservation.decisionId) ?? []).add(fiber)
								);
						...
						return yield* effect.pipe(
							Effect.ensuring(
								SynchronizedRef.update(state, (current) => {
									const currentLeases = current.activeLeases.get(reservation.decisionId);
			...
			return {
				...
				validate,
				withReservation,
				warnings: SynchronizedRef.get(state).pipe(Effect.map((current) => current.warnings))
			};
		})
	);
