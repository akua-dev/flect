...
export function ReferenceProductDiagnostic() {
  ...
  const [permissions, setPermissions] = useState<
    ReadonlyArray<ProductCapabilityProjection>
  >([]);
    ...
    (operationId: string, input: object = {}) => {
      void diagnostic.runtime
        ...
        .finally(() => {
          ...
          setCredentialState(
            diagnostic.controls.credentialApplied
  ...
  return (
    <main className="product-capability-diagnostic reference-product-diagnostic">
      <header>
        ...
        <p>
          One protected contract across offline work, fixed GraphQL, private
          host authentication, and bounded resumable events.
        </p>
      </header>

      <section
        aria-label="Reference product permissions"
        className="capsule-review"
      >
      ...
      <section
        aria-label="Reference product operations"
        className="capsule-review"
      >
        <h2>Named operations</h2>
        <div className="reference-product-diagnostic__actions">
          <button
            onClick={() => invoke(REFERENCE_OPERATIONS.status)}
            type="button"
          >
            Read offline status
          </button>
          ...
          <button
            onClick={() =>
              invoke(REFERENCE_OPERATIONS.archive, { projectId: "alpha" })
            }
            type="button"
          >
            Archive alpha
