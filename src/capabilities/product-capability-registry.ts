		...
		Effect.gen(function* () {
			...
			const catalog = Effect.fn('ProductCapabilityRegistry.catalog')(function* (
				...
				return options.operations.map((operation) => {
					const permission = projections.find(
						(projection) => projection.capabilityId === operation.capabilityId
					);
			...
			const invokeDetailed = Effect.fn('ProductCapabilityRegistry.invokeDetailed')(function* (
				...
				if (definition === undefined) {
					return yield* Effect.fail(productOperationFailure(invocation.operationId, 'denied'));
				}
				...
				const authorizedOperation = yield* Schema.decodeUnknownEffect(AuthorizedProductOperation)(
					authorization
				).pipe(
					Effect.mapError(() => productOperationFailure(invocation.operationId, 'invalid-input'))
				);
				yield* broker
				...
				const output = yield* broker
					.withReservation(
						...
						definition.execute(invocation.input).pipe(
							Effect.flatMap((candidate) => Schema.decodeUnknownEffect(Schema.Json)(candidate)),
							Effect.mapError(() =>
