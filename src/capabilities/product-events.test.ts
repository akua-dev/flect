...
const layerFor = (
  connector: ProductEventConnector,
...
describe("ProductEvents", () => {
  ...
  it.effect(
    "fails duplicate, regressing, mismatched, and oversized events closed",
    () =>
      Effect.gen(function* () {
            ...
            open: ({ emit }) =>
              Effect.gen(function* () {
                ...
                yield* emit({
                  ...
                  policyId:
                    second === "mismatched"
                      ? "another.events.policy"
                      : policy().id,
                  sequence:
                    second === "mismatched" || second === "oversized"
                      ? "3"
                      : second,
                  payload:
                    second === "oversized"
  ...
  it.effect(
    "reconnects with the last sequence and stops at the declared bound",
    () =>
      Effect.gen(function* () {
        ...
        const running = yield* Effect.gen(function* () {
          ...
          Effect.provide(layerFor(connector)),
        ...
        const exhausted = yield* Effect.gen(function* () {
          ...
          Effect.provide(layerFor(failing)),
          Effect.forkChild({ startImmediately: true }),
        );
        yield* Effect.yieldNow;
  ...
  it.effect(
    "backpressures at policy capacity and aborts owned work on cancellation",
    () =>
      Effect.gen(function* () {
