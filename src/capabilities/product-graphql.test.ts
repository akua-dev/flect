...
const document = 'query ProjectsList($limit: Int!) { projects(limit: $limit) { id } }';
...
describe('ProductGraphql', () => {
	...
	it.effect('rejects a document that does not match registered policy', () =>
		Effect.gen(function* () {
			...
			const error = yield* ProductGraphql.pipe(Effect.provide(layer), Effect.flip);
	...
	it.effect('bounds variables and response bytes', () =>
		Effect.gen(function* () {
			const registered = yield* registration;
			const fetch = vi.fn(() =>
				Promise.resolve(new Response(JSON.stringify({ data: 'x'.repeat(2_000) })))
			);
			const layer = makeProductGraphqlLayer({
				registrations: [registered],
				fetch
	...
	it.effect('sanitizes product and transport failure detail', () =>
		Effect.gen(function* () {
			...
			assert.strictEqual(productError.message, 'The product GraphQL operation failed safely.');
	...
	it.effect('sanitizes private credential resolver defects', () =>
		Effect.gen(function* () {
			...
			const layer = makeProductGraphqlLayer({
				registrations: [registered],
				credentialHeaders: () => Effect.die(new Error('vault host-secret must never escape'))
			});
			const error = yield* Effect.gen(function* () {
				return yield* (yield* ProductGraphql).invoke(request());
			}).pipe(Effect.provide(layer), Effect.flip);
