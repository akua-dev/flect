import { Context, Deferred, Effect, Layer, Option, Queue, Ref, Schema } from 'effect';
import type { AgentCommandSource, FlectCommand, FlectCommandError } from '../../shared/control';
...
export class AgentGatewayResult extends Schema.Class<AgentGatewayResult>('AgentGatewayResult')({
	type: Schema.Literals(['inspect', 'logs', 'propose-interface', 'propose-app', 'command']),
...
export interface AgentCommandBusShape {
	...
	) => Effect.Effect<AgentGatewayResult, AgentCommandBusError | FlectCommandError>;
	readonly take: Effect.Effect<AgentCommandRequest, AgentCommandBusError>;
	readonly shutdown: Effect.Effect<void>;
}

export class AgentCommandBus extends Context.Service<AgentCommandBus, AgentCommandBusShape>()(
...
export const AgentCommandBusLive = Layer.effect(
	...
	Effect.gen(function* () {
		...
		const pending = yield* Ref.make<
			ReadonlyMap<
				string,
				Deferred.Deferred<AgentGatewayResult, AgentCommandBusError | FlectCommandError>
			>
		>(new Map());
		...
		const submit = Effect.fn('AgentCommandBus.submit')(function* (
			...
			if (!accepted) {
				...
				return yield* Effect.fail(busError((yield* Ref.get(closed)) ? 'unavailable' : 'capacity'));
			}
			const timeout = busError('timeout');
			return yield* Deferred.await(response).pipe(
				Effect.timeoutOrElse({
					duration: '30 seconds',
					orElse: () => Deferred.fail(response, timeout).pipe(Effect.andThen(Effect.fail(timeout)))
				}),
				Effect.onInterrupt(() =>
