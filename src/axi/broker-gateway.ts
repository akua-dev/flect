...
import {
  FlectControlClient,
  type FlectControlClientError,
} from "../../cli/flect-client";
...
const transportError = (
  error: FlectControlClientError | ControlDescriptorError,
) =>
  FlectGatewayError.make({
    reason:
      error._tag === "FlectControlClientError" ? error.reason : "unavailable",
    message:
...
const commandError = (
  ...
  error._tag === "FlectControlClientError" ||
  error._tag === "ControlDescriptorError"
  ...
  Layer.effect(
    ...
    Effect.gen(function* () {
      ...
      return {
        ...
        events: (after: number) =>
          client.events(after).pipe(Stream.mapError(transportError)),
