...
describe("Flect AXI program", () => {
  ...
  it.effect(
    ...
    () => {
      ...
      return Effect.gen(function* () {
        ...
        const inspect = yield* runFlect([
          "--json",
          "share",
          "inspect",
          "dev.flect.weather",
        ]);
        const missing = yield* runFlect([
          "--json",
          "share",
          "inspect",
          "dev.flect.missing",
  ...
  it.effect(
    "dispatches share source and export actions through the command gateway",
    () => {
    ...
    () => {
      ...
      const snapshot = FlectWorkspaceSnapshot.make({
        ...
        extensions: PortableExtensionCatalogSnapshot.make({
          version: 1,
          entries: [
            roleState("app-weather", "app"),
            roleState("shape-weather", "shaper"),
          ],
        }),
      });
  ...
  it.effect(
    "reports canonical repository refs from reactive workspace state",
    () => {
  ...
  it.effect(
    "returns a definitive content-first home view when control is disabled",
    () => {
      const { layer } = makeGateway();
      return Effect.gen(function* () {
  ...
  it.effect(
    "keeps no-argument discovery useful before a descriptor exists",
    () => {
      const { layer } = makeGateway({
        status: Effect.fail(
  ...
  it.effect("routes native setup through typed Effect capabilities", () => {
    ...
    return Effect.gen(function* () {
      ...
      const uninstallStatus = yield* runFlect([
        "--json",
        "setup",
        "uninstall",
        "inspect",
      ]);
      const uninstallPrepared = yield* runFlect([
        "--json",
        "setup",
        "uninstall",
        "prepare",
  ...
  it.effect(
    "renders usage failures on stdout without touching the gateway",
    () => {
  ...
  it.effect(
    "discovers protected Shaper share editing commands in local help",
    () => {
      const { layer } = makeGateway();
      return Effect.gen(function* () {
  ...
  it.effect(
    "projects actions with availability and a definitive zero state",
    () => {
      ...
      return Effect.gen(function* () {
        const zero = yield* runFlect(["action", "list"]).pipe(
          Effect.provide(emptyLayer),
        );
        const actions = yield* runFlect(["action", "list"]).pipe(
        ...
        assert.include(
          actions.stdout,
          "actions[1]{nodeId,label,action,available}",
        );
        assert.include(
          actions.stdout,
  ...
  it.effect(
    "returns the invoked action and post-command workspace state",
    () => {
      let inspections = 0;
      const command = vi.fn<FlectCommandGatewayShape["command"]>(() =>
        Effect.succeed(
  ...
  it.effect(
    "returns a product operation result without exposing an HTTP primitive",
    () => {
      const command = vi.fn<FlectCommandGatewayShape["command"]>(() =>
        Effect.succeed(
          FlectCommandReceipt.make({
  ...
  it.effect(
    "dispatches semantic workbench target selection and reports the result",
    () => {
      const command = vi.fn<FlectCommandGatewayShape["command"]>(() =>
        Effect.succeed(
          FlectCommandReceipt.make({
  ...
  it.effect(
    "dispatches cancellation through the shared command gateway",
    () => {
      const command = vi.fn<FlectCommandGatewayShape["command"]>(() =>
        Effect.succeed(
          FlectCommandReceipt.make({
  ...
  it.effect(
    "rejects missing actions and Shaper invocation before dispatch",
    () => {
      ...
      return Effect.gen(function* () {
        ...
        const denied = yield* runFlect([
          "action",
          "invoke",
          "shape-interface",
        ]).pipe(Effect.provide(shaper.layer));
