...
import { AgentIntegration, type AgentIntegrationError } from '../lib/agent-integration';
...
import { type AxiFormat, type AxiFormatError, AxiPublicError, AxiRunResult } from './contracts';
import {
	FlectCommandGateway,
...
const publicError = (code: string, message: string, help: ReadonlyArray<string> = []) =>
...
const interfaceAuthoringContract = {
	...
	nodes: {
		...
		button: {
			...
			action: ['shape', 'safe-mode', 'accept-revision', 'reject-revision', 'rollback-revision']
		},
...
const errorProjection = (error: AxiExecutionError) => {
	switch (error._tag) {
		...
		case 'CommandConflict':
			return publicError('conflict', error.message, ['Inspect the current workspace and retry']);
		...
		case 'AgentIntegrationError':
			return publicError(error.reason === 'io' ? 'unavailable' : error.reason, error.message);
		case 'ShellLinkError':
			return publicError(error.reason === 'io' ? 'unavailable' : error.reason, error.message);
		case 'UninstallError':
			return publicError(error.reason, error.message);
...
const helpFor = (bin: string, path: ReadonlyArray<string>) => ({
	...
	usage:
		path[0] === 'model'
			...
			: path[0] === 'action'
				? ['flect action list', 'flect action inspect <node-id>', 'flect action invoke <node-id>']
				: path[0] === 'product'
					? ['flect product invoke <operation-id> [--input <json>]']
					: path[0] === 'permissions'
						? ['flect permissions list', 'flect permissions revoke <decision-id>']
						: path[0] === 'extensions'
							? [
...
const execute = Effect.fn('Axi.execute')(function* (parsed: ParsedAxiArguments) {
	...
	switch (command.kind) {
		...
		case 'inspect': {
			...
			return Object.fromEntries(command.fields.map((field) => [field, all[field]]));
		}
		case 'logs': {
			const logs = yield* gateway.logs;
			const operations = logs.operations
				.filter((operation) => command.role === undefined || operation.role === command.role)
				.filter(
					(operation) =>
						command.operationId === undefined || operation.operationId === command.operationId
				)
				.slice(-command.limit);
			return {
		...
		case 'action-list': {
			const snapshot = yield* gateway.inspect;
			const actions = projectInterfaceActions(snapshot.document, snapshot.shaping);
		...
		case 'action-inspect': {
			...
			return action === undefined
				? yield* gatewayFailure('not-found', `Interface action ${command.nodeId} was not found.`)
				: action;
		}
		case 'action-invoke': {
		...
		case 'product-invoke': {
			if (gateway.audience === 'shaper') {
				return yield* gatewayFailure('unauthorized', 'Shaper cannot invoke product operations.');
			}
			const receipt = yield* gateway.command(
				...
				gateway.audience === 'native'
					...
					: entries.filter(
							(entry) =>
								...
								entry.binding ===
									(gateway.binding ?? (gateway.audience === 'app' ? 'accepted' : 'candidate'))
						);
			return {
		...
		case 'revision-list': {
			...
			const revisions = [shaping.active, shaping.lastKnownGood, shaping.proposal].filter(
		...
		case 'repository-status': {
			...
			return repository === undefined
				? yield* gatewayFailure('unavailable', 'The canonical Git repository is still opening.')
				: repository;
		}
		case 'share-list': {
			...
			const shares = entries.map((entry) => ({
				...
				...(entry.pending === undefined ? {} : { pending: { lineage: entry.pending.lineage } })
			}));
		...
		case 'share-inspect': {
			...
			const review =
				snapshot.shareReview?.shareId === command.shareId ? snapshot.shareReview : undefined;
		...
		case 'prompt': {
			if (command.fromStdin) {
				return yield* unsupported('Standard input requires the native CLI adapter.');
			}
			const value =
				command.role === 'app'
		...
		case 'revision-rollback': {
			if (command.revisionId !== undefined) {
				return yield* unsupported('Targeted revision rollback is not available yet.');
			}
			const receipt = yield* gateway.command(RollbackRevision.make({ type: 'rollback-revision' }));
			const snapshot = yield* inspectAfterReceipt(gateway, receipt);
			return { receipt, workspace: compactWorkspace(snapshot) };
		}
		case 'command': {
		...
		case 'interface-propose': {
			const maybeInterface = yield* Effect.serviceOption(FlectInterfaceCommandGateway);
			if (Option.isNone(maybeInterface)) {
				return yield* unsupported('Interface files require the role-scoped sandbox adapter.');
			}
		...
		case 'app-propose': {
			if (gateway.audience !== 'shaper') {
				return yield* gatewayFailure('unauthorized', 'Only Shaper can author app source.');
			}
			const maybeInterface = yield* Effect.serviceOption(FlectInterfaceCommandGateway);
			if (Option.isNone(maybeInterface)) {
				return yield* unsupported('App source requires the role-scoped sandbox adapter.');
			}
			if (command.kind === 'app-propose') {
				return yield* maybeInterface.value.proposeApp(command.path, command.name);
			}
			const summary = yield* maybeInterface.value.validateApp(command.path, command.name);
		...
		case 'context': {
			...
			const base = {
				host: command.host,
				discovery: 'Flect is an adaptive interface shell with a live agent-first command surface.',
				suggestions: [
			...
			if (Result.isFailure(status) || !status.success.enabled || !status.success.connected) {
			...
			return {
				...
				roles: {
					...
					shaper: 'Changes interface revisions in Edit mode; acceptance remains a user decision.'
				},
		...
		case 'setup-status': {
			...
			if (Option.isNone(shell) || Option.isNone(agents)) {
				return yield* unsupported('Setup commands require the native Flect adapter.');
			}
			return {
		...
		case 'setup-shell': {
			...
			if (Option.isNone(shell)) {
				return yield* unsupported('Shell setup requires the native Flect adapter.');
			}
			return yield* command.action === 'install' ? shell.value.install : shell.value.remove;
		}
		case 'setup-agent': {
			const agents = yield* Effect.serviceOption(AgentIntegration);
			if (Option.isNone(agents)) {
				return yield* unsupported('Agent setup requires the native Flect adapter.');
			}
		...
		case 'setup-uninstall': {
			...
			if (Option.isNone(uninstall)) {
				return yield* unsupported('Uninstall preparation requires the native Flect adapter.');
			}
		...
		case 'mcp':
			return yield* unsupported('MCP mode is selected by the native Flect process adapter.');
	}
});
...
export const runFlect = Effect.fn('Axi.run')(function* (argv: ReadonlyArray<string>) {
	...
	if (Result.isFailure(parsed)) {
		return yield* failure(
			publicError(parsed.failure.code, parsed.failure.message, parsed.failure.help),
			format,
			2
		);
	}
	return yield* execute(parsed.success).pipe(
		...
		Effect.catch((error) => failure(errorProjection(error), parsed.success.format, 1))
	);
});
