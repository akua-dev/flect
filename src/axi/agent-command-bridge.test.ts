...
import { AgentCommandBridge, AgentCommandBridgeLive } from './agent-command-bridge';
...
const makeLayer = () => {
	const dispatch = vi.fn<FlectWorkspaceControllerShape['dispatch']>((envelope) =>
		envelope.command.type === 'submit-shaper-instruction'
			? Effect.fail(ControlUnauthorized.make({ message: 'App Agent cannot shape.' }))
			: Effect.succeed(
	...
	const agent = Layer.succeed(AgentWorkspace)({
		...
		proposeShaperInterface: (_source, document) => Effect.succeed({ status: 'proposed', document }),
		...
		diagnoseRecovery: () => Effect.succeed({ version: 1, message: 'Recovery available.' }),
		close: Effect.void
...
describe('AgentCommandBridge', () => {
	...
	it.effect('dispatches with the captured agent source and exact response', () => {
		const { dispatch, layer } = makeLayer();
		return Effect.gen(function* () {
			yield* AgentCommandBridge;
			const bus = yield* AgentCommandBus;
			...
			assert.strictEqual(
				envelope?.source.kind === 'agent' ? envelope.source.parentOperationId : '',
				'operation-app-parent'
			);
		}).pipe(Effect.provide(layer));
	});

	it.effect("returns the controller's authorization failure to the caller", () => {
