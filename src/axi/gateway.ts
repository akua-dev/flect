...
import type { ControlBrokerStatus, ControlLogsResponse } from '../../shared/control-channel';
...
export interface FlectCommandGatewayShape {
	...
	readonly events: (after: number) => Stream.Stream<FlectWorkspaceEvent, FlectGatewayError>;
	readonly command: (
...
export interface FlectInterfaceCommandGatewayShape {
	readonly validate: (path: string) => Effect.Effect<InterfaceDocument, FlectGatewayError>;
	readonly propose: (path: string) => Effect.Effect<unknown, FlectGatewayError | FlectCommandError>;
