...
import { type AxiFormat, AxiFormatError, type AxiPublicError, AxiRunResult } from './contracts';

export const AXI_DEFAULT_TEXT_LIMIT = 1_000;
export const AXI_DEFAULT_OUTPUT_LIMIT = 256 * 1_024;

const truncationSuffix = (length: number) => `… (truncated, ${length} chars total — use --full)`;
...
const projectBounded = (value: unknown, full: boolean, depth = 0): unknown => {
	...
	return Object.fromEntries(
		Object.entries(value).map(([key, item]) => [key, projectBounded(item, false, depth + 1)])
	);
};
...
const encodeOutput = Effect.fn('Axi.encodeOutput')(function* (
	...
	if (encoded === undefined) {
		return yield* Effect.fail(formatError('Flect output could not be encoded safely.'));
	}
	const stdout = `${encoded}\n`;
	if (new TextEncoder().encode(stdout).byteLength > maxBytes) {
		return yield* Effect.fail(formatError('Flect output exceeded its safe size limit.'));
	}
	return stdout;
});
