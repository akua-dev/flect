...
const parse = (argv: ReadonlyArray<string>) => parseAxiArguments(argv, 'native');

describe('AXI command parser', () => {
	it.effect('parses noun-first state commands with explicit desired state', () =>
		Effect.gen(function* () {
			const mode = (yield* parse(['mode', 'set', 'run'])).command;
			const target = (yield* parse(['target', 'shape'])).command;
			const action = (yield* parse(['action', 'invoke', 'run-report'])).command;
			const extensions = (yield* parse(['extensions', 'enable', 'shaper'])).command;
			const portableList = (yield* parse(['extensions', 'list'])).command;
			const portableDescribe = (yield* parse(['extensions', 'describe', 'weather-card'])).command;
			...
			const favorite = (yield* parse(['model', 'favorite', 'remove', 'openai/gpt-5.6'])).command;
			...
			const revoke = (yield* parse(['permissions', 'revoke', 'decision-capability-0001'])).command;

			assert.strictEqual(mode.kind, 'command');
			assert.strictEqual(mode.kind === 'command' ? mode.command.type : '', 'set-mode');
			assert.strictEqual(target.kind, 'command');
			assert.deepInclude(target.kind === 'command' ? target.command : undefined, {
				type: 'select-workbench-target',
				target: 'shape'
			});
			assert.strictEqual(action.kind, 'action-invoke');
			assert.strictEqual(action.kind === 'action-invoke' ? action.nodeId : '', 'run-report');
			assert.strictEqual(extensions.kind, 'command');
			if (extensions.kind !== 'command' || extensions.command.type !== 'set-external-extensions') {
			...
			if (cancel.kind !== 'command' || cancel.command.type !== 'cancel-role') {
			...
			if (favorite.kind !== 'command' || favorite.command.type !== 'set-model-favorite') {
	...
	it.effect('parses bounded share discovery and source commands', () =>
		Effect.gen(function* () {
			...
			assert.deepInclude((yield* parse(['share', 'inspect', 'dev.flect.weather'])).command, {
				kind: 'share-inspect',
			...
			const exported = (yield* parse(['share', 'export', 'dev.flect.weather'])).command;
			assert.strictEqual(exported.kind, 'command');
		})
	);

	it.effect('rejects unsafe share URLs, commits, identifiers, and extra flags', () =>
		Effect.gen(function* () {
			const cases = [
	...
	it.effect('fails closed on unknown flags before any gateway can run', () =>
		Effect.gen(function* () {
			const error = yield* parse(['action', 'list', '--stat']).pipe(Effect.flip);
			assert.strictEqual(error.code, 'unknown-flag');
	...
	it.effect('offers permission inspection and revocation but no grant command', () =>
		Effect.gen(function* () {
			const grant = yield* parse(['permissions', 'grant', 'product.projects.read']).pipe(
				Effect.flip
			);
			const invalidDecision = yield* parse(['permissions', 'revoke', 'not-a-decision']).pipe(
				Effect.flip
			);
	...
	it.effect('offers deferred portable extension discovery and calls but no grant command', () =>
		Effect.gen(function* () {
			const grant = yield* parse(['extensions', 'grant', 'weather-card']).pipe(Effect.flip);
			...
			const traversal = yield* parse(['extensions', 'describe', '../outside']).pipe(Effect.flip);
	...
	it.effect('parses the read and integration catalog without raw escape hatches', () =>
		Effect.gen(function* () {
			...
			yield* Effect.forEach(
				...
				([argv, kind]) =>
					parse(argv).pipe(
						Effect.tap((parsed) => Effect.sync(() => assert.strictEqual(parsed.command.kind, kind)))
					),
				{ discard: true }
			);
			const raw = yield* parse(['raw', '{}'] as const).pipe(Effect.flip);
			assert.strictEqual(raw.code, 'unknown-command');
			const host = yield* parse(['context', '--host', 'cursor']).pipe(Effect.flip);
