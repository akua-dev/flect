...
export const FLECT_COMMAND_METADATA: ReadonlyArray<FlectCommandMetadata> = [
  ...
  },
  {
    ...
    summary:
      "Inspect bounded inactive shared candidates and retained installations.",
    audiences: everyAudience,
  },
  {
    usage: "flect share open-url|open-git|reject|export",
    summary:
      "Route bounded shared-source actions through protected user review.",
    audiences: nativeOnly,
  },
  {
    usage:
    ...
    summary:
      "Checkpoint bounded Shaper sandbox files onto an exact retained share fork.",
    audiences: ["shaper"],
  },
  {
    usage:
    ...
    summary:
      "Submit an exact bounded Shaper resolution for every reviewed shared conflict path.",
    audiences: ["shaper"],
  },
  {
    usage: "flect model list|select|favorite",
    summary:
      "Inspect and select Pi-backed models without exposing credentials.",
    audiences: ["native", "app", "shaper"],
  },
  {
    usage: "flect extensions list|describe|call",
    summary:
      "Discover and call enabled portable extensions for the current role and binding.",
    audiences: everyAudience,
  },
  {
    usage: "flect trusted-extensions enable|disable <app|shaper>",
    summary:
      "Set opt-in loading for the selected Pi role's outside extensions.",
    audiences: ["native", "app"],
  },
  {
    usage: "flect safe enter|restore",
    summary:
      "Enter compiled recovery or restore the last-known-good interface.",
    audiences: ["native", "app"],
  },
  {
  ...
  },
  {
    ...
    summary:
      "Inspect or remove only Flect-owned integrations before moving the app to Trash.",
    audiences: nativeOnly,
  },
  {
    usage: "flect mcp",
...
export class AxiUsageError extends Schema.TaggedErrorClass<AxiUsageError>()(
  "AxiUsageError",
  {
...
const unknownFlag = (flag: string) =>
  failed("unknown-flag", `Unknown flag: ${flag}`, [
...
const parseLogs = Effect.fn("Flect.Axi.parseLogs")(function* (
    ...
    switch (flag) {
      ...
      case "--role":
        if (value !== "app" && value !== "shaper") {
          return yield* failed(
            "invalid-argument",
            "Log role must be app or shaper.",
          );
        }
        role = value;
        index += 1;
      ...
      case "--operation":
        if (value === undefined || value.startsWith("-")) {
          return yield* failed(
            "missing-argument",
            "--operation requires an identifier.",
          );
        }
        operationId = value;
        index += 1;
  ...
  switch (noun) {
    ...
    case "inspect": {
      ...
      if (value === undefined) {
        return yield* failed(
          "missing-argument",
          "--fields requires a comma-separated field list.",
        );
      }
      ...
      if (invented !== undefined) {
        return yield* failed(
          "invalid-argument",
          `Unknown inspect field: ${invented}`,
        );
      }
      return { kind: "inspect", fields };
    }
    ...
    case "action":
      ...
      if (verb === "inspect" || verb === "invoke") {
        if (value === undefined) {
          return yield* failed(
            "missing-argument",
            `action ${verb} requires a node identifier.`,
          );
        }
        yield* requireNoExtra(rest);
        return verb === "inspect"
    ...
    case "product": {
      ...
      const operationId = yield* Schema.decodeUnknownEffect(ProductOperationId)(
        value,
      ).pipe(
        Effect.mapError(() =>
          usage("invalid-argument", "Product operation ID is invalid."),
        ),
      );
      if (rest.length === 0) {
      ...
      const input = yield* Effect.try({
        try: () => JSON.parse(rest[1] ?? ""),
          ...
          usage("invalid-argument", "Product input must be valid JSON."),
      }).pipe(
    ...
    case "permissions": {
      ...
      yield* requireNoExtra(rest);
      const decisionId = yield* Schema.decodeUnknownEffect(
        ProductCapabilityDecisionId,
      )(value).pipe(
        Effect.mapError(() =>
          usage("invalid-argument", "Permission decision ID is invalid."),
        ),
      );
    ...
    case "interface":
      ...
      if (verb === "validate" || verb === "propose") {
        if (value === undefined) {
          return yield* failed(
            "missing-argument",
            `interface ${verb} requires a sandbox path.`,
          );
        }
        yield* requireNoExtra(rest);
        return {
          kind:
            verb === "validate" ? "interface-validate" : "interface-propose",
          path: value,
        };
      }
      return yield* failed(
        "unknown-command",
        `Unknown interface command: ${verb ?? ""}`,
      );
    case "proposal":
      yield* requireNoExtra(args.slice(2));
    ...
    case "repository":
      ...
      return verb === "status"
        ...
        : yield* failed(
            "unknown-command",
            `Unknown repository command: ${verb ?? ""}`,
          );
    case "share": {
      if (verb === "list") {
        yield* requireNoExtra(args.slice(2));
      ...
      if (verb === "open-url") {
        if (value === undefined) {
          return yield* failed(
            "missing-argument",
            "share open-url requires an HTTPS URL.",
          );
        }
        yield* requireNoExtra(rest);
        const source = yield* Schema.decodeUnknownEffect(ShareUrlSource)({
          ...
          Effect.mapError(() =>
            usage("invalid-argument", "Share URL is invalid."),
          ),
        );
        return {
          kind: "command",
      ...
      if (verb === "open-git") {
        ...
        const source = yield* Schema.decodeUnknownEffect(ShareGitSource)({
          ...
          Effect.mapError(() =>
            usage("invalid-argument", "Share Git source is invalid."),
          ),
        );
        return {
          kind: "command",
      ...
      if (verb === "export") {
        if (value === undefined) {
          return yield* failed(
            "missing-argument",
            "share export requires a share ID.",
          );
        }
        yield* requireNoExtra(rest);
        return {
    ...
    case "model":
      ...
      if (verb === "select") {
        ...
        if (model === undefined) {
          return yield* failed(
            "invalid-argument",
            "Model must be provider/id or auto.",
          );
        }
        yield* requireNoExtra(rest);
        return {
      ...
      if (verb === "favorite") {
        ...
        const model =
          modelValue === undefined ? undefined : modelSelection(modelValue);
      ...
      if (verb === "describe" || verb === "call") {
        ...
        const input = yield* Effect.try({
          try: (): unknown => JSON.parse(rest[1] ?? ""),
            ...
            usage(
              "invalid-argument",
              "Portable extension input must be valid JSON.",
            ),
        }).pipe(
          Effect.flatMap(Schema.decodeUnknownEffect(Schema.Json)),
          Effect.mapError(() =>
    ...
    case "setup":
      ...
      if (verb === "agent" && (value === "install" || value === "remove")) {
        ...
        if (agent !== "codex" && agent !== "claude" && agent !== "opencode") {
          return yield* failed(
            "invalid-argument",
            "Agent must be codex, claude, or opencode.",
          );
        }
        yield* requireNoExtra(rest.slice(1));
...
export const parseAxiArguments = Effect.fn("Flect.Axi.parseArguments")(
  function* (argv: ReadonlyArray<string>, audience: AxiAudience) {
    let index = 0;
