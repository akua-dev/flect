...
describe("CapsuleStore", () => {
  ...
  it("uninstalls only currently bound capsule objects and preserves unrelated data", async () => {
    ...
    await vfs.writeFile(
      "/flect-capsules/default/owner-note.txt",
    ...
    expect(
      await vfs.exists(
        `/flect-capsules/default/objects/${acceptedDigest}.flect`,
      ),
    ).toBe(false);
    expect(
      await vfs.exists(
        `/flect-capsules/default/objects/${candidateDigest}.flect`,
      ),
    ).toBe(false);
    expect(
      await vfs.exists(`/flect-capsules/default/objects/${orphanDigest}.flect`),
    ).toBe(true);
    expect(
      await vfs.readFileText("/flect-capsules/default/owner-note.txt"),
    ).toBe("not owned by the binding file");
  ...
  it("fails closed on corrupt bindings instead of guessing what to delete", async () => {
    ...
    expect(await vfs.exists("/flect-capsules/default/bindings.json")).toBe(
      true,
    );
  });
});
