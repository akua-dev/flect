...
const failure = () => CapsuleStoreError.make({ message: 'Capsule storage is unavailable.' });
...
export interface CapsuleStoreShape {
	...
	readonly save: (bindings: CapsuleArchiveBindings) => Effect.Effect<void, CapsuleStoreError>;
	/**
...
export class CapsuleStore extends Context.Service<CapsuleStore, CapsuleStoreShape>()(
	'flect/CapsuleStore'
) {}

const hash = async (archive: Uint8Array) => {
	const digest = await crypto.subtle.digest('SHA-256', Uint8Array.from(archive).buffer);
...
const readBindingDigests = async (vfs: Vfs) => {
	...
	if (
		...
		Object.keys(record).some(
			(key) => key !== 'version' && !bindingKeys.some((binding) => binding === key)
		)
	)
		throw new Error('invalid bindings');
...
export const CapsuleStoreLive = Layer.effect(
	CapsuleStore,
	Effect.promise(() =>
		browserPersistentStorage().then(({ vfs, persistence }) => makeCapsuleStore(vfs, persistence))
	)
);
