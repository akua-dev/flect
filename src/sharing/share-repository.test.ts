...
const makeGit = Effect.fn("test.makeShareGit")(function* () {
  ...
  const service: GitWorkspaceShape = {
    open: () =>
      Effect.succeed(
        GitOpened.make({ type: "opened", variant: "asyncify", existed: true }),
      ),
    ...
    exportRef: ({ branch, expectedCommit }) =>
      Effect.succeed(
        GitExported.make({
          ...
          fileCount:
            branch.endsWith("/fork") && expectedCommit === objectF ? 3 : 1,
        }),
      ),
      ...
      Ref.update(checkpoints, (current) => [...current, options]).pipe(
        Effect.tap(() =>
          Ref.update(refs, (current) =>
            new Map(current).set(options.branch, objectF),
          ),
        ),
        Effect.as(
    ...
    mergeRef: ({
      ...
      Effect.gen(function* () {
        ...
        yield* Ref.update(refs, (current) =>
          new Map(current).set(branch, objectM),
        );
    ...
    moveRef: ({ branch, expectedCommit, targetCommit }) =>
      Effect.gen(function* () {
        ...
        if (actual !== expectedCommit)
          return yield* Effect.die("stale test ref");
    ...
    deleteRef: ({ branch, expectedCommit }) =>
      Effect.gen(function* () {
        const current = yield* Ref.get(refs);
        if (current.get(branch) !== expectedCommit)
          return yield* Effect.die("stale test ref");
        ...
        Effect.andThen(
          Effect.gen(function* () {
            if (args[0] === "log" && args[1] === "--format=%H")
              return command(
                0,
                (yield* Ref.get(ancestry)) === "ancestor" ? "" : `${objectA}\n`,
              );
            if (args[0] === "merge")
              return command((yield* Ref.get(mergeConflict)) ? 1 : 0);
            ...
            if (args[0] === "commit") {
              ...
              if (candidate !== undefined)
                yield* Ref.update(refs, (current) =>
                  new Map(current).set(candidate, objectM),
                );
              return command();
            }
            if (args[0] === "rev-parse") {
...
describe("ShareRepository", () => {
  ...
  it.effect(
    "advances only the guarded user fork through an optimistic checkpoint",
    () =>
      Effect.gen(function* () {
        const fake = yield* makeGit();
        ...
        assert.deepStrictEqual(checkpoints[0]?.removals, [
          "components/old-weather.tsx",
        ]);
        ...
        assert.strictEqual(
          (yield* Ref.get(fake.refs)).get(names.fork),
          objectA,
        );
      }),
  );
  ...
  it.effect(
    "prepares fast-forward, replacement, clean merge, and conflict outcomes",
    () =>
      Effect.gen(function* () {
        const fake = yield* makeGit();
        const names = yield* deriveShareRefs("community.weather");
        ...
        if (fastForward._tag !== "fast-forward")
          return yield* Effect.die("wrong result");
        ...
        if (replacement._tag !== "replacement")
          return yield* Effect.die("wrong result");
        ...
        assert.strictEqual(
          (yield* Ref.get(fake.refs)).has(names.candidate),
          false,
        );
      }),
  );
  ...
  it.effect(
    "resolves only the recorded conflict paths into a guarded two-parent candidate",
    () =>
      Effect.gen(function* () {
        const fake = yield* makeGit();
        const names = yield* deriveShareRefs("community.weather");
  ...
  it.effect(
    "accepts a guarded candidate into upstream base and the user fork",
    () =>
      Effect.gen(function* () {
        const fake = yield* makeGit();
        const names = yield* deriveShareRefs("community.weather");
        ...
        assert.strictEqual(
          (yield* Ref.get(fake.refs)).get(names.base),
          objectB,
        );
        assert.strictEqual(
          (yield* Ref.get(fake.refs)).get(names.fork),
  ...
  it.effect(
    "removes installation refs before deleting only uninstalled fork data",
    () =>
      Effect.gen(function* () {
        const fake = yield* makeGit();
        const names = yield* deriveShareRefs("community.weather");
