...
describe('share signature verifier', () => {
	it.effect('keeps unsigned and unconfigured signatures non-authoritative', () =>
		Effect.gen(function* () {
			const verifier = yield* ShareSignatureVerifier;
			assert.strictEqual((yield* verifier.verify(manifest([]), 'c'.repeat(64))).status, 'unsigned');
	...
	it.effect('fails closed on malformed, duplicate, rejected, and defective claims', () =>
		Effect.gen(function* () {
			const verifier = yield* ShareSignatureVerifier;
			...
			Effect.provide(
				makeShareSignatureVerifierLayer({
					verify: () => Effect.failCause(Cause.die(new Error('private-key-material')))
				})
			)
		)
	);
