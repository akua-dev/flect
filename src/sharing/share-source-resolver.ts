...
export class ShareOpenStarted extends Schema.Class<ShareOpenStarted>(
  "ShareOpenStarted",
)({
...
export class ShareOpenProgress extends Schema.Class<ShareOpenProgress>(
  "ShareOpenProgress",
)({
  type: Schema.Literal("progress"),
  phase: Schema.Literals(["read", "download", "clone", "private"]),
  bytes: Schema.Int.check(
    Schema.isBetween({ minimum: 0, maximum: 64 * 1024 * 1024 }),
  ),
}) {}
...
export interface ShareSourceResolverShape {
  readonly open: (
    source: ShareSourceValue,
  ) => Stream.Stream<ShareOpenEvent, ShareSourceFailure>;
}
...
export const makeShareSourceResolverLayer = (options?: {
  ...
  Layer.effect(
    ...
    Effect.gen(function* () {
      ...
      const privateSources: PrivateShareSourceRegistryShape =
        yield* PrivateShareSourceRegistry;
      const maxArchiveBytes =
        options?.maxArchiveBytes ?? MAX_SHARE_ARCHIVE_BYTES;

      const download = Effect.fn("Flect.ShareSourceResolver.download")(
        (url: string) =>
          Effect.gen(function* () {
            ...
            const state = yield* response.stream.pipe(
              ...
              Effect.mapError((error) =>
                Schema.is(ShareSourceFailure)(error)
                  ? error
                  : failure("download"),
              ),
            );
