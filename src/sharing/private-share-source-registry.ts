...
export interface PrivateShareSourceRegistryShape {
  ...
  ) => Effect.Effect<Uint8Array, ShareSourceFailure>;
}
...
export const makePrivateShareSourceRegistryLayer = (options: {
  ...
  Layer.effect(
    PrivateShareSourceRegistry,
    Effect.gen(function* () {
      const definitions = yield* Ref.make(
        new Map<string, PrivateShareSourceDefinition>(),
      );

      const register = Effect.fn("Flect.PrivateShareRegistry.register")(
        function* (definition: PrivateShareSourceDefinition) {
...
export const PrivateShareSourceRegistryLive =
  makePrivateShareSourceRegistryLayer({ sources: [] });
