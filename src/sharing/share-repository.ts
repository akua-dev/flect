...
export class ShareRepositoryRefs extends Schema.Class<ShareRepositoryRefs>('ShareRepositoryRefs')({
	base: Schema.String,
...
export class ShareMergedUpdate extends Schema.TaggedClass<ShareMergedUpdate>()('merged', {
	upstream: ObjectId,
	fork: ObjectId,
	candidate: ObjectId,
...
export class ShareConflictUpdate extends Schema.TaggedClass<ShareConflictUpdate>()('conflict', {
	upstream: ObjectId,
	fork: ObjectId,
...
const sameBytes = (left: Uint8Array | undefined, right: Uint8Array | undefined) =>
	left === undefined
		? right === undefined
		: right !== undefined &&
...
const findThreeWayConflicts = (
	...
	return [...new Set([...before.keys(), ...personal.keys(), ...incoming.keys()])]
		.filter((path) => {
...
export class ShareRepository extends Context.Service<ShareRepository, ShareRepositoryShape>()(
	'flect/ShareRepository'
) {}

export const makeShareRepositoryLayer = (options?: { readonly workspaceId?: string }) =>
	Layer.effect(
		ShareRepository,
		Effect.gen(function* () {
			...
			const removeRef = (input: Parameters<GitWorkspaceShape['deleteRef']>[0]) =>
			...
			const restoreCandidate = Effect.fn('ShareRepository.restore')(function* (input: {
				...
				const restored = Effect.gen(function* () {
					...
					}).pipe(Effect.tap(() => Effect.sync(() => (candidateCreated = true))));
					yield* move({
						branch: names.fork,
					...
					}).pipe(Effect.tap(() => Effect.sync(() => (forkRestored = true))));
					yield* move({
						branch: names.base,
					...
					}).pipe(Effect.tap(() => Effect.sync(() => (baseRestored = true))));
				});
				yield* restored.pipe(
			...
			const acceptCandidate = Effect.fn('ShareRepository.accept')(function* (input: {
				...
				const accepted = Effect.gen(function* () {
					...
					}).pipe(Effect.tap(() => Effect.sync(() => (baseMoved = true))));
					yield* move({
						branch: names.fork,
					...
					}).pipe(Effect.tap(() => Effect.sync(() => (forkMoved = true))));
					yield* removeRef({
						branch: names.candidate,
						expectedCommit: input.refs.candidate,
						guards: [
					...
					}).pipe(Effect.tap(() => Effect.sync(() => (candidateRemoved = true))));
					return {
						refs: {
			...
			const prepareUpdate = Effect.fn('ShareRepository.prepareUpdate')(function* (input: {
				...
				const ancestry = yield* run(['log', '--format=%H', `${input.commit}..${input.refs.base}`]);
				...
				const [baseSnapshot, forkSnapshot, upstreamSnapshot] = yield* Effect.all([
					git.snapshotRef({
						branch: names.base,
						expectedCommit: input.refs.base,
				...
				const discard = Effect.gen(function* () {
					const resolved = yield* run(['rev-parse', names.candidate]).pipe(Effect.option);
					if (
						resolved._tag === 'None' ||
					...
					yield* requireCommand(['reset', '--hard', input.refs.fork]).pipe(Effect.ignore);
					yield* removeRef({
						branch: names.candidate,
			...
			const resolveConflict = Effect.fn('ShareRepository.resolveConflict')(function* (input: {
				...
				yield* open;
				const names = yield* deriveShareRefs(input.shareId);
				const expectedConflicts = yield* Effect.forEach(input.conflictPaths, (path) =>
					Schema.decodeUnknownEffect(Path)(path).pipe(
						Effect.mapError(() => failure('invalid-input'))
					)
				);
				const resolutionPaths = [...input.files.map((file) => file.path), ...input.removals];
				if (
				...
				if (
					...
					actualConflicts.some((path, index) => path !== expectedConflicts.toSorted()[index])
				) {
					return yield* Effect.fail(failure('stale-ref'));
				}
				const resolved = new Map(
					mergeThreeWayFiles(baseSnapshot.files, forkSnapshot.files, upstreamSnapshot.files).map(
			...
			const snapshotArtifact = Effect.fn('ShareRepository.snapshot')(function* (input: {
				...
				yield* open;
				const names = yield* deriveShareRefs(input.shareId);
				const root = yield* Schema.decodeUnknownEffect(Path)(input.sourceRoot).pipe(
					Effect.mapError(() => failure('invalid-input'))
				);
				...
				if (files.length === 0) return yield* Effect.fail(failure('invalid-input'));
				return files;
			});
