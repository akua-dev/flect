...
const makeLayer = (
	...
	const quarantine = Layer.succeed(ShareQuarantine)({
		inspect: (bytes) =>
			Ref.update(calls, (items) => [...items, `bytes:${bytes.byteLength}`]).pipe(
				Effect.as(candidate)
			),
		inspectGit: (url, revision) =>
			Ref.update(calls, (items) => [...items, `git:${url}:${revision}`]).pipe(Effect.as(candidate))
	});
	...
	const dependencies = Layer.mergeAll(quarantine, http, PrivateShareSourceRegistryLive);
	return makeShareSourceResolverLayer({ maxArchiveBytes }).pipe(Layer.provideMerge(dependencies));
};
...
describe('share source resolver', () => {
	it.effect('normalizes local, streamed URL, and exact public Git sources', () => {
	...
	it.effect('opens named private adapters through their trusted closure', () => {
		const calls = Ref.makeUnsafe<ReadonlyArray<string>>([]);
		return Effect.gen(function* () {
		...
		}).pipe(Effect.provide(makeLayer(() => Promise.resolve(new Response()), calls)));
	});

	it.effect('fails on declared and streamed oversize without inspecting bytes', () => {
		const calls = Ref.makeUnsafe<ReadonlyArray<string>>([]);
