...
export interface ShareCandidateStoreShape {
  ...
  readonly save: (
    archive: Uint8Array,
  ) => Effect.Effect<string, ShareCandidateStoreFailure>;
  readonly load: (
    digest: string,
  ) => Effect.Effect<Uint8Array | undefined, ShareCandidateStoreFailure>;
  readonly remove: (
    digest: string,
  ) => Effect.Effect<void, ShareCandidateStoreFailure>;
}
...
const decodeDigest = (digest: string) =>
  Schema.decodeUnknownEffect(Digest)(digest).pipe(
    Effect.mapError(() => failure("invalid-key")),
  );
  ...
  save: (archive) =>
    Effect.gen(function* () {
      if (
        ...
        archive.byteLength > MAX_SHARE_ARCHIVE_BYTES
      ) {
