...
const decodeStored = Effect.fn("Flect.ShareInstallationStore.decode")(
  function* (raw: string) {
    ...
    if (
      new Set(entries.map((entry) => entry.shareId)).size !== entries.length
    ) {
...
export interface ShareInstallationStoreShape {
  ...
  readonly get: (
    shareId: string,
  ) => Effect.Effect<ShareInstallationRecord | undefined>;
  readonly save: (
    record: ShareInstallationRecord,
  ) => Effect.Effect<void, ShareInstallationFailure>;
  readonly remove: (
    shareId: string,
  ) => Effect.Effect<void, ShareInstallationFailure>;
}
...
export const makeShareInstallationStoreLayer = () =>
  Layer.effect(
    ...
    Effect.gen(function* () {
      ...
      const remove = Effect.fn("Flect.ShareInstallationStore.remove")(
        (shareId: string) =>
          mutate((current) =>
            Effect.succeed(
              ShareInstallationSnapshot.make({
