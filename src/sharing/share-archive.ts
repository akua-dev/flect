...
import { decodeCapsule, MAX_CAPSULE_BYTES } from '../../packages/product/src/capsule';
import {
	decodePortableTar,
	encodePortableTar,
...
const verifyArtifacts = Effect.fn('ShareArchive.verifyArtifacts')(function* (
	...
	if (
		...
		artifacts.some((artifact, index) => artifact.path !== declared[index]?.path)
	) {
	...
	for (let index = 0; index < artifacts.length; index += 1) {
		...
		yield* decodeCapsule(artifact.contents).pipe(Effect.mapError(() => invalid()));
	}
});
