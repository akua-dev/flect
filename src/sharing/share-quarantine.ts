...
export class ShareQuarantineFailure extends Schema.TaggedErrorClass<ShareQuarantineFailure>()(
	...
	{
		reason: Schema.Literals(['archive', 'repository', 'manifest', 'integrity', 'unavailable']),
...
export class ShareQuarantine extends Context.Service<ShareQuarantine, ShareQuarantineShape>()(
...
const runWorkspace = <A, E>(
	...
	effect.pipe(
		...
		Effect.catchCause((cause) =>
			Cause.hasInterrupts(cause) ? Effect.failCause(cause) : Effect.fail(failure(reason))
		)
	);
...
const verifyArtifacts = Effect.fn('ShareQuarantine.verifyArtifacts')(function* (
	...
	const artifactsByPath = new Map(artifacts.map((artifact) => [artifact.path, artifact.contents]));
	for (const descriptor of manifest.artifacts) {
		const sourceFiles = files.filter((file) => file.path.startsWith(`${descriptor.sourceRoot}/`));
		const sourceDigest = yield* hashShareArtifactSource(descriptor.sourceRoot, sourceFiles).pipe(
		...
		yield* decodeCapsule(contents).pipe(Effect.mapError(() => failure('integrity')));
	}
});
...
export const makeShareQuarantineLayer = (options?: {
	...
	const withWorkspace = <A>(
		use: (workspace: GitWorkspaceShape) => Effect.Effect<A, ShareQuarantineFailure>
	) =>
		Effect.scoped(
			Effect.gen(function* () {
				...
				yield* Effect.acquireRelease(
					runWorkspace(workspace.open({ workspaceId, reset: true }), 'unavailable'),
					() =>
	...
	const inspect = Effect.fn('ShareQuarantine.inspect')((sourceBytes: Uint8Array) =>
		withWorkspace((workspace) =>
			Effect.gen(function* () {
				...
				yield* verifyArtifacts(decoded.manifest, inspected.files, decoded.artifacts);
				return {
