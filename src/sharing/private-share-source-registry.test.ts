...
describe("private share source registry", () => {
          ...
          sources: [
            {
              ...
              name: "Host library",
              open: (reference) =>
                Effect.succeed(new TextEncoder().encode(reference)),
            },
          ],
        }),
      ),
  ...
  it.effect("registers trusted closures without exposing credentials", () =>
    Effect.gen(function* () {
      ...
      yield* registry.register({
        ...
        open: (reference) =>
          Effect.succeed(
            new TextEncoder().encode(`archive:${reference}:private-token`),
          ),
      });
  ...
  it.effect(
    "rejects duplicate, missing, invalid, and defective adapters safely",
    () =>
      Effect.gen(function* () {
        const registry = yield* PrivateShareSourceRegistry;
        yield* registry.register({
          ...
          name: "Company share",
          open: () =>
            Effect.failCause(Cause.die(new Error("private-company-token"))),
        });
