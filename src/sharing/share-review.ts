...
export { ShareReview, ShareReviewArtifact, ShareReviewChange } from '../../shared/share-review';
...
const digest = (contents: Uint8Array) =>
	Effect.tryPromise({
		try: async () => {
			const value = await crypto.subtle.digest('SHA-256', Uint8Array.from(contents));
...
const category = (path: string): ShareReviewChange['category'] => {
	...
	if (lower === 'agents.md' || lower.includes('instruction') || lower.endsWith('prompt.md')) {
		return 'instructions';
	}
...
export const buildShareReview = Effect.fn('ShareReview.build')(function* (
	...
	for (const file of input.files) current.set(file.path, yield* digest(file.contents));
	for (const file of input.previousFiles) previous.set(file.path, yield* digest(file.contents));
	const paths = new Set([...current.keys(), ...previous.keys(), ...input.conflictPaths]);
	...
	const previousMigrations = JSON.stringify(input.previousManifest?.migrations ?? []);
	...
	const compatible =
		(yield* satisfiesVersion(input.flectVersion, input.manifest.compatibility.flect)) &&
	...
	const extensionChanged = input.manifest.artifacts.some(
		(artifact) => artifact.kind === 'extension' && !previousExtensionIds.has(artifact.id)
	);
	...
	if (input.conflictPaths.length > 0 || input.lineage === 'conflict') blockers.push('conflict');
	if (previousMigrations !== currentMigrations) blockers.push('migration-review-required');
	if (extensionChanged) blockers.push('extension-test-required');
	if (changes.some((change) => change.authorityAffecting)) blockers.push('grant-review-required');
