...
describe("share archive", () => {
  it.effect(
    "encodes deterministically and verifies repository and capsule bytes",
    () =>
      Effect.gen(function* () {
  ...
  it.effect(
    "fails closed on repository, capsule, and trailing-byte tampering",
    () =>
      Effect.gen(function* () {
        const capsule = yield* encodeCapsule(capsuleSource());
        const capsuleSha256 = yield* hashCapsuleArchive(capsule);
        ...
        for (const candidate of cases) {
          const result = yield* decodeShareArchive(candidate).pipe(
            Effect.result,
          );
          ...
          if (Result.isFailure(result)) {
            ...
            assert.strictEqual(
              ...
              "The .flect-share archive is invalid.",
            );
          }
        }
      }),
  ...
  it.effect(
    "rejects traversal, duplicate, link, device, and malformed ustar entries",
    () =>
      Effect.gen(function* () {
        const capsule = yield* encodeCapsule(capsuleSource());
        const capsuleSha256 = yield* hashCapsuleArchive(capsule);
