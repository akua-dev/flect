...
const sourceDigest =
  "d32d6b221997ecdf338aaf42ef0e0d2af95c71ac0db0b9bb6d92470a94fcd20c";
...
describe("share quarantine", () => {
  ...
  it.effect("rejects manifest drift without leaking private input", () => {
    ...
    return Effect.gen(function* () {
      ...
      if (Result.isFailure(drift)) {
        ...
        assert.notInclude(
          JSON.stringify(drift.failure),
          "private-manifest-value",
        );
      }
    }).pipe(
      Effect.provide(
  ...
  it.effect(
    "removes quarantine storage and terminates work on interruption",
    () => {
      const removals = Ref.makeUnsafe(0);
      return Effect.gen(function* () {
