...
import { type NativeSetupRuntime, nativeSetupRuntime } from '../lib/native-runtimes';
...
const defaultClient =
	nativeSetupRuntime === undefined ? undefined : clientFromRuntime(nativeSetupRuntime);

export function useNativeSetup(client: NativeSetupClient | undefined = defaultClient) {
	...
	return {
		...
		...(snapshot?.uninstall === undefined ? {} : { uninstall: snapshot.uninstall }),
		...(error === undefined ? {} : { error }),
		refresh,
		installShell: () => (client === undefined ? Promise.resolve() : mutate(client.installShell)),
		removeShell: () => (client === undefined ? Promise.resolve() : mutate(client.removeShell)),
		installAgent: (host: AgentIntegrationHost) =>
			client === undefined ? Promise.resolve() : mutate(() => client.installAgent(host)),
		removeAgent: (host: AgentIntegrationHost) =>
			client === undefined ? Promise.resolve() : mutate(() => client.removeAgent(host)),
		prepareUninstall: async () => {
			...
			try {
				...
				setSnapshot((current) => (current === undefined ? current : { ...current, uninstall }));
			} catch {
