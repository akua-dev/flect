...
export type WorkspaceRuntime = ManagedRuntime.ManagedRuntime<FlectWorkspaceController, unknown>;
...
export function useWorkspace(runtime: WorkspaceRuntime = flectRuntime): WorkspaceController {
	...
	const [capsulePresentation, setCapsulePresentation] = useState<CapsulePresentationState>({});
					...
					...(controller.capsulePresentationChanges === undefined
						...
						: [
								controller.capsulePresentationChanges.pipe(
									Stream.buffer({ capacity: 1, strategy: 'sliding' }),
									Stream.runForEach((next) => Effect.sync(() => setCapsulePresentation(next)))
								)
							])
				],
	...
	const runProviderAction = useCallback(
		<A, E>(use: (controller: FlectWorkspaceControllerShape) => Effect.Effect<A, E>) =>
			runtime.runPromise(
				Effect.gen(function* () {
					const controller = yield* FlectWorkspaceController;
	...
	const cancelProviderAuth = useCallback(
		(reference: AuthLoginReference) =>
			runProviderAction((controller) => controller.cancelProviderAuth(reference)),
		[runProviderAction]
	);
	...
	const readShareExport = useCallback(
		(shareId: string) =>
			runtime.runPromise(
				Effect.gen(function* () {
					return yield* (yield* FlectWorkspaceController).readShareExport(shareId);
				})
			),
		[runtime]
	);
	...
	const importCapsule = useCallback(
		(archive: Uint8Array) => dispatch(ImportCapsule.make({ type: 'import-capsule', archive })),
		[dispatch]
	);
	const invokeCapsuleIntent = useCallback(
		(capsuleId: string, binding: 'accepted' | 'candidate', intent: CapsuleIntent) => {
			const current = snapshotRef.current;
			if (current === undefined) {
