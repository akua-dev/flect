...
import type { ContinuityDrafts, ContinuityRecoveryReason } from '../../shared/role-continuity';
import {
	AgentWorkspace,
...
export type AgentWorkspaceRuntime = ManagedRuntime.ManagedRuntime<AgentWorkspace, unknown>;
...
export interface PreviewAppConversationController extends RoleConversationState {
	readonly role: 'app';
	readonly submit: (
		text: string,
...
export interface ShaperConversationController extends RoleConversationState {
	...
	readonly shape: (instruction: string, document: InterfaceDocument) => Promise<ShaperTurnOutcome>;
}

export interface AgentWorkspaceController {
	readonly drafts?: ContinuityDrafts;
	readonly setDraft?: (key: keyof ContinuityDrafts, value: string) => Promise<void>;
	readonly continuity?: {
	...
	readonly selectReasoning: (reasoningLevel: ReasoningLevel | undefined) => void;
	...
	readonly toggleExternalExtensions: (role: InteractiveAgentRole) => Promise<void>;
...
const conversationMessages = (
	...
	[
		...
		...conversation.activities.map((activity) => activityMessage(conversation.role, activity))
	].sort((left, right) =>
...
export function useAgentSession(
			...
			yield* Effect.all(
				[
					...
					workspace.providerAuthChanges.pipe(
						Stream.buffer({ capacity: 1, strategy: 'sliding' }),
						Stream.runForEach((next) => Effect.sync(() => setProviderAuth(next)))
					)
				],
	...
	const workspaceEffect = useCallback(
		() =>
			Effect.suspend(() => {
				...
				return workspace === undefined ? Effect.fail(unavailable()) : Effect.succeed(workspace);
			}),
		[]
	);
	...
	const refresh = useCallback(
		() =>
			runtime.runPromise(workspaceEffect().pipe(Effect.flatMap((workspace) => workspace.refresh))),
	...
	const toggleExternalExtensions = useCallback(
		(role: InteractiveAgentRole) =>
			runtime.runPromise(
				workspaceEffect().pipe(
					Effect.flatMap((workspace) =>
						workspace.setExternalExtensions(role, !(snapshot?.externalExtensions[role] ?? false))
					)
				)
			),
	...
	const selectReasoning = useCallback(
		(reasoningLevel: ReasoningLevel | undefined) => {
			runtime.runFork(
				workspaceEffect().pipe(
					Effect.flatMap((workspace) => workspace.selectReasoning(reasoningLevel))
				)
			);
		},
		[runtime, workspaceEffect]
	...
	const loginProvider = useCallback(
		(request: AuthLoginRequest) => {
			void runtime
				.runPromise(
					workspaceEffect().pipe(Effect.flatMap((workspace) => workspace.loginProvider(request)))
				)
				.catch(() => undefined);
		},
		[runtime, workspaceEffect]
	...
	const replyProviderAuth = useCallback(
		(reply: AuthSelectionReply) =>
			runtime.runPromise(
				workspaceEffect().pipe(Effect.flatMap((workspace) => workspace.replyProviderAuth(reply)))
			),
		[runtime, workspaceEffect]
	);
	const cancelProviderAuth = useCallback(
		(reference: AuthLoginReference) =>
			runtime.runPromise(
				workspaceEffect().pipe(
					Effect.flatMap((workspace) => workspace.cancelProviderAuth(reference))
				)
			),
		[runtime, workspaceEffect]
	);
	const refreshProviderAuth = useCallback(
		() =>
			runtime.runPromise(
				workspaceEffect().pipe(Effect.flatMap((workspace) => workspace.refreshProviderAuth))
			),
		[runtime, workspaceEffect]
	);
	const logoutProvider = useCallback(
		(providerId: string) =>
			runtime.runPromise(
				workspaceEffect().pipe(Effect.flatMap((workspace) => workspace.logoutProvider(providerId)))
			),
		[runtime, workspaceEffect]
	);
	...
	const submit = useCallback(
		(text: string) =>
			runtime.runPromise(
				workspaceEffect().pipe(
					Effect.flatMap((workspace) => workspace.submitAppPrompt(operation(), text)),
					Effect.exit,
					Effect.asVoid
				)
			),
	...
	const submitPreview = useCallback(
		(text: string, document: InterfaceDocument, revisionId: RevisionId) =>
			runtime.runPromise(
				workspaceEffect().pipe(
					Effect.flatMap((workspace) =>
						workspace.submitPreviewPrompt(operation(), text, document, revisionId)
					),
					Effect.exit,
					Effect.asVoid
				)
	...
	const shape = useCallback(
		(instruction: string, document: InterfaceDocument) =>
			runtime.runPromise(
				workspaceEffect().pipe(
					Effect.flatMap((workspace) =>
						workspace.submitShaperInstruction(operation(), instruction, document)
					)
				)
			),
		[runtime, workspaceEffect]
	...
	const diagnoseRecovery = useCallback(
		(reason: RecoveryReason) =>
			runtime.runPromise(
				workspaceEffect().pipe(Effect.flatMap((workspace) => workspace.diagnoseRecovery(reason)))
			),
		[runtime, workspaceEffect]
	);
