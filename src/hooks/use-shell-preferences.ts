...
type PreferencesRuntime = ManagedRuntime.ManagedRuntime<
  ShellPreferences,
  unknown
>;
...
export function useShellPreferences(
  ...
  const persist = useCallback(
    async (
      update: (current: ShellPreferencesValue) => ShellPreferencesValue,
    ) => {
      ...
      persist((current) => {
        ...
        return ShellPreferencesValue.make({
          ...
          modelFavorites: exists
            ? current.modelFavorites.filter(
                (candidate) => candidate !== modelKey,
              )
