...
import { FlectClient, type FlectClientShape, FlectUnavailableError } from '../lib/api';
import { Clipboard } from '../lib/clipboard';
import { InterfaceStorage, type InterfaceStorageShape } from '../lib/interface-store';
...
import { type AgentWorkspaceRuntime, useAgentSession } from './use-agent-session';
...
function createFakeRuntime({
	...
	const workspaceState: { promise: Promise<AgentWorkspaceShape> | undefined } = {
		promise: undefined
	};
		...
		execute: vi.fn((role, line, options) =>
			shellExecute(role, line, options).pipe(
				Effect.flatMap((result) => {
					...
					const currentWorkspace = workspaceState.promise;
	...
	workspaceState.promise = runtime.runPromise(AgentWorkspace);
...
describe('useAgentSession', () => {
	it('recreates a session with role-scoped external Pi extensions', async () => {
		...
		await waitFor(() => expect(result.current.externalExtensions.shaper).toBe(true));

		await act(async () => {
			await result.current.shaper.shape('Use the enabled extension', defaultInterfaceDocument);
		});
	...
	it('keeps App and Shaper conversations separate behind one session', async () => {
		...
		await act(async () => {
			...
			await result.current.shaper.shape('Change the interface', defaultInterfaceDocument);
		});
		...
		expect(result.current.app.messages[0]?.turnId).toBe(result.current.app.messages[1]?.turnId);
		expect(new Set(result.current.shaper.messages.map((message) => message.turnId))).toHaveLength(
			1
		);
		const shaperTimes = result.current.shaper.messages.map((message) => message.createdAt ?? 0);
		expect(shaperTimes).toEqual([...shaperTimes].sort((left, right) => left - right));
	...
	it('requires Pi setup when no authenticated models are available', async () => {
		...
		expect(result.current.error).toBe('Sign in to a Pi provider, then try again.');
		await act(async () => {
	...
	it('creates one session and appends streamed text', async () => {
		...
		expect(result.current.messages[0]?.turnId).toBe(result.current.messages[1]?.turnId);
		expect(result.current.messages[2]?.turnId).toBe(result.current.messages[3]?.turnId);
		expect(result.current.messages[0]?.turnId).not.toBe(result.current.messages[2]?.turnId);
	...
	it('services a Shaper shell request through the browser shell', async () => {
		...
		expect(shell.execute).toHaveBeenCalledWith('shaper', 'bun run src/index.ts', {
			agentContext: {
				sessionId: 'session-hook-test',
				parentOperationId: expect.stringMatching(/^operation-/),
				requestId: 'shell-018f8f4f-76d1-7f4d-8f35-71eebc5931d2'
		...
		expect(JSON.stringify(result.current.shaper.messages)).not.toContain('bun run src/index.ts');
		unmount();
		await runtime.dispose();
	});
	...
	it('releases the session after a fatal Shaper failure', async () => {
		...
		await waitFor(() => expect(client.closeSession).toHaveBeenCalledWith('session-hook-test'));
	...
	it('replaces a session after Pi returns a redacted turn error', async () => {
		...
		unmount();
		await waitFor(() => expect(client.closeSession).toHaveBeenCalledWith('session-2'));
		await runtime.dispose();
	});
	...
	it('cancels Shaper without changing the App conversation', async () => {
		...
		act(() => {
			shapeOutcome = result.current.shaper.shape('Keep shaping', defaultInterfaceDocument).then(
				() => undefined,
				(error: unknown) => error
			);
		});
	...
	it('keeps the session active when cancellation is rejected', async () => {
		...
		expect(result.current.error).toBe('The response could not be stopped. Try again.');
		unmount();
		await waitFor(() => expect(client.closeSession).toHaveBeenCalledOnce());
		await runtime.dispose();
	});
	...
	it('replaces the protected session when model selection changes', async () => {
		...
		await waitFor(() => expect(client.closeSession).toHaveBeenCalledWith('session-1'));
		await act(async () => {
	...
	it('invalidates the protected session when the runtime is refreshed', async () => {
		...
		expect(client.closeSession).toHaveBeenCalledWith('session-1');
		expect(client.createSession).toHaveBeenCalledTimes(2);
		expect(client.prompt).toHaveBeenLastCalledWith('session-2', 'After refresh');
		unmount();
		await waitFor(() => expect(client.closeSession).toHaveBeenCalledWith('session-2'));
		await runtime.dispose();
	});
	...
	it('uses the protected Guardian only for typed recovery diagnostics', async () => {
		...
		const diagnostic = await act(() => result.current.diagnoseRecovery('rollback-failed'));

		expect(diagnostic.message).toBe('The protected launcher remains available.');
		expect(client.diagnoseRecovery).toHaveBeenCalledWith('session-hook-test', 'rollback-failed');
		unmount();
		await waitFor(() => expect(client.closeSession).toHaveBeenCalledOnce());
		await runtime.dispose();
	});
	...
	it('releases the session after a fatal Guardian failure', async () => {
		...
		await act(async () => {
			await expect(result.current.diagnoseRecovery('rollback-failed')).rejects.toEqual(
				new FlectUnavailableError({
					message: 'The local Flect runtime is unavailable.'
				})
			);
