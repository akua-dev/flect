...
const makeRuntime = () => {
  ...
  const loginProvider = vi.fn<FlectWorkspaceControllerShape["loginProvider"]>(
    () => Effect.void,
  );
  const layer = Layer.effect(
    ...
    Effect.gen(function* () {
      ...
      const providerAuthState =
        yield* SubscriptionRef.make<ProviderAuthUiState>({ providers: [] });
      ...
      dispatch.mockImplementation((envelope) =>
        SubscriptionRef.update(state, (current) =>
          FlectWorkspaceSnapshot.make({
            ...
            sequence: current.sequence + 1,
            mode:
              envelope.command.type === "set-mode"
  ...
  return {
    ...
    setExternalProviderAuth: (connected: boolean) =>
      externalProviderAuth(connected),
  };
};
...
describe("useWorkspace", () => {
  ...
  it("dispatches user commands and renders outside state changes reactively", async () => {
    const { dispatch, runtime, setExternalMode, setExternalProviderAuth } =
      makeRuntime();
    const { result, unmount } = renderHook(() => useWorkspace(runtime));
    ...
    await waitFor(() =>
      expect(result.current.providerAuth.providers[0]?.status).toBe(
        "connected",
      ),
    );
  ...
  it("adapts a capsule intent into a role-bound command and correlated result", async () => {
    ...
    const outcome = await result.current.invokeCapsuleIntent(
      "dev.akua.projects",
      "accepted",
      {
        version: 1,
