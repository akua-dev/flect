...
export function App({ runtime = flectRuntime, initialPrompt }: AppProps = {}) {
	...
	useEffect(() => {
		if (initialPromptSent.current || initialPrompt === undefined || snapshot === undefined) {
			return;
		}
	...
	const workspace = useMemo<AgentWorkspaceController | undefined>(() => {
		...
		return {
			...
			continuity: {
				...
				...(continuity.recovery === undefined ? {} : { recovery: continuity.recovery })
			},
			exportContinuity,
			exportRepository,
			exportCapsule,
			...
			refreshProviderAuth,
			logoutProvider,
			refresh: () => command(RefreshRuntimeCommand.make({ type: 'refresh-runtime' })),
	...
	const preferences = useMemo(() => {
		...
		return {
			...
			toggleModelFavorite: async (key: string) => {
				const model = snapshot.agent.models.find((candidate) => modelKey(candidate) === key);
				if (model === undefined) {
					return;
				}
				await command(
	...
	if (snapshot === undefined || workspace === undefined || preferences === undefined) {
		return (
	...
	const failedOperation = snapshot.operations.findLast((operation) => operation.phase === 'failed');
	...
	const shaping: ShapingController = {
		...
		...(snapshot.agent.shaper.error === undefined && failedOperation === undefined
			? {}
			: {
					error:
			...
			command(
				RequestShapeHandoff.make({
					...
					handoff: WorkbenchHandoff.make({
						version: 1,
						instruction,
						revisionId: snapshot.shaping.proposal?.id ?? snapshot.shaping.active.id,
						selection,
		...
		fixFailure: (activity) =>
			command(
				RequestShapeHandoff.make({
					...
					handoff: WorkbenchHandoff.make({
						version: 1,
						instruction: 'Fix the interface behavior that caused this failed operation.',
						revisionId: snapshot.shaping.proposal?.id ?? snapshot.shaping.active.id,
						failureOperationId: activity.operationId,
						failureSummary:
							activity.resultSummary ??
							`${activity.toolName} failed${
								activity.exitCode === undefined ? '.' : ` with exit code ${activity.exitCode}.`
							}`
					})
				})
			),
		...
		rollback: () => command(RollbackRevision.make({ type: 'rollback-revision' }))
	};
	...
	return (
		<RoleAwareShell
			...
			controlledTarget={snapshot.workbench?.target ?? (snapshot.mode === 'run' ? 'use' : 'shape')}
			...
			onCapsuleIntent={(intent) => {
				...
				const capsule =
					binding === 'candidate' ? capsulePresentation.candidate : capsulePresentation.accepted;
			...
			onRejectShare={() =>
				dispatch(RejectShareCandidate.make({ type: 'reject-share-candidate' })).then(
					() => undefined
				)
			}
			onOpenShareUrl={(url) =>
			...
			onRemoveShare={(shareId) =>
				dispatch(RemoveShare.make({ type: 'remove-share', shareId })).then(() => undefined)
			}
			...
			workspace={workspace}
			useDisabled={
				snapshot.shaping.proposal === undefined && snapshot.shaping.active.source === 'built-in'
			}
		/>
	);
}
