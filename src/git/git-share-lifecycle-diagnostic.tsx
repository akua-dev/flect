    ...
    .runPromise(
      Effect.gen(function* () {
        ...
        if (merged._tag !== "merged") {
          return yield* Effect.fail(
            new Error(`Expected a clean merge, received ${merged._tag}.`),
          );
        }
        ...
        if (conflictAttempt._tag === "Failure") {
          ...
          const currentCandidate = yield* git.run([
            "rev-parse",
            names.candidate,
          ]);
          return yield* Effect.fail(
        ...
        yield* git
          ...
          .pipe(
            Effect.mapError(
              (error) => new Error(`Before removal: ${error.message}`),
            ),
          );
        ...
        yield* git
          ...
          .pipe(
            Effect.mapError(
              (error) => new Error(`After removal: ${error.message}`),
            ),
          );
        ...
        if (unrelatedSnapshot._tag === "Failure")
          return yield* Effect.fail(
            new Error(
              `Unrelated ref preservation failed: ${unrelatedSnapshot.failure.message}`,
            ),
          );
        ...
        Effect.catch(
          (error: GitWorkspaceFailure | ShareRepositoryFailure | Error) =>
...
export function GitShareLifecycleDiagnostic() {
  ...
  return (
    <main>
      <pre
        data-testid="git-share-lifecycle-result"
