...
const decodeResponse = Schema.decodeUnknownEffect(GitWorkerResponse, strictOptions);
...
const makeWorkerRequest = (
	...
	Effect.callback<GitWorkspaceResult, GitWorkspaceFailure>((resume) => {
		...
		const settle = (effect: Effect.Effect<GitWorkspaceResult, GitWorkspaceFailure>) => {
			if (completed) {
				return;
			}
			completed = true;
		...
		const onError = () => {
			settle(
				Effect.fail(
					failure(operationName(operation), 'worker', 'The embedded Git Worker failed safely.')
				)
			);
		};
		...
		try {
			...
			settle(
				Effect.fail(
					failure(operationName(operation), 'worker', 'The embedded Git request could not be sent.')
				)
			);
		}
		return Effect.sync(() => {
