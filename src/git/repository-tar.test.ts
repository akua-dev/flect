...
const archive = (entries: ReadonlyArray<RepositoryArchiveEntry>) =>
  makeRepositoryTar(entries);
...
describe("repository tar import", () => {
  it.effect(
    "returns only validated files and directories and sanitizes mutable Git state",
    () =>
      Effect.gen(function* () {
  ...
  it.effect(
    "rejects corrupt structure, duplicates, traversal, links, and devices",
    () =>
      Effect.gen(function* () {
        const valid = archive([
          {
        ...
        const traversal = archive([
          { path: "../private", kind: "file", contents: new Uint8Array() },
        ]);
        const link = archive([
          { path: "src/link", kind: "symlink", target: "../private" },
        ]);
  ...
  it.effect(
    "rejects protected refs, invalid object paths, and Git LFS pointers",
    () =>
      Effect.gen(function* () {
        const cases = [
          archive([
