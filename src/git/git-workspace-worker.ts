...
import initializeWasmGit, { type WasmGitModule } from 'wasm-git/lg2_opfs_async.js';
import {
	GitCheckpointed,
	GitCommandResult,
	GitCommitInspected,
...
const decodeRequest = Schema.decodeUnknownEffect(GitWorkerRequest, strictOptions);
...
const operationName = (operation: GitWorkspaceOperation): GitWorkspaceFailure['operation'] =>
	operation.type;
...
const mapUnknownFailure = (operation: GitWorkspaceFailure['operation'], error: unknown) => {
...
const requireRepository = () => {
	if (repositoryDirectory === undefined) {
		throw workspaceFailure('open', 'unavailable', 'Open a Git workspace before using it.');
	}
...
const clearMemfsTree = (module: WasmGitModule, path: string) => {
	try {
		...
		// Best-effort cleanup: a vanished memfs entry is already the goal state.
	}
};
...
const makeModule = async () => {
	...
	try {
		...
		// The directory may already exist from a previous module lifetime.
	}
	try {
...
const persistRepositoryIdentity = async (module: WasmGitModule, directory: string) => {
	...
	const managedEnd = managedStart === -1 ? -1 : source.indexOf(MANAGED_IDENTITY_END, managedStart);
	const unmanaged =
		managedStart === -1
			? source
			: managedEnd === -1
...
const runCommand = async (module: WasmGitModule, args: ReadonlyArray<string>) => {
...
const resolveRef = async (module: WasmGitModule, ref: string) => {
	...
	return result.exitCode === 0 && /^[0-9a-f]{40}$/.test(objectId) ? objectId : undefined;
};
...
const readLooseCommit = async (module: WasmGitModule, commit: string) => {
	...
	const bytes = typeof compressed === 'string' ? new TextEncoder().encode(compressed) : compressed;
	...
	let header: string;
	try {
		header = new TextDecoder('utf-8', { fatal: true }).decode(decoded.slice(0, separator));
	} catch {
		throw workspaceFailure(
			'inspect-commit',
			'corrupt',
...
const writeTwoParentMergeCommit = async (
	...
	if (!/^tree [0-9a-f]{40}$/.test(metadata[0] ?? '')) {
		throw workspaceFailure('merge-ref', 'corrupt', 'The shared merge tree could not be verified.');
	}
	const rewritten =
		[
			metadata[0],
	...
	const digest = new Uint8Array(await crypto.subtle.digest('SHA-1', Uint8Array.from(object)));
	const commit = Array.from(digest, (byte) => byte.toString(16).padStart(2, '0')).join('');
	let compressed: Uint8Array;
	try {
		const stream = new Blob([object]).stream().pipeThrough(new CompressionStream('deflate'));
...
const parseConflictPaths = (stdout: string) =>
	stdout
		...
		.flatMap((line) => {
			...
			if (!new Set(['AA', 'AU', 'DD', 'DU', 'UA', 'UD', 'UU']).has(status)) return [];
...
const detachAtCommit = async (
	...
	if ((await resolveRef(module, 'HEAD')) !== commit) {
		throw workspaceFailure(operation, 'corrupt', 'The detached Git head could not be verified.');
	}
};
	...
	const visit = (absolute: string, relative: string) => {
		...
		if (module.FS.isDir(stat.mode)) {
			...
			for (const name of module.FS.readdir(absolute)
				...
				.toSorted()) {
				visit(`${absolute}/${name}`, relative.length === 0 ? name : `${relative}/${name}`);
			}
			return;
		}
		...
		const contents = typeof value === 'string' ? new TextEncoder().encode(value) : value;
...
const readReachableLooseObject = async (module: WasmGitModule, objectId: string) => {
	let compressed: string | Uint8Array;
	try {
		compressed = await module.opfsReadFile(
	...
	const digest = new Uint8Array(await crypto.subtle.digest('SHA-1', Uint8Array.from(decoded)));
...
const treeLinks = (body: Uint8Array) => {
	...
	for (let offset = 0; offset < body.byteLength;) {
		...
		links.push(
			Array.from(body.slice(nul + 1, nul + 21), (byte) => byte.toString(16).padStart(2, '0')).join(
				''
			)
		);
		offset = nul + 21;
...
const exportReachableRef = async (module: WasmGitModule, commit: string) => {
	...
	const prefixes = [...new Set([...retained.keys()].map((id) => id.slice(0, 2)))].toSorted();
	const entries: Array<RepositoryArchiveEntry> = [
...
const collectQuarantineArchive = (module: WasmGitModule, root: string) => {
	...
	const visit = (absolute: string, relative: string) => {
		...
		totalBytes += contents.byteLength;
		if (totalBytes > MAX_EXPORT_BYTES) {
			throw workspaceFailure('inspect-share', 'oversized', 'The shared repository is too large.');
		}
		entries.push({ path: relative, kind: 'file', contents });
	};
...
const collectSourceFiles = async (module: WasmGitModule, root: string) => {
	...
	const visit = async (absolute: string, relative: string): Promise<void> => {
		...
		if (module.FS.isDir(stat.mode)) {
			for (const name of module.FS.readdir(absolute)
				.filter(
					(entry) => entry !== '.' && entry !== '..' && !(relative.length === 0 && entry === '.git')
				)
				.toSorted()) {
				await visit(`${absolute}/${name}`, relative.length === 0 ? name : `${relative}/${name}`);
			}
			return;
		}
		if (module.FS.isLink(stat.mode)) {
...
const executeOperation = async (operation: GitWorkspaceOperation): Promise<GitWorkspaceResult> => {
	...
	switch (operation.type) {
		...
		case 'write': {
			...
			const directory = requireRepository();
			await module.opfsWriteFile(`${directory}/${relative}`, operation.contents);
		...
		case 'export-ref': {
			...
			if ((await resolveRef(module, operation.branch)) !== operation.expectedCommit) {
				throw workspaceFailure(
					'export-ref',
					'stale-ref',
			...
			const exported = await exportReachableRef(module, operation.expectedCommit);
		...
		case 'import-repository': {
			...
			try {
				entries = await Effect.runPromise(decodeRepositoryTar(operation.archive));
			} catch {
				throw workspaceFailure(
					'import-repository',
					'corrupt',
			...
			for (const entry of entries) {
				if (
					...
					await module.opfsWriteFile(`${directory}/${entry.path}`, entry.contents);
				}
			}
			await persistRepositoryIdentity(module, directory);
			await refreshRepositoryCache(module);
			...
			for (const protectedRef of ['flect/accepted', 'flect/last-known-good', 'flect/authoring']) {
		...
		case 'import-objects': {
			...
			for (const object of objects) {
				...
				await module.opfsWriteFile(`${directory}/${object.path}`, object.contents);
			}
			await refreshRepositoryCache(module);
			await requireRefGuards('import-objects', module, operation.guards);
			if ((await resolveRef(module, operation.commit)) !== operation.commit) {
		...
		case 'merge-ref': {
			...
			const resolvedFiles = operation.files.map((file) => {
				...
				if (
					...
					throw workspaceFailure(
						'merge-ref',
						path === undefined || mergePaths.has(path) ? 'invalid-path' : 'oversized',
			...
			if (
				(await resolveRef(module, operation.branch)) !== operation.expectedCommit ||
				(await resolveRef(module, operation.upstreamBranch)) !== operation.expectedUpstreamCommit
			) {
				throw workspaceFailure(
					'merge-ref',
					'stale-ref',
			...
			if (changed) {
				...
				const removals = forkFiles.map((file) => file.path).filter((path) => !retained.has(path));
				...
				await requireCommandSuccess('merge-ref', module, ['commit', '-m', operation.message]);
			...
			await requireCommandSuccess('merge-ref', module, ['reset', '--hard', commit]);
			const committedFiles = await collectSourceFiles(module, requireRepository());
		...
		case 'checkpoint': {
			...
			const files = operation.files.map((file) => {
				...
				if (file.contents.byteLength > MAX_FILE_BYTES || totalBytes > MAX_EXPORT_BYTES) {
					throw workspaceFailure(
						'checkpoint',
						'oversized',
			...
			if (operation.expectedCommit !== undefined) {
				...
				if (sharedBranch) {
					...
					await requireCommandSuccess('checkpoint', module, ['checkout', operation.branch]);
			...
			await requireCommandSuccess('checkpoint', module, ['commit', '-m', operation.message]);
			if (createBranchAfterCommit) {
				await requireCommandSuccess('checkpoint', module, ['checkout', '-b', operation.branch]);
		...
		case 'read-at-ref': {
			...
			if (operation.branch.startsWith('flect/shared/')) {
				...
				await requireCommandSuccess('read-at-ref', module, ['checkout', operation.branch]);
		...
		case 'snapshot-ref': {
			...
			if (operation.branch.startsWith('flect/shared/')) {
				...
				await requireCommandSuccess('snapshot-ref', module, ['checkout', operation.branch]);
		...
		case 'move-ref': {
			...
			if (operation.branch.startsWith('flect/shared/')) {
				if (
					...
					(operation.expectedCommit !== undefined && actual !== operation.expectedCommit)
				) {
					throw workspaceFailure(
						'move-ref',
						'stale-ref',
			...
			if (operation.expectedCommit === undefined) {
				...
				await requireCommandSuccess('move-ref', module, ['checkout', operation.targetCommit]);
				await requireCommandSuccess('move-ref', module, ['checkout', '-b', operation.branch]);
			} else {
				if (actual !== operation.expectedCommit) {
					throw workspaceFailure(
						'move-ref',
				...
				await requireCommandSuccess('move-ref', module, ['checkout', operation.branch]);
			...
			const commit = await resolveRef(module, operation.branch);
			if (commit !== operation.targetCommit) {
				throw workspaceFailure('move-ref', 'corrupt', 'The moved ref could not be verified.');
			}
			return GitRefMoved.make({
				type: 'ref-moved',
				branch: operation.branch,
		...
		case 'delete-ref': {
			...
			await module.opfsRemoveTree(`${requireRepository()}/.git/refs/heads/${operation.branch}`);
			await refreshRepositoryCache(module);
			if ((await resolveRef(module, operation.branch)) !== undefined) {
				throw workspaceFailure('delete-ref', 'corrupt', 'The deleted shared ref is still present.');
			}
		...
		case 'status': {
			...
			const lastKnownGoodCommit = await resolveRef(module, 'flect/last-known-good');
			...
			const status = await requireCommandSuccess('status', module, ['status', '--short']);
			const lines = status.stdout.split('\n').filter((line) => line.length > 3);
			const conflictCodes = new Set(['DD', 'AU', 'UD', 'UA', 'DU', 'AA', 'UU']);
			const conflictPaths = lines.flatMap((line) => {
				const path = validateRelativePath(line.slice(3).trim());
				return path !== undefined && conflictCodes.has(line.slice(0, 2)) ? [path] : [];
			});
