...
const runDiagnostic = async (workspaceId: string) => {
  ...
  try {
    ...
    if (cached !== null) {
      const previous = await Schema.decodeUnknownPromise(
        GitWorkspaceDiagnosticResult,
      )(JSON.parse(cached));
    ...
    const first = await runtime.runPromise(
      Effect.gen(function* () {
        ...
        yield* git.write(
          "flect.json",
          encoder.encode('{"revision":"initial"}\n'),
        );
        yield* git.run(["add", "flect.json"]).pipe(Effect.map(requireSuccess));
        yield* git
          .run(["commit", "-m", "Initial interface"])
          .pipe(Effect.map(requireSuccess));
        const initialCommit = requireSuccess(
          yield* git.run(["rev-parse", "HEAD"]),
        ).stdout.trim();
        yield* git
          .run(["checkout", "-b", "flect/accepted"])
          .pipe(Effect.map(requireSuccess));
        yield* git
          .run(["checkout", "-b", "flect/proposal/e2e"])
          .pipe(Effect.map(requireSuccess));
        yield* git.write(
          "flect.json",
          encoder.encode('{"revision":"candidate"}\n'),
        );
        const diff = requireSuccess(yield* git.run(["diff"])).stdout;
        yield* git.run(["add", "flect.json"]).pipe(Effect.map(requireSuccess));
        yield* git
          .run(["commit", "-m", "Candidate interface"])
          .pipe(Effect.map(requireSuccess));
        const proposalCommit = requireSuccess(
          yield* git.run(["rev-parse", "HEAD"]),
        ).stdout.trim();
        yield* git
          .run(["checkout", "flect/accepted"])
          .pipe(Effect.map(requireSuccess));
        yield* git.write(
          "flect.json",
          encoder.encode('{"revision":"accepted"}\n'),
        );
        yield* git.run(["add", "flect.json"]).pipe(Effect.map(requireSuccess));
        yield* git
          .run(["commit", "-m", "Accepted interface"])
          .pipe(Effect.map(requireSuccess));
        const acceptedCommit = requireSuccess(
          yield* git.run(["rev-parse", "HEAD"]),
        ).stdout.trim();
        yield* git.run(["merge", "flect/proposal/e2e"]);
        const status = requireSuccess(
          yield* git.run(["status", "--short"]),
        ).stdout;
        if (!status.includes("flect.json")) {
          return yield* Effect.fail(
            new Error(
              "The divergent interface edit did not produce a conflict.",
            ),
          );
        }
        yield* git
          .run(["reset", "--hard", acceptedCommit])
          .pipe(Effect.map(requireSuccess));
        return {
    ...
    try {
      const completed = await reopenedRuntime.runPromise(
        Effect.gen(function* () {
          ...
          yield* git
            .run(["reset", "--hard", first.initialCommit])
            .pipe(Effect.map(requireSuccess));
