...
const validatePackedRefs = Effect.fn("Flect.RepositoryTar.validatePackedRefs")(
...
const validateGitEntry = Effect.fn("Flect.RepositoryTar.validateGitEntry")(
  function* (
    ...
    if (entry.kind === "directory") {
      if (
        ...
        if (
          path.startsWith(".git/refs/heads/flect/") ||
          path === ".git/refs/heads/flect"
        ) {
          return yield* Effect.fail(repositoryArchiveError("prohibited"));
        }
        return true;
...
export const decodeRepositoryTar = Effect.fn("Flect.RepositoryTar.decode")(
  function* (
    archive: Uint8Array,
  ): Effect.fn.Return<
    ReadonlyArray<DecodedRepositoryArchiveEntry>,
    RepositoryArchiveError
  > {
    if (
      archive.byteLength > MAX_ARCHIVE_BYTES ||
      archive.byteLength < BLOCK_BYTES * 3
    ) {
      return yield* Effect.fail(
        repositoryArchiveError(
          archive.byteLength > MAX_ARCHIVE_BYTES ? "oversized" : "malformed",
        ),
      );
    }
    ...
    for (let offset = 0; offset < archive.byteLength; ) {
      ...
      if (header.every((byte) => byte === 0)) {
        if (
          ...
          !archive
            .subarray(offset, offset + BLOCK_BYTES * 2)
            .every((byte) => byte === 0) ||
          !archive
            .subarray(offset + BLOCK_BYTES * 2)
            .every((byte) => byte === 0)
        ) {
          return yield* Effect.fail(repositoryArchiveError("malformed"));
        }
      ...
      const [rawPath, sizeText, checksumText, magic, version, prefix] =
        yield* Effect.all([
      ...
      const kind =
        type === "5" ? "directory" : type === "0" ? "file" : undefined;
      const path =
        kind === "directory" && rawPath.endsWith("/")
          ? rawPath.slice(0, -1)
          : rawPath;
      if (
        kind === undefined ||
...
const makeHeader = (entry: RepositoryArchiveEntry) => {
  if (encoder.encode(entry.path).byteLength > 100) {
    throw new Error(
      "The repository contains a path too long for portable tar export.",
    );
  }
  ...
  writeText(
    ...
    100,
    8,
    octal(entry.kind === "directory" ? 0o755 : 0o644, 8),
  );
  ...
  writeText(
    ...
    124,
    12,
    octal(entry.kind === "file" ? contents.byteLength : 0, 12),
  );
...
export const makeRepositoryTar = (
  entries: ReadonlyArray<RepositoryArchiveEntry>,
) => {
  ...
  for (const entry of entries) {
    ...
    const paddedLength =
      entry.kind === "file"
        ? Math.ceil(contents.byteLength / BLOCK_BYTES) * BLOCK_BYTES
        : 0;
