				...
				SubscriptionRef.update(state, (current) =>
					FlectWorkspaceSnapshot.make({
						...
						workbench: WorkbenchSnapshot.make({
							...
							binding: 'accepted',
							transitionSequence: (current.workbench?.transitionSequence ?? 0) + 1
						})
					})
				);
					...
					const next =
						...
						command.type === 'submit-conversation-prompt'
							...
							: FlectWorkspaceSnapshot.make({
									...current,
									sequence,
									mode: command.type === 'set-mode' ? command.mode : current.mode,
									control:
...
describe('App', () => {
		...
		await waitFor(() =>
			expect(
				dispatch.mock.calls.some(
					([envelope]) => envelope.command.type === 'submit-conversation-prompt'
				)
			).toBe(true)
		);
		expect(await screen.findByText('Focused project overview')).toBeVisible();
		expect(screen.queryByRole('region', { name: 'Import decision' })).not.toBeInTheDocument();
		expect(screen.getByRole('textbox', { name: 'Message Flect' })).toBeVisible();
		expect(screen.getAllByRole('textbox')).toHaveLength(1);
		expect(
			dispatch.mock.calls.some(([envelope]) => envelope.command.type === 'accept-proposal')
		).toBe(false);

		await user.click(screen.getByRole('button', { name: 'Open settings' }));
		await user.click(await screen.findByRole('button', { name: 'Enable local control' }));
		await waitFor(() =>
			expect(screen.getByRole('button', { name: 'Disable local control' })).toBeVisible()
		);
	...
	it('renders outside state changes without a parallel local mode', async () => {
		...
		expect(await screen.findByRole('textbox', { name: 'Message Flect' })).toBeVisible();
	...
	it('keeps the protected recovery shell available in safe mode', async () => {
		...
		expect(await screen.findByText('Your interface is protected.')).toBeVisible();
