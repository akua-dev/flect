    ...
    if (flag === "--write") {
      ...
      const resolvedWorkspacePath =
        requestedWorkspacePath === undefined
          ? undefined
          : workspacePath(requestedWorkspacePath);
      if (value === undefined || resolvedWorkspacePath === undefined)
        return undefined;
  ...
  defineCommand(options.hiddenName, async (args, commandContext) => {
    ...
    if (args[0] === "share" && args[1] === "resolve") {
      ...
      try {
        ...
        await Effect.runPromise(
          options.bus.submit(source, { type: "command", command }),
          { signal: commandContext.signal },
        );
        return {
    ...
    try {
      const result = await Effect.runPromise(
        runFlect(args).pipe(Effect.provide(gateway)),
        { signal: commandContext.signal },
      );
      return {
