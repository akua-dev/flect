...
describe('SandboxedShell', () => {
	...
	it.effect('keeps accepted App, candidate App, and Shaper filesystems isolated', () =>
		Effect.gen(function* () {
			const commandLayer = makeBunCommandTestLayer(() => Effect.succeed(operationResult('unused')));
			...
			const results = yield* Effect.gen(function* () {
				const shell = yield* SandboxedShell;
				const app = yield* shell.execute('app', 'cat role.txt; echo app > marker');
				const shaper = yield* shell.execute('shaper', 'cat role.txt; test ! -e marker');
	...
	it.effect('replaces one protected Shaper conflict tree atomically', () =>
		Effect.gen(function* () {
			...
			const result = yield* Effect.gen(function* () {
				const shell = yield* SandboxedShell;
				yield* shell.replaceTree('shaper', '/workspace/.flect/share-conflicts/weather', [
					{
		...
		Effect.gen(function* () {
			...
			const results = yield* Effect.gen(function* () {
				...
				return [
					...
					yield* shell.execute('shaper', 'bun add flect-fixture@1.0.0 | tee install.log'),
	...
	it.effect('does not let guest shell features shadow the reserved command', () =>
		Effect.gen(function* () {
			const calls = yield* Ref.make(0);
			const commandLayer = makeBunCommandTestLayer(() =>
				Ref.update(calls, (count) => count + 1).pipe(Effect.as(operationResult('reserved')))
			);
			const shellLayer = makeSandboxedShellLayer({
				role: 'app',
				files: {}
		...
		Effect.gen(function* () {
			...
			const run = (
				...
				Effect.gen(function* () {
					...
					Effect.provide(makeSandboxedShellLayer({ role, files }).pipe(Layer.provide(commandLayer)))
				);
