...
const branchLines = (role: FlectAgentRole, repository: GitRepositoryStatus) => {
  ...
  return branches
    .map((branch) => `${branch === selected ? "*" : " "} ${branch}`)
    .join("\n");
};
...
const readWorkspaceSource = (fileSystem: IFileSystem) =>
  Effect.tryPromise({
    try: async () =>
      (await snapshotWorkspaceFiles(fileSystem)).filter((file) =>
...
const changed = (
  ...
  left === undefined
    ? right !== undefined
    : right === undefined || !bytesEqual(left, right);
...
const workspaceStatus = Effect.fn("Flect.Shell.gitStatus")(function* (
  ...
  const index =
    state.staged === undefined
      ? base
      : new Map(
          state.staged.map((file) => [file.path, file.contents] as const),
        );
    ...
    const indexCode = changed(before, staged)
      ? before === undefined
        ...
        : staged === undefined
          ...
          : "M"
      : " ";
    const workCode = changed(staged, working)
      ? working === undefined
        ? "D"
        : "M"
      : " ";
    return indexCode === " " && workCode === " "
      ? []
      : [`${indexCode}${workCode} ${path}`];
  });
});
...
const execute = Effect.fn("Flect.Shell.git")(function* (
  ...
  switch (command) {
    ...
    case "branch": {
      if (
        rest.length > 1 ||
        (rest[0] !== undefined && rest[0] !== "--show-current")
      ) {
        return { stdout: "", stderr: `${usage}\n`, exitCode: 2 };
      }
      return {
        ...
        return {
          ...
          stderr:
            options.git === undefined
              ? "fatal: embedded Git engine unavailable\n"
              : `${usage}\n`,
          exitCode: 2,
        };
      }
      yield* options.git.open({ workspaceId: "default" });
      const result = yield* options.git.run([
        "log",
        currentRef(options.role, repository),
      ]);
      return {
    ...
    case "commit": {
      ...
      const baseBranch =
        repository.authoringCommit === undefined
          ...
          : "flect/authoring";
      const baseCommit =
        repository.authoringCommit ?? repository.acceptedCommit;
      ...
      yield* Effect.tryPromise({
        try: async () => {
          ...
          for (const file of desired) {
            ...
            if (slash > 0) {
              await options.fileSystem.mkdir(
                `/workspace/${file.path.slice(0, slash)}`,
                { recursive: true },
              );
            }
