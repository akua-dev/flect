...
describe('persistent role workspace filesystem', () => {
	it('persists a namespaced workspace and restores its synchronous index', async () => {
		...
		await first.writeFile('/workspace/src/index.ts', 'export const value = 42;\n');
		...
		expect(await reopened.readFile('/workspace/package.json')).toBe('{"name":"fixture"}\n');
		expect(await reopened.readFile('/workspace/src/index.ts')).toBe('export const value = 42;\n');
	...
	it('restores the durable tree when a copy replacement fails', async () => {
		...
		vfs.failNextWrite();
		await expect(workspace.cp('/workspace/source.txt', '/workspace/copy.txt')).rejects.toThrow(
			/write failed/
		);
	...
	it('denies canonical metadata, traversal, links, and read-only writes', async () => {
		...
		await expect(readOnly.writeFile('/workspace/index.ts', 'changed\n')).rejects.toThrow(
			/read-only/i
		);
		await expect(readOnly.writeFile('/workspace/.git/config', 'unsafe\n')).rejects.toThrow(
			/protected/i
		);
		await expect(readOnly.writeFile('/workspace/../outside', 'unsafe\n')).rejects.toThrow(
			/outside/i
		);
		await expect(readOnly.symlink('/workspace/index.ts', '/workspace/link')).rejects.toThrow(
			/links/i
		);
		await expect(readOnly.link('/workspace/index.ts', '/workspace/hard-link')).rejects.toThrow(
			/links/i
		);
	});
});
