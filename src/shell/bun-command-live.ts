...
import { BunCommandFailed, BunCommandResult, type WorkspaceDelta } from '../../shared/bun-command';
import {
	BunModuleExecution,
	BunModuleExecutionLive,
	type BunWorkspace
} from '../execution/bun-module-execution';
import { BunPackageMutation, makeBunPackageMutationLayer } from '../execution/bun-package-mutation';
import { NPM_REGISTRY_ORIGIN, trustedNpmRegistryFetch } from '../execution/npm-registry';
import { BunCommand, type BunOperationCall, makeBunCommandService } from './bun-command';
...
const snapshotWorkspace = (fs: IFileSystem) =>
	Effect.tryPromise({
		try: async (): Promise<BunWorkspace> => {
			...
			const paths = fs
				.getAllPaths()
				.filter((path) => workspacePath(path) && path !== '/workspace/.flect-root')
				.sort();
			...
			if (Object.keys(files).length > FILE_LIMIT || byteLength > BYTE_LIMIT) {
				throw failure('workspace', 'The disposable workspace exceeds its limit.');
			}
			return { files };
		},
		catch: () => failure('workspace', 'The disposable workspace could not be read.')
	});
...
const applyDelta = (fs: IFileSystem, delta: WorkspaceDelta) =>
	Effect.tryPromise({
		try: async () => {
			...
			for (const change of delta.files) {
				if (!workspacePath(change.path)) {
					throw failure('workspace', 'The workspace delta contains an invalid path.');
				}
		...
		catch: () => failure('workspace', 'The workspace delta could not be applied.')
	});

const packageResult = (operation: 'install' | 'add' | 'remove', packageCount: number) =>
...
export const makeShellBunCommandLiveLayer = (options: {
	...
	const dependencies = Layer.merge(options.moduleLayer ?? BunModuleExecutionLive, packageLayer);
	return Layer.effect(
		BunCommand,
		Effect.gen(function* () {
