...
describe("reserved browser flect command", () => {
  ...
  it.effect("lets only Shaper import a capsule from its sandbox", () =>
    Effect.gen(function* () {
      ...
      const result = yield* shell.execute(
        "shaper",
        "flect capsule import app.flect",
        {
          agentContext: context,
        },
      );
  ...
  it.effect(
    "lets Shaper checkpoint bounded sandbox files onto an inspected shared fork",
    () =>
      Effect.gen(function* () {
        const bus = yield* AgentCommandBus;
  ...
  it.effect(
    "checkpoints staged Shaper source onto a guarded authoring ref",
    () =>
      Effect.gen(function* () {
        const checkpoints: Array<
          Parameters<GitWorkspaceShape["checkpoint"]>[0]
        > = [];
        ...
        const output = yield* Effect.gen(function* () {
          ...
          Effect.provide(
            makeLayer("shaper", { "/workspace/new.txt": "new\n" }, git),
          ),
        );
        ...
        assert.isTrue(
          checkpoints[0]?.guards?.some(
            (guard) =>
              guard.branch === "flect/proposal/revision-1" &&
              guard.commit === "c".repeat(40),
          ),
        );
      }),
  );
