...
export interface PortableExtensionSourceShape {
  readonly list: (
    ...
    ReadonlyArray<ResolvedPortableExtension>,
    PortableExtensionSourceFailure
  >;
}
    ...
    return {
      list: Effect.fn("Flect.PortableExtensionSource.list")((binding) =>
        Effect.gen(function* () {
          ...
          const archive =
            binding === "accepted" ? archives.accepted : archives.candidate;
          if (archive === undefined) return [];
          const capsule = yield* decodeCapsule(archive);
          const decoder = new TextDecoder("utf-8", { fatal: true });
          return yield* Effect.forEach(
            capsule.manifest.extensions ?? [],
            (manifest) =>
              Effect.try({
                try: () => {
                  const bundle = capsule.files.find(
                    (file) => file.path === manifest.bundle,
                  );
...
const unavailable = (
  extensionId: string,
  reason: PortableExtensionUnavailable["reason"],
) =>
...
export interface PortableExtensionHostShape {
  readonly list: (
    ...
    ReadonlyArray<PortableExtensionDescriptor>,
    PortableExtensionSourceFailure
  >;
  readonly describe: (
    role: PortableExtensionRole,
...
export const PortableExtensionHostLive = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const descriptor = Effect.fn("Flect.PortableExtensionHost.descriptor")(
      function* (
        role: PortableExtensionRole,
        binding: "accepted" | "candidate",
        extensionId: string,
        ...
        if (resolved === undefined) {
          const otherBinding =
            binding === "accepted" ? "candidate" : "accepted";
          ...
          return yield* Effect.fail(
            unavailable(
              extensionId,
              existsElsewhere ? "wrong-binding" : "missing",
            ),
          );
        }
        ...
        if (entry === undefined)
          return yield* Effect.fail(unavailable(extensionId, "disabled"));
    ...
    const describe = Effect.fn("Flect.PortableExtensionHost.describe")(
      function* (
        role: PortableExtensionRole,
        binding: "accepted" | "candidate",
        extensionId: string,
