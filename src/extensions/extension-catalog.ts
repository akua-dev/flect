...
const failure = (
  reason: ExtensionCatalogFailure["reason"],
): ExtensionCatalogFailure =>
...
const decodeStored = Effect.fn("Flect.ExtensionCatalog.decodeStored")(
  (raw: string) =>
    Effect.try({
      ...
      Effect.flatMap(
        Schema.decodeUnknownEffect(PortableExtensionCatalogRecord, strict),
      ),
      Effect.map((record) =>
        PortableExtensionCatalogSnapshot.make({
          version: 1,
...
const storageFailure = (_error: InterfaceStorageError) =>
...
export const makeExtensionCatalogLayer = () =>
  Layer.effect(
    ...
    Effect.gen(function* () {
      ...
      const updateEntry = Effect.fn("Flect.ExtensionCatalog.updateEntry")(
        <A>(
          ...
          change: (
            ...
            readonly [A, PortableExtensionRoleState],
            ExtensionCatalogFailure
          >,
        ) =>
          mutate((current) => {
            ...
            if (index < 0 || entry === undefined)
              return Effect.fail(failure("missing"));
      ...
      const disable = Effect.fn("Flect.ExtensionCatalog.disable")(
        (key: PortableExtensionKey) =>
          updateEntry(key, (entry) =>
            Effect.succeed([
              undefined,
      ...
      const remove = Effect.fn("Flect.ExtensionCatalog.remove")(
        (key: PortableExtensionKey) =>
          mutate((current) => {
            const entries = current.entries.filter(
              (entry) => !sameKey(entry, key),
            );
      ...
      const promoteCandidate = mutate((current) => {
        ...
        const acceptedKeys = new Set(
          candidates.map(
            (entry) =>
              `${entry.capsuleId}\u0000${entry.extensionId}\u0000${entry.role}`,
          ),
        );
        const entries = [
