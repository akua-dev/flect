...
describe("PortableExtensionHost", () => {
  it.effect(
    "discovers only enabled packages for the authenticated role and binding",
    () => {
  ...
  it.effect(
    "executes with the exact role grants and marks only that candidate tested",
    () => {
      ...
      return Effect.gen(function* () {
        ...
        assert.strictEqual(
          snapshot.entries.find((entry) => entry.extensionId === "app-weather")
            ?.tested,
          true,
        );
        assert.strictEqual(
          snapshot.entries.find(
            (entry) => entry.extensionId === "shape-weather",
          )?.tested,
          false,
        );
      }).pipe(Effect.provide(harness.layer));
  ...
  it.effect(
    "records capability denial without applying an intent or exposing details",
    () => {
      const harness = makeHarness({ proposedIntent: true });
      return Effect.gen(function* () {
        yield* stage;
  ...
  it.effect("contains sandbox failure to the offending package role", () => {
    ...
    return Effect.gen(function* () {
      ...
      assert.strictEqual(
        snapshot.entries.find((entry) => entry.extensionId === "shape-weather")
          ?.state,
        "available",
      );
    }).pipe(Effect.provide(harness.layer));
