...
const key = (role: 'app' | 'shaper', binding: 'accepted' | 'candidate' = 'candidate') => ({
	capsuleId: 'dev.akua.weather',
...
describe('ExtensionCatalog', () => {
	it.effect('stages separate inactive candidate records for each declared role', () => {
		...
		return Effect.gen(function* () {
			...
			assert.deepStrictEqual(
				snapshot.entries.map((entry) => [entry.role, entry.binding, entry.state]),
				[
			...
			assert.isTrue(snapshot.entries.every((entry) => entry.grantedCapabilities.length === 0));
		}).pipe(Effect.provide(harness.layer));
	});

	it.effect('enables only one explicit role and requires its required grants', () => {
		const harness = makeLayer();
		return Effect.gen(function* () {
			const catalog = yield* ExtensionCatalog;
			yield* catalog.stageCandidate(candidate);
			...
			yield* catalog.enable(key('app'), ['interface:read', 'interface:propose']);
			const snapshot = yield* catalog.snapshot;
			const app = snapshot.entries.find((entry) => entry.role === 'app');
			const shaper = snapshot.entries.find((entry) => entry.role === 'shaper');
			assert.strictEqual(app?.state, 'enabled');
			assert.deepStrictEqual(app?.grantedCapabilities, ['interface:read', 'interface:propose']);
			assert.strictEqual(shaper?.state, 'available');
			assert.deepStrictEqual(shaper?.grantedCapabilities, []);
		}).pipe(Effect.provide(harness.layer));
	});

	it.effect('requires candidate testing before promotion and keeps roles isolated', () => {
		const harness = makeLayer();
		return Effect.gen(function* () {
			const catalog = yield* ExtensionCatalog;
			yield* catalog.stageCandidate(candidate);
			...
			const snapshot = yield* catalog.snapshot;
			assert.isTrue(snapshot.entries.every((entry) => entry.binding === 'accepted'));
			assert.strictEqual(snapshot.entries.find((entry) => entry.role === 'app')?.tested, true);
			assert.strictEqual(snapshot.entries.find((entry) => entry.role === 'shaper')?.tested, false);
		}).pipe(Effect.provide(harness.layer));
	});

	it.effect('preserves pins and local forks as update conflicts without widening grants', () => {
		const harness = makeLayer();
		return Effect.gen(function* () {
			const catalog = yield* ExtensionCatalog;
			yield* catalog.stageCandidate(candidate);
			...
			assert.deepStrictEqual(nextApp?.grantedCapabilities, ['interface:read']);
			...
			const unresolvedFork = yield* catalog.resolveUpdate(key('app'), 'fork').pipe(Effect.flip);
	...
	it.effect('records bounded failure evidence, disables, removes, and rejects candidates', () => {
		const harness = makeLayer();
		return Effect.gen(function* () {
			const catalog = yield* ExtensionCatalog;
			yield* catalog.stageCandidate(candidate);
			...
			const failed = (yield* catalog.snapshot).entries.find((entry) => entry.role === 'app');
			assert.strictEqual(failed?.state, 'failed');
			assert.strictEqual(failed?.failure?.message, 'The portable extension failed safely.');
			assert.notProperty(failed?.failure ?? {}, 'stack');
			assert.notProperty(failed?.failure ?? {}, 'error');
			const failedPromotion = yield* catalog.promoteCandidate.pipe(Effect.flip);
			assert.strictEqual(failedPromotion.reason, 'untested-candidate');

			yield* catalog.disable(key('app'));
			const disabled = (yield* catalog.snapshot).entries.find((entry) => entry.role === 'app');
	...
	it.effect(
		...
		() => {
			...
			return Effect.gen(function* () {
				...
				const corrupt = makeLayer('{"version":1,"entries":[{"token":"secret"}]}');
