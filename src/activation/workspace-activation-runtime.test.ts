...
it.effect("completes when the deferred workspace reports readiness", () =>
  Effect.gen(function* () {
    const activation = yield* waitForAstroIsland(document).pipe(
      Effect.forkChild,
    );
    ...
    assert.strictEqual(
      document.documentElement.dataset.flectOpenRequested,
      "true",
    );
  }),
);
...
it.effect("fails closed instead of leaving activation pending forever", () =>
  Effect.gen(function* () {
    const activation = yield* waitForAstroIsland(document, "1 second").pipe(
      Effect.forkChild,
    );
